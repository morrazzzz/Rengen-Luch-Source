...
void TfraSpawn::OnItemFocused(ListItemsVec& items)
{
	...
	ESceneSpawnTools* g_tools = dynamic_cast<ESceneSpawnTools*>(Scene->GetOTools(OBJCLASS_SPAWNPOINT));

	R_ASSERT(g_tools);

	if (g_tools)
		g_tools->AnObjectSelectedInOList();
...
void __fastcall TfraSpawn::evTogglePreview(TObject *Sender)
{
	ESceneSpawnTools* g_tools = dynamic_cast<ESceneSpawnTools*>(Scene->GetOTools(OBJCLASS_SPAWNPOINT));
