...
CSceneObject* __fastcall TfrmEditLibrary::RayPick(const Fvector& start, const Fvector& direction, SRayPickInfo* pinf)
{
	...
	if (form->cbPreview->Checked || form->previewEnabled_)
	{
    	float dist=UI->ZFar();
...
//		ebExportLWO->Enabled 	= !bReadOnly;
        ...
        if (cbPreview->Checked || previewEnabled_ || m_Props->Visible)
		{
...
//.            	modif_map.insert(mk_pair(m_pEditObject->GetRefName(),FS_QueryItem(0,0,0)));
		    ...
		    if (cbPreview->Checked || previewEnabled_)
				mt = true;
        }
    }else{
		ChangeReference(0);
    ...
    ebMakeLOD_high->Enabled	= (cbPreview->Checked || previewEnabled_);
    ebMakeLOD_low->Enabled	= (cbPreview->Checked || previewEnabled_);
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmEditLibrary::ebMakeThmClick(TObject *Sender)
{
	...
	if (node&&FHelper.IsObject(node))
	{
		...
		if (obj && (cbPreview->Checked || previewEnabled_))
		{
...
bool TfrmEditLibrary::GenerateLOD(ListItem* prop, bool bHighQuality)
{
	...
	VERIFY2(cbPreview->Checked || previewEnabled_, "LOD dds will be empty since object previewing is not enabled. LOD generator needs it to make images of an object from different angles");
    ...
    if (O && O->IsMUStatic())
	{
        ...
        bool result = ImageLib.CreateLODTexture(O, tex_name.c_str(), LOD_IMAGE_SIZE, LOD_IMAGE_SIZE, LOD_SAMPLE_COUNT, O->Version(), bHighQuality ? 4 : 1);

		if(!result)
		{
			ELog.Msg(mtError, "LOD for object '%s' Was not created",  O->GetName());
		}
...
bool TfrmEditLibrary::GenerateLOD_Object(CEditableObject* e_object, bool bHighQuality)
{
	VERIFY(e_object);

	VERIFY2(cbPreview->Checked || previewEnabled_, "LOD dds will be empty since object previewing is not enabled. LOD generator needs it to make images of an object from different angles");

	if (e_object && e_object->IsMUStatic())
	{
		LPCSTR lib_name = e_object->GetName();

		form->ChangeReference(lib_name);

		BOOL bLod = e_object->m_Flags.is(CEditableObject::eoUsingLOD);

		e_object->m_Flags.set(CEditableObject::eoUsingLOD, FALSE);

		xr_string tex_name;
		tex_name = EFS.ChangeFileExt(lib_name, "");
		...
		ELog.Msg(mtInformation, "Running LOD creation '%s'. Editor Used RAM %u KB", tex_name.c_str(), ETOOLS::GetTotalUsedProcessRAM() / 1024);

		bool result = ImageLib.CreateLODTexture(e_object, tex_name.c_str(), LOD_IMAGE_SIZE, LOD_IMAGE_SIZE, LOD_SAMPLE_COUNT, e_object->Version(), bHighQuality ? 4 : 1);
		
		if(!result)
		{
			ELog.Msg(mtError, "LOD for object '%s' Was not created", e_object->GetName());
		}
		
		e_object->OnDeviceDestroy();
		e_object->m_Flags.set(CEditableObject::eoUsingLOD, bLod);

		ELog.Msg(mtInformation, "LOD for object '%s' successfully created. Editor Used RAM %u KB", e_object->GetName(), ETOOLS::GetTotalUsedProcessRAM() / 1024);
...
void TfrmEditLibrary::MakeLOD(bool bHighQuality)
{
	...
	if (node && (cbPreview->Checked || previewEnabled_))
	{
        LockForm();
...
extern ECORE_API xr_vector<shared_str> objectsWithMisingLODS_;

void __fastcall TfrmEditLibrary::AutoGenerateLODS()
{

		ELog.Msg(mtInformation, "Starting Library Editor");

		TfrmEditLibrary::ShowEditor();

		ELog.Msg(mtInformation, "Starting generating misisng lods. Count = %u, Current Used RAM = %u KB", objectsWithMisingLODS_.size(), ETOOLS::GetTotalUsedProcessRAM() / 1024);

		form->previewEnabled_ = true;
		
		bool is_high_quality = false;
		
		int res = ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, "Generate HIGH QUALITY LODS?(10 times slower) \n Yes = high. No = low.");

		if (res == 6)
			is_high_quality = true;

		Msg("High Quality Lods? %d", is_high_quality);

		for (u32 i = 0; i < objectsWithMisingLODS_.size(); i++)
		{
			Msg(LINE_SPACER);
			ELog.Msg(mtInformation, "***GENERATING LOD for '%s'", objectsWithMisingLODS_[i].c_str());

			CEditableObject* hujila = Lib.CreateEditObject(objectsWithMisingLODS_[i].c_str());

			form->GenerateLOD_Object(hujila, is_high_quality);

			if (UI->NeedAbort())
				break;
			
			Lib.RemoveEditObject(hujila);
		}
...
//.		form->tvItemsItemFocused(0);
        TElTreeItem *node = m_Items->GetSelected();
        bool mt=false;
        if ((cbPreview->Checked || previewEnabled_) && node && FHelper.IsObject(node))
		{
            AnsiString name;
            FHelper.MakeName(node,0,name,false);
            ChangeReference(name.c_str());
        ...
        ebMakeLOD_high->Enabled = !bReadOnly && (cbPreview->Checked || previewEnabled_);
        ebMakeLOD_low->Enabled 	= !bReadOnly && (cbPreview->Checked || previewEnabled_);
        UI->RedrawScene();
    }
}
//---------------------------------------------------------------------------
