...
void __fastcall TfraBottomBar::ClickOptionsMenuItem(TObject *Sender)
{
    ...
    if (mi){
        ...
        else if (mi==miAutoCloseLog)			psDeviceFlags.set(rsAutoCloseLog,mi->Checked);
...
void __fastcall TfraBottomBar::pmOptionsPopup(TObject *Sender)
{
    ...
    miAutoCloseLog->Checked			= psDeviceFlags.is(rsAutoCloseLog);
...
void __fastcall TfraBottomBar::ebTranslateCameraPos(TObject *Sender)
{
	Msg("Translate Cam Button");

	Fvector pos = EDevice.m_Camera.GetPosition();

	if (NumericVectorRun("Move camera to", &pos, 3))
