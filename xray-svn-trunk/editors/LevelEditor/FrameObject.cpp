...
extern TfrmSplash* startupSplashForm_;
...
__fastcall TfraObject::~TfraObject()
{
	Msg("Waiting for Object List Thread to finish");

	FlushLog();

	while (isRefreshing_) // Подождать, пока не закончится рефреш, чтобы избежать краша.
		Sleep(100);
...
void __fastcall TfraObject::OnItemFocused(ListItemsVec& items)
{
	...
	if (ParentTools && !isRefreshing_)
		ParentTools->AnObjectSelectedInOList();
}
//------------------------------------------------------------------------------
...
{
	TfraObject* frameObj_;
	ListItemsVec* itemArray_;
};

void RefreshListMT(void *params)
{

	BTHREAD_params	P = *((BTHREAD_params*)params);

	ListItemsVec items = *P.itemArray_;

	P.frameObj_->m_Items->AssignItems(items, false, true);

	P.frameObj_->isRefreshing_ = false;
}
...
void TfraObject::RefreshList()
{
	if (isRefreshing_)
	{
		Msg("List is already refreshing");
	...
	// fill list
	if (EPrefs->object_flags.is(epoMT_OListLoading))
	{
		Msg("Loading Item List in Another Thread");
		FlushLog();

		BTHREAD_params thread_params = { this, &items };

		thread_spawn(RefreshListMT, "List Loading thread", 0, &thread_params);
	}
	else
	{
		Msg("Loading Item List:Non MT");
		FlushLog();
...
void __fastcall TfraObject::FormCreate(TObject *Sender)
{
	...
	if (startupSplashForm_)
		startupSplashForm_->SetStatus("Creating Objects List");
...
{
	int button = ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, "Refreshing list takes some anoying time. Do it?");

	if(button == 6)
		RefreshList();	
}

void __fastcall TfraObject::ExtBtn10Click(TObject *Sender)
{
	if (ParentTools)
		ParentTools->SwitchPreview();
}
//---------------------------------------------------------------------------
