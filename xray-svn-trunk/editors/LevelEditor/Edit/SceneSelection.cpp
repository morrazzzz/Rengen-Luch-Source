...
{
	Fbox midpoint_box;
	midpoint_box.invalidate();
	
    if (cls == OBJCLASS_DUMMY)
	{
		Msg("No Edit Mode is Selected: Resetting camera");
	...
	else
	{
        ...
        if (mt)
			mt->GetBBox(midpoint_box, bSel);
    }
	
    if (midpoint_box.is_valid())
		EDevice.m_Camera.ZoomExtents(midpoint_box);
    else
