...
#define MISSING_OBJ_COMP_TABLE_LTX "missing_objects_comp_table.ltx"
#define MISSING_OBJ_COMP_TABLE_SECTION "object_compatibility_table"
...
public:
	...
	bool applyReplacerToAllFound_; // yes to all switch for substitute table
	bool skipLTXReplacerInquiry_; // no to all switch for substitute table
	bool applySelectedFromLibToAllFound_; // yes to all switch for library remembered substitute selection
	bool skipLibraryRememberedInquiry_; // no to all switch for library remembered substitute selection

	CInifile* missingObjsCompTable_;
...
public:
	...
	bool 			CheckIfModifiedAndAsk(LPCSTR mesage);
...
public:
	...
	int				totalCObjectsInstances_;
	xr_vector<CSceneObject*>	visibleStaticObject_;
	
	void			CalculateVertsFaces (u32& vertexes, u32& faces);
};
