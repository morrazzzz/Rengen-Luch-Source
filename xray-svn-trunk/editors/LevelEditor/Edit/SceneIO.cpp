...
bool EScene::Load(LPCSTR map_name, bool bUndo)
{
	...
	applyReplacerToAllFound_ = false;
	skipLTXReplacerInquiry_ = false;
	applySelectedFromLibToAllFound_ = false;
	skipLibraryRememberedInquiry_= false;
	
    if (FS.exist(full_name.c_str()))
	{
        CTimer T; T.Start();

		xr_delete(missingObjsCompTable_);
	
		string_path table_ltx_path;
		FS.update_path(table_ltx_path, "$game_data$", MISSING_OBJ_COMP_TABLE_LTX);

		R_ASSERT2(FS.exist(table_ltx_path), make_string("Missing missing object compatibility table ltx: %s", table_ltx_path));

		if (FS.exist(table_ltx_path))
		{
			missingObjsCompTable_ = xr_new<CInifile>(table_ltx_path, FALSE, TRUE, FALSE);
			
			Msg(LINE_SPACER);
			Msg("Checking paths specified in '%s'", table_ltx_path);
			
			int line_count	= missingObjsCompTable_->line_count(MISSING_OBJ_COMP_TABLE_SECTION);
			for (int l_id = 0; l_id < line_count; l_id++)
			{
				LPCSTR line_name, line_value;
				
				if (missingObjsCompTable_->r_line(MISSING_OBJ_COMP_TABLE_SECTION, l_id, &line_name, &line_value))
				{
					string_path temp;
					string_path temp_extension;
					FS.update_path(temp, _objects_, line_value);
					xr_sprintf(temp_extension, sizeof(temp), "%s.object", temp);

					R_ASSERT2(FS.exist(temp_extension), make_string("%s has an incorrect path %s = %s", MISSING_OBJ_COMP_TABLE_LTX, line_name, line_value));
				}
			}
			
			Msg("Loaded Objects compatability table. '%s'", table_ltx_path);
		}
		else
			Msg("Objects compatability table is missing. %s: '%s'", MISSING_OBJ_COMP_TABLE_LTX, table_ltx_path);
...
*/
	...
	{
		res = false;
		VERIFY2(FS.exist(full_name.c_str()), make_string("Temp file dissapeared!: %s", full_name.c_str()));
	}
...
{
	Msg("'Copy/Cut' command is prompted");
	...
	if(pCurrentTools)
	{
		selected_count = pCurrentTools->SelectionCount(true);
		unselected_count = pCurrentTools->SelectionCount(false);
	}
	
	Msg("Selected count = %i. Unselected count = %i", selected_count, unselected_count);
	
	if(selected_count <= 0)
	{
		Msg("Nothing Is Selected to be stored");
		return;
	}
...
void EScene::PasteSelection() 
{
	...
	if(OpenClipboard(0))
	{
		...
		{
			SceneClipData *sceneclipdata = (SceneClipData *)GlobalLock(hmem);
			Msg("Pasting from temp file %s", sceneclipdata->m_FileName);
			
			bool result = LoadSelection(sceneclipdata->m_FileName);
			if(!result)
				Msg("Temporery editor file is absent");
