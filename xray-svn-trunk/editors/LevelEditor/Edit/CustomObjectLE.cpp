...
{
	R_ASSERT(owner);
    R_ASSERT2( ((!m_pOwnerObject) || (m_pOwnerObject==owner) ), "Object is already in this group!");
...
void CCustomObject::FillProp(LPCSTR pref, PropItemVec& items)
{
	...
	V = PHelper().CreateAngle3(items, PrepareKey(pref, "Transform\\Rotation"), &PRotation, -10000, 10000, 0.01, 4);
