...
void ESceneGroupTools::GroupObjects(bool bUndo)
{
	...
	{
		bool has_attachable_objects = false;
		for (ObjectIt itt = lst.begin(); itt != lst.end(); itt++)
		{
			if ((*itt)->CanAttach())
			{
				has_attachable_objects = true;
				break;
			}
		}

		if (has_attachable_objects) // Create group only if at least one object is groupable
		{
			group->GroupObjects(lst);
		}
		else
		{
			ELog.DlgMsg(mtError, "Selection does not contain attachable objects");
		}
	}
	...
	{
	    ELog.DlgMsg(mtError,"Group was not created.");
        xr_delete(group);
    }
}
//----------------------------------------------------
...
void ESceneGroupTools::CheckGroups()
{
	Msg("Checking Groups");
	...
	for (ObjectIt it = m_Objects.begin(); it != m_Objects.end(); it++)
	{
		if (*it)
		{
			...
			{
				ELog.DlgMsg(mtInformation, "Group '%s' has no objects and will be removed.", group->Name);
				group->DeleteThis();
			}
		}
	}
...
void ESceneGroupTools::GetBBox(Fbox& BB, bool bSelOnly)
{
	Fbox bb;
	ObjectList lst;
	int res = Scene->GetQueryObjects(lst, OBJCLASS_GROUP, bSelOnly, 1, -1);

	if (res > 0)
	{
		for (ObjectIt g_iter = lst.begin(); g_iter != lst.end(); g_iter++)
		{
			CGroupObject* group = dynamic_cast<CGroupObject*>(*g_iter);
