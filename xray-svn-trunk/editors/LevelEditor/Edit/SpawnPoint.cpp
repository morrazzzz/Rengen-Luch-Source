...
#include "../../Ecore/EditorRender/SkeletonAnimated.h"
...
#define SPAWNPOINT_CHUNK_ENVMOD4		0xE427
...
void CSpawnPoint::SSpawnData::Create(LPCSTR _entity_ref)
{
    ...
    if (m_Data){
		...
		LPCSTR class_texted = pSettings->r_string(m_Data->name(), "class");

		if(!xr_strcmp(class_texted, "AI_GRAPH")) // dull, but at least somehow
			isGraphPoint_ = true;
...
void CSpawnPoint::SSpawnData::OnFrame()
{
    ...
    if (m_Motion)
    {
    	...
    	if (m_Motion->animator)
    		m_Motion->animator->Update(TimeDelta());
    }
   ...
   // 	return;
	...
	{		
		if (!Tools->GetSettings(etfNUScale))
			m_EM_Radius = _max(_max(PScale.x, PScale.y), PScale.z);

		boxProportions_ = PScale;
	}
}
...
bool CSpawnPoint::GetBox( Fbox& box ) const
{
    ...
    case ptEnvMod: 	
        box.set		(PPosition, PPosition);
		if(isBox_)
			box.grow(Selected() ? boxProportions_ : Fvector().set(0.25f, 0.25f, 0.25f));
		else
...
void CSpawnPoint::DrawFlagshtocks()
{
	if (psDeviceFlags.is(rsDrawFlashtok))
	{
		// render icon
		ESceneSpawnTools* st = dynamic_cast<ESceneSpawnTools*>(ParentTools);
	
		VERIFY(st);
	
		ref_shader s = EDevice.m_WireShader;
		
		if(m_SpawnData.isGraphPoint_)
		{
			if(Selected())
				DU_impl.DrawTriangularFlag(0xFF856AC4, s, 1.0f);
			else if (m_bSpawnEnabled)
				DU_impl.DrawTriangularFlag(0xFFBD8CFF, s, 1.0f);
			else
				DU_impl.DrawTriangularFlag(0x00FF0000, s, 1.0f);
		}
		else
		{
			if(Selected())
				DU_impl.DrawRectangularFlag(0xFF77F700, s, 1.0f);
			else if (m_bSpawnEnabled)
				DU_impl.DrawRectangularFlag(0xFFE2C77C, s, 1.0f);
			else
				DU_impl.DrawRectangularFlag(0x00FF0000, s, 1.0f);
		}
                ...
                switch (m_Type)
                {
                    case ptRPoint:
                    {
                    	...
                    	if( NULL==st->get_draw_visual(m_RP_TeamID, m_RP_Type, m_RP_GameType) )
                        {
                            ...
                            DU_impl.DrawRectangularFlag(c.get(), EDevice.m_WireShader, 1.0f);
                        }
                    }break;
                    case ptEnvMod:
                    {
                        ...
                        if (Selected())
							if(isBox_)
							{
								DU_impl.DrawAABB(Fidentity, PPosition, boxProportions_, 0x30FFAE00, 0x30FFAE00, true, true);
							}
							else
	...
	if (priority == 3)
	{
		if (!strictB2F)
		{
			if(Selected())
			{
				...
				u32 clr = Locked() ? 0xFFFF0000 : 0xFFFFFF70;
...
bool CSpawnPoint::ExportGame(SExportStreams* F)
{
	...
	if (m_SpawnData.Valid()){
        ...
        case ptEnvMod:
			...
			F->envmodif.stream.w_u8(isBox_);
			F->envmodif.stream.w_fvector3(PScale);
...
void CSpawnPoint::FillProp(LPCSTR pref, PropItemVec& items)
{
    ...
    if (m_SpawnData.Valid())
    {
        ...
        case ptEnvMod:{
			PHelper().CreateBOOL(items, PrepareKey(pref,"Environment Modificator\\Is Box"),	&isBox_);
			
			PHelper().CreateVector(items, PrepareKey(pref, "Environment Modificator\\Box Boundaries"), &boxProportions_, 0, 5000, 0.01, 4);
        	PHelper().CreateFloat(items, PrepareKey(pref,"Environment Modificator\\Sphere Radius"),	&m_EM_Radius, EPS_L, 10000.f);
