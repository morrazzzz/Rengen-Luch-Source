...
bool EDetailManager::Export(LPCSTR path) 
{
    ...
    int res				= ImageLib.CreateMergedTexture(textures,do_tex_name.c_str(),STextureParams::tfDXT3,256,8192,256,8192,offsets,scales,rotated,remap);
	if (1 != res){ ELog.DlgMsg(mtError, "Failed to create merged details types texture"); bRes = false ;}
    ...
    if (bRes){
		...
		if (!bRes)
			ELog.DlgMsg(mtError, "Failed to save to '%s'", fn.c_str());
    }
