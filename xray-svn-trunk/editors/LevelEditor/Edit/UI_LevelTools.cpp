...
bool needRecalcBeforeRedraw_ = false;
...
bool __fastcall CLevelTools::MouseStart(TShiftState Shift)
{
    ...
    if(pCurTools&&pCurTools->pCurControl)
    {
    	if ((pCurTools->pCurControl->Action()!=etaSelect)&&
	    	(!pCurTools->IsEditable()|| !pCurTools->AllowMouseStart() ||(pCurTools->ClassID==OBJCLASS_DUMMY))) 
	    	return false;
...
void __fastcall CLevelTools::MouseWheel		(TShiftState Shift)
{
	if(pCurTools&&pCurTools->pCurControl) 
		pCurTools->pCurControl->ToolsMouseWheel(Shift);
}

void __fastcall CLevelTools::MouseWheelUp	(TShiftState Shift)
{
	if(pCurTools&&pCurTools->pCurControl) 
		pCurTools->pCurControl->ToolsMouseWheelUp(Shift);
}

void __fastcall CLevelTools::MouseWheelDown	(TShiftState Shift)
{
	if(pCurTools&&pCurTools->pCurControl) 
		pCurTools->pCurControl->ToolsMouseWheelDown(Shift);
}
...
{
	needRecalcBeforeRedraw_ = true; // Since some instances have their ::OnFrame already done - we set it so they have chance to recalc again in ::Render
...
void __fastcall CLevelTools::RealSetTarget   (ObjClassID tgt,int sub_tgt,bool bForced)
{
	...
	Msg("Current Edit Mode ID = %u : %s", CurrentClassID(), pCurTools->ClassDesc());
}
//---------------------------------------------------------------------------
...
bool CLevelTools::IfModified()
{
	...
	default:{ Msg("State = %u", UI->GetEState()); THROW; }
    }
    return false;
}
//---------------------------------------------------------------------------
...
{
	if (needRecalcBeforeRedraw_) needRecalcBeforeRedraw_ = false;

	Scene->OnFrame		(TimeDelta());
    EEditorState est 	= UI->GetEState();
