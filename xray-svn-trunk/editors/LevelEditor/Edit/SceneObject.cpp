...
bool CSceneObject::IsRender()
{
	...
	bool is_rendered = inherited::IsRender();

	if (is_rendered)
		Scene->visibleStaticObject_.push_back(this);
...
{
    if (m_iBlinkTime>0){
        if (m_iBlinkTime>(int)EngineTimeU()){
        	int alpha = iFloor(sqrtf(float(m_iBlinkTime - EngineTimeU()) / BLINK_TIME) * 64);
...
{
	m_BlinkSurf		= surf;
    m_iBlinkTime	= EngineTimeU() + BLINK_TIME + TimeDeltaU();
}
//----------------------------------------------------
