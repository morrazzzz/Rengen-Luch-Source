...
void CCustomObject::AnimationUpdate(float t, bool is_loading_in_progress)
{
    Fvector R,P,r;   
...
//    Log("speed: ",speed);
    ...
    if (m_CO_Flags.is(flCameraView) && !is_loading_in_progress) // check for that this is not a save loading to not touch camera after undo
...
void CCustomObject::AnimationOnFrame()
{
    ...
    if (Selected()&&m_MotionParams->bPlay)
    {
        ...
        m_MotionParams->Update	(TimeDelta(), 1.f, true);
    }
}
