...
#define MAX_WALLMARK_COUNT			5000
...
bool ESceneWallmarkTools::Valid()
{
	bool res = !marks.empty();
	if(!res)
		ELog.Msg (mtInformation,"No Wallmarks");
	
	return !marks.empty();
}

bool ESceneWallmarkTools::IsNeedSave()
{
	return marks.size();
}
...
{
	bool operator()(const ESceneWallmarkTools::wallmark*& x)
	{
		return x == 0;
	}
};
