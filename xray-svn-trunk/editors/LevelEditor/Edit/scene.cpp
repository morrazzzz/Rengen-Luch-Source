...
EScene::~EScene()
{
	...
	VERIFY2(totalCObjectsInstances_ == 0, make_string("Objects left after scene is destroyed. totalCObjectsInstances_ = %i; Probably because of Copy\Pasting. contact Rafa", totalCObjectsInstances_)); // Verify no objects were lost in memory - otherwise we need to debug
}
...
bool EScene::CheckIfModifiedAndAsk(LPCSTR mesage)
{
	if (locked()){ 
        ELog.DlgMsg( mtError, "Scene sharing violation" );
    ...
    if (m_RTFlags.is(flRT_Unsaved) && (ObjCount()||!Tools->GetEditFileName().IsEmpty())){
        int mr = ELog.DlgMsg(mtConfirmation, mesage);
...
{
   	ESceneCustomOTools* t 			= Scene->GetOTools(OBJCLASS_LIGHT);
    if(!t)
	    return;

	ESceneLightTools* lt = dynamic_cast<ESceneLightTools*>(t); 
	VERIFY(lt);
...
void EScene::CalculateVertsFaces(u32& vertexes, u32& faces)
{
	if (LTools->CurrentClassID() == OBJCLASS_SCENEOBJECT){
		...
		for (ObjectIt it = lst.begin(); it != lst.end(); it++)
		{
			CSceneObject* scene_object = (CSceneObject*)(*it);

			if (scene_object->Selected())
			{
				vertexes += scene_object->GetVertexCount();
				faces += scene_object->GetFaceCount();
			}
		}
	}
}
