...
#include "../../Ecore/EditorRender/DetailFormat.h"
#include "../ECore/Editor/EDetailModel.h"
#include "../ECore/Editor/Library.h"
#include "customobject.h"
#include "../../Ecore/EditorRender/DetailManager.h"
...
public:
    ...
    virtual bool		Valid					()
	{
		bool res = dtSlots||objects.size()||m_Base.Valid()||m_SnapObjects.size();
		
		if(!res)
		{
			ELog.Msg (mtError, "DETAILS: No Details:");
			
			if(!dtSlots)
				ELog.Msg (mtError, "DETAILS: No detail slots");
			
			if(!objects.size())
				ELog.Msg (mtError, "DETAILS: No detail objects");
			
			if(!objects.size())
				ELog.Msg (mtError, "DETAILS: m_Base.Valid() = false");
			
			if(!m_SnapObjects.size())
				ELog.Msg (mtError, "DETAILS: No snap objects");
		}
    ...
    // IO
    virtual bool   		IsNeedSave				(){return ValidSilent();}
