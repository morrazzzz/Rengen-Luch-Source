...
Fmatrix	previewRotation_;
bool previewObjectEnabled_ = false;
float previewScale_ = 0.2f;
...
BOOL ESceneObjectTools::FrustumPick(ObjectList& lst, const CFrustum& frustum)
{
	Msg("Picking static objects. visible = %u", Scene->visibleStaticObject_.size());

	for (u32 i = 0; i < Scene->visibleStaticObject_.size(); i++)
	{
		CSceneObject* s_obj = Scene->visibleStaticObject_[i];

		if (s_obj)
		{
			if (s_obj->Visible() && s_obj->FrustumPick(frustum))
				lst.push_back(s_obj);
		}
	}
...
BOOL ESceneObjectTools::RayPick(CCustomObject*& object, float& distance, const Fvector& start, const Fvector& direction, SRayPickInfo* pinf)
{
	object = 0;
	...
	for (u32 i = 0; i < Scene->visibleStaticObject_.size(); i++)
	{
		...
		if (s_obj)
		{
			if (s_obj->Visible() && s_obj->RayPick(distance, start, direction, pinf))
				object = s_obj;
		}
	}
...
void ESceneObjectTools::OnRender(int priority, bool strictB2F)
{
	inherited::OnRender(priority, strictB2F);

	if (previewObjectEnabled_ && LTools->CurrentClassID() == OBJCLASS_SCENEOBJECT)
	{
		if (previewObject_)
		{
			...
			previewObject_->Render(M, 1, false);
		}
	}
}

void ESceneObjectTools::AnObjectSelectedInOList()
{
	if (pFrame)
	{
		LPCSTR ref_name = ((TfraObject*)pFrame)->Current();

		if (ref_name)
		{
			previewObject_ = Lib.CreateEditObject(ref_name);
...
void ESceneObjectTools::SwitchPreview()
{
