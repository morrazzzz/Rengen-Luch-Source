...
#include "../../Ecore/EditorRender/ResourceManager.h"
...
class CSceneStat
{
	...
	{
		VERIFY((ix < bb_sx) && (iz < bb_sz)); return svertices[iz*bb_sx + ix];
	}
	...
	{
		VERIFY((ix < bb_sx) && (iz < bb_sz)); return muvertices[iz*bb_sx + ix];
	}
public:
...
BOOL SceneBuilder::BuildObject(CSceneObject* obj)
{
	...
	{
		temp.sprintf("Saving scene to obj");
	}
...
BOOL SceneBuilder::BuildMUObject(CSceneObject* obj)
{
	...
	if (-1 == model_idx || m_save_as_object)
	{
		...
		{
			// build LOD
			lod_id = BuildObjectLOD(Fidentity, O, sect_num);
		...
		if(M.lod_id == (u16)-1)
			ELog.Msg(mtError, "MU object has undefined LOD id '%s'.", M.name);
		
		Msg("MU Object %s LOD id = %u", M.name, M.lod_id);
...
BOOL SceneBuilder::CompileStatic(bool b_selected_only)
{
	...
	if (!m_save_as_object)
	{
		...
		if (bResult&&!l_lods.empty())
		{
			...
			Msg("Merge LOD textures: Stage 1");
			...
			Msg("Merge LOD textures: Stage 2");
			...
			if (1 == ImageLib.CreateMergedTexture(2, images, merged_image, 512, 8192, 512, 8192, offsets, scales, rotated, remap))
			{
				...
				if (ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, "Do you want to make LEVEL LOD texture or skip it to test other building stages?. \n Creating LOD texture takes about 30 minutes") == mrYes)
				{			
					Msg("Merge LOD textures: MakeGameTexture; This stage can take up to 30 mins");
				...
				{
					Msg("Merge LOD textures: Skiped by developer");
				}

				Msg("Merge LOD textures: Stage 3");
