...
CCommandVar GenerateMisingLODS_ForScene(CCommandVar p1, CCommandVar p2)
{
	if (Scene->ObjCount() || (LUI->GetEState() != esEditScene))
	{
		if (LUI->GetEState() == esEditLibrary)
		{
			int res = ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, "Generate mising LODS that were detected while rendering scene. The Library Editor will be started (Will need to wait) and then LOD generation will be done in auto mode for = %u objects", objectsWithMisingLODS_.size());

			if (res == 6)
			{
				TfrmEditLibrary::AutoGenerateLODS();
			}
		}
		else
			ELog.DlgMsg(mtError, "Scene must be empty before editing library!");
...
CCommandVar CLevelTools::CommandEnableTarget(CCommandVar p1, CCommandVar p2)
{
    ...
    }else
    {
        if (!Scene->IfModified())
        {
            ...
            if(res)
		    	M->m_EditFlags.set(ESceneCustomMTools::flEnable,FALSE);
        }
		if (res)        	
...
{
	Scene->CheckIfModifiedAndAsk("Do you want to save map before further actions?");
}
...
CCommandVar CommandDeselectAllEditModes(CCommandVar p1, CCommandVar p2)
{
    if(!Scene->locked())
	{
...
CCommandVar CommandRestObjRot(CCommandVar p1, CCommandVar p2)
{
	if(!Scene->locked())
	{
		ObjectList lst;
...
CCommandVar CommandRestObjSc(CCommandVar p1, CCommandVar p2)
{
	if(!Scene->locked())
	{
		ObjectList lst;
...
void CLevelMain::RegisterCommands()
{
	...
	REGISTER_CMD_S	    (COMMAND_GENERATE_MISING_LODS,           	GenerateMisingLODS_ForScene);
	...
	REGISTER_CMD_S	    (COMMAND_RESET_OBJECT_ROT,          CommandRestObjRot);
	REGISTER_CMD_S	    (COMMAND_RESET_OBJECT_SC,           CommandRestObjSc);
	...
	REGISTER_CMD_SE	    (COMMAND_DESELECT_ALL,              "Selection\\Unselect Cur. E. Mode", CommandDeselectAll, false);
	REGISTER_CMD_SE	    (COMMAND_DESELECT_ALL_E_MODES,		"Selection\\Unselect All E. Modes", CommandDeselectAllEditModes, false);
...
//---------------------------------------------------------------------------
void CLevelMain::UpdateVertsFacesInfo(u32 vertexes, u32 faces)
{
	VERIFY(fraBottomBar);

    AnsiString s;
	
    s.sprintf("V: %u, F: %u", vertexes, faces);

    fraBottomBar->paV_F_info->Caption = s;
	fraBottomBar->paV_F_info->Repaint();
}
//---------------------------------------------------------------------------
