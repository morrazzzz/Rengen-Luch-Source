...
xr_vector<TElTreeItem*> foundList_;

void TfrmObjectList::ProcessFindItemInList(TElTreeItem* from, AnsiString str)
{
	if(foundList_.size())
	{
		for(u32 i = 0; i < foundList_.size(); i++)
		{
			foundList_[i]->ParentColors = true;
		}
    ...
    for (TElTreeItem* node = from; node; node = node->GetNext())
    {
        ...
        if(strstr(O->FName.c_str(), str.c_str()))
        {
            ...
            node->BkColor = clYellow;

			foundList_.push_back(node);
         }
	}                
}
...
{
	return; // dont clear the highlighting, if edit box is unfocused
