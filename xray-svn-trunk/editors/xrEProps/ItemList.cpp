...
void TItemList::ClearParams(TElTreeItem* node)
{
	if (node){
        ...
        // real clear
		for (ListItemsIt it = itemsList_.begin(); it != itemsList_.end(); it++)
    		xr_delete			(*it);
...
void __fastcall TItemList::AssignItems(ListItemsVec& items, bool full_expand, bool full_sort)
{
	...
	WideString store_title;

	if (Parent)
		store_title = tvItems->HeaderSections->Item[0]->Text;
	else
		store_title = Caption;
	
	u32 stat_update_delta = 0;
	u32 loaded_count = 0;
	...
	for (ListItemsIt it = itemsList_.begin(); it != itemsList_.end(); it++)
	{
		...
		else
		{
			...
			if (!prop->item)
			{
				std::string debug_msg = make_string("TItemList::AssignItems : !Duplicate item name found: '%s'", *prop->key);
				ETOOLS::AlertMsg(debug_msg.c_str());
		...
		if (stat_update_delta > 50)
		{
			string128 info;
			sprintf(info, "Ready %u of %u", loaded_count, itemsList_.size());

			if (Parent)
				tvItems->HeaderSections->Item[0]->Text = info;
			else
				Caption = info;
	...
	if (Parent)
		tvItems->HeaderSections->Item[0]->Text = store_title;
	else
		Caption = store_title;
	
	// some kind of items numeration
	if(0) // not good, since some parts of code use the node name for creating file name and etc.
    for (TElTreeItem* item = tvItems->Items->GetFirstNode(); item; item = item->GetNext())
	{
        if (!item->Hidden)
		{
			...
			if(prop && AnsiString(item->Text).Length() < 128) // if this is a refference item - numerate it
			{
				lineNumberCounter_++;
				string128 text;
			
				xr_sprintf(text, sizeof(text), "%i - %s", lineNumberCounter_, AnsiString(item->Text).c_str());
				
				item->Text = text;
			}
			else // if this is a filter item - reset counter
				lineNumberCounter_ = 0;
		}
    }

	paStatus->Caption = AnsiString(" Items count: ") + itemsList_.size();
}
//---------------------------------------------------------------------------
...
//	for (it=itemsList_.begin(); it!=itemsList_.end(); it++){
//		ListItem* prop 		= *it;
