		...
		for (u32 _mx=0; _mx<dm_cache1_line; _mx++)
		{
			...
			for (u32 _i=0; _i < dwCC ; _i++){
				...
				if (CurrentFrame() > S.frame)
				{
					...
					S.frame = CurrentFrame() + Random.randI(15, 30);
...
void __stdcall	CDetailManager::MT_CALC		()
{
	...
	if (m_frame_calc != CurrentFrame())
		if ((m_frame_rendered + 1) == CurrentFrame()) //already rendered
		{
#ifndef _EDITOR
			Fvector		EYE				= RDEVICE.vCameraPosition_saved;
#else
			...
			m_frame_calc = CurrentFrame();
		}
	MT.Leave					        ();


#if defined (MEASURE_MT) && !defined (_EDITOR)
