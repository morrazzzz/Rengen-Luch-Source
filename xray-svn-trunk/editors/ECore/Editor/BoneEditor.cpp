...
void CBone::ShapeScale(const Fvector& _amount)
{
	...
	case SBoneShape::stBox:
	{
		...
		if
			(shape.box.m_halfsize.x < EPS) shape.box.m_halfsize.x = EPS;
		if
			(shape.box.m_halfsize.y < EPS) shape.box.m_halfsize.y = EPS;
		if
			(shape.box.m_halfsize.z < EPS) shape.box.m_halfsize.z = EPS;
	}
	break;
	case SBoneShape::stSphere:
		shape.sphere.R += _amount.x;
		if
			(shape.sphere.R < EPS) shape.sphere.R = EPS;
		break;
	case SBoneShape::stCylinder:
		shape.cylinder.m_height += _amount.z;
		if
			(shape.cylinder.m_height < EPS) shape.cylinder.m_height = EPS;
		shape.cylinder.m_radius += _amount.x;
		if
			(shape.cylinder.m_radius < EPS) shape.cylinder.m_radius = EPS;
		break;
	}
}
...
void CBone::ShapeRotate(const Fvector& _amount)
{
	...
	Fmatrix _IT; _IT.invert(_LTransform());
	if
		(Tools->GetSettings(etfCSParent)) _IT.transform_dir(amount);
	switch (shape.type)
	{
	case SBoneShape::stBox:
	{
...
void CBone::AntirevertBones()
{
	...
	if
		(IK_data.limits[1].limit.x > 0)
		IK_data.limits[1].limit.x *= -1.0f;
	if
		(IK_data.limits[1].limit.y < 0)
