...
void __fastcall TfrmImageLib::EditLib(AnsiString& title, bool bImport)
{
	if (!form){
		...
		form->ebSyncSelected->Enabled = !bImport;
...
ETextureThumbnail* TfrmImageLib::FindUsedTHM(const shared_str& name)
{
	for (THMIt it = m_THM_Used.begin(); it != m_THM_Used.end(); it++)
		if (0 == xr_strcmp((*it)->SrcName(), name)) return *it;
	return 0;
}
...
void __fastcall TfrmImageLib::SyncSelectedTextures()
{
	E_IMGE = true;
	VERIFY(!bReadonlyMode);
	...
	if (0 != m_ItemList->GetSelected(items))
	{
		for (RStringVecIt it = items.begin(); it != items.end(); it++)
		{
			...
			FS_FileSetIt itt = texture_map.find(FS_File(it->c_str()));
			
			R_ASSERT(itt != texture_map.end());
			
			if(itt != texture_map.end())
				textures_to_sync.insert(*itt);
		}
	}
	
	if(textures_to_sync.size())
	{
		LockForm();
		
		AStringVec modif;
		ImageLib.SynchronizeTextures(true, true, true, &textures_to_sync, &modif);
...
void TfrmImageLib::InitItemsList()
{
	...
	for (; it != _E; it++)
	{
		...
		I->tag = EDevice.Resources->m_textures_description.GetTextureType(it->name.c_str());
...
void __fastcall TfrmImageLib::ebSyncSelectedClick(TObject*)
{
	if (bFormLocked) return;

	SyncSelectedTextures();
}
...
void TfrmImageLib::OnItemsFocused(ListItemsVec& items)
{
	...
	paImage->Repaint();
	m_ItemProps->tvProperties->MultiSelect = false;
...
{
    const ListItemsVec&	items	= m_ItemList->GetItemsList();
