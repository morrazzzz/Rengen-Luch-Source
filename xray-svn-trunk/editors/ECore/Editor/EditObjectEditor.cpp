...
void CEditableObject::Render(const Fmatrix& parent, int priority, bool strictB2F)
{
	...
	{
		R_ASSERT2(false, "CEditableObject::Render Error");
	}
}
...
void CEditableObject::RenderLOD(const Fmatrix& parent)
{
	...
	}
	{
		...
		for (int i = 0; i < 4; i++)
		{
			LOD[i].p.set(p[i]); LOD[i].t.set(t[i]);
		}
		RCache.set_xform_world(parent);
...
ECORE_API xr_vector<shared_str> objectsWithMisingLODS_;
...
void CEditableObject::DefferedLoadRP()
{
	try
	{
		...
		if (m_Flags.is(eoUsingLOD) && IsMUStatic())
		{
			// создать LOD shader
			...
			{
				// Add object to list
				bool alreadty_in_list = false;
				for (u32 i = 0; i < objectsWithMisingLODS_.size(); i++)
				{
					if (!xr_strcmp(objectsWithMisingLODS_[i].c_str(), l_name.c_str()))
						alreadty_in_list = true;
				}

				if (!alreadty_in_list)
					objectsWithMisingLODS_.push_back(m_LibName.c_str());

				Msg("LOD Texture [%s] was not found - Using dummy LOD texture DEBUG_NO_LOD_TEXTURE.dds", fname.c_str());

				l_name = "DEBUG_NO_LOD_TEXTURE";
				fname = "DEBUG_NO_LOD_TEXTURE.dds";

				R_ASSERT2(FS.exist(_game_textures_, fname.c_str()), make_string("Can't find DEBUG_NO_LOD_TEXTURE.dds, create a 128x64 texture and name it. It will be better for you. You will be able to see objects without LOD texture"));
	...
	{
		R_ASSERT2(false, "CEditableObject::DefferedLoadRP() Error");
	}
}
...
#include "..\EditorRender\Blenders\Blender.h"
...
bool CEditableObject::CheckShaderCompatible()
{
	...
	for (SurfaceIt s_it = m_Surfaces.begin(); s_it != m_Surfaces.end(); s_it++)
	{
		...
		if (!B || !C){
			ELog.Msg(mtError, "Object '%s': invalid or missing shader [Editor shader:'%s', Compiler shader(shaders_xrlc.xr):'%s']", GetName(), (*s_it)->_ShaderName(), (*s_it)->_ShaderXRLCName());
			bRes = false;
		}
		else{
