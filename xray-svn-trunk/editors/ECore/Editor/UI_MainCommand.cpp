...
#include "../EditorRender/ResourceManager.h"
...
float keyboardCamSpeed_ = 25.0f;
bool cmaSpeedUpOn_ = false;
bool cmaSpeedDownOn_ = false;
...
CCommandVar 	TUI::CommandRenderResize(CCommandVar p1, CCommandVar p2)
{
	...
	else
	{
		m_D3DWindow->Left = 8;
		m_D3DWindow->Top = 8;
		m_D3DWindow->Width = m_D3DPanel->Width - 16;
		m_D3DWindow->Height = m_D3DPanel->Height - 16;
	}
	UI->RedrawScene();
	return 1;
}
...
{
	float speed = keyboardCamSpeed_ * EDevice.m_Camera.GetFlySpeed();
...
CCommandVar CommandCamForward(CCommandVar p1, CCommandVar p2)
{
	EDevice.m_Camera.Pan(0, -GetMovementSpeed());
...
CCommandVar CommandCamBackward(CCommandVar p1, CCommandVar p2)
{
	EDevice.m_Camera.Pan(0, GetMovementSpeed());
...
CCommandVar CommandCamLeft(CCommandVar p1, CCommandVar p2)
{
	EDevice.m_Camera.Pan(-GetMovementSpeed(), 0);
...
CCommandVar CommandCamRight(CCommandVar p1, CCommandVar p2)
{
	EDevice.m_Camera.Pan(GetMovementSpeed(), 0);
...
CCommandVar CommandCamUp(CCommandVar p1, CCommandVar p2)
{
	EDevice.m_Camera.Scale(-GetMovementSpeed());
...
CCommandVar CommandCamDown(CCommandVar p1, CCommandVar p2)
{
	EDevice.m_Camera.Scale(GetMovementSpeed());
...
CCommandVar CommandCamSpeedUp(CCommandVar p1, CCommandVar p2)
{
	cmaSpeedUpOn_ = !cmaSpeedUpOn_;
	cmaSpeedDownOn_ = false;

	return TRUE;
}

CCommandVar CommandCamSpeedDown(CCommandVar p1, CCommandVar p2)
{
	cmaSpeedUpOn_ = false;
	cmaSpeedDownOn_ = !cmaSpeedDownOn_;
...
void TUI::RegisterCommands()
{
	...
	REGISTER_CMD_SE(COMMAND_CAM_F, "Scene\\Camera Move Forward", CommandCamForward, false);
	REGISTER_CMD_SE(COMMAND_CAM_B, "Scene\\Camera Move Backward", CommandCamBackward, false);
	REGISTER_CMD_SE(COMMAND_CAM_L, "Scene\\Camera Move Left", CommandCamLeft, false);
	REGISTER_CMD_SE(COMMAND_CAM_R, "Scene\\Camera Move Right", CommandCamRight, false);
	REGISTER_CMD_SE(COMMAND_CAM_U, "Scene\\Camera Move Up", CommandCamUp, false);
	REGISTER_CMD_SE(COMMAND_CAM_D, "Scene\\Camera Move Down", CommandCamDown, false);

	REGISTER_CMD_SE(COMMAND_CAM_SPEED_UP, "Scene\\Camera Speed Up", CommandCamSpeedUp, false);
	REGISTER_CMD_SE(COMMAND_CAM_SPEED_DOWN, "Scene\\Camera Speed Down", CommandCamSpeedDown, false);
}
