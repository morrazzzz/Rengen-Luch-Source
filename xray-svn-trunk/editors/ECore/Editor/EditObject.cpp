...
bool CSurface::CreateImageData()
{
	...
	bool sexful = ImageLib.LoadTextureData(*m_ImageData->name, m_ImageData->layers.back(), m_ImageData->w, m_ImageData->h);
...
CSurface*	CEditableObject::FindSurfaceByName(const char* surf_name, int* s_id){
	for (SurfaceIt s_it = m_Surfaces.begin(); s_it != m_Surfaces.end(); s_it++)
		...
		{ 
			if (s_id) *s_id = s_it - m_Surfaces.begin(); return *s_it; 
		}
	return 0;
}
