...
extern bool cmaSpeedUpOn_;
extern bool cmaSpeedDownOn_;
...
void CUI_Camera::Update(float dt)
{
	if (m_bMoving)
	{
		...
		if ((m_Style == csFreeFly) && (bLeftDn || bRightDn) && !(bLeftDn&&bRightDn))
		{
			...
			float result = m_FlySpeed * dt;

			if (cmaSpeedUpOn_)
				result *= 4.f;
			else if (cmaSpeedDownOn_)
				result /= 4.f;
...
bool CUI_Camera::Process(TShiftState Shift, int dx, int dy)
{
	if (m_bMoving)
	{
		...
		if (xx != 0.f || yy != 0.f)
		{
			...
			case csFreeFly:
			{	
				if (m_Shift.Contains(ssLeft) || m_Shift.Contains(ssRight) || m_Shift.Contains(ssAlt))
					Rotate(xx, yy);
