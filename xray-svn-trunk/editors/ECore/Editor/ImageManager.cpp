...
{
	Msg("MakingGameTexture 1: %s", game_name);
...
{
	Msg("MakingGameTexture 2: %s", game_name);
	...
	for (; it != _E; it++)
	{
		...
		if (bForceGame || (sync_game&&bGame))
		{
			...
			if (IsValidSize(w, h))
			{
				...
				// if base tga texture has been changed in terms of size or alpha - update thm file, to avoid desync in sdk and game
				if(THM->_Width() != w || THM->_Height() != h || THM->_Alpha() != a)
				{
					Msg("Fixing THM file '%s'", THM->Name() ? THM->Name() : "");
					
					FMT.width = w;
					FMT.height = h;
					FMT.flags.set(STextureParams::flHasAlpha, true);
					
					Msg("Saving THM file '%s'", THM->Name() ? THM->Name() : "");
		...
		if (bUpdated)
		{
			...
			if (bForceBaseAge)
			{
				...
				FS.set_file_age(tga_fn, age, true);
				FS.set_file_age(thm_fn, age, true);
				FS.set_file_age(dds_fn, age, true);
			}
			else
			{
				FS.set_file_age(tga_fn, m_age, true);
				FS.set_file_age(thm_fn, m_age, true);
				FS.set_file_age(dds_fn, m_age, true);
			}
		}
	}
...
{
	//types
	ltx_ini->set_override_names(true);
	ltx_ini->w_u32("types", base_name, fmt.type);
	ltx_ini->set_override_names(false);
}
...
void CImageManager::RemoveTexture(LPCSTR fname, EItemType type, bool& res)
{
	if (TYPE_FOLDER == type)
	{
		...
		if (FS.exist(_textures_, src_name.c_str())){
			...
			res = false;
			int cnt = 0;
			while ((false == (res = ltx_ini->save_as())) && (cnt++ < 10))
			{
				Sleep(1000);
			}
