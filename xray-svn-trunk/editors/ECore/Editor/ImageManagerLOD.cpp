...
bool CImageManager::CreateLODTexture(CEditableObject* OBJECT, U32Vec& lod_pixels, U32Vec& nm_pixels, u32 tgt_w, u32 tgt_h, int _samples, int quality)
{
	// build hemi light
	BLVec						simple_hemi;
	// fill simple hemi
	...
	for (SurfaceIt surf_it = OBJECT->Surfaces().begin(); surf_it != OBJECT->Surfaces().end(); surf_it++){
		...
		{
			if (!surf->CreateImageData())
			{
				ELog.Msg(mtError, "Can't create LOD for object '%s'. Probably because of mising texture", OBJECT->GetName());
				return false;
			}
		}
	}
	...
	for (SurfaceIt surf_itt = OBJECT->Surfaces().begin(); surf_itt != OBJECT->Surfaces().end(); surf_itt++)
	{
		CSurface* surf = *surf_itt;
		if (surf->m_ImageData)
		{
...
bool CImageManager::CreateLODTexture(CEditableObject* OBJECT, LPCSTR tex_name, u32 tgt_w, u32 tgt_h, int samples, int age, int quality)
{
	U32Vec 						lod_pixels, nm_pixels;

	bool result = CreateLODTexture(OBJECT, lod_pixels, nm_pixels, tgt_w, tgt_h, samples, quality);
	...
	FS.set_file_age(out_name, age, true);
