...
#include "EditorRender/dxRenderFactory.h"
...
void CEditorRenderDevice::FrameMove()
{
	...
	timeDelta_ = 0.1f * timeDelta_ + 0.9f*fPreviousFrameTime;			// smooth random system activity - worst case ~7% error
	if (timeDelta_ > .1f) timeDelta_ = .1f;									// limit to 15fps minimum

	timeGlobal_ = TimerGlobal.GetElapsed_sec(); //float(qTime)*CPU::cycles2seconds;
	timeGlobalU_ = TimerGlobal.GetElapsed_ms();	//u32((qTime*u64(1000))/CPU::cycles_per_second);
	timeDeltaU_ = iFloor(timeDelta_*1000.f + 0.5f);
	timeContinual_ = timeGlobalU_;

	m_Camera.Update(timeDeltaU_);
...
float CEditorRenderDevice::time_factor()
{
