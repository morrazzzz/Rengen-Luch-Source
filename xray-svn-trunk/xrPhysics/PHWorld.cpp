...
#include "../xr_3da/xrgame/PHSynchronize.h"
#include "../xr_3da/xrgame/phnetstate.h"
...
void CPHWorld::OnFrame()
{
	...
	// Msg									("------------- physics: %d / %d",u32(CurrentFrame()),u32(m_steps_num));
...
#ifdef MEASURE_MT
	const CRenderDevice& device = static_cast<CRenderDevice&>(inl_ph_world().Device());
	device.Statistic->mtPhysicsTime_ += measure_mt.GetElapsed_sec();
#endif
}
...
void CPHWorld::FrameStep(dReal step)
{
	...
	start_time = EngineTimeU();// - u32(m_frame_time*1000);
