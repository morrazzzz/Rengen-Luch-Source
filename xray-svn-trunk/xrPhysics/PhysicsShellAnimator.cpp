...
void	CPhysicsShellAnimator::	CreateJoints( LPCSTR controled )
{
		...
		for(u16 i = 0 ; nb > i ; ++i )
		{
			...
			VERIFY2( bid != BI_NONE, make_string( "shell_animation - controled bone %s not found! object: %s, model: %s", n, obj->NameObject(), obj->ObjectNameVisual()) );
			CPHElement *e = smart_cast<CPHElement*>( m_pPhysicsShell->get_Element( bid ) );
			VERIFY2( e, make_string( "shell_animation - controled bone %s has no physics collision! object: %s, model: %s", n, obj->NameObject(), obj->ObjectNameVisual() ) );
			CreateJoint( e );
		}
}
...
void CPhysicsShellAnimator::OnFrame()
{
	...
	for (xr_vector<CPhysicsShellAnimatorBoneData>::iterator i=m_bones_data.begin();i!=m_bones_data.end();i++)
	{
		...
		m_pPhysicsShell->PKinematics()->CalculateBones(force_recalc);
