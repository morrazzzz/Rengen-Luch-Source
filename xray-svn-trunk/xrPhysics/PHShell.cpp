...
void CPHShell::SetCallbacks()
{
	...
	struct set_bone_reference
	{
		...
		set_bone_reference() = default;
		set_bone_reference(set_bone_reference&) = delete;
		set_bone_reference& operator=(set_bone_reference&) = delete;
...
void CPHShell::SetGravity_K(float K)
{
	ELEMENT_I i, e;
	i = elements.begin(); e = elements.end();
	for (; i != e; ++i)
...
float CPHShell::GetGravity_K()
{
	if (elements.empty())
		return	(false);

	VERIFY(elements.front());
	return		(elements.front()->GetGravity_K());
}
