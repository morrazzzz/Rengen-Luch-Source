...
void CPHObject::spatial_move_intern()
{
	get_spatial_params();
...
void	CPHObject::		CollideDynamics					()
{
	...
	for(;i!=e;++i)
	{
		...
		if(obj2==this || !obj2->m_flags.test(st_dirty) || !obj2->dSpacedGeom())
			continue;
...
void CPHObject::spatial_register_intern()
{
	get_spatial_params();
