...
CPhysicsShell*	P_build_Shell( IPhysicsShellHolder* obj, bool not_active_state,BONE_P_MAP* p_bone_map, LPCSTR	fixed_bones )
{
	...
	if(fixed_bones)
	{
		...
		for (int i=0 ;i<count; ++i) 
		{
			...
			_GetItem					(fixed_bones,i,fixed_bone)			;
			u16 fixed_bone_id=pKinematics->LL_BoneID(fixed_bone)			;
			R_ASSERT2(BI_NONE!=fixed_bone_id,make_string("[%s] wrong fixed bone [%d]", obj->NameObject(), fixed_bone_id));
...
CPhysicsShell*	P_build_Shell( IPhysicsShellHolder* obj, bool not_active_state, LPCSTR	fixed_bones )
{
	...
	if(fixed_bones){
		...
		for (int i=0 ;i<count; ++i){
			...
			R_ASSERT2(BI_NONE!=f_bones.back(),make_string("[%s] wrong fixed bone %d", obj->NameObject(), f_bones.back()));
		}
	}
...
void	phys_shell_verify_object_model( IPhysicsShellHolder& O )	
{
	...
	VERIFY2( K, make_string( "Can not create physics shell for object %s, model %s is not skeleton", O.NameObject(), O.ObjectNameVisual() ) );

	VERIFY2( has_physics_collision_shapes( *K ), make_string( "Can not create physics shell for object %s, model %s has no physics collision shapes set", O.NameObject(), O.ObjectNameVisual() )/*+ make_string("\n object dump: \n") + dbg_object_full_dump_string( &O )*/  );
...
bool __stdcall	can_create_phys_shell( string1024 &reason, IPhysicsShellHolder& O )
{
	...
	if(!K)
	{
		xr_strcpy( reason,	make_string( "Can not create physics shell for object %s, model %s is not skeleton", O.NameObject(), O.ObjectNameVisual() ).c_str() );
		return false;
	}
	if(!has_physics_collision_shapes( *K ))
	{
		xr_strcpy( reason,	make_string( "Can not create physics shell for object %s, model %s has no physics collision shapes set", O.NameObject(), O.ObjectNameVisual() ).c_str() );
		return false;
	}
