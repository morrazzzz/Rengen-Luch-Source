	...
	void	Collector::calc_adjacency	(xr_vector<u32>& dest)
	{
...

		{
			...
			for ( ; EI != EE; ++EI) {
				...
				if ((*EI).vertex_id0 != (*J).vertex_id0)
					continue;

				if ((*EI).vertex_id1 != (*J).vertex_id1)
					continue;

				dest[(*EI).face_id*3 + (*EI).edge_id]	= (*J).face_id;
				dest[(*J).face_id*3 + (*J).edge_id]	= (*EI).face_id;
			}
		}
#	if 0
...
//		flags.reserve	(apx_faces);
	...
	void CollectorPacked::add_face_ptr(const Fvector & v0, const Fvector & v1, const Fvector & v2, void * ptr)
	{
		TRI T;
		T.verts[0] = VPack(v0);
		T.verts[1] = VPack(v1);
