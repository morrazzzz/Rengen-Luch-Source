...
inline_ BOOL AABBTreeCollider::TriBoxOverlap(const Point& center, const Point& extents)
{
	...
	float minn,maxx;
	// Find min, max of the triangle in x-direction, and test for overlap in X
	FINDMINMAX(v0.x, v1.x, v2.x, minn, maxx);
	if(minn >extents.x || maxx <-extents.x) return FALSE;
	...
	FINDMINMAX(v0.y, v1.y, v2.y, minn, maxx);
	if(minn >extents.y || maxx <-extents.y) return FALSE;
	...
	FINDMINMAX(v0.z, v1.z, v2.z, minn, maxx);
	if(minn >extents.z || maxx <-extents.z) return FALSE;
#endif
...
inline_ BOOL OBBCollider::TriBoxOverlap()
{
	...
	// Find min, max of the triangle in x-direction, and test for overlap in X
	FINDMINMAX(v0.x, v1.x, v2.x, minn, maxx);
	if(minn >mBoxExtents.x || maxx <-mBoxExtents.x) return FALSE;

	FINDMINMAX(v0.y, v1.y, v2.y, minn, maxx);
	if(minn >mBoxExtents.y || maxx <-mBoxExtents.y) return FALSE;

	FINDMINMAX(v0.z, v1.z, v2.z, minn, maxx);
	if(minn >mBoxExtents.z || maxx <-mBoxExtents.z) return FALSE;
#endif
	// 2) Test if the box intersects the plane of the triangle
	// compute plane equation of triangle: normal*x+d=0
	// ### could be precomputed since we use the same leaf triangle several times
