...
public:
	...
	*/
	...
	void			walk(const ISpatial_NODE* N, const Fvector& n_C, const float n_R, xr_vector<ISpatial*>& R)
	{
		...
		{
			ISpatial*		S	= *_it;
			if (mask!=(S->spatial.s_type&mask))	continue;
...
void	ISpatial_DB::q_ray(xr_vector<ISpatial*>& R, u32 _o, u32 _mask_and, const Fvector&	_start, const Fvector&	_dir, float _range)
{
	...
	if (_o & O_ONLYFIRST)
	{
		if (_o & O_ONLYNEAREST) { walker<false, true, true>		W(this, _mask_and, _start, _dir, _range);	W.walk(m_root_node, m_spatial_db_center, m_spatial_db_bounds, R); }
		else { walker<false, true, false>	W(this, _mask_and, _start, _dir, _range);	W.walk(m_root_node, m_spatial_db_center, m_spatial_db_bounds, R); }
	}
	else {
		if (_o & O_ONLYNEAREST) { walker<false, false, true>	W(this, _mask_and, _start, _dir, _range);	W.walk(m_root_node, m_spatial_db_center, m_spatial_db_bounds, R); }
		else { walker<false, false, false>	W(this, _mask_and, _start, _dir, _range);	W.walk(m_root_node, m_spatial_db_center, m_spatial_db_bounds, R); }
	}
