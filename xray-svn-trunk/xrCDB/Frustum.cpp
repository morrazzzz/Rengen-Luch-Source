	...
	for (int i=0; i<p_count; i++, bit<<=1)
	{
		if (test_mask&bit) {
			...
			if (_abs(cls)>=r) test_mask&=~bit;			// fully - no need to test this plane
				...
				else if (fcvNone== vis){ test_mask=0; return fcvNone;	}	// none - return
			}
		}
	}
	return test_mask ? fcvPartial:fcvFully;
...
void CFrustum::CreateFromParams(float fov, Fvector base_pos, Fvector yaw_pitch_roll, float far_palne_dist, float near_plane_dist, float aspect, u32 mask)
{
	Fmatrix m;

	m.setHPB(yaw_pitch_roll.x, yaw_pitch_roll.y, yaw_pitch_roll.z);
	m.c = base_pos;

	Fmatrix m_projection, m_view, m_full;
	m_view.build_camera_dir(m.c, m.k, m.j);

	m_projection.build_projection(deg2rad(fov), aspect, near_plane_dist, far_palne_dist);
	
	m_full.mul(m_projection, m_view);

	CreateFromMatrix(m_full, mask);
}
...
sPoly*	CFrustum::ClipPoly(sPoly& srcc, sPoly& destt) const
{
	sPoly*	src		= &destt;
	sPoly*	dest	= &srcc;
	for (int i=0; i<p_count; i++)
