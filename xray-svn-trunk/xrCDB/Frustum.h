...
public:
	...
	void			CreateFromPlanes	(Fplane* p,		int count);
					// fov in degrees; start position; yaw_pitch_roll = rotation, front tilt, side tilt; range/dist/far_plane;
	void			CreateFromParams	(float fov, Fvector base_pos, Fvector yaw_pitch_roll, float far_palne_dist, float near_plane_dist = 0.1f, float aspect = 1.f, u32 mask = FRUSTUM_P_LRTB | FRUSTUM_P_FAR);

	sPoly*			ClipPoly			(sPoly& srcc, sPoly& destt) const;
