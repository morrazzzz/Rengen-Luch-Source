...
{
	if (dwFrame == CurrentFrame())return;
#ifndef _EDITOR
	...
	// color
    float tf		= g_pGamePersistent->Environment().environmentTimeFactor_;
...
//	LFState			previous_state = m_State;
    ...
    case lfsShow: 
        m_StateBlend 	= m_Current?(m_StateBlend + m_Current->m_StateBlendUpSpeed * TimeDelta() * tf):1.f+EPS;
        if (m_StateBlend>=1.f) m_State=lfsIdle;
    break;
    case lfsHide: 
        m_StateBlend 	= m_Current?(m_StateBlend - m_Current->m_StateBlendDnSpeed * TimeDelta() * tf):0.f-EPS;
        if (m_StateBlend<=0.f){ 	
            m_State		= lfsShow;
            m_Current	= desc;
	        m_StateBlend= m_Current?m_Current->m_StateBlendUpSpeed * TimeDelta() * tf:0;
        }
    break;
    }
//	Msg				("%6d : [%s] -> [%s]", CurrentFrame(), state_to_string(previous_state), state_to_string(m_State));
    ...
    if (Tools->RayPick(Device.m_Camera.GetPosition(),vSunDir,dist))
		fBlend = fBlend - BLEND_DEC_SPEED * TimeDelta();
	else
		fBlend = fBlend + BLEND_INC_SPEED * TimeDelta();
#else
	...
	//blend_lerp(fBlend,TP.vis,BLEND_DEC_SPEED,TimeDelta());
	blend_lerp(fBlend,fVisResult,BLEND_DEC_SPEED,TimeDelta());
