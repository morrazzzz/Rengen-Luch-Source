...
CCF_Skeleton::CCF_Skeleton(CObject* O) : ICollisionForm(O, cftObject)
{
	if (!O->Visual())
	{
		R_ASSERT3(false, "Can't create skeleton without Visual", O->SectionNameStr());

		return;
	}

	IKinematics* K = PKinematics(O->Visual()); R_ASSERT3(K, "Can't create skeleton without Kinematics.", *O->VisualName());
...
void CCF_Skeleton::BuildState()
{
	...
	//K->CalculateBones(); // This actualy slowdowns everything, since it interacts with critical sections. No need so much precision of bones
		...
		switch (I->type){
			case SBoneShape::stBox:{
				...
				if (!b)	{
					...
					Msg						("visual name %s",owner->VisualName().c_str());
					Msg						("object name %s",owner->ObjectName().c_str());
...
{
	protectRayQuery_.Enter();

	if (dwFrameTL != CurrentFrame())
		BuildTopLevel();
