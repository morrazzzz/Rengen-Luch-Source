...
u32 CConsole::get_mark_color( Console_mark type )
{
	...
	switch ( type )
	{
	case mark0:  color = color_rgba( 255, 216,   0, 255 ); break;
	case mark1:  color = color_rgba( 255,  94,  76, 255 ); break;
	case mark2:  color = color_rgba( 100, 100, 255, 255 ); break;
	case mark3:  color = color_rgba( 221, 116,	24, 155	); break;
	case mark4:  color = color_rgba( 193, 126, 229, 255 ); break;
	case mark5:  color = color_rgba( 155,  55, 170, 255 ); break;
	case mark6:  color = color_rgba(  25, 200,  50, 255 ); break;
	case mark7:  color = color_rgba( 184, 224,  51, 255 ); break;
...
#ifdef MEASURE_ON_FRAME
	Device.Statistic->onframe_Console_ += measure_on_frame.GetElapsed_ms_f();
#endif
}
...
{
	if ( !bVisible || Render->currentViewPort != MAIN_VIEWPORT)
	{
		return;
	}
	...
	if ( !m_disable_tips && m_tips.size() )
	{
		...
		for ( u32 i = 0; itb != ite ; ++itb, ++i ) // tips
		{
			...
			if ( i >= u32((bGame) ? VIEW_TIPS_COUNT : VIEW_TIPS_COUNT_2) - 1)
			{
				break; //for
			}
		}	
	...
	string16 q;
	itoa( log_line + scroll_delta, q, 10 );
	u32 qn = xr_strlen( q );

	pFont->SetColor( total_font_color );
	pFont->OutI(0.70f - 0.03f * qn, fMaxY - 1.0f * LDIST, "[%i] : [%d]", scroll_delta, log_line);
...
void CConsole::DrawBackgrounds( bool bGame )
{
	...
	UIRender->StartPrimitive( 6 + 12 + (((bGame) ? VIEW_TIPS_COUNT : VIEW_TIPS_COUNT_2) + 1)*6 + 12, IUIRender::ptTriList, IUIRender::pttTL );
	...
	float tips_h    = _min( m_tips.size(), (u32)((bGame) ? VIEW_TIPS_COUNT : VIEW_TIPS_COUNT_2)) * font_h;
	...
	pr.y1 = UI_BASE_HEIGHT * ((bGame) ? 0.5f : 0.825f);
	...
	if ( tips_sz > u32((bGame) ? VIEW_TIPS_COUNT : VIEW_TIPS_COUNT_2))
	{
		Frect rb, rs;
		...
		float u_height = (back_height * ((bGame) ? VIEW_TIPS_COUNT : VIEW_TIPS_COUNT_2)) / float(tips_sz);
...
void CConsole::ExecuteCommand( LPCSTR cmd_str, bool record_cmd )
{
	...
	{
		Log( "! Unknown console command: ", first);
	}
...
//		( g_pGamePersistent && g_pGamePersistent->m_pMainMenu && g_pGamePersistent->m_pMainMenu->IsActive() ))
	...
	if (pInput->our_app_has_mouse_priority())
	{
...
void CConsole::AssignCFGFile()
{
	if (Engine.External.XrGameModuleExist())
		xr_strcpy(ConfigFile, "user.ltx");
	else
		xr_strcpy(ConfigFile, "xr3da_cfg.ltx");
