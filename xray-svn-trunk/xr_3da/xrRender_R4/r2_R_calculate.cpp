...
float		r_ssaDISCARD_VP1; // special for after frame stuff (Details calc)

void CRender::CalculateRendering(ViewPort viewport)
{
	...
	for (u32 _it=0; _it<lstRenderables.size(); _it++)
	{
		...
		spatial->spatial_updatesector(CurrentFrame());

		CSector* sector = (CSector*)spatial->GetCurrentSector();

		if (0 == sector)
			continue; // disassociated from S/P structure

		VERIFY(spatial->spatial.s_type & STYPE_LIGHTSOURCE);
