...
#pragma todo("Rafa: ssao_ultra does not work without shader model 5, which is broken")
	RImplementation.o.ssao_ultra = 0;//ps_r_ssao > 3;
	if (RImplementation.o.dx10_msaa)
	...
	b_bloom_new = xr_new <CBlender_bloom_new>();
	b_dof = xr_new <CBlender_DOF>();
	b_mblur = xr_new <CBlender_MBLUR>();

	//PostEffects
	b_visor = xr_new <CBlender_Visor>();
	b_rain_drops = xr_new <CBlender_Rain_Drops>();
	b_nightvision = xr_new<CBlender_Night_Vision>();
	b_smaa = xr_new<CBlender_SMAA>();

	b_blood = xr_new<CBlender_Blood>();
	...
	if (RImplementation.o.dx10_msaa)
	{
		...
		for (int i = 0; i < bound; ++i)
		{
			...
			b_reflections_msaa[i] = xr_new<CBlender_reflections_MSAA>();
			...
			static_cast<CBlender_reflections_MSAA*>						(b_reflections_msaa[i])->SetDefine("ISAMPLE", SampleDefs[i]);
		}
	}
	...
	//	NORMAL
	{
		...
		rt_Shader_params.create(r2_RT_shader_params, vp_params_main_secondary, D3DFMT_A16B16G16R16F, SampleCount);
		...
		//Sun Shafts RTs
		rt_SunShaftsMask.create(r2_RT_SunShaftsMask, vp_params_main_secondary, D3DFMT_A8R8G8B8);
		rt_SunShaftsMaskSmoothed.create(r2_RT_SunShaftsMaskSmoothed, vp_params_main_secondary, D3DFMT_A8R8G8B8);
		rt_SunShaftsPass0.create(r2_RT_SunShaftsPass0, vp_params_main_secondary, D3DFMT_A8R8G8B8);

		
		//Bloom RTs
		rt_bloom_treshold.create(r2_RT_treshold, RtCreationParams((u32)(w / 2), (u32)(h / 2), MAIN_VIEWPORT), D3DFMT_A16B16G16R16F);
		rt_bloom_blur.create(r2_RT_texBloom2, RtCreationParams((u32)(w / 4), (u32)(h / 4), MAIN_VIEWPORT), D3DFMT_A16B16G16R16F);
		rt_bloom_final.create(r2_RT_texBloom3, RtCreationParams((u32)(w / 16), (u32)(h / 16), MAIN_VIEWPORT), D3DFMT_A16B16G16R16F);
		...
		rt_Reflections.create(r2_RT_reflections, vp_params_main_secondary, D3DFMT_A8R8G8B8);
	}

	//Sun Shafts
	s_SunShafts.create(b_sunshafts, "r3\\SunShafts");
	//Bloom
	s_bloom_new.create(b_bloom_new, "r3\\bloom_new");
	//DOF
	s_dof.create(b_dof, "r3\\depth_of_field");
	//MBLUR
	s_mblur.create(b_mblur, "r3\\motion_blur");
//PostEffects
	//Visor
	s_visor.create(b_visor, "r3\\visor_effect");
	//Rain drops
	s_rain_drops.create(b_rain_drops, "r3\\raindrops_effect");
	//Night vision
	...
	//Blood
	s_blood.create(b_blood, "r3\\blood");

	//Reflections
	s_reflections.create(b_reflections, "r3\\ogse_wet_reflections");
	...
	if (RImplementation.o.HW_smap)
	{
		...
		rt_smap_surf.create			(r2_RT_smap_surf, RtCreationParams(size, size, MAIN_VIEWPORT), D3DFMT_A16B16G16R16F);
		//rt_smap_ZB					= NULL;
	...
	//	Or make DX10 rain switch dynamic?
	{
		...
		{
			static LPCSTR SampleDefs[] = { "0", "1", "2", "3", "4", "5", "6", "7" };
			CBlender_rain_msaa blender_msaa[8];
			...
			for (int i = 0; i < bound; ++i)
			{
				blender_msaa[i].SetDefine("ISAMPLE", SampleDefs[i]);
				s_rain_msaa[i].create(&blender_msaa[i], "null");
	...
	//temp
	{
		...
		rt_temp_without_samples.create(r2_RT_temp_without_samples, RtCreationParams(Device.m_SecondViewport.screenWidth, Device.m_SecondViewport.screenHeight, MAIN_VIEWPORT), D3DFORMAT::D3DFMT_A16B16G16R16);
	}
...
CRenderTarget::~CRenderTarget()
{
	...
	if (RImplementation.o.dx10_msaa)
	{
		...
		for (int i = 0; i < bound; ++i)
		{
			...
			xr_delete(b_reflections_msaa[i]);
		}
	}
...
void CRenderTarget::render_simple_quad(ref_rt& DEST_RT, ref_selement& SHADER, float downscale)
{
	...
	// Fill vertex buffer
	FVF::TL* pv = (FVF::TL*)RCache.Vertex.Lock(4, g_combine->vb_stride, Offset);
	pv->set(0, _h, EPS_S, 1, C, p0.x, p1.y); pv++;
	pv->set(0, 0, EPS_S, 1, C, p0.x, p0.y); pv++;
	pv->set(_w, _h, EPS_S, 1, C, p1.x, p1.y); pv++;
	pv->set(_w, 0, EPS_S, 1, C, p1.x, p0.y); pv++;
	RCache.Vertex.Unlock(4, g_combine->vb_stride);
