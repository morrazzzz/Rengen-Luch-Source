...
void CRender::level_Load(IReader* fs)
{
	...
	// Shaders
	g_pGamePersistent->LoadTitle("st_loading_shaders", "Loading Shaders");
	...
	// VB,IB,SWI
	g_pGamePersistent->LoadTitle("st_loading_geometry", "Loading Geometry");
	...
	// Visuals
	g_pGamePersistent->LoadTitle("st_loading_spatial_db", "Loading Visuals");
	...
	// Details
	g_pGamePersistent->LoadTitle("st_loading_details", "Loading Details");
	Details->Load				();

	// Sectors
	g_pGamePersistent->LoadTitle("st_loading_sectors_portals", "Loading Sectors");
	LoadSectors					(fs);

	// 3D Fluid
	g_pGamePersistent->LoadTitle("st_loading_valumetric_fog", "Loading Valumetric Fog");
	Load3DFluid					();
	...
	if (viewPortBuffer1.mainRenderPrior0DrawingDsBuffer_)
		viewPortBuffer1.mainRenderPrior0DrawingDsBuffer_->mapLOD.clear();

	if (viewPortBuffer2.mainRenderPrior0DrawingDsBuffer_)
		viewPortBuffer2.mainRenderPrior0DrawingDsBuffer_->mapLOD.clear();
...
void CRender::ClearRuntime()
{
	// Clear
	viewPortBuffer1.LP_normalNextFrame_.ClearPackage();
	viewPortBuffer1.LP_pendingNextFrame_.ClearPackage();
	viewPortBuffer2.LP_normalNextFrame_.ClearPackage();
	viewPortBuffer2.LP_pendingNextFrame_.ClearPackage();
	Lights.ldbViewPortBuffer1.rawPackage_.ClearPackage();
	Lights.ldbViewPortBuffer1.rawPackageDeffered_.ClearPackage();
	Lights.ldbViewPortBuffer2.rawPackage_.ClearPackage();
	Lights.ldbViewPortBuffer2.rawPackageDeffered_.ClearPackage();
	...
	viewPortBuffer1.defferedGlowsVect_.clear();
	viewPortBuffer2.defferedGlowsVect_.clear();
	...
	for (u8 i = 0; i < 3; i++)
	{
		...
		viewPortBuffer1.sunCascadesRuntime_[i] = xr_new<SSunCascadeBuffer>();
		viewPortBuffer2.sunCascadesRuntime_[i] = xr_new<SSunCascadeBuffer>();
		sunCascedsDsBuffersVect_[i] = xr_new<DsGraphBuffer>();

		sunCascedsDsBuffersVect_[i]->debugName.printf("sun cascade dsgraph_buffer %u %u", i, sunCascedsDsBuffersVect_[i]->visMarkerId_);
	}
	...
	viewPortBuffer1.objectsToCalcWallMarks_.clear();
	viewPortBuffer2.objectsToCalcWallMarks_.clear();
}

void CRender::SetDsBuffersNames()
{
	viewPortBuffer1.mainRenderPrior0DsBuffer1_->debugName.printf("VP 1 Prior0 dsgraph_buffer 1 %u", viewPortBuffer1.mainRenderPrior0DsBuffer1_->visMarkerId_);
	viewPortBuffer1.mainRenderPrior0DsBuffer2_->debugName.printf("VP 1 Prior0 dsgraph_buffer 2 %u", viewPortBuffer1.mainRenderPrior0DsBuffer2_->visMarkerId_);
	viewPortBuffer2.mainRenderPrior0DsBuffer1_->debugName.printf("VP 2 Prior0 dsgraph_buffer 1 %u", viewPortBuffer2.mainRenderPrior0DsBuffer1_->visMarkerId_);
	viewPortBuffer2.mainRenderPrior0DsBuffer2_->debugName.printf("VP 2 Prior0 dsgraph_buffer 2 %u", viewPortBuffer2.mainRenderPrior0DsBuffer2_->visMarkerId_);

	mainRenderPrior1DsBuffer_->debugName.printf("Forward dsgraph_buffer %u", mainRenderPrior1DsBuffer_->visMarkerId_);
	wetRainDsBuffer_->debugName.printf("Wet Rain dsgraph_buffer %u", wetRainDsBuffer_->visMarkerId_);
}
