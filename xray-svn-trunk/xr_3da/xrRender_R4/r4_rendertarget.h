...
private:
	...
	//Bloom
	ref_shader					s_bloom_new;
	
	ref_rt						rt_bloom_treshold;
	ref_rt						rt_downsample;
	ref_rt						rt_bloom_blur;
	ref_rt						rt_bloom_final;
...
public:
	...
	void						phase_smap_direct		(CLightSource* L,	u32 sub_phase);
	void						phase_smap_direct_tsh	(CLightSource* L,	u32 sub_phase);
	void						phase_smap_spot_clear	();
	void						phase_smap_spot			(CLightSource* L);
	void						phase_smap_spot_tsh		(CLightSource* L);
	...
	void						phase_rain				();
	void						draw_rain				(CLightSource& RainSetup);
	...
	BOOL						enable_scissor			(CLightSource* L);		// true if intersects near plane
	void						enable_dbt_bounds		(CLightSource* L);

	void						disable_aniso			();

	void						draw_volume				(CLightSource* L);
	...
	void						accum_point				(CLightSource* L);
	void						accum_spot				(CLightSource* L);
	void						accum_reflected			(CLightSource* L);
	//	Igor: for volumetric lights
	void						accum_volumetric		(CLightSource* L);
	void						phase_bloom				();
