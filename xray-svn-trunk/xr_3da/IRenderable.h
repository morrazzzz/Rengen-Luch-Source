...
public:
	struct 
	{
		...
		BOOL							pROS_Allowed;
		bool							needToDelete; // Signals render that this renderable needs to be deleted, so that we don't add it to the lists
	}	renderable;
public:
										IRenderable					();
	virtual								~IRenderable				();
	IRender_ObjectSpecific*				renderable_ROS				()	;
	virtual	void						renderable_Render			(IRenderBuffer& render_buffer)	= 0;
