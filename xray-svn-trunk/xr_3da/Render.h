...
{
	MAIN_VIEWPORT,
	SECONDARY_WEAPON_SCOPE
};

#define VIEW_PORTS_CNT 2
...
public:
	...
	virtual void					set_use_static_occ	(bool val)							= 0;
	virtual bool					get_use_static_occ	()									= 0;

	virtual ~IRender_Light();
	IRender_Light();

	bool							readyToDestroy_;
	bool							alreadyInDestroyQueue_;
};
...
public:
	...
	IRenderable*						owner_;
public:
	AccessLock							protectHemiCubeSetGet_;

	virtual	void						force_mode(u32 mode) = 0;
	virtual IC float					get_luminocity() = 0;
	virtual IC float					get_luminocity_hemi() = 0;
	virtual IC float*					get_luminocity_hemi_cube() = 0;
	virtual IC float					get_ai_luminocity() = 0;
	bool								needed_for_ai;
...
class ENGINE_API IViewPortBuffer
{
public:
	IViewPortBuffer()
	{
...
// A buffer for storing runtime values for culling and drawing
class ENGINE_API IRenderBuffer
{
public:
	IRenderBuffer() 
	{
		setRenderable_				= nullptr;
		isForHud_					= false;
		allowInvisibles_			= false;
	};

	virtual ~IRenderBuffer() {};

	CFrustum		dsgraphBufferViewFrustum_;
	Fmatrix			renderingMatrix_;
	IRenderable*	setRenderable_;

	bool			isForHud_;
	bool			allowInvisibles_;

	CFrustum*		GetViewFrustum() { return &dsgraphBufferViewFrustum_; };
...
public:
	...
	virtual void					add_Visual				(IRenderVisual*	V, IRenderBuffer& render_buffer)	= 0; // add visual leaf	(no culling performed at all)
	virtual void					add_Geometry			(IRenderVisual*	V, IRenderBuffer& render_buffer, xr_vector<CFrustum>* sector_frustums)	= 0; // add visual(s)	(all culling performed)
	...
	//	Prefer this function when possible
	virtual void					add_SkeletonWallmark	(const Fmatrix* xf, IKinematics* obj, IWallMarkArray *pArray, const Fvector& start, const Fvector& dir, float size, IRenderBuffer* render_buffer)=0;
	...
	// Main
	virtual void					CalculateRendering		(ViewPort viewport)							= 0;
	virtual void					RenderFrame				(ViewPort viewport)							= 0;
	
	virtual void					Screenshot				(ScreenshotMode mode=SM_NORMAL, LPCSTR name = 0) = 0;
	virtual void					ScreenshotAsyncBegin	() = 0;
	virtual void					ScreenshotAsync			() = 0;
	...
	// Second VP
	virtual void					BeforeViewRender		(ViewPort viewport)							= 0; //--#SM+#-- Перед рендерингом мира
	virtual void					AfterViewRender			(ViewPort viewport)							= 0; //--#SM+#-- После рендеринга мира (до UI)

	virtual IViewPortBuffer&		GetViewPortBuffer1		() = 0;
	virtual IViewPortBuffer&		GetViewPortBuffer2		() = 0;
