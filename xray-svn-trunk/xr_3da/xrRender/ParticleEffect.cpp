...
void CParticleEffect::OnFrame(u32 frame_dt) // Is done in MT if option is on. Make sure there is no thread-unsafe stuff in here
{
	float adjusted_distane = Device.vpSavedView1.GetDistFromCameraMT(m_InitialPosition);

	if (GetHudMode())
		adjusted_distane = 0;

	if (m_InitialPosition.x == 0 && m_InitialPosition.y == 0 && m_InitialPosition.z == 0)
		adjusted_distane = 0;
	...
	if (m_Def && m_RT_Flags.is(flRT_Playing))
	{
		...
		for (; StepCount; StepCount--)
		{
			...
			// collision
			if (adjusted_distane < particlesCollisionDistance_ || particlesCollisionDistance_ >= 990)
			{
				if (particlesCollision_ != 0 && particlesCollision_ == 2 || m_Def->m_Flags.is(CPEDef::dfCollision))
...
void CParticleEffect::Render(float)
{
	...
	float adjusted_distane = Device.GetDistFromCamera(m_InitialPosition);
	...
	if (p_cnt > 0 && (adjusted_distane < ps_r2_particle_distance || ps_r2_particle_distance >= 990))
	{
		if (m_Def && m_Def->m_Flags.is(CPEDef::dfSprite))
		{
			...
			if (dwCount)
			{
				...
				RCache.BackendRender(D3DPT_TRIANGLELIST, dwOffset, 0, dwCount, 0, dwCount / 2);
