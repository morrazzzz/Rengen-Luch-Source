...
static class cl_svp_params : public R_constant_setup //--#SM+#--
{
	virtual void setup(R_constant* C) { RCache.set_c(C, g_pGamePersistent->weaponSVPShaderParams->svpParams); }
} bind_svp_params;

static class cl_vp_params : public R_constant_setup //--#SM+#--
{
	virtual void setup(R_constant* C) { RCache.set_c(C, Device.m_SecondViewport.IsSVPActive(), (float)Render->currentViewPort, 0, 0); }
} bind_vp_params;
...
{
	// misc
	r_Constant				("svp_params",		&bind_svp_params);	//--#SM+#--
	r_Constant				("svp_screen_res",	&binder_spv_screen_res);	
	r_Constant				("view_port_params",&bind_vp_params);	//--#SM+#--
