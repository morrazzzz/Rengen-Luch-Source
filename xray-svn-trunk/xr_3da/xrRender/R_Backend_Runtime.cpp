...
void CBackend::OnRenderFinish(ViewPort vp)
{
	HW.pContext->ClearState();
	Invalidate();
}

void CBackend::OnRenderBegin(ViewPort vp)
{
	PGO(Msg("PGO:*****frame[%d]*****", CurrentFrame()));

	Invalidate();

	HW.SwitchViewPort(vp);
...
#pragma optimize( "", off )
#endif
void CBackend::set_Textures(STextureList* t)
{
	if (T == t)
		return;
	...
	STextureList::iterator	_it		= t->begin	();
	STextureList::iterator	_end	= t->end	();
