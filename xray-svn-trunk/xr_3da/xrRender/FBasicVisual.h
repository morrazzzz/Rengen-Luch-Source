...
public:
	...
	IRenderVisual*				parentHierarhyHolder_;
	AccessLock					protectDrawingProcess_;

	AccessLock					protectVisMarker_; // if placed in vis_data struct - render starts to crash
public:
	...
	ICF AccessLock&				GetDrawingLock				() { if (parentHierarhyHolder_) return ((dxRender_Visual*)parentHierarhyHolder_)->GetDrawingLock(); else return protectDrawingProcess_; };
