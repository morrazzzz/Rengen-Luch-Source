...
void CPortalTraverser::traverse(IRender_Sector* start, CFrustum& F, Fvector& vBase, Fmatrix& mXFORM, u32 options, DsGraphBuffer& buffer)
{
	...
	buffer.sectorS.clear();

	_scissor scissor;
	scissor.set(0, 0, 1, 1);
	scissor.depth = 0;

	i_start->traverse(F, scissor, buffer);
	...
	if (options & VQ_SCISSOR)
	{
		...
		// merge scissor info
		for (u32 s = 0; s < buffer.sectorS.size(); s++)
		{
			CSector* S = (CSector*)buffer.sectorS[s];

			S->r_scissor_merged.invalidate();
			S->r_scissor_merged.depth = flt_max;

			LocalSubrenderSectorData& subrender_data = S->GetSubrenderData(buffer.visMarkerId_);

			for (u32 it = 0; it < subrender_data.r_scissors.size(); it++)
			{
				S->r_scissor_merged.merge(subrender_data.r_scissors[it]);

				if (subrender_data.r_scissors[it].depth < S->r_scissor_merged.depth)
					S->r_scissor_merged.depth = subrender_data.r_scissors[it].depth;
			}
		}
	}
}
...
void CPortalTraverser::fade_render()
{
	...
	RCache.BackendRender(D3DPT_TRIANGLELIST, _offset, _pcount);
...
{
	RCache.OnRenderFinish		(MAIN_VIEWPORT);
	RCache.set_xform_world	(Fidentity);
