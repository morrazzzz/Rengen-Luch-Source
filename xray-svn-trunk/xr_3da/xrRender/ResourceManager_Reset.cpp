...
{
	Msg("# Deleting shader templates");

	for (u32 i = 0; i < v_shaders_templates.size(); i++)
	{
		if (v_shaders_templates[i])
		{
			xr_delete(v_shaders_templates[i]);
		}
	}
	...
	for (size_t i = 0; i < ramTestingPool.size(); ++i)
		xr_delete(ramTestingPool[i]);
}
...
void CResourceManager::Dump(bool bBrief)
{
	...
	Msg		("* RM_Dump: created textures : %u | deleted textures : %u", createdTexturesCnt_, deletedTexturesCnt_);
}
