...
void CWallmarksEngine::AddSkeletonWallmark(const Fmatrix* xf, CKinematics* obj, ref_shader& sh, const Fvector& start, const Fvector& dir, float size, IRenderBuffer* render_buffer)
{
	DsGraphBuffer* buffer = render_buffer ? (DsGraphBuffer*)render_buffer : RImplementation.actualViewPortBufferNow->mainRenderPrior0DsBuffer1_; // because can be added from bullet manager

	if (buffer->phaseType_ != CRender::PHASE_NORMAL)
		return;
...
void CWallmarksEngine::AddSkeletonWallmark(intrusive_ptr<CSkeletonWallmark> wm, DsGraphBuffer& buffer)
{
	if (buffer.phaseType_ != CRender::PHASE_NORMAL)
		return;
...
#ifdef	DEBUG
		wm->used_in_render = CurrentFrame();
#endif
...
ICF void FlushStream(ref_geom hGeom, ref_shader shader, u32& w_offset, FVF::LIT*& w_verts, FVF::LIT*& w_start, BOOL bSuppressCull)
{
	...
	if (w_count)
	{
		...
		RCache.BackendRender(D3DPT_TRIANGLELIST, w_offset, w_count / 3);
...
void CWallmarksEngine::Render()
{
	...
	for (WMSlotVecIt slot_it = marks.begin(); slot_it != marks.end(); slot_it++)
	{
		...
		for (StaticWMVecIt w_it = slot->static_items.begin(); w_it != slot->static_items.end();)
		{
			...
			float dst = Device.GetDistFromCamera(W->bounds.P);
			float ssa = (W->bounds.R / dst) * 100.f;

			if (ssa >= ps_r__SWallmarkDiscard && RImplementation.baseViewFrustum_.testSphere_dirty(W->bounds.P, W->bounds.R))
			{
				...
				W->ttl -= 0.1f * TimeDelta();	// visible wallmarks fade much slower
			}
			else
			{
				W->ttl -= TimeDelta();
		...
		for (xr_vector<intrusive_ptr<CSkeletonWallmark> >::iterator w_it = slot->skeleton_items.begin(); w_it != slot->skeleton_items.end(); w_it++)
		{
			...
			float dst = Device.GetDistFromCamera(W->m_Bounds.P);

			if ((dst < ps_r__DWallmarkDiscard) && RImplementation.baseViewFrustum_.testSphere_dirty(W->m_Bounds.P, 1.f))
			{
	...
	// Level-wmarks
	RImplementation.r_dsgraph_render_wmarks(*RImplementation.actualViewPortBufferNow->mainRenderPrior0DrawingDsBuffer_);
