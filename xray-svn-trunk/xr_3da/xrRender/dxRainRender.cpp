...
void dxRainRender::Render(CEffect_Rain &owner, SRainParams* p)
{
	...
	for (u32 I=0; I<owner.items.size(); I++){
		...
		if (one.dwTime_Hit<EngineTimeU())		owner.Hit (one.Phit);
		if (one.dwTime_Life<EngineTimeU())	owner.Born(one,p->source_radius);

		// последн€€ дельта ??
		//.		float xdt		= float(one.dwTime_Hit-EngineTimeU())/1000.f;
		//.		float dt		= TimeDelta();//xdt<TimeDelta()?xdt:TimeDelta();
		float dt		= TimeDelta();
		...
		sR				= sC.magnitude();
		sC.add			(pos_trail);
		if (!::Render->baseViewFrustum_.testSphere_dirty(sC, sR))	continue;
...

	{
		...
		while (P)	{
			...
			// Render
			if (::Render->baseViewFrustum_.testSphere_dirty(P->bounds.P, P->bounds.R))
			{
				// Build matrix
				...
				if (pcount >= p->particles_cache) {
					...
					RCache.BackendRender	(D3DPT_TRIANGLELIST, v_offset, 0, vCount_Lock, i_offset, dwNumPrimitives);
