...
void CSector::traverse(CFrustum &F, _scissor& R_scissor, DsGraphBuffer& buffer)
{
	// Find local data for this subrender
	LocalSubrenderSectorData& subrebder_data = GetSubrenderData(buffer.visMarkerId_);

	// Register traversal process
	if (subrebder_data.r_marker != buffer.portalTraverserMark)
	{
		subrebder_data.r_marker = buffer.portalTraverserMark;
		buffer.sectorS.push_back(this);

		subrebder_data.r_frustums.clear();
		subrebder_data.r_scissors.clear();
	}

	subrebder_data.r_frustums.push_back(F);
	subrebder_data.r_scissors.push_back(R_scissor);
	...
	{
		if (m_portals[I]->marker == PortalTraverser.iportal_marker)
			continue;
		...
		if (PortalTraverser.i_options&CPortalTraverser::VQ_SCISSOR && (!PORTAL->bDualRender))
		{
			...
			if (depth < EPS)
			{
				...
				if ((PortalTraverser.i_options&CPortalTraverser::VQ_HOM) && (!RImplementation.HOM.hom_visible(*P)))
					continue;
			}
			else
			{
				...
				if ((PortalTraverser.i_options&CPortalTraverser::VQ_HOM) && (!RImplementation.HOM.hom_visible(scissor, depth)))
					continue;
			}
		}
		else
		...
		PORTAL->marker = PortalTraverser.iportal_marker;
		PORTAL->bDualRender = FALSE;

		pSector->traverse(Clip, scissor, buffer);
	}
}
