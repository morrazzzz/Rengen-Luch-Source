...
void dxEnvDescriptorRender::OnDeviceCreate(CEnvDescriptor &owner)
{
	...
	{
		if (!sky_texture._get())
			sky_texture.create(owner.sky_texture_name.c_str(), true);
		else
			Msg("Already exist!");
	}
	...
	{
		if (!sky_texture_env._get())
			sky_texture_env.create(owner.sky_texture_env_name.c_str(), true);
		else
			Msg("Already exist!");
	}
	...
	{
		if (!clouds_texture._get())
			clouds_texture.create(owner.clouds_texture_name.c_str(), true);
		else
			Msg("Already exist!");
	}
}
...
void dxEnvironmentRender::OnFrame(CEnvironment &env)
{
	...
	R_ASSERT(mixRen.sky_r_textures[0].second);
	R_ASSERT(mixRen.sky_r_textures[1].second);
...
//	RCache.set_Textures			(&env.CurrentEnv->sky_r_textures);
	...
	RCache.BackendRender		(D3DPT_TRIANGLELIST, v_offset, 0, 12, i_offset, 20);
...
void dxEnvironmentRender::RenderClouds(CEnvironment &env)
{
	...
	RCache.BackendRender		(D3DPT_TRIANGLELIST, v_offset, 0, env.CloudsVerts.size(), i_offset, env.CloudsIndices.size() / 3);
