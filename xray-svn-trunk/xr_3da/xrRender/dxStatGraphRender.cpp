...
void dxStatGraphRender::OnRender(CStatGraph &owner)
{
	...
	if (TriElem)
	{
		...
		RCache.BackendRender(D3DPT_TRIANGLELIST, dwOffsetTri, 0, dwCount, 0, dwCount / 2);
	};
	...
	if (LineElem)
	{
		...
		RCache.BackendRender(D3DPT_LINELIST, dwOffsetLine, dwCount / 2);
	};
...
void dxStatGraphRender::RenderBack(CStatGraph &owner)
{
	...
	RCache.set_Geometry(hGeomLine);
	RCache.BackendRender(D3DPT_LINESTRIP, dwOffset, 4);
	...
	RCache.set_Geometry		(hGeomLine);
	RCache.BackendRender	(D3DPT_LINELIST, dwOffset, dwCount / 2);
}
