...
struct	FTreeVisual_setup
{
	...
	void calculate()
	{
		...
		float	tm_rot = PI_MUL_2 * EngineTime() / desc.m_fTreeRotation;
		...
		// setup constants
		wave.set(desc.m_fTreeWave.x, desc.m_fTreeWave.y, desc.m_fTreeWave.z, EngineTime() * desc.m_fTreeSpeed); // wave
		wave.div(PI_MUL_2);
	}
};
...
void FTreeVisual_ST::Render(float LOD)
{
	...
	RCache.BackendRender(D3DPT_TRIANGLELIST, vBase, 0, vCount, iBase, dwPrimitives);
...
void FTreeVisual_PM::Render(float LOD)
{
	...
	RCache.set_Geometry(rm_geom);
	RCache.BackendRender(D3DPT_TRIANGLELIST, vBase, 0, SW.num_verts, iBase + SW.offset, SW.num_tris);
