...
#include "../xrRender_R4/light_GI.h"
...
class CLightSource : public IRender_Light, public ISpatial
{
public:
	struct {
		u32			type	:	4;
	...
	bool			isOmniPart_;
	CLightSource*	omniParent_;

	// use GPU occ calc for static geom visability. Not for moving lights
	bool			useOccOnStaticGeom_;
	...
	struct _vis
	{
		u32			timeToTestOcc_;	// engine time the test is sheduled to
...
#ifndef XRCPU_PIPE_EXPORTS
	vis_list		m_shadowed_details[3];

	DsGraphBuffer*	lightDsGraphBuffer_;

	AccessLock		protectMtLightCalc_;
	bool			canCalcDeffered_;

	LightViewProtbuffer	lightViewPortBuffer1;
	LightViewProtbuffer	lightViewPortBuffer2;
#endif

	bool			usefullForDetailsShadows_;
	u16				detailWithShadowsCount_;
...
public:
	...
	virtual void	set_use_static_occ		(bool val)						{ useOccOnStaticGeom_ = val; };
	virtual bool	get_use_static_occ		()								{ return useOccOnStaticGeom_; };
