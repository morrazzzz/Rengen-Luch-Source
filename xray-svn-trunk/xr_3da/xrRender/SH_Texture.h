#ifndef SH_TEXTURE_H
#define SH_TEXTURE_H
#pragma once

#include "../../xrCore/xr_resource.h"

class  ENGINE_API CAviPlayerCustom;
class  CTheoraSurface;

class  ECORE_API CTexture : public xr_resource_named
{
public:
	//	Since DX10 allows up to 128 unique textures, 
	//	distance between enum values should be at leas 128
	enum ResourceShaderType	//	Don't change this since it's hardware-dependent
	{
		rstPixel = 0,	//	Default texture offset
		rstVertex = D3DVERTEXTEXTURESAMPLER0,
		rstGeometry = rstVertex+256,
		rstHull = rstGeometry+256,
		rstDomain = rstHull+256,
		rstCompute = rstDomain+256,
        rstInvalid = rstCompute+256
	};

public:
	void	__stdcall					apply_load		(u32 stage);
	void	__stdcall					apply_theora	(u32 stage);
	void	__stdcall					apply_avi		(u32 stage);
	void	__stdcall					apply_seq		(u32 stage);
	void	__stdcall					apply_normal	(u32 stage);

	void								Preload			();
	void								LoadTexture		(bool already_in_mt, bool mt_anyway = false);
	void								PostLoad		();
	void								UnloadTexture	();
	void								PreDelete		();
//	void								Apply			(u32 dwStage);

	void	__stdcall					LoadMT			();
	void	__stdcall					LoadMTFinish	();

	void								surface_set		(ID3DBaseTexture* surf);
	void								SurfaceSetRT	(ID3DBaseTexture* surf, ID3DShaderResourceView* sh_res_view); // special for render target

	ID3DShaderResourceView*				CreateShaderRes	(ID3DBaseTexture* surf);

	ID3DBaseTexture*					surface_get 	();

	void								surface_null	();

	IC BOOL								isUser			()				{ return flags.bUser; }
	IC u32								get_Width		()				{ desc_enshure(); return desc.Width; }
	IC u32								get_Height		()				{ desc_enshure(); return desc.Height; }

	const D3D_TEXTURE2D_DESC&			Description		()		{ return desc; };

	void								video_Sync		(u32 _time)		{ m_play_time=_time; }
	void								video_Play		(BOOL looped, u32 _time = 0xFFFFFFFF);
	void								video_Pause		(BOOL state);
	void								video_Stop		();
	BOOL								video_IsPlaying	();

	CTexture							();
	virtual ~CTexture					();
	
	ID3DShaderResourceView*				get_SRView		()		{ return m_pSRView; }

private:
	IC BOOL								desc_valid		()		{ return pSurface==desc_cache; }
	IC void								desc_enshure	()		{ if (!desc_valid()) desc_update(); }
	void								desc_update		();

	void								Apply			(u32 dwStage);
	void								ProcessStaging();
	D3D_USAGE							GetUsage(ID3DBaseTexture* surface);

	// MT post load
	ID3DBaseTexture*					mtLoadedSurface_;
	ID3DShaderResourceView*				mtLoadedViewRes_;

	AccessLock							loadLock_; // just in case, two threads gets to load same texture simultaniously

	//	Class data
public:	//	Public class members (must be encapsulated furthur)
	struct 
	{
		u32					bLoaded		: 1;
		u32					bUser		: 1;
		u32					seqCycles	: 1;
		u32					MemoryUsage	: 28;
		u32					bLoadedAsStaging : 1;

	} flags;

	fastdelegate::FastDelegate1<u32>	bind;


	CAviPlayerCustom*					pAVI;
	CTheoraSurface*						pTheora;
	float								m_material;
	shared_str							m_bumpmap;

	union
	{
		u32								m_play_time;		// sync theora time
		u32								seqMSPF;			// Sequence data milliseconds per frame
	};

	bool								alreadyInDestroyQueue_; //criticaly valuable mark

private:
	ID3DBaseTexture*					pSurface;
	// Sequence data
	xr_vector<ID3DBaseTexture*>			seqDATA;

	// Description
	ID3DBaseTexture*					desc_cache;
	D3D_TEXTURE2D_DESC					desc;

	AccessLock							porotectIsProcessingMT;
	bool								isProcessingMT_;

	ID3DShaderResourceView*				m_pSRView;
	// Sequence view data
	xr_vector<ID3DShaderResourceView*>	m_seqSRView;

	AccessLock							porotectReadyToDestroy_;
	// flag for assertion check
	bool								readyToDestroy_;

public:

	void								SetIsProcessingMT(bool val)	{ porotectIsProcessingMT.Enter(); isProcessingMT_ = val; porotectIsProcessingMT.Leave(); };
	bool								GetIsProcessingMT()			{ porotectIsProcessingMT.Enter(); bool ret = isProcessingMT_; porotectIsProcessingMT.Leave(); return ret; };

	void								SetReadyToDestroy(bool val)	{ porotectReadyToDestroy_.Enter(); readyToDestroy_ = val; porotectReadyToDestroy_.Leave(); };
	bool								IsReadyToDestroy()			{ porotectReadyToDestroy_.Enter(); bool ret = readyToDestroy_; porotectReadyToDestroy_.Leave(); return ret; };
};

struct 		resptrcode_texture	: public resptr_base<CTexture>
{
	void				create			(LPCSTR	_name, bool mt_anyway = false);
	void				destroy			()					{ _set(NULL); }
	shared_str			bump_get		()					{ return _get()->m_bumpmap; }
	bool				bump_exist		()					{ return 0 != bump_get().size(); }

	virtual	void		_dec			();
};

typedef	resptr_core<CTexture,resptrcode_texture > ref_texture;

#endif
