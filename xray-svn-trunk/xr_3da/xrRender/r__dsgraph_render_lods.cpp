...
void R_dsgraph_structure::r_dsgraph_render_lods(bool _setup_zb, bool _clear, DsGraphBuffer& render_buffer)
{
	if (_setup_zb)
		render_buffer.mapLOD.getLR(lstLODs);	// front-to-back
	else
		render_buffer.mapLOD.getRL(lstLODs);	// back-to-front
		...
		for (u32 uiPass = 0; uiPass < SHADER_PASSES_MAX; ++uiPass)
		{
			...
			for (u32 g = 0; g<lstLODgroups.size(); g++)
			{
				...
				if (uiPass<uiNumPasses)
				{
					...
					RCache.BackendRender(D3DPT_TRIANGLELIST, vCurOffset, 0, 4 * p_count, 0, 2 * p_count);
				}
