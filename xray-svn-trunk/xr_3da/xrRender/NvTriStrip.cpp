...
void GenerateStrips(const u16* in_indices, const s32 in_numIndices, xr_vector<PrimitiveGroup> &primGroups)
{
	...
	if(bListsOnly)
	{
		...
		for(int j = 0; j < tempStrips.size(); j++)
		{
			numIndices += tempStrips[j]->m_faces.size() * 3;
		}
		...
		{
			for(int j = 0; j < tempStrips[k]->m_faces.size(); j++)
			{
				primGroups[0].indices[indexCtr++] = u16(tempStrips[k]->m_faces[j]->m_v0);
				primGroups[0].indices[indexCtr++] = u16(tempStrips[k]->m_faces[j]->m_v1);
				primGroups[0].indices[indexCtr++] = u16(tempStrips[k]->m_faces[j]->m_v2);
			}
		}

		//do lists
		for(u32 j = 0; j < tempFaces.size(); j++)
		{
			primGroups[0].indices[indexCtr++] = u16(tempFaces[j]->m_v0);
			primGroups[0].indices[indexCtr++] = u16(tempFaces[j]->m_v1);
			primGroups[0].indices[indexCtr++] = u16(tempFaces[j]->m_v2);
		}
	}
	else
	{
		...
		for	(stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++)
		{
			...
			if(numSeparateStrips != 1)
			{
				...
				int j;
				for(j = startingLoc; j < stripIndices.size(); j++)
				{
					if(stripIndices[j] == -1)
						break;
				}
			...
			for(int j = startingLoc; j < stripLength + startingLoc; j++)
				primGroups[stripCtr].indices[indexCtr++] = u16(stripIndices[j]);
		...
		if(tempFaces.size() != 0)
		{
			...
			{
				primGroups[faceGroupLoc].indices[indexCtr++] = u16(tempFaces[j]->m_v0);
				primGroups[faceGroupLoc].indices[indexCtr++] = u16(tempFaces[j]->m_v1);
				primGroups[faceGroupLoc].indices[indexCtr++] = u16(tempFaces[j]->m_v2);
			}
		}
	}
	...
	for(u32 k = 0; k < tempStrips.size(); k++)
	{
		for(int j = 0; j < tempStrips[k]->m_faces.size(); j++)
		{
			xr_delete(tempStrips[k]->m_faces[j]);
		}
		xr_delete(tempStrips[k]);
	}

	//_delete faces
	for(u32 j = 0; j < tempFaces.size(); j++)
	{
		xr_delete(tempFaces[j]);
	}
}
