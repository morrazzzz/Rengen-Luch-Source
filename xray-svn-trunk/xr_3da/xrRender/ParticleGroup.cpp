...
void CParticleGroup::SItem::OnFrame(u32 u_dt, const CPGDef::SEffect& def, Fbox& box, bool& bPlaying)  // Is done in MT if option is on. Make sure there is no thread-unsafe stuff in here
{
	CParticleEffect* E = static_cast<CParticleEffect*>(_effect);
	...
	if (!_children_related.empty())
	{
		for (it = _children_related.begin(); it != _children_related.end(); it++)
		{
			CParticleEffect* child_E = static_cast<CParticleEffect*>(*it);

			if (child_E)
			{
				child_E->OnFrame(u_dt);

				if (child_E->IsPlaying())
				{
					bPlaying = true;

					if (child_E->vis.box.is_valid())
						box.merge(child_E->vis.box);
				}
				else
				{
					if (def.m_Flags.is(CPGDef::SEffect::flOnPlayChildRewind))
					{
						child_E->Set_InitialPosition(m_InitialPosition);
						child_E->Play();
					}
				}
			}
	...
	if (!_children_free.empty())
	{
		...
		for (it = _children_free.begin(); it != _children_free.end(); it++)
		{
			...
			if (child_E)
			{
				...
				{
					bPlaying = true;
					if (child_E->vis.box.is_valid()) box.merge(child_E->vis.box);
				}
				else
				{
					rem_cnt++;
...
void CParticleGroup::OnFrame(u32 u_dt) // Is done in MT if option is on. Make sure there is no thread-unsafe stuff is here
{
