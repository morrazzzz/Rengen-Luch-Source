...
u32 m_SelectionRect = D3DCOLOR_RGBA(206, 165, 15, 64);
...
void CDrawUtilities::OnDeviceCreate()
{
	...
	axisMarkFont_	= xr_new <CGameFont>("ui_font_graffiti22_russian");
...
void CDrawUtilities::DrawRectangularFlag(u32 clr, ref_shader shade_with, float size)
{
	R_ASSERT2(shade_with, "To Coder! Specify shader at this point");

	DU_DRAW_SH(shade_with);

	_VertexStream* Stream = &RCache.Vertex;
	u32	vBase;

	// Draw flags banner
	FVF::LIT* pv = (FVF::LIT*)Stream->Lock(6, vs_LIT->vb_stride, vBase); // fill VB

	// shared vertex for all 4 triangles for FAN technique of triangle drawing
	pv->set(0.f, 1.f * size, 0.025f * size, clr, 0.f, 0.f);
	pv++;

	//front side
	pv->set(0.f, 1.f * size, .5f * size, clr, 0.f, 0.f);
	pv++;
	pv->set(0.f, .5f * size, .5f * size, clr, 0.f, 0.f);
	pv++;
	pv->set(0.f, .5f * size, 0.025f * size, clr, 0.f, 0.f);
	pv++;
	...
	// Render 4 triangles from 6 vertexes using FAN technique
	DU_DRAW_DP(D3DPT_TRIANGLEFAN, vs_LIT, vBase, 4);


	// Draw flag's pole
	pv = (FVF::LIT*)Stream->Lock(6, vs_LIT->vb_stride, vBase); // fill VB

	// shared vertex for all 4 triangles for FAN technique of triangle drawing
	pv->set(0.f, 0.f, 0.f, 0x0FF6B3128, 0.f, 0.f);
	pv++;

	//front side
	pv->set(0.f, 1.f * size, 0.f, 0x0FF6B3128, 0.f, 0.f);
	pv++;
	pv->set(0.f, 1.f * size, 0.025f * size, 0x0FF6B3128, 0.f, 0.f);
	pv++;
	pv->set(0.f, 0.f, 0.025f * size, 0x0FF6B3128, 0.f, 0.f);
	pv++;
...
void CDrawUtilities::DrawTriangularFlag(u32 clr, ref_shader shade_with, float size)
{
	R_ASSERT2(shade_with, "To Coder! Specify shader at this point");
	...
	// Draw flags banner
	FVF::LIT* pv = (FVF::LIT*)Stream->Lock(4, vs_LIT->vb_stride, vBase); // fill VB

	// shared vertex for all 2 triangles for FAN technique of triangle drawing
	pv->set(0.f, 1.f * size, 0.025f * size, clr, 0.f, 0.f);
	pv++;

	//front side
	pv->set(0.f, 0.75f * size, 0.5f * size, clr, 0.f, 0.f);
	pv++;
	pv->set(0.f, 0.5f * size, 0.025f * size, clr, 0.f, 0.f);
	pv++;
	...
	Stream->Unlock(4, vs_LIT->vb_stride);

	// Render 2 triangles from 4 vertexes using FAN technique
	DU_DRAW_DP(D3DPT_TRIANGLEFAN, vs_LIT, vBase, 2);
...
void CDrawUtilities::DrawSoundIcon(u32 clr, ref_shader shade_with, float size)
{
	R_ASSERT2(shade_with, "To Coder! Specify shader at this point");
	...
	// shared vertex for all 4 triangles for FAN technique of triangle drawing
	pv->set(0.f, 0.25f * size, 0.05f * size, 0x0FF332721, 0.f, 0.f);
	pv++;

	//front side
	pv->set(0.f, 0.25f * size, 0.5f * size, 0x0FF332721, 0.f, 0.f);
	pv++;
	pv->set(0.f, -0.25f * size, 0.5f * size, 0x0FF332721, 0.f, 0.f);
	pv++;
	pv->set(0.f, -0.25f * size, 0.05f * size, 0x0FF332721, 0.f, 0.f);
	...
	// shared vertex for all 4 triangles for FAN technique of triangle drawing
	pv->set(0.f, 0.25f, 0.05f, clr, 0.f, 0.f);
	pv++;

	//front side
	pv->set(0.f, 0.65f * size, -0.5f, clr, 0.f, 0.f);
	pv++;
	pv->set(0.f, -0.65f * size, -0.5f * size, clr, 0.f, 0.f);
	pv++;
	pv->set(0.f, -0.25f, 0.05f * size, clr, 0.f, 0.f);
	pv++;
...
void CDrawUtilities::DrawSnowFlake(const Fvector& p, float r, u32 c){
...
void CDrawUtilities::DrawSelectionBox(const Fvector& C, const Fvector& S, u32* clr)
{
	...
	// and Render it as triangle list
	DU_DRAW_RS(D3DRS_ZENABLE, FALSE); // Make our premitive drawn on top of output

    DU_DRAW_DP	(D3DPT_LINELIST,vs_L,vBase,boxvertcount/2);

	DU_DRAW_RS(D3DRS_ZENABLE, TRUE);
...
void CDrawUtilities::DrawSphere(const Fmatrix& parent, const Fvector& center, float radius, u32 clr_s, u32 clr_w, BOOL bSolid, BOOL bWire, bool mul_by_parent_M)
{
    Fmatrix B;
...
void CDrawUtilities::DrawAxis(const Fmatrix& T)
{
	...
	{
#ifdef __BORLANDC__
		pv->color = c[i]; pv->transform(p[i], EDevice.mFullTransform);
#else
    ...
    axisMarkFont_->SetColor(0xFF909090);
	axisMarkFont_->Out(p[1].x, p[1].y, "x");
	axisMarkFont_->Out(p[3].x, p[3].y, "y");
	axisMarkFont_->Out(p[5].x, p[5].y, "z");

	axisMarkFont_->SetColor(0xFF000000);
	axisMarkFont_->Out(p[1].x-1,p[1].y-1,"x");
	axisMarkFont_->Out(p[3].x-1,p[3].y-1,"y");
	axisMarkFont_->Out(p[5].x-1,p[5].y-1,"z");
#else
	if (!m_axis_object)
...
void CDrawUtilities::DrawObjectAxis(const Fmatrix& T, float sz, BOOL sel)
{
#ifdef __BORLANDC__
	VERIFY(EDevice.b_is_Ready);
	Fmatrix& device_full_transform = EDevice.mFullTransform;
#else
	VERIFY(Device.b_is_Ready);
	Fmatrix& device_full_transform = Device.mFullTransform;
#endif

	_VertexStream*	Stream	= &RCache.Vertex;
    Fvector c, r, n, d;

	float w = T.c.x*device_full_transform._14 + T.c.y*device_full_transform._24 + T.c.z*device_full_transform._34 + device_full_transform._44;

    if (w < 0) return; // culling

	float s = w * sz;
	device_full_transform.transform(c, T.c);

	r.mul(T.i, s);
	r.add(T.c);

	device_full_transform.transform(r);

	n.mul(T.j, s);
	n.add(T.c);

	device_full_transform.transform(n);

	d.mul(T.k, s);
	d.add(T.c);

	device_full_transform.transform(d);
    ...
    u32 vBase;
	FVF::TL* pv	= (FVF::TL*)Stream->Lock(18,vs_TL->vb_stride,vBase);
	...
	for (u32 i = 0; i < 3; i++)
	{
		...
		pv->set(c.x + off, c.y, 0, 1, 0xFF222222, 0, 0);
		pv++;
		pv->set(d.x + off, d.y, 0, 1, sel ? 0xFF0000FF : 0xFF000080, 0, 0);
		pv++;
		pv->set(c.x + off, c.y, 0, 1, 0xFF222222, 0, 0);
		pv++;
		pv->set(r.x + off, r.y, 0, 1, sel ? 0xFFFF0000 : 0xFF800000, 0, 0);
		pv++;
		pv->set(c.x + off, c.y, 0, 1, 0xFF222222, 0, 0);
		pv++;
		pv->set(n.x + off, n.y, 0, 1, sel ? 0xFF00FF00 : 0xFF008000, 0, 0);
		pv++;
	}

	Stream->Unlock(18, vs_TL->vb_stride);
    ...
    DU_DRAW_DP	(D3DPT_LINELIST, vs_TL, vBase, 9);
	DU_DRAW_RS	(D3DRS_SHADEMODE, SHADE_MODE);


	axisMarkFont_->SetColor(sel ? 0xFF000000 : 0xFF909090);
	axisMarkFont_->Out(r.x, r.y, "x");
	axisMarkFont_->Out(n.x, n.y, "y");
	axisMarkFont_->Out(d.x, d.y, "z");

	axisMarkFont_->SetColor(sel ? 0xFFFFFFFF : 0xFF000000);
	axisMarkFont_->Out(r.x - 1, r.y - 1, "x");
	axisMarkFont_->Out(n.x - 1, n.y - 1, "y");
	axisMarkFont_->Out(d.x - 1, d.y - 1, "z");
}
