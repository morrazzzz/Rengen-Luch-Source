...
CGlow::~CGlow()
{
	...
	R_ASSERT2(GetCurrentThreadId() == Core.mainThreadID, "Glows must be destructed only within main thread, to avoid mt bugs");
	R_ASSERT2(readyToDestroy, "Glows should be destroyed via Glows manager, to avoid mt bugs");
	...
	for (u32 j = 0; j < VIEW_PORTS_CNT; ++j)
	{
		...
		// Remove glow from deffered vector
		auto I = std::find_if(vpbuffer.defferedGlowsVect_.begin(), vpbuffer.defferedGlowsVect_.end(), GlowRemoverPred(this));

		if (I != vpbuffer.defferedGlowsVect_.end())
			vpbuffer.defferedGlowsVect_.erase(I);

		I = std::find_if(vpbuffer.defferedGlowsVect_.begin(), vpbuffer.defferedGlowsVect_.end(), GlowRemoverPred(this));

		R_ASSERT(I == vpbuffer.defferedGlowsVect_.end());
	}
}
...
void CGlow::spatial_move_intern()
{
...
void CGlowManager::Load(IReader* fs)
{
	...
	for (; count; count--)
	{
		...
		G->fade = 1.f;
		G->dwFrame = CurrentFrame();
		G->bTestResult = TRUE;

		G->spatial.s_type = STYPE_RENDERABLE;
...
void CGlowManager::add(ref_glow G_)
{
	...
	if (G->dwFrame == CurrentFrame())
		return;

	int frame_div = CurrentFrame() - G->dwFrame;
...
void CGlowManager::DeleteGlow(IRender_Glow*& p_)
{
	if (!p_)
		return;

	protectGlowDeletion.Enter();

	if (engineState.test(FRAME_PROCESING))
	{
		if (!p_->alreadyInDestroyQueue)
		{
			glowsToDelete.push_back(p_);
	...
	protectGlowDeletion.Leave();

	p_ = nullptr;
}

void CGlowManager::DeleteQueue()
{
	protectGlowDeletion.Enter();

#ifdef DEBUG
	u32 cnt = glowsToDelete.size();
#endif

	for (u32 it = 0; it < glowsToDelete.size(); it++)
	{
		R_ASSERT2(glowsToDelete[it], "Deleting glow, that is already deleted?");

		glowsToDelete[it]->readyToDestroy = true;
		xr_delete(glowsToDelete[it]);
	}
...
#ifdef DEBUG
	if (cnt)
		Msg("* Post frame glows deletion: %u glows deleted", cnt);
#endif
}
...
void CGlowManager::render_selected()
{
	...
	for (; pos < Selected.size();) 
	{
		...
		if (vCount)
		{
			...
			RCache.BackendRender(D3DPT_TRIANGLELIST, vOffset, 0, vCount, 0, vCount / 2);
		}
	}
