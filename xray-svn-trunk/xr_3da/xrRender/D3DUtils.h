...
//----------------------------------------------------
//for unknown reasons virtual, stdcall, inline and any kind of optimizations don't work propetly with borland builder in this file
//it jumps to wrong function's, don't try to use it until y know what you are doing
...
public:
    ...
    //----------------------------------------------------
     ...
     void  DrawCross(const Fvector& p, float sz, u32 clr, BOOL bRot45=false)
	 {
		 DrawCross(p, sz,sz,sz, sz,sz,sz, clr, bRot45); 
	 }

	 void	DrawRectangularFlag					(u32 clr, ref_shader shade_with, float size);
	 void	DrawTriangularFlag					(u32 clr, ref_shader shade_with, float size);

	 void	DrawSoundIcon						(u32 clr, ref_shader shade_with, float size);
	 ...
	 void  DrawSnowFlake(const Fvector& p, float radius, u32 clr);
     ...
     void  DrawFaceNormal(const Fvector* p, float size, u32 clr)
	 {
		 DrawFaceNormal(p[0],p[1],p[2],size,clr);
	 }

     void  DrawFaceNormal(const Fvector& C, const Fvector& N, float size, u32 clr)
	 {
	 ...
	 void  DrawSphere	(const Fmatrix& parent, const Fvector& center, float radius, u32 clr_s, u32 clr_w, BOOL bSolid, BOOL bWire, bool mul_by_parent_M = true);

	 void  DrawSphere	(const Fmatrix& parent, const Fsphere& S, u32 clr_s, u32 clr_w, BOOL bSolid, BOOL bWire)
	 {
     ...
     void  OutText	(const Fvector& pos, LPCSTR text, u32 color=0xFF000000, u32 shadow_color=0xFF909090);
	...
	void __stdcall			DrawRectangularFlag			(u32 clr, ref_shader shade_with, float size);
	void __stdcall			DrawTriangularFlag			(u32 clr, ref_shader shade_with, float size);

	void __stdcall			DrawSoundIcon				(u32 clr, ref_shader shade_with, float size);
	...
	virtual void __stdcall DrawSnowFlake(const Fvector& p, float radius, u32 clr);
	...
	virtual void __stdcall DrawSphere	(const Fmatrix& parent, const Fvector& center, float radius, u32 clr_s, u32 clr_w, BOOL bSolid, BOOL bWire, bool mul_by_parent_M = true);
