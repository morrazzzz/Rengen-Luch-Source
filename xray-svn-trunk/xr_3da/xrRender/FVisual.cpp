...
{
	if (m_fast && RImplementation.render_phase == CRender::PHASE_SMAP && !RCache.is_TessEnabled())
	{
		RCache.set_Geometry(m_fast->rm_geom);
		RCache.BackendRender(D3DPT_TRIANGLELIST, m_fast->vBase, 0, m_fast->vCount, m_fast->iBase, m_fast->dwPrimitives);
