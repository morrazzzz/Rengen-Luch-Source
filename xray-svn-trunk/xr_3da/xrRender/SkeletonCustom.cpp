...
float CKinematics::m_bloodmark_lifetime;
bool CKinematics::IsStaticInited = false;
...
void CKinematics::LL_Validate()
{
    ...
    if (bCheckBreakable)
	{
		...
		tempGroups.clear();
        LL_GetBoneGroups			(tempGroups);

        xr_vector<u16>* b_parts = xr_new<xr_vector<u16>>(LL_BoneCount(), BI_NONE);

        CBoneData* root 			= &LL_GetData(LL_GetBoneRoot());
        u16 last_id					= 0;
        iBuildGroups    			(root,*b_parts,0,last_id);

        for (u16 g = 0; g < (u16)tempGroups.size(); ++g)
		{
            xr_vector<u16>&	group = tempGroups[g];

            u16 bp_id = (*b_parts)[group[0]];

			for (u32 b = 1; b < tempGroups[g].size(); b++)
				if (bp_id != (*b_parts)[tempGroups[g][b]])
				{
...
void CKinematics::AddWallmark(const Fmatrix* parent_xform, const Fvector3& start, const Fvector3& dir, ref_shader r_shader, float size)
{
	...
	for (u32 wm_idx = 0; wm_idx < wallmarks.size(); wm_idx++)
	{
		...
		if (wm->Similar(r_shader,cp,0.02f))
		{ 
	...
	intrusive_ptr<CSkeletonWallmark> wm = xr_new<CSkeletonWallmark>(this, parent_xform, r_shader, cp, EngineTime());
	...
	for (u32 i = 0; i < children.size(); i++)
	{
		CSkeletonX* skel = LL_GetChild(i);

		for (U16It b_it = test_bones.begin(); b_it != test_bones.end(); b_it++)
			skel->FillVertices(mView, *wm, normal, size, *b_it);
	}
...
void CKinematics::CalculateWallmarks(DsGraphBuffer& buffer, CFrustum& view_frust)
{
	//protectWalmarkCalc_.Enter(); // no need it now, since this is done in min thread

	if (!wallmarks.empty() && (wm_frame != CurrentFrame()))
	{
		wm_frame = CurrentFrame();

		for (SkeletonWMVecIt it = wallmarks.begin(); it != wallmarks.end();)
		{
			intrusive_ptr<CSkeletonWallmark>& wm = *it;

			float w = (EngineTime() - wm->TimeStart()) / m_bloodmark_lifetime;

			if (w < 1.f)
			{
				// append wm to WallmarkEngine
				if (view_frust.testSphere_dirty(wm->m_Bounds.P, wm->m_Bounds.R))
					::RImplementation.add_SkeletonWallmark(wm, buffer);
...
void CKinematics::StaticInitClass()
{
	if (IsStaticInited)
		return;

	m_bloodmark_lifetime = READ_IF_EXISTS(pSettings, r_float, "interface", "bloodmark_life_time", 30.0f);
