...
void CEffect_Rain::RenewItem(Item& dest, float height, BOOL bHit)
{
    ...
    if (bHit)
	{
		dest.dwTime_Life= EngineTimeU() + iFloor(1000.f * height / dest.fSpeed) - TimeDeltaU();
		dest.dwTime_Hit	= EngineTimeU() + iFloor(1000.f * height / dest.fSpeed) - TimeDeltaU();
		dest.Phit.mad	(dest.P,dest.D,height);
	}
	else
	{
		dest.dwTime_Life= EngineTimeU() + iFloor(1000.f * height / dest.fSpeed) - TimeDeltaU();
		dest.dwTime_Hit	= EngineTimeU() + iFloor(2 * 1000.f * height / dest.fSpeed) - TimeDeltaU();
...
void CEffect_Rain::OnFrame()
{
	...
	if (E&&E->renderable_ROS())
	{
		...
		E->renderable_ROS()->protectHemiCubeSetGet_.Enter();

		CopyMemory(hemi_cube, E->renderable_ROS()->get_luminocity_hemi_cube(), 6*sizeof(float));

		E->renderable_ROS()->protectHemiCubeSetGet_.Leave();
