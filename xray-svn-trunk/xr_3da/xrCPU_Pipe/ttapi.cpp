...
//	ttapi_workers_count = ID->n_cores;
	...
	DWORD dwOverride = 0;
	if ( pszTemp )
		if ( sscanf_s( pszTemp + xr_strlen( szSearchFor ) , "%u" , &dwOverride ) )
	...
	if ( ( ttapi_threads_handles = (LPHANDLE) xr_malloc( sizeof(HANDLE)*ttapi_threads_count ) ) == NULL )
		return 0;
	if ( ( ttapi_worker_params = (PTTAPI_WORKER_PARAMS) xr_malloc( sizeof(TTAPI_WORKER_PARAMS)*ttapi_workers_count ) ) == NULL )
		return 0;

	// Clearing params
	for ( DWORD i = 0 ; i < ttapi_workers_count ; i++ )
		ZeroMemory(&ttapi_worker_params[i], sizeof( TTAPI_WORKER_PARAMS ));
	...
	for ( DWORD i = 0 ; i < ttapi_threads_count ; i++ ) {
		...
		// Setting thread name
		xr_sprintf( szThreadName , "Helper Thread #%u" , i );
...
VOID ttapi_Done()
{
	...
	// Freeing resources
	xr_free( ttapi_threads_handles );		ttapi_threads_handles = NULL;
	xr_free( ttapi_worker_params );		ttapi_worker_params = NULL;
