...
static bool g_exclusive_mouse = true;
static void on_error_dialog			(bool before)
{
	...
	if (before)
	{
		pInput->unacquire_mouse	();
		pInput->unacquire_keyboard();

		return;
	}

	pInput->acquire_mouse(pInput->our_app_has_mouse_priority());
	pInput->acquire_keyboard(false);
}

CInput::CInput						( BOOL bExclusive, int deviceForInit)
{
	g_exclusive_mouse					= !!bExclusive;
	...
	u32 kb_input_flags = DISCL_NONEXCLUSIVE | DISCL_FOREGROUND | DISCL_NOWINKEY;
...
void CInput::KeyUpdate	( )
{
	...
	{
		b_altF4				= TRUE;
		string4096	disconect_reason = "Alt F4";
		Engine.Event.Defer("KERNEL:disconnect", u64(xr_strdup(disconect_reason)), 0);
		Engine.Event.Defer("KERNEL:quit", u64(xr_strdup(disconect_reason)), 0);
	}
	...
	#endif
	{
		...
		for (i = 0; i < dwElements; i++)
		{
			...
			if(od[i].dwData & 0x80)
			{
				...
				// For when no input is set
				if (!Engine.External.XrGameModuleExist() && KBState[DIK_GRAVE])
				{
					if (Console->bVisible)
						Console->Hide();
					else
...
#ifdef MEASURE_ON_FRAME
	Device.Statistic->onframe_Input_ += measure_on_frame.GetElapsed_ms_f();
#endif
}
...
void CInput::unacquire_mouse		()
{
	pMouse->Unacquire				();
}

void CInput::unacquire_keyboard()
{
	pKeyboard->Unacquire();
}

void CInput::acquire_mouse			(const bool &exclusive)
{
...
void CInput::acquire_keyboard(const bool &exclusive)
{
...
void CInput::exclusive_mouse		(const bool &exclusive)
{
	g_exclusive_mouse = exclusive;
	unacquire_mouse();
	acquire_mouse(exclusive);
}

void CInput::exclusive_keyboard		(const bool &exclusive)
{
	g_exclusive_mouse = exclusive;
	unacquire_keyboard();
	acquire_keyboard(exclusive);
}

bool CInput::our_app_has_mouse_priority()
{
	return g_exclusive_mouse;
}
