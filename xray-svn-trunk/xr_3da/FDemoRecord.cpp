...
void CDemoRecord::MakeLevelMapProcess()
{
	...
	case DEVICE_RESET_PRECACHE_FRAME_COUNT+90:
		{
			...
			if(m_iLMScreenshotFragment==-1)
				xr_sprintf(tmp, sizeof(tmp), "map_%s, bb[%3.3f, %3.3f]-[%3.3f, %3.3f]", *g_pGameLevel->name(), bb.min.x, bb.min.z, bb.max.x, bb.max.z);
			else
				xr_sprintf(tmp, sizeof(tmp), "map_%s, hq. fragment %d, bb[%3.3f, %3.3f]-[%3.3f, %3.3f]", *g_pGameLevel->name(), m_iLMScreenshotFragment, bb.min.x, bb.min.z, bb.max.x, bb.max.z);
...
#pragma todo("bind these lerps to time delta")
		...
		m_vT.mul(m_vVelocity, speed);
		m_vR.mul(m_vAngularVelocity, ang_speed);
...
void CDemoRecord::IR_OnKeyboardPress	(int dik)
{
	...
	if (dik == DIK_RETURN && CApplication::isDeveloperMode)
	{	
...
void CDemoRecord::IR_OnKeyboardHold	(int dik)
{
	...
	case DIK_A:
	case DIK_NUMPAD1:
	case DIK_LEFT:		vT_delta.x -= 100.0f * FrameTimeDelta(); break; // Slide Left
	case DIK_D:
	case DIK_NUMPAD3:
	case DIK_RIGHT:		vT_delta.x += 100.0f * FrameTimeDelta(); break; // Slide Right
	case DIK_S:			vT_delta.y -= 100.0f * FrameTimeDelta(); break; // Slide Down
	case DIK_W:			vT_delta.y += 100.0f * FrameTimeDelta(); break; // Slide Up

	// rotate	
	case DIK_NUMPAD2:	vR_delta.x -= 50.0f * FrameTimeDelta(); break; // Pitch Down
	case DIK_NUMPAD8:	vR_delta.x += 50.0f * FrameTimeDelta(); break; // Pitch Up
	case DIK_E:	
	case DIK_NUMPAD6:	vR_delta.y += 50.0f * FrameTimeDelta(); break; // Turn Left
	case DIK_Q:	
	case DIK_NUMPAD4:	vR_delta.y -= 50.0f * FrameTimeDelta(); break; // Turn Right
	case DIK_NUMPAD9:	vR_delta.z -= 50.0f * FrameTimeDelta(); break; // Turn Right
	case DIK_NUMPAD7:	vR_delta.z += 50.0f * FrameTimeDelta(); break; // Turn Right
...
void CDemoRecord::IR_OnMouseMove		(int dx, int dy)
{
	...
	if (dx || dy)
	{
		...
		vR_delta.x += float(dy) * scale * (3.f / 4.f) * ((psMouseInvert.test(1)) ? -1 : 1); // pitch
	}
...
void CDemoRecord::IR_OnMouseHold		(int btn)
{
	...
	switch (btn)
	{
	case 0:			vT_delta.z += 100.0f * FrameTimeDelta(); break; // Move Backward
	case 1:			vT_delta.z -= 100.0f * FrameTimeDelta(); break; // Move Forward
	}
