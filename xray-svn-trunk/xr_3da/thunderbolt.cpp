...
void CEffect_Thunderbolt::Bolt(shared_str id, float period, float lt)
{
    ...
    if (next_v<environment.p_second_prop){
		next_lightning_time = EngineTime() + lt + EPS_L;
    }else{
		next_lightning_time = EngineTime() + period + Random.randF(-period*0.3f, period*0.3f);
...
void CEffect_Thunderbolt::OnFrame(shared_str id, float period, float duration)
{
	...
	if (bEnabled!=enabled){
    	bEnabled			= enabled;
		next_lightning_time = EngineTime() + period + Random.randF(-period*0.5f, period*0.5f);
	}
	else if (bEnabled && (EngineTime() > next_lightning_time))
	{
