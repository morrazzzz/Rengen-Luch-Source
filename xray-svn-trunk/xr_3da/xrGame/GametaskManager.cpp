...
CGameTask*	CGameTaskManager::GiveGameTaskToActor(CGameTask* t, u32 timeToComplete, bool bCheckExisting)
{
	...
	for (u32 i = 0; i < t->m_Objectives.size(); ++i){
		...
		if( std::find_if(article_vector.begin(), article_vector.end(), pred) == article_vector.end() ){
			...
			article_vector.push_back(ARTICLE_DATA(obj->article_id, GetGameTime(), article.data()->articleType));
			}
		}
...
void CGameTaskManager::UpdateActiveTask				()
{
	...
	{
		bool bDone								=false;
		GameTasksVec::iterator it			= GetGameTasks().begin();
		GameTasksVec::iterator it_e		= GetGameTasks().end();
...
void CGameTaskManager::DumpTasks()
{
	...
	for (; it != it_e; ++it)
	{
		...
		auto itt = gt->m_Objectives.begin();
		auto itt_e = gt->m_Objectives.end();

		Msg(" Task ID=[%s], prio=[%d]", gt->m_ID.c_str(), gt->m_priority);

		for (; itt != itt_e; ++itt)
		{
			const SGameTaskObjective* obj = &(*itt);

			const ETaskState val = obj->TaskState();

			Msg(" Obj ID=[%s] state=[%s]", gt->m_ID.c_str(), sTaskStates[val]);
		}
	}
}