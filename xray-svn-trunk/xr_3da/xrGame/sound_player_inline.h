...
IC	u32	CSoundPlayer::active_sound_count(bool only_playing) const
{
	...
	if (!only_playing) {
		for ( ; I != E; ++I)
			if ((*I).m_sound->_feedback() || ((*I).m_start_time <= EngineTimeU()))
				++count;
	}
	else {
		for ( ; I != E; ++I)
...
IC	const CSoundPlayer::SOUND_COLLECTIONS &CSoundPlayer::sound_collections	() const
{
	return				(m_sound_collections);
}
