...
{
#	ifdef XRSE_FACTORY_EXPORTS
	PHelper().CreateFloat		(items, PrepareKey(pref,*s_name,"hold position time").c_str(), 	&m_hold_position_time,	0.f, 60.f);
	RListValue *value			= PHelper().CreateRList	 (items,	PrepareKey(pref,*s_name,"description").c_str(),			&m_description,			&*fp_data.smart_covers.begin(),	fp_data.smart_covers.size());
	value->OnChangeEvent.bind	(this,&CSE_SmartCover::OnChangeDescription);

	PHelper().CreateFloat		(items, PrepareKey(pref,*s_name,"enter min enemy distance").c_str(),&m_enter_min_enemy_distance,	0.f, 100.f);
	PHelper().CreateFloat		(items, PrepareKey(pref,*s_name,"exit min enemy distance").c_str(),	&m_exit_min_enemy_distance,		0.f, 100.f);

	if (is_combat_cover(m_description)) {
		PHelper().CreateBOOL	(items, PrepareKey(pref, *s_name, "is combat cover").c_str(), &m_is_combat_cover);
		PHelper().CreateBOOL	(items, PrepareKey(pref, *s_name, "can fire").c_str(), &m_can_fire);
	}
