...
//	m_bExplodeEventSent		= false;
	...
	isReadyToExpload_		= false;

	hasDistExplodeSnd_		= false;
}
...
void CExplosive::LightCreate()
{
	...
	m_pLight->set_use_static_occ(false);
}
...
CExplosive::~CExplosive(void) 
{
	...
	if (hasDistExplodeSnd_)
		sndExplodeDist.destroy();
}

void CExplosive::LoadCfg(LPCSTR section) 
{
...
void CExplosive::Load(CInifile const *ini,LPCSTR section)
{
	...
	if (ini->line_exist(section, "snd_explode_dist"))
	{
		snd_name = ini->r_string(section, "snd_explode_dist");
		sndExplodeDist.create(snd_name, st_Effect, m_eSoundExplode);

		hasDistExplodeSnd_ = true;
	}
...
void CExplosive::DestroyClientObj()
{
...
#endif
//	Msg("---------CExplosive Explode [%d] frame[%d]",cast_game_object()->ID(), CurrentFrame());
	OnBeforeExplosion();

	//играем звук взрыва

	if (hasDistExplodeSnd_ && pos.distance_to(Device.vCameraPosition) > GameConstants::GetDistantSndApplyDistance())
		Sound->play_at_pos(sndExplodeDist, 0, pos, false);
	else
...
void CExplosive::UpdateCL() 
{
	...
	if (isReadyToExpload_)
	{
		VERIFY(0xffff != Initiator());

		SetInitiator(Initiator());
		ExplodeParams(exploadPos_, exploadNormal_);
		Explode();
...
//		Msg("---------CExplosive OnAfterExplosion [%d] frame[%d]",cast_game_object()->ID(), CurrentFrame());
	} 
	else
	{		
...
void CExplosive::OnAfterExplosion()
{
	if(m_pExpParticle){
		m_pExpParticle->Stop();
		DestroyParticleInstance(m_pExpParticle);
		m_pExpParticle = NULL;
	}
...
void CExplosive::OnBeforeExplosion()
{
	...
	{
		HideExplosive();
		//	Msg("---------CExplosive OnBeforeExplosion setVisible(false) [%d] frame[%d]",cast_game_object()->ID(), CurrentFrame());
	}
}
...
{
	exploadPos_ = pos;
	exploadNormal_ = normal;

	isReadyToExpload_ = true;
}
...
void CExplosive::RemoveLinksToCLObj(CObject* O)
{
