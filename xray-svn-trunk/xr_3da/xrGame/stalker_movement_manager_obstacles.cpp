...
//		Msg							( "%6d stalker %s waits for the some door to be open/closed", EngineTimeU(), object().ObjectName().c_str() );
		float						desirable_speed = old_desirable_speed();
		set_desirable_speed			(0.f);
...
//	Msg								( "%6d stalker %s is going", EngineTimeU(), object().ObjectName().c_str() );
	...
	if (EngineTimeU() < (m_last_fail_time + fail_check_time)) {
		inherited::move_along_path	( movement_control, dest_position, time_delta);
		return;
	}
