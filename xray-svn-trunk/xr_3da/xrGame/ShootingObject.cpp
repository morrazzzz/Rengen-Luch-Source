...
#define SHADOWED_FIRE_DIST 25.f
...
void CShootingObject::LoadCfg	(LPCSTR section)
{
...
void CShootingObject::Light_Start	()
{
	...
	if (CurrentFrame() != light_frame)
	{
		light_frame = CurrentFrame();
...
void CShootingObject::Light_Render	(const Fvector& P)
{
	...
	light_render->set_shadow	((gameObjectLightShadows_ == TRUE) ? (Device.GetDistFromCamera(P) < SHADOWED_FIRE_DIST ? true : false) : false);
...
void CShootingObject::StopParticles (CParticlesObject*&	pParticles)
{
	...
	pParticles->Stop		();
	DestroyParticleInstance(pParticles);
}
...
void CShootingObject::UpdateFlameParticles	()
{
	...
	{
		m_pFlameParticles->Stop();
		DestroyParticleInstance(m_pFlameParticles);
	}
}
								 ...
								 bool send_hit)
{
	...
	if (m_bUseAimBullet)
	{
		if (ParentMayHaveAimBullet())
		{
			...
			{
				if ((EngineTime() - m_fPredBulletTime) >= m_fTimeToAim)
				{
					aim_bullet=true;
				}
				else
