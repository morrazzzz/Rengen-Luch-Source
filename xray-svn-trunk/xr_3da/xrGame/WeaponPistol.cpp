...
void CWeaponPistol::DestroyClientObj()
{
	inherited::DestroyClientObj();
}


void CWeaponPistol::LoadCfg	(LPCSTR section)
{
	inherited::LoadCfg		(section);

	m_sounds.LoadSound(section, "snd_close", "sndClose", 0, false, m_eSoundClose);
}

void CWeaponPistol::BeforeAttachToParent()
{
