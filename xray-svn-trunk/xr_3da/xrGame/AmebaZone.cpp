...
void CAmebaZone::LoadCfg(LPCSTR section)
{
...
void  CAmebaZone::Affect(SZoneObjectInfo* O) 
{
...
#ifdef DEBUG
	char l_pow[255]; 
	xr_sprintf(l_pow, "zone hit. %.1f", Power(distance_to_center(O->object), Radius()));
	...
	float power = Power(distance_to_center(O->object), Radius());
	...
	for(it = m_ObjectInfoMap.begin(); m_ObjectInfoMap.end() != it; ++it) 
	{
		...
		if(EA)
		{
			...
			if(mc)
			{
				...
				//vel.invert();
				//vel.mul(mc->GetMass());
				if(distance_to_center(EA) < effective_radius(Radius()))
