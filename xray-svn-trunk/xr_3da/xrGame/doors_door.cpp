	...
	m_locked					( false )
{
	...
	m_object.spatial.s_type		|=	STYPE_VISIBLEFORAI;
}
...
#ifdef DEBUG
	if ( g_debug_doors)
		Msg						( "door[%s] is locked", m_object.ObjectName().c_str() );
#endif // #ifdef DEBUG
}
...
#ifdef DEBUG
	if ( g_debug_doors)
		Msg						( "door[%s] is unlocked", m_object.ObjectName().c_str() );
#endif // #ifdef DEBUG
}
...
#ifdef DEBUG
	if ( g_debug_doors)
		Msg						( "door[%s] started to change its state to [%s]", m_object.ObjectName().c_str(), m_target_state == door_state_open ? "open" : "closed" );
#endif // #ifdef DEBUG
}
...
#ifdef DEBUG
		if ( g_debug_doors)
			Msg					( "door[%s] added initiator[%s] to keep door %s", m_object.ObjectName().c_str(), initiator->get_name(), m_target_state == door_state_open ? "open" : "closed" );
#endif // #ifdef DEBUG
...
#ifdef DEBUG
		if ( g_debug_doors)
			Msg					( "door[%s] removed initiator[%s] to keep door %s", m_object.ObjectName().c_str(), initiator->get_name(), m_target_state == door_state_open ? "open" : "closed" );
#endif // #ifdef DEBUG
//		if ( !xr_strcmp( "sim_default_duty_28212", initiator->get_name()) ) {
//			int i=0; (void)i;
//		}
...
#ifdef DEBUG
		if ( g_debug_doors)
			Msg					( "door[%s] restores its state to %s", m_object.ObjectName().c_str(), m_target_state == door_state_open ? "open" : "closed" );
#endif // #ifdef DEBUG
//		if ( !xr_strcmp( "sim_default_duty_28212", initiator->get_name()) ) {
//			int i=0; (void)i;
//		}
...
{
	return						m_object.ObjectName().c_str();
}
