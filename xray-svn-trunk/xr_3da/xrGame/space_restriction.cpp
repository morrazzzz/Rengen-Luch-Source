...
#ifdef DEBUG
	if (psAI_Flags2.test(aiTestCorrectness))
	{
		if (m_out_space_restriction) {
...
{
	R_ASSERT(this); // Retarded, but happens
	...
	{
		Msg(LINE_SPACER);
		Msg("^ Space Restr Name %s, requested pos = %f %f %f", this ? name().c_str() : "", VPUSH(position));
		LogStackTrace("accessible_nearest Error");
		Msg(LINE_SPACER);
