...
public:
	...
	virtual void StartTalk		(CInventoryOwner* talk_partner, bool start_trade = false);
	...
	bool IsNoInvWnd()					{ return noInvWnd_; };

	void EnableTrade			()		{m_bAllowTrade = true;}
	void DisableTrade			()		{m_bAllowTrade = false;}
	bool IsTradeEnabled			()		{ return m_bAllowTrade;}

	void EnableInvUpgrade		()		{ m_bAllowUpgrade = true; }
	void DisableInvUpgrade		()		{ m_bAllowUpgrade = false; }
...
public:
	...
	virtual void			SetIORank		(CHARACTER_RANK_VALUE);
	virtual void			ChangeIORank	(CHARACTER_RANK_VALUE);
	...
	IC CHARACTER_RANK_VALUE			GetIORank()		const { return CharacterInfo().GetCharacterRank().value(); };
	CHARACTER_REPUTATION_VALUE		Reputation()	const { return CharacterInfo().Reputation().value(); };
	CHARACTER_RANK_VALUE			T_GetRank()		const { return CharacterInfo().GetCharacterRank().value(); };
	float							Sympathy()		const { return CharacterInfo().Sympathy(); }

	IC bool							IsTrader()		const { return isTrader_; };
...
public:
	...
	virtual void			OnItemBelt				(CInventoryItem *inventory_item, const EItemPlace& previous_place);
	virtual void			OnItemRuck				(CInventoryItem *inventory_item, const EItemPlace& previous_place);
	virtual void			OnItemSlot				(CInventoryItem *inventory_item, const EItemPlace& previous_place);
