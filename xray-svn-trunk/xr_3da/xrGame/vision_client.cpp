...
void vision_client::eye_pp_s2()
{
	...
	feel_vision_update(m_object, m_position, float(dwDT) / 1000.f, visual().GetTransparencyThreshold());
...
void vision_client::ScheduledUpdate(u32 dt)
{
...
#ifdef MEASURE_UPDATES
	Device.Statistic->scheduler_VisionClient_ += measure_sc_update.GetElapsed_sec();
#endif
}

shared_str vision_client::SchedulerName() const
{
	string256 temp;
	xr_sprintf(temp,"vision_client[%s]",*object().ObjectName());
