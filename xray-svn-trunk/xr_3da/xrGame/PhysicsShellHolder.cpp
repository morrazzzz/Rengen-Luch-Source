...
BOOL CPhysicsShellHolder::SpawnAndImportSOData				(CSE_Abstract*	DC)
{
	CParticlesPlayer::OnClientObjectSpawned		();
	st_enable_state=(u8)stNotDefitnite;
	b_sheduled									=	true;
	BOOL ret=inherited::SpawnAndImportSOData(DC);//load
		//create_physic_shell			();
...
void CPhysicsShellHolder::correct_spawn_pos()
{
	...
	R_ASSERT2( _valid( c ), make_string( "object: %s model: %s ", CObject::ObjectName().c_str(), VisualName().c_str() ) );
	R_ASSERT2( _valid( size ), make_string( "object: %s model: %s ", CObject::ObjectName().c_str(), VisualName().c_str() ) );
	R_ASSERT2( _valid( XFORM() ), make_string( "object: %s model: %s ", CObject::ObjectName().c_str(), VisualName().c_str() ) );
...
void CPhysicsShellHolder::activate_physic_shell()
{
	...
	if(H_Parent()&&H_Parent()->Visual())
	{
		...
		smart_cast<IKinematics*>(H_Parent()->Visual())->CalculateBones	(force_recalc);
	}
...
LPCSTR CPhysicsShellHolder::NameObject()const
{
	return CObject::ObjectName().c_str();
}
LPCSTR CPhysicsShellHolder::ObjectNameVisual()const
{
	return VisualName().c_str();
}
