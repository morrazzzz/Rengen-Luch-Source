...
CScriptSound::~CScriptSound		()
{
	...
	{
		Msg("* Stopping ScriptSound. Name = %s", m_sound._handle() ? m_sound._handle()->file_name() : "unknown");
		Stop(); // Instead of crashing in next line - it's better to just stop script sound before destroying it
	}
...
//	Msg							("%6d : CScriptSound::Play (%s), delay %f, flags %d",EngineTimeU(),m_sound._handle()->file_name(),delay,flags);
