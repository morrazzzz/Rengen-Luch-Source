...
#pragma todo("Make these params to look for values, not only for existnce of any value; ex: param = '0' still makes it true with current code")
	...
	s = g_uiSpotXml->ReadAttrib(path_base, 0, "hint_priority", NULL);
	if (s)
	{
		int str_to_num = std::stoi(s);

		R_ASSERT2(str_to_num <= 255 && str_to_num >= 0, "Possible bug here, contact coders");

		m_hint_priority = (u8)str_to_num;
	}
	...
	s = g_uiSpotXml->ReadAttrib(path_base, 0, "is_location_mark", NULL); // will show, only if show locations button is toggled in pda map window
	if (s)
		m_flags.set(eMapLocation, TRUE);

	m_ttl = g_uiSpotXml->ReadAttribInt(path_base, 0, "ttl", 0);
	if(m_ttl>0){
		m_flags.set( eTTL, TRUE);
		m_actual_time = EngineTimeU()+m_ttl*1000;
	}
...
{
	R_ASSERT(m_cached.m_updatedFrame != CurrentFrame());


	if (m_flags.test(eTTL))
	{
		if (m_actual_time < EngineTimeU())
		{
			m_cached.m_Actuality = false;
			m_cached.m_updatedFrame = CurrentFrame();
...
void CMapLocation::UpdateSpot(CUICustomMap* map, CMapSpot* sp )
{
	if (map->MapName() == GetLevelName())
	{
		...
		if (!IsUserDefined() && b_alife && !m_owner_se_object)
		{
		...
		if (!IsUserDefined() && b_alife && m_flags.test(eHideInOffline) && !m_owner_se_object->m_bOnline)
		{
			is_shown = false;
		...
		if (!IsUserDefined() && b_alife && m_owner_se_object->m_flags.test(CSE_ALifeObject::flVisibleForMap) == FALSE)
		{
			is_shown = false;
		...
		CSE_ALifeCreatureAbstract* alive_obj = m_owner_se_object ? smart_cast<CSE_ALifeCreatureAbstract*>(m_owner_se_object) : nullptr;
		...
		if (!!m_flags.test(eMapLocation) && !drawMapLocations_)
		{
			is_shown = false;
			return;
		}
