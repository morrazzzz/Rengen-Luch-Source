...
void CMaterialManager::LoadCfg			(LPCSTR section)
{
	R_ASSERT3				(pSettings->line_exist(section,"material"),"Material not found in the section ",*(m_object->SectionName()));
	m_my_material_idx		= GMLib.GetMaterialIdx(pSettings->r_string(section,"material"));
		...
		if( debug_character_material_load )
		{
			...
			if( entity_alive )
				...
				VERIFY( m );
				Msg( "(CMaterialManager::LoadCfg(LPCSTR section)) material: %s loaded for %s, from section: %s ", m->m_Name.c_str(), entity_alive->ObjectName().c_str(), section ); 
			}
		}
#endif
}
...
//		}
		...
		if( debug_character_material_load )
		{
			...
			SGameMtl *m = GMLib.GetMaterialByIdx( m_my_material_idx );
			VERIFY( m );
			Msg( "(CMaterialManager::reinit) material: %s loaded for %s ", m->m_Name.c_str(), entity_alive->ObjectName().c_str() ); 
		}
#endif
	}
}
