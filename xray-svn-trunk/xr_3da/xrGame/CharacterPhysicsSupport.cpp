...
void CCharacterPhysicsSupport::in_Load( LPCSTR section )
{
	...
	CPHDestroyable::LoadCfg( section );
}
...
void CCharacterPhysicsSupport::in_NetSpawn( CSE_Abstract* e )
{
	...
	if( m_EntityAlife.use_simplified_visual	( ) )
	{
		...
		ka->CalculateBones(BonesCalcType::force_recalc);
...
{
	if( pSettings->line_exist( ea.SectionName().c_str(), "hack_terrible_donot_collide_on_spawn") &&
		pSettings->r_bool( ea.SectionName().c_str(), "hack_terrible_donot_collide_on_spawn")
		) return true;
	return false;
}
...
void CCharacterPhysicsSupport::SpawnInitPhysics( CSE_Abstract* e )
{
	...
	{
#ifdef DEBUG
		if( ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject)&&stricmp( PH_DBG_ObjectTrackName( ), *m_EntityAlife.ObjectName( ) )==0 )
		{
	...
	{
#ifdef DEBUG
		Msg("! saved bones %d , current bones %d, object :%s", saved_bones.size(), m_EntityAlife.PHGetSyncItemsNumber(), m_EntityAlife.ObjectName().c_str() );
#endif
...
{
	m_sv_hit = H;
	m_hit_valide_time = EngineTimeU() + hit_valide_time;
	...
	/*if( m_flags.test( fl_block_hit ) )
	{
		...
		if(EngineTimeU() - m_EntityAlife.GetLevelDeathTime( ) >= 2000 )
	...
	{
		// accumulate impulse
		Fvector3 newImpulse = Fvector3(H.dir).normalize().mul(H.impulse);
	...
	if( !( m_pPhysicsShell && m_pPhysicsShell->isActive( ) ) )
	{
		...
		m_pPhysicsShell->applyHit(H.bone_space_position(), impulseDir, impulseMagnitude, H.bone(), H.hit_type);
...
void CCharacterPhysicsSupport::in_UpdateCL( )
{
	...
	else if( ik_controller( ) )
	{
		...
		ik_controller( )->UpdateKinematics();
	}
	
	if (!fis_zero(m_death_impulse_vec.magnitude())) {
		float reducedMagnitude = m_death_impulse_vec.magnitude() - m_death_impulse_cooldown_speed * m_character_shell_control.TimeDeltaCalced();
...
#ifdef DEBUG	
	Msg("shell for %s[%d] created in %f ms",*m_EntityAlife.ObjectName(),m_EntityAlife.ID(),t.GetElapsed_sec()*1000.f);
#endif
}
...
{
	m_physics_shell_animated_time_destroy = EngineTimeU() + physics_shell_animated_destroy_delay;
...
void CCharacterPhysicsSupport::update_animation_collision		( )
{
	if( animation_collision( ) )
	{
			...
			if(EngineTimeU() > m_physics_shell_animated_time_destroy )
						destroy_animation_collision		( );
	}
}
#ifdef	DEBUG
...
if( dbg_draw_ragdoll_spawn )
{
	...
	//if( false &&  anim_mov_ctrl && anim_mov_blend && anim_mov_blend->blend != CBlend::eFREE_SLOT &&  anim_mov_blend->timeCurrent + Device.TimeDelta()*anim_mov_blend->speed < anim_mov_blend->timeTotal-SAMPLE_SPF-EPS)//.
	//{
	//	const Fmatrix sv_xform = mXFORM;
	//	mXFORM.set( start_xform );
	//	//anim_mov_blend->blendPower = 1;
	//	anim_mov_blend->timeCurrent  += Device.TimeDelta() * anim_mov_blend->speed;
	//	m_pPhysicsShell->AnimToVelocityState( Device.TimeDelta(), 2 * default_l_limit, 10.f * default_w_limit );
	//	mXFORM.set( sv_xform );
	//}
	IKinematics* K=smart_cast<IKinematics*>( m_EntityAlife.Visual( ) );
...
void CCharacterPhysicsSupport::in_ChangeVisual()
{
	...
	if( KA )
	{
		m_death_anims.setup( KA, m_EntityAlife.SectionName().c_str() , pSettings );
		if( Type( ) != etBitting )
...
{
	if( m_hit_valide_time < EngineTimeU() || !m_sv_hit.is_valide() )
	{
		if( m_EntityAlife.use_simplified_visual( ) )
			return;
		ActivateShell( NULL );
