...
static void interactive_motion_diag( LPCSTR message, const CBlend &b, CPhysicsShell *s, float time_left )
{
	...
	Msg( "death anims - interactive_motion:- %s, motion: %s, blend time %f , total blend time %f , time left: %f , obj: %s, model:  %s ", message, motion_name, b.timeCurrent, b.timeTotal, time_left, O->ObjectName().c_str(), O->VisualName().c_str());
#endif
}
...
#ifdef	DEBUG
	if( collide_obj )
		return make_string("collide obj: %s", collide_obj->ObjectName().c_str() );
	else
