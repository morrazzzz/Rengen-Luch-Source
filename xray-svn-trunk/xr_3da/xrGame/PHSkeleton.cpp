...
void CPHSkeleton::LoadCfg(LPCSTR section)
{
...
void CPHSkeleton::Update(u32 dt)
{
	...
	if (b_removing&&
		EngineTimeU() > m_remove_time&&
		//(EngineTimeU()-m_unsplit_time)*phTimefactor>remove_time&&
		m_unsplited_shels.empty()
		&& Device.GetDistFromCamera(PPhysicsShellHolder()->Position()) > GameConstants::GetDontRemovedDestroyableDist() * 0.8f // 0.8 because to avoid case when object is removed before its parts
		) 
...
{
	CSE_Abstract*				D = F_entity_Create("ph_skeleton_object");//*SectionName()
	R_ASSERT(D);
	/////////////////////////////////////////////////////////////////////////////////////////////
...
void CPHSkeleton::UnsplitSingle(CPHSkeleton* SO)
{
	...
	pKinematics->CalculateBones(BonesCalcType::force_recalc);
	...
	newKinematics->CalculateBones(BonesCalcType::force_recalc);
	...
	SO->CopySpawnInit		();
	CopySpawnInit			();
	VERIFY3(CheckObjectSize(pKinematics),*(O->VisualName()),"Object unsplit whith no size");
	VERIFY3(CheckObjectSize(newKinematics),*(O->VisualName()),"Object unsplit whith no size");
...
{
	b_removing=true;
	m_remove_time=EngineTimeU()+iFloor(time/phTimefactor);
	SetNotNeedSave();
