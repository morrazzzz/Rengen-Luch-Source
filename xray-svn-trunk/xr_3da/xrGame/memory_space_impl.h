...
IC	void CObjectParams<T>::fill				(const T *game_object)
{
	...
	if (game_object && ai().get_level_graph() && ai().level_graph().valid_vertex_id(m_level_vertex_id) && !ai().level_graph().inside(m_level_vertex_id,game_object->Position())) {
		Fvector temp = ai().level_graph().vertex_position(m_level_vertex_id);

		SetMemorizedPos(Fvector().set(temp.x, game_object->Position().y, temp.z));
	...
	*/
	...
	{
		Fvector temp;
		game_object->Center(temp);

		SetMemorizedPos(temp);
		SetMemorizedPos(Fvector().set(game_object->Position().x, m_memorized_position.y, game_object->Position().z));
	}
	else
		SetMemorizedPos(Fvector().set(0.f, 0.f, 0.f));
}
...
#endif
#ifdef USE_GAME_TIME
	m_game_time				= GetGameTime();
#endif
#ifdef USE_LEVEL_TIME
	m_level_time			= EngineTimeU();
#endif
