...
void CCameraFirstEye::LoadCfg(LPCSTR section)
{
...
void CCameraFirstEye::Move( int cmd, float val, float factor )
{
	...
	switch (cmd){
	case kDOWN:		pitch	-= val?val:(rot_speed.y*TimeDelta()/factor);	break;
	case kUP:		pitch	+= val?val:(rot_speed.y*TimeDelta()/factor);	break;
	case kLEFT:		yaw		-= val?val:(rot_speed.x*TimeDelta()/factor);	break;
	case kRIGHT:	yaw		+= val?val:(rot_speed.x*TimeDelta()/factor);	break;
	}
