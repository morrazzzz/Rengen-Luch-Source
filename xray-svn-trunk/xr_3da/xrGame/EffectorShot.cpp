...
void CWeaponShotEffector::Shot( CWeapon* weapon )
{
	...
	float angle	= m_cam_recoil.camDispersion    * weapon->cur_silencer_koef.cam_dispersion;
	angle      += m_cam_recoil.camDispersionInc * weapon->cur_silencer_koef.cam_disper_inc * (float)m_shot_numer;
	Shot2( angle );
}

void CWeaponShotEffector::Shot2( float angle )
{
	m_angle_vert += angle * ( m_cam_recoil.camDispersionFrac + m_Random.randF(-1.0f, 1.0f) * (1.0f - m_cam_recoil.camDispersionFrac) );

	clamp( m_angle_vert, -m_cam_recoil.camMaxAngleVert, m_cam_recoil.camMaxAngleVert );
	if ( fis_zero(m_angle_vert - m_cam_recoil.camMaxAngleVert) )
	{
		m_angle_vert *= m_Random.randF( 0.96f, 1.04f );
	}
	
	float rdm  = m_Random.randF( -1.0f, 1.0f );
	m_angle_horz += (m_angle_vert / m_cam_recoil.camMaxAngleVert) * rdm * m_cam_recoil.camStepAngleHorz;

	clamp( m_angle_horz, -m_cam_recoil.camMaxAngleHorz, m_cam_recoil.camMaxAngleHorz );
...
{
	float time_to_relax    = _abs(m_angle_vert) / m_cam_recoil.camRelaxSpeed;
	...
	{
		m_angle_vert -= m_cam_recoil.camRelaxSpeed * dt;
...
{
	if ( m_actived && m_cam_recoil.camReturnMode /*|| m_single_shot*/ )
	{
		Relax();
	}
	
	if ( !m_cam_recoil.camReturnMode && m_shot_end && !m_single_shot )
	{
		m_actived = false;
	}
...
//	Msg( " <<[%d]  v=%.4f  dv=%.4f   a=%d s=%d  fr=%d", m_shot_numer, m_angle_vert, m_delta_vert, m_actived, m_first_shot, CurrentFrame() );
}
...
//		m_actived = false;
//	}
//	Msg( "[%d]  pitch = %.4f   yaw = %.4f    fs=%d    a=%d  fr=%d", m_shot_numer, *pitch, *yaw, m_first_shot, m_actived, CurrentFrame() );
