...
{
	CSE_Abstract* server_ogj = object_factory().server_object(pSettings->r_clsid(section, "class"), section);

	if (!server_ogj)
	{
		LPCSTR classs = pSettings->r_string(section, "class");
		return 0;
		//R_ASSERT2(false, make_string("Server object creation failed: class %s, section %s", classs, section));
	}
...
BOOL CSE_Abstract::Spawn_Read				(NET_Packet	&tNetPacket)
{
	...
	string256					temp;
	u8							game_id_deprecated;
	tNetPacket.r_stringZ		(temp);
	set_name_replace			(temp);
	tNetPacket.r_u8				(game_id_deprecated); // game id
...
#ifdef XRGAME_EXPORTS
#	ifdef DEBUG
    PHelper().CreateU16			(items,	PrepareKey(pref, "Respawn Time (s)").c_str(),	&RespawnTime,	0,43200);
