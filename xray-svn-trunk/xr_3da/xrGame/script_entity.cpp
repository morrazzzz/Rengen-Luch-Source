...
#ifdef DEBUG
	if (bScriptControl)
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeInfo,"Script %s set object %s under its control",*caSciptName,*object().ObjectName());
	else
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeInfo,"Script %s freed object %s from its control",*caSciptName,*object().ObjectName());
#endif
	if (!bScriptControl)
...
bool CScriptEntity::CheckObjectVisibility(const CGameObject *tpObject)
{
	...
	return				(m_monster->memory().VisualMManager().visible_now(tpObject));
}
...
bool CScriptEntity::CheckTypeVisibility(const char* section_name)
{
	...
	CVisualMemoryManager::VISIBLES::const_iterator	I = m_monster->memory().VisualMManager().GetMemorizedObjects().begin();
	CVisualMemoryManager::VISIBLES::const_iterator	E = m_monster->memory().VisualMManager().GetMemorizedObjects().end();
	for ( ; I != E; ++I) {
		VERIFY			((*I).m_object);
		if (!xr_strcmp(section_name, *(*I).m_object->SectionName()))
			return		(true);
	}
	return				(false);
}
...
void CScriptEntity::vfFinishAction(CScriptEntityAction *tpEntityAction)
{
	...
	if (!tpEntityAction->m_tParticleAction.m_bAutoRemove)
		DestroyParticleInstance(tpEntityAction->m_tParticleAction.m_tpParticleSystem);
}
...
#ifdef _DEBUG
//		if (!xr_strcmp("m_stalker_wounded",*object().ObjectName()))
//			Msg			("%6d Processing action : %s",EngineTimeU(),*l_tpEntityAction->m_tAnimationAction.m_caAnimationToPlay);
#endif
...
#ifdef _DEBUG
//		if (!xr_strcmp("m_stalker_wounded",*object().ObjectName()))
//			Msg			("%6d Action completed : %s",EngineTimeU(),*l_tpEntityAction->m_tAnimationAction.m_caAnimationToPlay);
#endif

		vfFinishAction(l_tpEntityAction);

#ifdef DEBUG
		if (psAI_Flags.is(aiDebugMsg))
			Msg("Entity Action removed!!!");
#endif
		if (true /*psAI_Flags.is(aiLua)*/ )
		{
			object().callback(GameObject::eActionTypeRemoved)(object().lua_game_object(),u32(ScriptEntity::eActionTypeRemoved));
		}
...
#ifdef DEBUG
		if (empty_queue)
			ai().script_engine().script_log	(ScriptStorage::eLuaMessageTypeInfo,"Object %s has an empty script queue!",*object().ObjectName());
#endif
		return;
	}
...
#ifdef _DEBUG
//				Msg									("%6d Starting sound %s",EngineTimeU(),*l_tSoundAction.m_caSoundToPlay);
#endif
...
//			Msg			("%6d Object %s, position [%f][%f][%f]",EngineTimeU(),*l_tpGameObject->ObjectName(),VPUSH(l_tpGameObject->Position()));
		...
		case CScriptMovementAction::eGoalTypePathPosition : {
			...
			if (!ai().level_graph().valid_vertex_id(vertex_id)) {
				string256	S;
				xr_sprintf		(S,"Cannot find corresponding level vertex for the specified position [%f][%f][%f] for monster %s",VPUSH(l_tMovementAction.m_tDestinationPosition),*m_monster->ObjectName());
...
void CScriptEntity::DestroyClientObj()
{
	m_initialized			= false;
}
...
#ifdef DEBUG
	if (!GetScriptControl()) {
		ai().script_engine().script_log(eLuaMessageTypeError,"Object %s is not under script control while you are trying to get patrol path name!",*m_object->ObjectName());
		return "";
	}
#endif
	if (m_tpActionQueue.empty())
...
BOOL CScriptEntity::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
	m_initialized					= true;
...
void CScriptEntity::ScheduledUpdate(u32 DT)
{
...
#ifdef MEASURE_UPDATES
	Device.Statistic->scheduler_ScriptEntity_ += measure_sc_update.GetElapsed_sec();
#endif
}
...
#ifdef DEBUG
			//if (!xr_strcmp("m_stalker_wounded",*object().ObjectName()))
			//	Msg				("%6d Playing animation : %s , Object %s",EngineTimeU(),*GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay, *object().ObjectName());
#endif
...
#ifdef MEASURE_UPDATES
	Device.Statistic->updateCL_ScriptEntity_ += measure_updatecl.GetElapsed_sec();
#endif
}
