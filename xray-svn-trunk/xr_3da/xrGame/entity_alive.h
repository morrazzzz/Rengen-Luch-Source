...
//	virtual float			g_MaxHealth				()	const;
	...
	// Visibility related
	virtual	float			GetEyeFovValue			()	const			= 0;	
	virtual	float			GetEyeRangeValue		()	const			= 0;	
	
	virtual bool			human_being				() const			{return	(false);}

	virtual void			OnStartTrade			();
	virtual void			OnStopTrade				();
	virtual void			OnPerformTrade			(u32 money_get, u32 money_put);
public:
...
public:
	...
	void						SetBoneSurfListActual(bool val)	{ protectBonesSurfBool_.Enter(); m_hit_bone_surface_areas_actual = val; protectBonesSurfBool_.Leave(); };
	bool						IsBoneSurfListActual()			{ protectBonesSurfBool_.Enter(); bool ret = m_hit_bone_surface_areas_actual; protectBonesSurfBool_.Leave(); return ret; };

private:
			void				fill_hit_bone_surface_areas	();

private:
	typedef xr_vector< std::pair<u16,float> >	hit_bone_surface_areas_type;

	AccessLock							protectGetLocalPoint_;
	AccessLock							protectBonesSurfBool_;

private:
	hit_bone_surface_areas_type	m_hit_bone_surface_areas;
	CRandom						m_hit_bones_random;
	bool						m_hit_bone_surface_areas_actual;
};
