...
bool CAI_Stalker::tradable_item(CInventoryItem* inventory_item, const u16& current_owner_id)
{
	...
	return(trade_parameters().enabled(CTradeParameters::action_sell(0), inventory_item->object().SectionName()));
}
...
void CAI_Stalker::attach_available_ammo(CWeapon *weapon)
{
	...
	for (; I != E; ++I)
	{
		...
		if (std::find(weapon->m_ammoTypes.begin(), weapon->m_ammoTypes.end(), (*I).m_item->object().SectionName()) == weapon->m_ammoTypes.end())
			continue;
...
void CAI_Stalker::choose_detector()
{
	...
	for (; I != E; ++I)
	{
		...
		CAnomDetectorBase* detector = smart_cast<CAnomDetectorBase*>((*I).m_item);
...
{
	if (!g_Alive())
		return(trade_parameters().enabled(CTradeParameters::action_show(0), item->object().SectionName()));
...
bool CAI_Stalker::enough_ammo(const CWeapon *new_weapon) const
{
	...
	for (; I != E; ++I)
	{
		if (std::find(new_weapon->m_ammoTypes.begin(), new_weapon->m_ammoTypes.end(), (*I)->object().SectionName()) == new_weapon->m_ammoTypes.end())
			continue;
...
void CAI_Stalker::remove_personal_only_ammo(const CInventoryItem* item)
{
	...
	for (; I != E; ++I)
	{
		...
		for (i = inventory().allContainer_.begin(); i != e; ++i)
		{
			if (xr_strcmp(*I, (*i)->object().SectionName()))
				continue;
...
void CAI_Stalker::on_after_take(const CGameObject *object)
{
	...
	if (!READ_IF_EXISTS(pSettings, r_bool, SectionName(), "use_single_item_rule", true))
		return;
