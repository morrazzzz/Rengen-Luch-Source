...
void CCameraLook::LoadCfg(LPCSTR section)
{
...
{
	switch (cmd){
	case kCAM_ZOOM_IN:	dist	-= val?val:(rot_speed.z*TimeDelta());	break;
	case kCAM_ZOOM_OUT:	dist	+= val?val:(rot_speed.z*TimeDelta());	break;
	case kDOWN:			pitch	-= val?val:(rot_speed.x*TimeDelta()/factor);	break;
	case kUP:			pitch	+= val?val:(rot_speed.x*TimeDelta()/factor);	break;
	case kLEFT:			yaw		-= val?val:(rot_speed.y*TimeDelta()/factor);	break;
	case kRIGHT:		yaw		+= val?val:(rot_speed.y*TimeDelta()/factor);	break;
	}
...
void CCameraLook2::LoadCfg(LPCSTR section)
{
	CCameraLook::LoadCfg		(section);
	m_cam_offset			= pSettings->r_fvector3	(section,"offset");
	dist				= 1.1f;
}

void CCameraFixedLook::LoadCfg(LPCSTR section)
{
	CCameraLook::LoadCfg(section);
	style = csFixed;
}
...
{
	Fquaternion	new_dir;
	new_dir.slerp(m_current_dir, m_final_dir, TimeDelta());	//1 sec
