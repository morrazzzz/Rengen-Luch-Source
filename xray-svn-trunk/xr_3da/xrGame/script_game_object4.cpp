...
u32	CScriptGameObject::add_sound		(bool in_aux_thread, LPCSTR prefix, u32 max_count, ESoundTypes type, u32 priority, u32 mask, u32 internal_type, LPCSTR bone_name)
{
	CCustomMonster				*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		...
		return					(monster->sound().add(in_aux_thread, prefix, max_count, type, priority, mask, internal_type, bone_name));
}

u32	CScriptGameObject::add_sound		(bool in_aux_thread, LPCSTR prefix, u32 max_count, ESoundTypes type, u32 priority, u32 mask, u32 internal_type)
{
	return						(add_sound(in_aux_thread, prefix, max_count, type, priority, mask, internal_type, "bip01_head"));
}

u32 CScriptGameObject::add_sound(LPCSTR prefix, u32 max_count, ESoundTypes type, u32 priority, u32 mask, u32 internal_type)
{
	return						(add_sound(false, prefix, max_count, type, priority, mask, internal_type, "bip01_head"));
}
...
u32	CScriptGameObject::add_combat_sound(LPCSTR prefix, u32 max_count, ESoundTypes type, u32 priority, u32 mask, u32 internal_type, LPCSTR bone_name)
{
	...
	if (!stalker) {
		...
		return					(stalker->sound().add(false, prefix, max_count, type, priority, mask, internal_type, bone_name, xr_new<CStalkerSoundData>(stalker)));
}
...
#pragma todo("Cop merge A: Fix it")
