...
void CMincer::LoadCfg (LPCSTR section)
{
...
BOOL CMincer::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
...
{
	float power = Power(dist, Radius()) * GetPullDeltaPower() * m_fThrowInHitKoefAlive;
	//Fvector dir;
	//dir.random_dir(throw_in_dir,2.f*M_PI);
	if(!smart_cast<CActor*>(EA))
	{
		// Msg("Hit %s by pull: %.2f, dist: %.2f, delta: %.2f", EA->ObjectName().c_str(), power, dist, GetPullDeltaPower());
...
{
	float power = Power(dist, Radius()) * GetPullDeltaPower() * m_fThrowInHitKoefCar;
