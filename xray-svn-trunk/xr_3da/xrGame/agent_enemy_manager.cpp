...
void CAgentEnemyManager::permutate_enemies		()
{
	...
	if (!m_only_wounded_left) {
		...
		for ( ; I != E; ++I)
		{
			CHitMemoryManager&	hit = (*I)->object().memory().HitMManager();

			ENEMIES::iterator	i = m_enemies.begin();
			ENEMIES::iterator	e = m_enemies.end();
			for ( ; i != e; ++i)
			{
				if ((*I)->object().memory().VisualMManager().visible_now((*i).m_object))
				{
...
//		Msg							("wounded enemy [%s] is assigned to member [%s]",*enemy->m_object->ObjectName(),*processor->ObjectName());
...
//	Msg								("[%6d] assigned = %x",EngineTimeU(),assigned);
//	ENEMIES::iterator				I = m_enemies.begin();
//	ENEMIES::iterator				E = m_enemies.end();
//	for ( ; I != E; ++I)
//		Msg							("[%6d] [%s] = %x",EngineTimeU(),*(*I).m_object->ObjectName(),(*I).m_distribute_mask.get());
}
