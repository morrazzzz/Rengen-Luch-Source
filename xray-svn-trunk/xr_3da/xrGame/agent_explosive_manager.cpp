...
void CAgentExplosiveManager::register_explosive	(const CExplosive *explosive, const CGameObject *game_object)
{
	...
	m_explosives.push_back				(CDangerExplosive(explosive,game_object,0,EngineTimeU()));
	...
	object().location().add				(xr_new <CDangerObjectLocation>(game_object,EngineTimeU(),interval,GRENADE_RADIUS));
}
...
bool CAgentExplosiveManager::process_explosive			(CMemberOrder &member)
{
	...
	for ( ; I != E; ++I) {
		if (!member.object().memory().VisualMManager().visible_now((*I).m_game_object))
			continue;
