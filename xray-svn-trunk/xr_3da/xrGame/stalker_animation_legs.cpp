...
float CStalkerAnimationManager::legs_switch_factor		() const
{
	if	(
...
bool CStalkerAnimationManager::need_look_back				() const
{
	...
	if ((m_change_direction_time + need_look_back_time_delay) > EngineTimeU())
		return					(false);
...
void CStalkerAnimationManager::legs_assign_direction		(float switch_factor, const EMovementDirection &direction)
{
	...
	VERIFY						(m_direction_start <= EngineTimeU());
	if ((EngineTimeU() - m_direction_start) <= (u32)iFloor(switch_factor*direction_switch_interval))
		return;
...
MotionID CStalkerAnimationManager::legs_move_animation		()
{
	...
	if (m_previous_speed_direction != speed_direction) {
		if (m_change_direction_time < EngineTimeU())
			m_change_direction_time	= EngineTimeU();
