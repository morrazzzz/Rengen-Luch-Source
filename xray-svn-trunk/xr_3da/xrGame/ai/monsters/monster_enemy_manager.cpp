...
void CMonsterEnemyManager::update()
{
	...
	if ((prev_enemy == enemy) && (time_last_seen != EngineTimeU()))	flags.or(FLAG_ENEMY_LOST_SIGHT);		
...
{
	return (monster->memory().VisualMManager().visible_right_now(enemy));
}
...
{
	return monster->memory().VisualMManager().visible_now(enemy);
}
...
bool CMonsterEnemyManager::enemy_see_me_now()
{
	...
	else 
	{
		...
		if ( cm ) 
		{
			return cm->memory().VisualMManager().visible_right_now(monster);
		}
	}
...
{
	if (object0->Position().distance_to(object1->Position()) > object0->GetEyeRangeValue())
		return		(false);
	...
	fYawFov			= angle_normalize_signed(object0->GetEyeFovValue() * PI/180.f);
	fRange			= object0->GetEyeRangeValue();
