...
void CControlAnimation::play_part(SAnimationPart &part, PlayCallback callback)
{
	...
	//Msg				("%6d Playing animation : %s , %s , Object %s",EngineTimeU(), K->LL_MotionDefName_dbg(part.motion).first,K->LL_MotionDefName_dbg(part.motion).second, *(m_object->ObjectName()));
#endif
...
///////////////////////////////////////////////////////////////////////////////
//#ifdef _DEBUG	
//	Msg("Monster[%s] Time[%u] Anim[%s]",*(m_object->ObjectName()), EngineTimeU(),*(m_object->anim().GetAnimTranslation(part.motion)));
//#endif
///////////////////////////////////////////////////////////////////////////////
...
void CControlAnimation::check_events(SAnimationPart &part)
{
	if (part.get_motion().valid() && part.actual && part.blend) {
		...
		if (it != m_anim_events.end()) {
			...
			float cur_perc = float(EngineTimeU() - part.time_started) / ((part.blend->timeTotal / part.blend->speed) * 1000);
