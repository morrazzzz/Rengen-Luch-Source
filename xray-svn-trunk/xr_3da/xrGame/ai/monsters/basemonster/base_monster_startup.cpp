...
void CBaseMonster::LoadCfg(LPCSTR section)
{
	m_section						= section;
	...
	monsterRank_					= (pSettings->line_exist(section,"rank")) ? int(pSettings->r_u32(section,"rank")) : 0;
...
#define LOAD_SOUND(sound_name,_type,_prior,_mask,_int_type)		\
	if (pSettings->line_exist(section,sound_name))						\
		sound().add(true, pSettings->r_string(section,sound_name), DEFAULT_SAMPLE_COUNT,_type,_prior,u32(_mask),_int_type,"bip01_head");
...
BOOL CBaseMonster::SpawnAndImportSOData(CSE_Abstract* data_containing_so) 
{
...
void CBaseMonster::DestroyClientObj()
{
...
void CBaseMonster::on_before_sell	(CInventoryItem *item)
{
	...
	CSE_Abstract					*object = Level().Server->Server_game_sv_base->get_entity_from_eid(item->object().ID());
	VERIFY							(object);
	CSE_ALifeObject					*alife_object = smart_cast<CSE_ALifeObject*>(object);
	if (alife_object)
...
void CBaseMonster::load_critical_wound_bones()
{
	...
	if (pSettings->line_exist(SectionName(),"critical_wound_anim_head")) {
		fill_bones_body_parts			("critical_wound_bones_head",	critical_wound_type_head);
		m_critical_wound_anim_head		= pSettings->r_string(SectionName(),"critical_wound_anim_head");
	} 

	if (pSettings->line_exist(SectionName(),"critical_wound_anim_torso")) {
		fill_bones_body_parts			("critical_wound_bones_torso",	critical_wound_type_torso);
		m_critical_wound_anim_torso		= pSettings->r_string(SectionName(),"critical_wound_anim_torso");

	} 
	
	if (pSettings->line_exist(SectionName(),"critical_wound_anim_legs")) {
		fill_bones_body_parts			("critical_wound_bones_legs",	critical_wound_type_legs);
		m_critical_wound_anim_legs		= pSettings->r_string(SectionName(),"critical_wound_anim_legs");
	} 
}

void CBaseMonster::fill_bones_body_parts	(LPCSTR body_part, CriticalWoundType wound_type)
{
	LPCSTR					body_parts_section = pSettings->r_string(SectionName(),body_part);
