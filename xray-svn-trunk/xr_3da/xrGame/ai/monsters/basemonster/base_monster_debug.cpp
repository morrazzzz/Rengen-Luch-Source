...
CBaseMonster::SDebugInfo CBaseMonster::show_debug_info()
{
	...
	xr_sprintf(text, "-- Monster : [%s]  Current Time = [%u]", *ObjectName(), EngineTimeU());
	...
	if (EnemyMan.get_enemy()) {
		xr_sprintf(text, "Current Enemy = [%s]", *EnemyMan.get_enemy()->ObjectName());
	} else 
	...
	if (CorpseMan.get_corpse()) {
		xr_sprintf(text, "Current Corpse = [%s] Satiety = [%.2f]", *CorpseMan.get_corpse()->ObjectName(), GetSatiety());
	} else 
	...
	if (HitMemory.is_hit()) {
		if (HitMemory.get_last_hit_object()) {
			xr_sprintf(text,"Hit Info: object=[%s] time=[%u]", *(HitMemory.get_last_hit_object()->ObjectName()), HitMemory.get_last_hit_time());
		} else {
...
void   add_enemy_debug_info (debug::text_tree& root_s, const CCustomMonster* pThis, const CEntityAlive* pEnemy)
{
	...
	root_s.add_line("I_See_Enemy", pThis->memory().VisualMManager().visible_right_now(pEnemy));
	...
	{
		seen_now = cm->memory().VisualMManager().visible_right_now(pThis);
	}
...
void   CBaseMonster::add_debug_info (debug::text_tree& root_s)
{
	...
	current_visual_s.add_line(*VisualName());
	...
	update_range_fov (object_range,object_fov,monsterEyeRange_,deg2rad(monsterEyeFov_));
	...
	if ( actor )
	{
		visuals_s.add_line("Actor_Visible",  memory().VisualMManager().visible_now(actor));
	}
	...
	corpse_s.add_line("Current_Corpse",  CorpseMan.get_corpse() ? 
		                                *CorpseMan.get_corpse()->ObjectName() : "none");
	...
	sound_player_s.add_line("Num_Sounds", sound().sound_collections().size());

	typedef CSoundPlayer::SOUND_COLLECTIONS::const_iterator SoundIterator;

	u32 object_count = 0;
	for ( SoundIterator i=sound().sound_collections().begin(), e=sound().sound_collections().end();
		  i!=e; ++i )
	{
