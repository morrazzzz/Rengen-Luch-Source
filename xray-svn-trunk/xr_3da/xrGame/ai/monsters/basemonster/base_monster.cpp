...
void CBaseMonster::update_pos_by_grouping_behaviour ()
{
	...
	{
		m_last_grouping_behaviour_update_tick = EngineTimeU();
	}	

	const float dt = 0.001f * (EngineTimeU() - m_last_grouping_behaviour_update_tick);
...
bool CBaseMonster::enemy_accessible ()
{
	...
	if ( EngineTimeU() < m_first_tick_enemy_inaccessible + 3000 )
		return							true;
...
bool CBaseMonster::at_home ()
{
	return										!m_first_tick_object_not_at_home ||
												(EngineTimeU() < m_first_tick_object_not_at_home + 4000);
}
...
void CBaseMonster::update_enemy_accessible_and_at_home_info	()
{
	if ( !Home->at_home() )
	{
		if ( !m_first_tick_object_not_at_home )
			m_first_tick_object_not_at_home	=	EngineTimeU();
	}
	else
		m_first_tick_object_not_at_home		=	0;
	...
	{
		if ( !m_first_tick_enemy_inaccessible )
			m_first_tick_enemy_inaccessible	=	EngineTimeU();

		m_last_tick_enemy_inaccessible		=	EngineTimeU();
	}
	else
	{
		if ( m_last_tick_enemy_inaccessible && EngineTimeU() - m_last_tick_enemy_inaccessible > 3000 )
		{
			m_first_tick_enemy_inaccessible	=	0;
			m_last_tick_enemy_inaccessible	=	0;
		}
...
#ifdef MEASURE_UPDATES
	Device.Statistic->updateCL_BaseMonster_ += measure_updatecl.GetElapsed_sec();
#endif
}

void CBaseMonster::ScheduledUpdate(u32 dt)
{
#ifdef MEASURE_UPDATES
...
void CBaseMonster::RemoveLinksToCLObj(CObject *O)
{
