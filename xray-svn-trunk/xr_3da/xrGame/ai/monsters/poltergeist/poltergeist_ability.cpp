...
{
	DestroyParticleInstance	(m_particles_object);
	DestroyParticleInstance	(m_particles_object_electro);
}
...
{
	if (m_particles_object)			DestroyParticleInstance(m_particles_object);
	if (m_particles_object_electro) DestroyParticleInstance(m_particles_object_electro);
}
...
{
	if (m_object->g_Alive() && (pHDS->hit_type == ALife::eHitTypeFireWound) && (CurrentFrame() != m_last_hit_frame)) {
