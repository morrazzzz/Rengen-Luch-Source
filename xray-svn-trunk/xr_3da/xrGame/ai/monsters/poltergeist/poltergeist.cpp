...
void CPoltergeist::LoadCfg(LPCSTR section)
{
...
void   CPoltergeist::update_detection ()
{
	...
	float const time_passed_sec			=	float(EngineTimeU() - m_last_detection_time) / 1000.f;
	m_last_detection_time				=	EngineTimeU();
...
void CPoltergeist::renderable_Render(IRenderBuffer& render_buffer)
{
	Visual()->getVisData().lastHomTestFrame = CurrentFrame();
...
void CPoltergeist::UpdateCL()
{
	...
	//	Visual()->getVisData().hom_frame = CurrentFrame();
}
...
void CPoltergeist::ScheduledUpdate(u32 dt)
{
...
BOOL CPoltergeist::SpawnAndImportSOData (CSE_Abstract* data_containing_so) 
{
...
void CPoltergeist::DestroyClientObj()
{
...
void CPoltergeist::RemoveLinksToCLObj(CObject *O)
{
