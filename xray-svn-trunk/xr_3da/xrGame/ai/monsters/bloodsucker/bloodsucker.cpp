...
#pragma todo("Cop merge: check skyloader commits")
...
void CAI_Bloodsucker::LoadCfg(LPCSTR section) 
{
...
void CAI_Bloodsucker::reload(LPCSTR section)
{
	...
	sound().add(true, pSettings->r_string(section,"Sound_Vampire_Grasp"),				DEFAULT_SAMPLE_COUNT,	SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eHighPriority + 4,	MonsterSound::eBaseChannel,	eVampireGrasp,					"bip01_head");
	sound().add(true, pSettings->r_string(section,"Sound_Vampire_Sucking"),			DEFAULT_SAMPLE_COUNT,	SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eHighPriority + 3,	MonsterSound::eBaseChannel,	eVampireSucking,				"bip01_head");
	sound().add(true, pSettings->r_string(section,"Sound_Vampire_Hit"),				DEFAULT_SAMPLE_COUNT,	SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eHighPriority + 2,	MonsterSound::eBaseChannel,	eVampireHit,					"bip01_head");
	sound().add(true, pSettings->r_string(section,"Sound_Vampire_StartHunt"),			DEFAULT_SAMPLE_COUNT,	SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eHighPriority + 5,	MonsterSound::eBaseChannel,	eVampireStartHunt,				"bip01_head");
	sound().add(true, pSettings->r_string(section,"Sound_Invisibility_Change_State"),	DEFAULT_SAMPLE_COUNT,	SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eNormalPriority,	MonsterSound::eChannelIndependent << 1,	eChangeVisibility,	"bip01_head");
	sound().add(true, pSettings->r_string(section,"Sound_Growl"),						DEFAULT_SAMPLE_COUNT,	SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eHighPriority + 6,	MonsterSound::eBaseChannel,	eGrowl,							"bip01_head");
	sound().add(true, pSettings->r_string(section,"Sound_Alien"),						DEFAULT_SAMPLE_COUNT,	SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eCriticalPriority,	u32(MonsterSound::eCaptureAllChannels),	eAlien,				"bip01_head");
}
...
void  CAI_Bloodsucker::BoneCallback(CBoneInstance *B)
{
	...
	this_class->Bones.Update(B, EngineTimeU());
}
...
BOOL CAI_Bloodsucker::SpawnAndImportSOData(CSE_Abstract* data_containing_so) 
{
...
void   CAI_Bloodsucker::set_visibility_state (visibility_t new_state)
{
	...
	if ( EngineTimeU() <	m_visibility_state_last_changed_time + 
								get_visibility_state_change_min_delay() )
	{
		return;
	}

	m_visibility_state_last_changed_time	=	EngineTimeU();
...
void   CAI_Bloodsucker::update_invisibility ()
{
	...
	else if ( EngineTimeU() < m_runaway_invisible_time + default_runaway_invisible_time/6 )
	{
		set_visibility_state				(partial_visibility);
	}
	else if ( EngineTimeU() < m_runaway_invisible_time + default_runaway_invisible_time )
	{
...
void CAI_Bloodsucker::ScheduledUpdate(u32 dt)
{
#ifdef MEASURE_UPDATES
...
#ifdef MEASURE_UPDATES
	Device.Statistic->scheduler_VariousMonsters_ += measure_sc_update.GetElapsed_sec();
#endif
}
...
void CAI_Bloodsucker::predator_start()
{
	...
	SetVisualName(m_visual_predator);
...
void CAI_Bloodsucker::predator_stop()
{
	...
	SetVisualName(*m_visual_default);
...
void   CAI_Bloodsucker::renderable_Render (IRenderBuffer& render_buffer)
{
