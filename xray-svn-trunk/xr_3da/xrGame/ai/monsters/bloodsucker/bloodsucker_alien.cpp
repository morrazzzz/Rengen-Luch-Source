...
BOOL CAlienEffectorPP::Process(SPPInfo& pp)
{
	...
	def_lerp			(factor,target_factor, PERIOD_SPEED, TimeDelta());
...
BOOL CAlienEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	// set angle 
	if (angle_lerp(dangle_current.x, dangle_target.x, ANGLE_SPEED, TimeDelta())) {
		dangle_target.x = angle_normalize(Random.randFs(DELTA_ANGLE_X));
	}

	if (angle_lerp(dangle_current.y, dangle_target.y, ANGLE_SPEED, TimeDelta())) {
		dangle_target.y = angle_normalize(Random.randFs(DELTA_ANGLE_Y));
	}

	if (angle_lerp(dangle_current.z, dangle_target.z, ANGLE_SPEED, TimeDelta())) {
	...
	def_lerp(m_inertion, 1 - rel_dist, rel_dist, TimeDelta());
	...
	def_lerp(m_current_fov, m_target_fov, FOV_SPEED, TimeDelta());
...
//.	Actor()->inventory().setSlotsBlocked			(true);
	...
	CurrentGameUI()->ShowGameIndicators(IB_BLOODSUCKER_ALIEN, false);
	CurrentGameUI()->ShowCrosshair(CB_BLOODSUCKER_ALIEN, false);
	CurrentGameUI()->AllowInput(GINPUT_BLOODSUCKER_ALIEN, false);
...
void CBloodsuckerAlien::deactivate()
{
	...
	CurrentGameUI()->ShowGameIndicators(IB_BLOODSUCKER_ALIEN, true);
	CurrentGameUI()->ShowCrosshair(CB_BLOODSUCKER_ALIEN, true);
	CurrentGameUI()->AllowInput(GINPUT_BLOODSUCKER_ALIEN, true);
