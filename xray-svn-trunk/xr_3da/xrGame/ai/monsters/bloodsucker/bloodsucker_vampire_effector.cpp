...
BOOL CVampireCameraEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	if (time_left_perc < 0.2f) {
		...
		angle_lerp(dangle_current.x, dangle_target.x, _abs(dangle_current.x / fLifeTime + 0.001f), TimeDelta());
		angle_lerp(dangle_current.y, dangle_target.y, _abs(dangle_current.y / fLifeTime + 0.001f), TimeDelta());
		angle_lerp(dangle_current.z, dangle_target.z, _abs(dangle_current.z / fLifeTime + 0.001f), TimeDelta());
