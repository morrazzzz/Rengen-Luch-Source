...
void CControlPathBuilder::update_schedule() 
{
	...
	if (m_data.enable) {
		...
		if (m_data.path_type == MovementManager::ePathTypeGamePath) {
			...
			// set target
			// TODO: make it VERIFY
//			VERIFY3(m_data.target_node != u32(-1), "Error: Object set wrong path params! Object name:",*inherited_com::m_object->ObjectName());
...
bool CControlPathBuilder::build_special(const Fvector &target, u32 node, u32 vel_mask)
{
	...
	if (!path_completed() && (detail().time_path_built() >= EngineTimeU()))	return true;
...
void CControlPathBuilder::fix_position(const Fvector &pos, u32 node, Fvector &res_pos)
{
	...
	if (!accessible(res_pos)) {
		...
		VERIFY3((level_vertex_id == node) || show_restrictions(m_restricted_object),"Invalid restrictions (see log for details) for object ",*(CControl_Com::m_object->ObjectName()));
#endif
	}
}
