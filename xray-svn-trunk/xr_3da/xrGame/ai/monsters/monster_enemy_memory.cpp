...
void CMonsterEnemyMemory::update() 
{
	...
	objects_list const& objects	=	monster->memory().EnemyManager().objects();
	...
	if ( monster_hit_memory.is_hit() && time() < monster_hit_memory.get_last_hit_time() + 1000 )
	{
		...
		{
			if ( monster->CCustomMonster::useful(&monster->memory().EnemyManager(), enemy) &&
	...
	if ( monster->SoundMemory.IsRememberSound() )
	{
		...
		if ( dangerous && EngineTimeU() < sound.time + 2000 )
		{
									  ...
									  ++I	) 
	{
		...
		if ( feel_enemy || monster->memory().VisualMManager().visible_now(*I) )
			add_enemy(*I);
	}
	...
	if ( g_actor )
	{
		...
		if ( xz_dist < feel_enemy_max_distance && 
			 y_dist < 10 &&
			 monster->memory().EnemyManager().is_useful(g_actor) &&
			 g_actor->memory().visual().visible_now(monster) )
		{
			add_enemy(g_actor);
		}
...
void CMonsterEnemyMemory::remove_non_actual() 
{
							...
							it	=	nit	)
	{
		...
		if ( !it->first									|| 
			 ...
			 !monster->memory().EnemyManager().is_useful(it->first) )
		{
			m_objects.erase (it);
		}
	}
