...
void CStateZombieChokeExecuteAbstract::initialize()
{
	...
	CurrentGameUI()->ShowGameIndicators(IB_ZOMBIE_ATTACK, false);
	CurrentGameUI()->ShowCrosshair(CB_ZOMBIE_ATTACK, false);
	CurrentGameUI()->AllowInput(GINPUT_ZOMBIE_ATTACK, false);

	Actor()->SetWeaponHideState(IB_ZOMBIE_ATTACK, true);
...
{
	CurrentGameUI()->ShowGameIndicators(IB_ZOMBIE_ATTACK, true);
	CurrentGameUI()->ShowCrosshair(CB_ZOMBIE_ATTACK, true);
	CurrentGameUI()->AllowInput(GINPUT_ZOMBIE_ATTACK, true);
...
void CStateZombieChokeExecuteAbstract::execute_choke_prepare()
{
	...
	time_choke_started				= EngineTimeU();
}
...
void CStateZombieChokeExecuteAbstract::execute_choke_continue()
{
	...
	if (time_choke_started + CHOKE_TIME_HOLD < EngineTimeU()) {
