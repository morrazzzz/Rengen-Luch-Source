...
void CZombie::LoadCfg(LPCSTR section)
{
...
BOOL CZombie::SpawnAndImportSOData(CSE_Abstract* data_containing_so) 
{
...
//	inherited::Hit(P,dir,who,element,p_in_object_space,impulse,hit_type);
	...
	if ((pHDS->hit_type == ALife::eHitTypeFireWound) && (CurrentFrame() != last_hit_frame)) {
		if (!com_man().ta_is_active() && (time_resurrect + TIME_RESURRECT_RESTORE < EngineTimeU()) && (conditions().GetHealth() < health_death_threshold)) {
...
void CZombie::ScheduledUpdate(u32 dt)
{
	...
	{
		if (time_dead_start + TIME_FAKE_DEATH < EngineTimeU())
		{
			time_dead_start = 0;
			fakedeath_is_active = false;
