...
void CStateGhostBossShieldAbstract::initialize()
{
	...
	m_last_shield_started =  EngineTimeU();
	m_next_particle_allowed = 0;
	m_action = ACTION_IDLE;
...
void CStateGhostBossShieldAbstract::execute()
{
	...
	case ACTION_SHIELD_STARTED:
		if (object->m_shield_keep_particle && EngineTimeU() > m_next_particle_allowed)
		{
			...
			m_next_particle_allowed = EngineTimeU() + object->m_shield_keep_particle_period;
		}
		break;
...
{
	if (EngineTimeU() > m_last_shield_started + object->m_shield_time + object->m_shield_cooldown)
	{
...
{
	if (EngineTimeU() <= m_last_shield_started + object->m_shield_time)
	{
