...
{
	if (time_started + object->m_tele_time_to_hold > EngineTimeU()) return;
	...
	while (i < object->CTelekinesis::get_objects_count()) {
		...
		if ((tele_object.get_state() == TS_Keep) && (tele_object.time_keep_started + 1500 < EngineTimeU())) {
	...
	if (object_found) {
		...
		if (!IsActiveObjects() || (time_started + MAX_TIME_CHECK_FAILURE < EngineTimeU())) {
