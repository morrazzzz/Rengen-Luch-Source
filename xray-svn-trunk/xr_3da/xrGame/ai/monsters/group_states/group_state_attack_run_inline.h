...
{	
 	if ( EngineTimeU() > m_intercept_tick + m_intercept_length )
 	{
 		m_intercept_tick   = EngineTimeU();
 	...
 	if ( EngineTimeU() > m_memorized_tick + memory_update_ms )
 	{
 		m_predicted_vel  = (enemy_pos-m_memorized_pos) * (1000.f / (EngineTimeU()-m_memorized_tick));
	...
	if ( !enemy_at_home || EngineTimeU() < time_state_started + m_encircle_time )
	{
...
// 
// 	//	if ( g_bDogsDirMode )
// 	if ( EngineTimeU()-time_state_started < m_max_encircle_time )
// 	{
//  		CMonsterSquad *squad	= monster_squad().get_squad(object);
//  		if (squad && squad->SquadActive()) {
//  			// Получить команду
