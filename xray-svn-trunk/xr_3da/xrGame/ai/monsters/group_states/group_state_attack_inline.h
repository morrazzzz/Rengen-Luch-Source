				...
				switch( prev_substate )
				{
					...
					case eStateCustom:
						if ( get_state_current()->check_completion() ) 
						{
							...
							m_time_start_drive_out = EngineTimeU();
					...
					case eStateAttack_ControlFire:
						if ( object->Position().distance_to(enemy_pos) > 7.f + m_delta_distance || EngineTimeU() - m_time_start_drive_out > object->m_drive_out_time )
						{
							if ( object->get_custom_anim_state() ) 
							{
								object->anim_end_reinit();
							...
							if ( EngineTimeU() - m_time_start_drive_out > object->m_drive_out_time ) 
							{
								m_drive_out = true;
							}
