...
{	
	if (data.action.time_out !=0) {
		if (time_state_started + data.action.time_out < EngineTimeU()) return true;
	} 
...
{	
	if ( data.action.time_out != 0 )
	{
		if ( time_state_started + data.action.time_out < EngineTimeU() ) 
			return					true;
	} 
	...
	if ( EngineTimeU() < time_state_started + 200 )
	{
