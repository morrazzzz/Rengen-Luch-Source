...
bool CStateMonsterAttackAbstract::check_find_enemy_state()
{
	...
	if (object->EnemyMan.get_enemy_time_last_seen() + FIND_ENEMY_DELAY < EngineTimeU()) 
		return true;
	return false;
}
...
bool CStateMonsterAttackAbstract::check_run_away_state()
{
	...
	if (prev_substate == eStateAttack_RunAway) {
		if (!get_state(eStateAttack_RunAway)->check_completion()) 
			return true;
		else m_time_next_run_away = EngineTimeU() + 10000;
	} else if ((object->EnemyMan.get_enemy() != Actor()) && object->Morale.is_despondent() && (m_time_next_run_away < EngineTimeU())) {
		return true;
	}
