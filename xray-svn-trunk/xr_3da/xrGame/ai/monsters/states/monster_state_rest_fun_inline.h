...
void CStateMonsterRestFunAbstract::execute()
{
	...
	if ((dist < object->db().m_fDistToCorpse + 0.5f) && (time_last_hit + MIN_DELAY < EngineTimeU())) {
...
bool CStateMonsterRestFunAbstract::check_completion()
{
	...
	if (time_state_started + TIME_IN_STATE < EngineTimeU()) return true;
	return false;
}
