...
: inherited(ppi)
{
	...
	m_time_state_started	= EngineTimeU();
...
BOOL CPPEffectorPsyDogAura::update()
{
	...
	if (m_effector_state == eStatePermanent) {
		...
		m_factor = float(EngineTimeU() - m_time_state_started) / float(m_time_to_fade);
...
void CPsyDogAura::update_schedule()
{
	...
	CVisualMemoryManager::VISIBLES::const_iterator	I = m_actor->memory().visual().GetMemorizedObjects().begin();
	CVisualMemoryManager::VISIBLES::const_iterator	E = m_actor->memory().visual().GetMemorizedObjects().end();
	for ( ; I != E; ++I) {
