...
bool CTelekineticObject::init(CTelekinesis* tele,CPhysicsShellHolder *obj, float s, float h, u32 ttk, bool rot) 
{
	...
	time_fire_started	= 0;
	//time_raise_started	= EngineTimeU();
...
{
	if (time_keep_started + time_to_keep < EngineTimeU()) return true;
	return false;
}

bool CTelekineticObject::time_fire_elapsed()
{
	if (time_fire_started + FIRE_TIME < EngineTimeU()) return true;
	return false;
}
...
void CTelekineticObject::keep()
{
	...
	//if (time_keep_updated + KEEP_IMPULSE_UPDATE > EngineTimeU()) return;
...
{
	if (time_raise_started + RAISE_MAX_TIME < EngineTimeU())
		return true;
