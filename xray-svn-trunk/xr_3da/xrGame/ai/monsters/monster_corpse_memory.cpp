...
{
	for (xr_vector<const CGameObject *>::const_iterator I = monster->memory().ItemManager().objects().begin(); I != monster->memory().ItemManager().objects().end(); ++I) {
		if (monster->memory().VisualMManager().visible_now(*I)) {
