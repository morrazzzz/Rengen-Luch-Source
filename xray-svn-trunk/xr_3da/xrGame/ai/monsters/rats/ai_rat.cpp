...
void CAI_Rat::reload					(LPCSTR	section)
{
	...
	sound().add		(true, pSettings->r_string(section,"sound_death"),	100, SOUND_TYPE_MONSTER_DYING,		0, u32(eRatSoundMaskDie),		eRatSoundDie,		head_bone_name);
	sound().add		(true, pSettings->r_string(section,"sound_hit"),	100, SOUND_TYPE_MONSTER_INJURING,	1, u32(eRatSoundMaskInjuring),	eRatSoundInjuring,	head_bone_name);
	sound().add		(true, pSettings->r_string(section,"sound_attack"),	100, SOUND_TYPE_MONSTER_ATTACKING,	2, u32(eRatSoundMaskAttack),	eRatSoundAttack,	head_bone_name);
	sound().add		(true, pSettings->r_string(section,"sound_voice"),	100, SOUND_TYPE_MONSTER_TALKING,	4, u32(eRatSoundMaskVoice),		eRatSoundVoice,		head_bone_name);
	sound().add		(true, pSettings->r_string(section,"sound_eat"),	100, SOUND_TYPE_MONSTER_EATING	,	3, u32(eRatSoundMaskEat),		eRatSoundEat,		head_bone_name);
}
...
void CAI_Rat::LoadCfg(LPCSTR section)
{ 
	init();
...
BOOL CAI_Rat::SpawnAndImportSOData	(CSE_Abstract* data_containing_so)
{
	//////////////////////////////////////////////////////////////////////////
	...
	if (!inherited::SpawnAndImportSOData(data_containing_so))
		return(FALSE);
	// model
	if (!CEatableItem::SpawnAndImportSOData(data_containing_so))
		return(FALSE);
	...
	monsterEyeFov_					= tpSE_Rat->fEyeFov;
	monsterEyeRange_				= tpSE_Rat->fEyeRange;
	...
	for (int j=0; j<iPointCount; ++j)
		if (ai().game_graph().mask(movement().locations().vertex_types()[j].tMask,ai().game_graph().vertex(ai_location().game_vertex_id())->vertex_type())) {
			m_time_to_change_graph_point= EngineTimeU() + ::Random32.random(60000) + 60000;
			break;
		}
...
void CAI_Rat::ExportDataToServer(NET_Packet& P)
{
...
void CAI_Rat::ScheduledUpdate(u32 dt)
{
...
// Rat as eatable item
/////////////////////////////////////
void CAI_Rat::AfterAttachToParent		()
{
	inherited::AfterAttachToParent		();
	CEatableItem::AfterAttachToParent	();
}

void CAI_Rat::BeforeAttachToParent		()
{
	setVisible					(FALSE);
	setEnabled					(FALSE);
...
void CAI_Rat::BeforeDetachFromParent	()
{
	CEatableItem::BeforeDetachFromParent	(TRUE);
	inherited::BeforeDetachFromParent	(TRUE);
...
void CAI_Rat::AfterDetachFromParent	()
{
