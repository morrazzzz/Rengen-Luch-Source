...
//					tpGlobalAnimation = m_tRatAnimations.tNormal.tGlobal.tpaIdle[0];
	...
	if (psAI_Flags.is(aiAnimation)) {
		...
		Msg					("%6d %s animation : %s (%f,%f)",EngineTimeU(),"Global",skeleton_animated->LL_MotionDefName_dbg(m_tpCurrentGlobalAnimation),movement().m_body.current.yaw,movement().m_body.target.yaw);
	}
#endif
}
