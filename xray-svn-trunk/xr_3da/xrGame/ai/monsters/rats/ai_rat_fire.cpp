...
void CAI_Rat::Exec_Action(float /**dt/**/)
{
	switch (m_tAction) {
		case eRatActionAttackBegin : {
			...
			if (memory().EnemyManager().selected() && memory().EnemyManager().selected()->g_Alive() && (dwTime - m_dwStartAttackTime > m_dwHitInterval)) {
				m_bActionStarted = true;
				m_dwStartAttackTime = dwTime;
				Fvector tDirection;
				Fvector position_in_bone_space;
				position_in_bone_space.set(0.f,0.f,0.f);
				tDirection.sub(memory().EnemyManager().selected()->Position(),this->Position());
				vfNormalizeSafe(tDirection);
				
				if (memory().EnemyManager().selected()) 
				{
					CEntityAlive	*entity_alive = const_cast<CEntityAlive*>(memory().EnemyManager().selected());
					VERIFY			(entity_alive);
...
float CAI_Rat::evaluate		(const CItemManager *manager, const CGameObject *object) const
{
	...
	if (!entity_alive->g_Alive()) {
		if ((EngineTimeU() - entity_alive->GetLevelDeathTime() < m_dwEatCorpseInterval) && (entity_alive->m_fFood > 0) && (m_bEatMemberCorpses || (entity_alive->g_Team() != g_Team())) && (m_bCannibalism || (entity_alive->CLS_ID != CLS_ID)))
