...
Msg("*   %s",*visible_objects()[i]->ObjectName());\
}\
/**
#define WRITE_TO_LOG(s) {\
	Msg("Monster %s : \n* State : %s\n* Time delta : %7.3f\n* Global time : %7.3f",*ObjectName(),s,m_fTimeUpdateDelta,float(EngineTimeU())/1000.f);\
...
void CAI_Rat::Death()
{
	...
	if (m_fFood <= 0) {
		...
		setVisible(false);
		if (EngineTimeU() - m_previous_query_time > 10000) {
//			setEnabled			(FALSE);
//			NET_Packet			P;
...
void CAI_Rat::FreeHuntingActive()
{
	...
	CHECK_IF_SWITCH_TO_NEW_STATE_THIS_UPDATE((memory().EnemyManager().selected() && ((memory().EnemyManager().selected()->Position().distance_to(m_tSafeSpawnPosition) < m_fMaxPursuitRadius) || (Position().distance_to(m_tSafeSpawnPosition) > m_fMaxHomeRadius))),aiRatAttackRun);
	...
	if	(
			...
			(
				...
				(
					(!memory().ItemManager().selected() || (memory().ItemManager().selected()->ID() != m_tLastSound.tpEntity->ID())) &&
					(m_tLastSound.tpEntity->g_Team() != g_Team())
				)
			) && 
			!memory().EnemyManager().selected()
		)
	{
		SWITCH_TO_NEW_STATE_THIS_UPDATE(aiRatFreeRecoil);
	}
    
	CHECK_IF_SWITCH_TO_NEW_STATE_THIS_UPDATE(memory().ItemManager().selected(),aiRatEatCorpse);
...
void CAI_Rat::UnderFire()
{
	...
	//	Msg					("%6d : Rat %s, %f -> %f [%f]",EngineTimeU(),*ObjectName(),movement().m_body.current.pitch,movement().m_body.target.pitch,get_custom_pitch_speed(0.f));
	...
	if (m_bStateChanged)//(EngineTimeU() - m_previous_query_time > TIME_TO_GO) || !m_previous_query_time)
...
void CAI_Rat::AttackFire()
{
	...
	CHECK_IF_GO_TO_PREV_STATE(!memory().EnemyManager().selected());

	CHECK_IF_GO_TO_NEW_STATE((memory().EnemyManager().selected()->Position().distance_to(Position()) > m_fAttackDistance),aiRatAttackRun)
	...
	CHECK_IF_GO_TO_NEW_STATE(angle_difference(movement().m_body.current.yaw,sTemp.yaw) > m_fAttackAngle,aiRatAttackRun)
		...
		Fvector			tDistance;
	tDistance.sub	(Position(),memory().EnemyManager().selected()->Position());
...
void CAI_Rat::AttackRun()
{
	...
	CHECK_IF_GO_TO_NEW_STATE_THIS_UPDATE(memory().EnemyManager().selected() && (m_tSafeSpawnPosition.distance_to(memory().EnemyManager().selected()->Position()) > m_fMaxPursuitRadius),aiRatReturnHome);

	CHECK_IF_GO_TO_PREV_STATE_THIS_UPDATE(!memory().EnemyManager().selected());
	...
	CHECK_IF_GO_TO_NEW_STATE_THIS_UPDATE((memory().EnemyManager().selected()->Position().distance_to(Position()) <= m_fAttackDistance) && (angle_difference(movement().m_body.target.yaw, sTemp.yaw) <= m_fAttackAngle),aiRatAttackFire)

		if ((EngineTimeU() - m_previous_query_time > TIME_TO_GO) || !m_previous_query_time) {
			m_tGoalDir.set(memory().EnemyManager().selected()->Position());
		}
...
void CAI_Rat::Retreat()
{
	...
	if	(!memory().EnemyManager().selected() ||
		(memory().EnemyManager().selected() &&
		(
		!memory().EnemyManager().selected()->g_Alive()
		|| 
		(
	...
	if (memory().EnemyManager().selected() && memory().EnemyManager().selected()->g_Alive()) {
		...
		if ((memory().EnemyManager().selected()->Position().distance_to(Position()) <= m_fAttackDistance) && (angle_difference(movement().m_body.target.yaw, sTemp.yaw) > m_fAttackAngle)) {
			m_fSpeed = 0.f;
			return;
		}
...
void CAI_Rat::Pursuit()
{
	...
	if (memory().EnemyManager().selected() && (EngineTimeU() - memory().memory(memory().EnemyManager().selected()).m_level_time >= m_dwLostMemoryTime)) {
		memory().enable(memory().EnemyManager().selected(),false);
		GO_TO_PREV_STATE_THIS_UPDATE;
	}

	CHECK_IF_SWITCH_TO_NEW_STATE_THIS_UPDATE(memory().EnemyManager().selected(),aiRatAttackRun);
	...
	if ((EngineTimeU() - m_previous_query_time > TIME_TO_GO) || !m_previous_query_time)
		m_tGoalDir.set(memory().memory(memory().EnemyManager().selected()).m_object_params.m_position);
...
void CAI_Rat::FreeRecoil()
{
	...
	if (m_bStateChanged) {
		m_dwLostRecoilTime	= EngineTimeU();
	...
	CHECK_IF_GO_TO_NEW_STATE_THIS_UPDATE(memory().EnemyManager().selected() && (EngineTimeU() - memory().memory(memory().EnemyManager().selected()).m_level_time >= m_dwLostRecoilTime),aiRatPursuit);
...
void CAI_Rat::ReturnHome()
{
	...
	if (memory().EnemyManager().selected() && (m_tSafeSpawnPosition.distance_to(memory().EnemyManager().selected()->Position()) < m_fMaxPursuitRadius)) {
		m_fGoalChangeTime = 0;
	...
	CHECK_IF_GO_TO_PREV_STATE(!memory().EnemyManager().selected() || !memory().EnemyManager().selected()->g_Alive() || Position().distance_to(m_tSafeSpawnPosition) < m_fMaxHomeRadius);
...
void CAI_Rat::EatCorpse()
{
	...
	CHECK_IF_GO_TO_PREV_STATE_THIS_UPDATE((memory().EnemyManager().selected() && ((memory().EnemyManager().selected()->Position().distance_to(m_tSafeSpawnPosition) < m_fMaxPursuitRadius) || (Position().distance_to(m_tSafeSpawnPosition) > m_fMaxHomeRadius))));

	CHECK_IF_GO_TO_PREV_STATE_THIS_UPDATE(!memory().ItemManager().selected() || (m_fMorale < m_fMoraleNormalValue));
	...
	//	IKinematicsAnimated					*V= smart_cast<IKinematicsAnimated*>(const_cast<CGameObject*>(memory().ItemManager().selected())->Visual());
	//	R_ASSERT							(V);
	//	u16									head_bone = V->LL_BoneID("bip01_head");
	//	Fmatrix								l_tMatrix;
	//	l_tMatrix.mul_43					(const_cast<CGameObject*>(memory().ItemManager().selected())->XFORM(),smart_cast<IKinematics*>(const_cast<CGameObject*>(memory().ItemManager().selected())->Visual())->LL_GetBoneInstance(head_bone).mTransform);
	//	Fvector								temp_position = l_tMatrix.c;
	Fvector								temp_position;
	memory().ItemManager().selected()->Center						(temp_position);
	...
	if (a && angle_difference(y,-movement().m_body.current.yaw) < PI_DIV_6) {
		m_fSpeed						= 0;
		if (EngineTimeU() - m_previous_query_time > m_dwHitInterval) {
			m_previous_query_time		= EngineTimeU();
			const CEntityAlive			*const_corpse = smart_cast<const CEntityAlive*>(memory().ItemManager().selected());
			VERIFY						(const_corpse);
