...
void CBurer::reload(LPCSTR section)
{
	...
	// add specific sounds
	sound().add				(true, pSettings->r_string(section,"sound_gravi_attack"),	DEFAULT_SAMPLE_COUNT,	
							SOUND_TYPE_MONSTER_ATTACKING,	MonsterSound::eHighPriority + 2,	
							u32(MonsterSound::eBaseChannel),	eMonsterSoundGraviAttack, "head");

	sound().add				(true, pSettings->r_string(section,"sound_tele_attack"),	DEFAULT_SAMPLE_COUNT,
...
void CBurer::LoadCfg(LPCSTR section)
{
...
void CBurer::ScheduledUpdate(u32 dt)
{
...
void xr_stdcall CBurer::StaminaHit ()
{
	...
	if ( do_weapon_drop )
	{
		...
		if ( !Actor()->inventory().InputAction((u16)kDROP, CMD_STOP) )
		{
			Actor()->g_PerformDrop		();
		}
	}
...
void CBurer::UpdateGraviObject()
{
	...
	float dt = float(EngineTimeU() - m_gravi_object.time_last_update);
	...
	m_gravi_object.time_last_update		= EngineTimeU();
...
void CBurer::RemoveLinksToCLObj(CObject *O)
{
