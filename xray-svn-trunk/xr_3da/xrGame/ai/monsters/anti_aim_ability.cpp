...
void   anti_aim_ability::activate ()
{
	...
	m_last_activated_tick			=	EngineTimeU();
	
	m_animation_hit_tick			=	EngineTimeU() + 
	...
	m_animation_end_tick			=	EngineTimeU() + (TTime)(anim_length*1000);
...
void   anti_aim_ability::start_camera_effector ()
{
	...
	m_camera_effector_end_tick		=	EngineTimeU() + (TTime)(cam_eff->GetAnimatorLength()*1000);
...
void   anti_aim_ability::deactivate ()
{
	...
	m_effector_id					=	0;
	m_last_detection_tick			=	EngineTimeU();
	m_last_angle					=	M_PI;
...
void   anti_aim_ability::update_schedule ()
{
	...
	if ( is_active() )
	{
		...
		if ( EngineTimeU() < m_animation_hit_tick )
		{
			return;
		}
		if ( m_effector_id == 0 )
		...
		if ( EngineTimeU() < m_camera_effector_end_tick )
		{
			return;
		}
		do_deactivate					();
	...
	if ( EngineTimeU() < m_last_activated_tick + (TTime)(m_timeout*1000) )
	{
#ifdef DEBUG_STATE
	...
	float const detect_delta		=	(EngineTimeU()-m_last_detection_tick) / 1000.f;
	m_last_detection_tick			=	EngineTimeU();
