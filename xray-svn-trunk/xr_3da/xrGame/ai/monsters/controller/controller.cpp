...
#pragma todo("Cop merge: check lost alpha and skyloader commits")
...
void CController::LoadCfg(LPCSTR section)
{
...
BOOL CController::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
...
/*
	...
	time_control_hit_started	= EngineTimeU();
*/
}
...
void CController::UpdateCL()
{
	...
	if (active_control_fx) {
		...
		float percent		= float((EngineTimeU() - time_control_hit_started)) / float(time_to_show);
...
void CController::ScheduledUpdate(u32 dt)
{
#ifdef MEASURE_UPDATES
...
void CController::DestroyClientObj()
{
...
void CController::RemoveLinksToCLObj(CObject *O)
{
...
bool CController::is_relation_enemy(const CEntityAlive *tpEntityAlive) const
{
	...
	if (xr_strcmp(*(tpEntityAlive->SectionName()), "stalker_zombied") == 0) return false;
