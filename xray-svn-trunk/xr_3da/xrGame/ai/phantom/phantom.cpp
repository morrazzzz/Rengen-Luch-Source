...
// A ghosted monster. For psy and controller aura
...
//---------------------------------------------------------------------
void CPhantom::LoadCfg( LPCSTR section )
{
...
BOOL CPhantom::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
	CSE_ALifeCreaturePhantom*	OBJ	= smart_cast<CSE_ALifeCreaturePhantom*>(data_containing_so); VERIFY(OBJ);
	
	// select visual at first
	LPCSTR vis_name = OBJ->get_visual();
	if (!(vis_name&&vis_name[0])){
		LPCSTR visuals	= pSettings->r_string(SectionName(),"visuals");
...
void CPhantom::DestroyClientObj()
{
	...
	sdata.sound.stop			();
	DestroyParticleInstance	(m_fly_particles);
}
...
void CPhantom::ScheduledUpdate(u32 DT)
{
...
void CPhantom::UpdatePosition(const Fvector& tgt_pos) 
{
	...
	angle_lerp		(vHP.x,tgt_h,fASpeed,TimeDelta());
	angle_lerp		(vHP.y,tgt_p,fASpeed,TimeDelta());

	cur_dir.setHP	(vHP.x,vHP.y);

	Fvector prev_pos=Position();
	XFORM().rotateY (-vHP.x);
	Position().mad	(prev_pos,cur_dir,fSpeed*TimeDelta());
}
...
void CPhantom::ExportDataToServer(NET_Packet& P)					// export to server
{
	u8					flags = 0;
