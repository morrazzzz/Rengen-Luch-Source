...
const CCoverPoint *CAI_Stalker::find_best_cover(const Fvector &position_to_cover_from)
{
	...
	if (!best_weapon())
		...
		if (!weapon)
			...
			if (weapon->BaseSlot() != RIFLE_SLOT && weapon->BaseSlot() != RIFLE_2_SLOT)
				m_ce_best->can_use_smart_covers(false);
			else
...
void CAI_Stalker::update_best_cover_actuality(const Fvector &position_to_cover_from)
{
	...
	if (m_best_cover->m_is_smart_cover) {
		float							value;
		const smart_cover::cover*		cover = static_cast<const smart_cover::cover *>(m_best_cover);
...
//		m_best_cover_actual				= false;
//		Msg								("* [%6d][%s] cover became too bad2",EngineTimeU(),*ObjectName());
//		return;
//	}
	...
	m_best_cover = find_best_cover(position_to_cover_from);
}
...
{
#ifdef _DEBUG
//	Msg									("* [%6d][%s] cover is blocked",EngineTimeU(),*ObjectName());
#endif
	m_best_cover_actual					= false;
}
