...
public:
	...
	void _stdcall						ScheduledUpdateMT					();
	...
	void								ProcessHitAlive						(SHit* HDSin, SHit& HDSout); // hit alive stalker
	void								ProcessHitDead						(SHit* HDSin, SHit& HDSout); // hit dead stalker =(
...
//	virtual CInventoryItem				*GetCurrentEquipment	() const; <- moved to InventoryOwner::GetCurrentOutfit
	...
	virtual bool						CanPutInSlot			(PIItem item, u32 slot)		{return(slot != OUTFIT_SLOT && slot != RIFLE_2_SLOT && slot != PDA_SLOT);};
...
private:
	...
	bool				canHitFriends_;
	bool				canHitFriendsWGrenade_;

	u32					m_pick_frame_id;
	collide::rq_results	rq_storage;
	collide::rq_results	rq_storage_2;

	xr_vector<ISpatial*>hittingSpatials_;

	xr_vector<CObject*> temp_pool_1;
private:
			...
			void						CheckCanHitFriends		();
			void						CheckCanHitFriendsWGrenade();
public:
			bool						can_kill_member			();
			bool						can_kill_enemy			();

			// Are there friends in the firing frustum?
	IC		bool						CanHitFriends			() { return canHitFriends_; };
	IC		bool						CanHitFriendsWGrenade	() { return canHitFriendsWGrenade_; };

			float						pick_distance			();
	IC		float						start_pick_distance		() const;
			bool						fire_make_sense			();

			// changes some combat properties, which makes npc get out of cover(Helps in situations, when NPC doesn't want to fight)
			void						PokeNPCInFight			();
...
public:
			...
			void						PlaySearchSound								(u32 max_start_time = 0, u32 min_start_time = 0, u32 max_stop_time = 0, u32 min_stop_time = 0, u32 id = u32(-1));
			void						PlayLostSound								(u32 max_start_time = 0, u32 min_start_time = 0, u32 max_stop_time = 0, u32 min_stop_time = 0, u32 id = u32(-1));
			void						PlayFinishFightSound						(u32 max_start_time = 0, u32 min_start_time = 0, u32 max_stop_time = 0, u32 min_stop_time = 0, u32 id = u32(-1));
			u32							ActiveSoundsNum								(bool only_playing = true);
