...
void CAI_Stalker::debug_text			()
{
	...
	// memory
	DBG_OutText	("memory");
	DBG_OutText	("%sname          : %s",indent,*ObjectName());
	DBG_OutText	("%ssection       : %s",indent,*SectionName());
	...
	if (g_Alive()) {
		...
		if (memory().sound().sound()) {
			...
			DBG_OutText	("%s%s%sobject    : %s",indent,indent,indent,memory().sound().sound()->m_object ? *memory().sound().sound()->m_object->ObjectName() : "unknown");
		...
		DBG_OutText	("%s%slast hit object name : %s",indent,indent,object ? *object->ObjectName() : "");
#ifdef USE_SELECTED_HIT
		if (memory().hit().hit()) {
			DBG_OutText	("%s%sselected",indent,indent);
			DBG_OutText	("%s%s%spower     : %f",indent,indent,indent,memory().hit().hit()->m_amount);
			DBG_OutText	("%s%s%sobject    : %s",indent,indent,indent,memory().hit().hit()->m_object ? *memory().hit().hit()->m_object->ObjectName() : "unknown");
	...
	if (g_Alive()) {
		...
		for ( ; I != E; ++I)
			DBG_OutText	("%s%s%s%s : %s",indent,indent,indent,*(*I)->ObjectName(),memory().visual().visible_now(*I) ? "visible" : "invisible");
	}
	...
	if (memory().enemy().selected()) {
		...
		DBG_OutText	("%s%s%sobject    : %s",indent,indent,indent,*memory().enemy().selected()->ObjectName());
		if (g_Alive()) {
	...
	if (memory().danger().selected() && memory().danger().selected()->object()) {
		...
		DBG_OutText	("%s%s%stime      : %.3f (%.3f)",indent,indent,indent,float(memory().danger().selected()->time())/1000.f,float(EngineTimeU() - memory().danger().selected()->time())/1000.f);
		DBG_OutText	("%s%s%sinitiator : %s",indent,indent,indent,*memory().danger().selected()->object()->ObjectName());
		...
		if (memory().danger().selected()->dependent_object() && !!memory().danger().selected()->dependent_object()->ObjectName()) {
			DBG_OutText("%s%s%sdependent : %s",indent,indent,indent,*memory().danger().selected()->dependent_object()->ObjectName());
			if (g_Alive())
	...
	if (g_Alive())
		...
		if (g_Alive()) {
			...
			if (agent_manager().member().member(this).member_death_reaction().m_processing)
				DBG_OutText("%react on death : %s",indent,*agent_manager().member().member(this).member_death_reaction().m_member->ObjectName());

			if (agent_manager().member().member(this).grenade_reaction().m_processing)
				DBG_OutText("%react on grenade : %s",indent,agent_manager().member().member(this).grenade_reaction().m_game_object ? *agent_manager().member().member(this).grenade_reaction().m_game_object->ObjectName() : "unknown");
		}
	}
	...
	DBG_OutText	("%s%sactive item         : %s",indent,indent,inventory().ActiveItem() ? *inventory().ActiveItem()->object().ObjectName() : "");
	DBG_OutText	("%s%sbest weapon         : %s",indent,indent,best_weapon() ? *best_weapon()->object().ObjectName() : "");
	DBG_OutText	("%s%sitem to kill        : %s",indent,indent,item_to_kill() ? *m_best_item_to_kill->object().ObjectName() : "");
	DBG_OutText	("%s%sitem can kill       : %s",indent,indent,item_can_kill() ? "+" : "-");
	DBG_OutText	("%s%smemory item to kill : %s",indent,indent,remember_item_to_kill() ? *m_best_found_item_to_kill->object().ObjectName() : "");
	DBG_OutText	("%s%smemory ammo         : %s",indent,indent,remember_ammo() ? *m_best_found_ammo->object().ObjectName() : "");
	...
	if (inventory().ActiveItem()) {
		...
		DBG_OutText	("%s%s%sobject         : %s",indent,indent,indent,inventory().ActiveItem() ? *inventory().ActiveItem()->object().ObjectName() : "");
	...
	if (movement().current_params().cover_fire_object())
		DBG_OutText	("%s%s%sfire object current   : %s",indent,indent,indent,movement().current_params().cover_fire_object()->ObjectName().c_str());

	if (movement().target_params().cover_fire_object())
		DBG_OutText	("%s%s%sfire object target    : %s",indent,indent,indent,movement().target_params().cover_fire_object()->ObjectName().c_str());
	...
	switch (sight().current_action().sight_type()) {
		...
		case SightManager::eSightTypeObject : {
			DBG_OutText	("%s%sobject          : %s",indent,indent,*sight().current_action().object().ObjectName());
...
void CAI_Stalker::dbg_draw_vision	()
{
	...
	if (!smart_cast<CGameObject*>(Level().CurrentEntity()))
		...
		DBG().object_info(this, this).add_item(*ObjectName(), D3DCOLOR_XRGB(255, 0, 0), 0);
...
static void draw_animation_bones	(CAI_Stalker& self, Fmatrix const& transform, IKinematicsAnimated* kinematics_animated, LPCSTR animation_id)
{
	...
	u16									spine_bone_id = 
		(u16)kinematics->LL_BoneID(
			pSettings->r_string(self.SectionName().c_str(),"bone_spin")
		);
	u16									shoulder_bone_id = 
		(u16)kinematics->LL_BoneID(
			pSettings->r_string(self.SectionName().c_str(),"bone_shoulder")
		);
	u16									weapon_bone_id0 = 
		(u16)kinematics->LL_BoneID(
			pSettings->r_string(self.SectionName().c_str(),"weapon_bone0")
		);
	u16									weapon_bone_id1 = 
		(u16)kinematics->LL_BoneID(
			pSettings->r_string(self.SectionName().c_str(),"weapon_bone2")
		);
...
//	self.setVisible						(FALSE);
	...
	Fvector							pos,ypr;
	pos								= pSettings->r_fvector3		(weapon->SectionName(),"position");
	ypr								= pSettings->r_fvector3		(weapon->SectionName(),"orientation");
	ypr.mul							(PI/180.f);
	...
	Fvector	const vLoadedFirePoint	= pSettings->r_fvector3		( weapon->SectionName(), "fire_point" );
