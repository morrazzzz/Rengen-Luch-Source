...
void CSpaceRestrictionHolder::register_restrictor				(CSpaceRestrictor *space_restrictor, const RestrictionSpace::ERestrictorTypes &restrictor_type)
{
	...
	shared_str					space_restrictors = space_restrictor->ObjectName();
...
{
	shared_str				restrictor_id = space_restrictor->ObjectName();
...
IC	void CSpaceRestrictionHolder::collect_garbage			()
{
	...
	for ( ; I != E; ) {
		if (!(*I).second->shape() && !(*I).second->m_ref_count && (EngineTimeU() >= (*I).second->m_last_time_dec + time_to_delete)) {
			J				= I;
			++I;
			xr_delete		((*J).second);
