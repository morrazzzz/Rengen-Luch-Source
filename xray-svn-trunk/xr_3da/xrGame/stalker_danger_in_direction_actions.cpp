...
void CStalkerActionDangerInDirectionTakeCover::initialize						()
{
	...
	float										distance = object().memory().DangerManager().selected()->position().distance_to(object().Position());
...
void CStalkerActionDangerInDirectionDetour::execute								()
{
	...
	if (!object().memory().DangerManager().selected()->object())
		return;

	CMemoryInfo							mem_object = object().memory().memory(object().memory().DangerManager().selected()->object());
...
void CStalkerActionDangerInDirectionSearch::execute							()
{
	...
	if (object().movement().path_completed()) {
		object().m_ce_ambush->setup		(position,mem_object.m_self_params.GetMemorizedPos(),10.f);
		...
		if (object().movement().path_completed() && completed()) {
			if (object().memory().DangerManager().selected()->object())
				object().memory().enable(object().memory().DangerManager().selected()->object(), false);
			else
				object().memory().DangerManager().time_line(EngineTimeU());
		}
	}
