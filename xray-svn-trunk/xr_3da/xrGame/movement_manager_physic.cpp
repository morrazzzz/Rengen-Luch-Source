...
void dump_collision_hit(CPHMovementControl *movement_control)
{
	...
	Msg( "ai unit: %s hited by collision; power: %f, spawn frame %d, current frame %d ", obj->ObjectName().c_str(), movement_control->gcontact_HealthLost, obj->spawn_time(), CurrentFrame() ); 
...
//		Msg				("[%6d][%s] no move, curr_tp=%d",CurrentFrame(),*object().ObjectName(),detail().m_current_travel_point);
		return;
	}
...
#pragma todo("Dima to ? : is this correct?")
		...
		m_speed			= 0.f;
		//Msg				("[%6d][%s] strange exit, curr_tp=%d",CurrentFrame(),*object().ObjectName(),detail().m_current_travel_point);
		return;
	}
//	Msg					("[%6d][%s] curr_tp=%d",CurrentFrame(),*object().ObjectName(),detail().m_current_travel_point);
