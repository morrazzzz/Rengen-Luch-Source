...
#endif
	m_start_level_time	= EngineTimeU();
	m_first_time		= true;
}
...
{
	return				(m_start_level_time + m_inertia_time <= EngineTimeU());
}
...
IC	void CBaseAction::debug_log			(const EActionStates state_state) const
{
	switch (state_state) {
		case eActionStateConstructed : {
			Msg			("[%6d] action %s is constructed",EngineTimeU(),m_action_name);
			break;
		}
		case eActionStateSetup : {
			Msg			("[%6d] action %s is setup",EngineTimeU(),m_action_name);
			break;
		}
		case eActionStateInitialized : {
			Msg			("[%6d] action %s is initialized",EngineTimeU(),m_action_name);
			break;
		}
		case eActionStateExecuted : {
			Msg			("[%6d] action %s is executed",EngineTimeU(),m_action_name);
			break;
		}
		case eActionStateFinalized : {
			Msg			("[%6d] action %s is finalized",EngineTimeU(),m_action_name);
			break;
		}
		default : NODEFAULT;
	}
