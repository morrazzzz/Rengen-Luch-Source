...
public:
	...
	virtual	void	set_level_dest_vertex			(u32 const& level_vertex_id, float satisfying_distance = -1.f);
	...
	IC		void	set_detail_path_type			(EDetailPathType detail_path_type);
			...
			void	set_nearest_accessible_position	(Fvector desired_position, u32 level_vertex_id, float satisfying_distance = -1.f);
			float	speed							(const EMovementDirection &movement_direction);
			void	setup_speed_from_animation		(const float &speed);

				// Helps stop before destination, if evaluation of NPC getting on needed spot is done in scheduler update, which can be delayed for several frames and eventually does not stop NPC when needed
	IC		void	SetDestinationStopDistance		(float distance_to_targed_where_stop)	{ stopBeforeDestination_ = true; stopBeforeDestinationDistance_ = distance_to_targed_where_stop; };
...
private:
	...
	bool					stopBeforeDestination_;
	float					stopBeforeDestinationDistance_;
