...
//		Msg					("[%6d][%s] actuality is false 2",CurrentFrame(),*object().ObjectName());
	...
	switch (m_path_state) {
		...
		case ePathStatePathVerification : {
			if (!patrol().actual()) {
//				Msg				("[%6d][%s] actuality is false 3",CurrentFrame(),*object().ObjectName());
				m_path_state	= ePathStateSelectPatrolPoint;
			}
			else
...
//							Msg				("[%6d][%s] detail path is completed",CurrentFrame(),*object().ObjectName());
//							Msg				(
...
//							);
		...
		case ePathStatePathCompleted : {
			if (!patrol().actual()) {
//				Msg				("[%6d][%s] actuality is false 4",CurrentFrame(),*object().ObjectName());
				m_path_state	= ePathStateSelectPatrolPoint;
			}
			break;
		}
