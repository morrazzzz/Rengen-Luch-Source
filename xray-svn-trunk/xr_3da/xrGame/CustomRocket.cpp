...
BOOL CCustomRocket::SpawnAndImportSOData(CSE_Abstract* data_containing_so) 
{
	m_eState = eInactive;
...
void CCustomRocket::DestroyClientObj() 
{
//	Msg("---------DestroyClientObj [%d] frame[%d]",ID(), CurrentFrame());
...
//	}
	...
	m_time_to_explode = EngineTime() + pSettings->r_float(SectionName(), "force_explode_time") / 1000.0f;
#ifdef	DEBUG
	gbg_rocket_speed1=0;
	gbg_rocket_speed2=0;
#endif	
...
void CCustomRocket::LoadCfg(LPCSTR section) 
{
...
void CCustomRocket::BeforeAttachToParent()
{
	VERIFY(m_eState == eInactive);
	inherited::BeforeAttachToParent();

}
void CCustomRocket::AfterAttachToParent()
{
	VERIFY(m_eState == eInactive);
...
void CCustomRocket::BeforeDetachFromParent(bool just_before_destroy) 
{
...
void CCustomRocket::AfterDetachFromParent() 
{
...
void CCustomRocket::UpdateCL()
{
	...
	{
		if (m_time_to_explode < EngineTime())
		{
...
{
	float force = m_fEngineImpulse*fixed_step;// * TimeDelta();
	float k_back=1.f;
	Fvector l_pos, l_dir; 
	l_pos.set(0, 0,-2.f);
	l_dir.set(XFORM().k);
	...
	l_dir.set(0, 1.f, 0);
	force = m_fEngineImpulseUp*fixed_step;// * TimeDelta();
...
void CCustomRocket::UpdateEngine				()
{
	...
	if( !getVisible() ){
		Msg("! CCustomRocket::UpdateEngine called, but false==getVisible() id[%d] frame[%d]", ID(), CurrentFrame());
	}
