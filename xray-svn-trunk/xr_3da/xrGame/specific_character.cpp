...
void CSpecificCharacter::load_shared(LPCSTR section)
{
#if 0
	CTimer			timer;
	timer.Start		();
	...
	R_ASSERT2(&item_data, make_string("Error reading specific character profile [%s]. It is missing in xmls. [%s]", m_OwnId.c_str(), section ? section : "no sexion"));
	...
	data()->m_can_upgrade = can_upgrade;
	...
	data()->speificStoryID_ = pXML->ReadInt("story_id", 0, -1);

	data()->s_map_spot		= pXML->Read("map_spot_pointer", 0, "null");
	
#ifdef  XRGAME_EXPORTS
	data()->isTrader_		= !!pXML->ReadInt("is_trader", 0, 0);
	...
	data()->sCharRank_			= pXML->ReadInt("rank", 0, NO_RANK);

	R_ASSERT3(data()->sCharRank_ != NO_RANK, "'rank' field not fulfiled for specific character", *m_OwnId);
...
CHARACTER_RANK_VALUE CSpecificCharacter::GetSpecCRank() const
{
	return	data()->sCharRank_;
}
