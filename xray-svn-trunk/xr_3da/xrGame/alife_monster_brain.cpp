...
{
	R_ASSERT2						(object().m_smart_terrain_id != 0xffff, object().name());
	...
	R_ASSERT2						(m_smart_terrain, object().name());
	return							(*m_smart_terrain);
}
...
void CALifeMonsterBrain::process_task			()
{
	...
	R_ASSERT2						(task, make_string("smart terrain [%s] returned nil task, while npc [%s] is registered in it",smart_terrain().name_replace(), object().name()));
