...
void CGroupHierarchyHolder::register_in_group			(CEntity *member)
{
	...
	VERIFY3						(I == m_members.end(),"Specified group member has already been found",*member->ObjectName());
...
#pragma note("cop: this commented code might be needed")
	if (monster)
	{
		//monster->memory().VisualMManager().set_squad_objects(&visible_objects());
		//monster->memory().SoundMManager().set_squad_objects(&sound_objects());
		//monster->memory().HitMManager().set_squad_objects(&hit_objects());
	}

	R_ASSERT(visible_objects().size() < 40000000);
	R_ASSERT(sound_objects().size() < 40000000);
	R_ASSERT(hit_objects().size() < 40000000);
}
...
void CGroupHierarchyHolder::unregister_in_group			(CEntity *member)
{
	VERIFY						(member);
	MEMBER_REGISTRY::iterator	I = std::find(m_members.begin(),m_members.end(),member);
	VERIFY3						(I != m_members.end(),"Specified group member cannot be found",*member->ObjectName());
	m_members.erase				(I);
}
...
#pragma note("cop: this commented code might be needed")
	if (monster)
	{
		//monster->memory().VisualMManager().set_squad_objects(0);
		//monster->memory().SoundMManager().set_squad_objects(0);
		//monster->memory().HitMManager().set_squad_objects(0);
	}
}
