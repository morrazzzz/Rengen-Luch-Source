...
void CMapManager::RemoveMapLocationByObjectID(u16 id) //call on destroy object
{
	...
	{
		if(g_actor && Level().GameTaskManagerP())
			Level().GameTaskManagerP()->MapLocationRelcase((*it).location);
...
void CMapManager::Update()
{
	...
	{
		bool bForce		= CurrentFrame()%3 == idx%3;
...
#ifdef MEASURE_MT
	Device.Statistic->mtMapTime_ += measure_mt.GetElapsed_sec();
#endif
}
