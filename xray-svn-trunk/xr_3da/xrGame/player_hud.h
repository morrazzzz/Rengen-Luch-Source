...
{
public:
	custom_addon(LPCSTR section);
	~custom_addon();
public:
	Fvector attach_offset[2]; // pos,rot
...
struct hud_item_measures
{
	...
	Fvector							m_hands_offset_ironsight[2][3];//pos,rot/ normal,aim,GL
	Fvector							m_hands_offset_scope[2][3];//pos,rot/ normal,aim,GL
	...
	Fvector                         m_strafe_offset[3][1]; // pos,rot,data/ normal --#SM+#--
	...
	Fvector                         m_custom_addon_offset[2][3]; //pos,rot/ scope,silencer,launcher

	Fmatrix GetAddonsParams(u8 index);
...
struct attachable_hud_item
{
	...
	void render_me					(IRenderBuffer& render_buffer);	
	void render_item_ui				();
...
public: 
	...
	void			render_hud			(IRenderBuffer& render_buffer);	
	void			render_item_ui		();
	bool			render_item_ui_query();
	...
	xr_vector<custom_addon> addons_list;
