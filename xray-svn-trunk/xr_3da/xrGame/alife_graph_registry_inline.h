...
IC	void CALifeGraphRegistry::SetGraphItterTimeLimit(const float& process_time)
{
	graphProcessTimeLimit_ = process_time;

	if (m_level)
	{
		level().set_is_time_limited(true);
		level().set_process_time(graphProcessTimeLimit_);
	}
}
