...
#ifdef USE_SCHEDULER_IN_AGENT_MANAGER
void CAgentManager::ScheduledUpdate(u32 time_delta)
{
	START_PROFILE("Agent_Manager")

	ISheduled::ScheduledUpdate	(time_delta);
...
void CAgentManager::update				()
{
	...
	if (EngineTimeU() <= m_last_update_time)
		return;

	if (EngineTimeU() - m_last_update_time < m_update_rate)
		return;
...
#ifdef MEASURE_UPDATES
	Device.Statistic->scheduler_AgentManager_ += measure_sc_update.GetElapsed_sec();
#endif
}
