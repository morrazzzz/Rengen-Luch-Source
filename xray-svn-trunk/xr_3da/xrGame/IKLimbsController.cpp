...
void CIKLimbsController::Destroy(CGameObject* O)
{
	...
	Device.RemoveFromAuxthread4Pool(fastdelegate::FastDelegate0<>(this, &CIKLimbsController::UpdateIK));
}
...
void CIKLimbsController::UpdateIK()
{
	...
	CFrustum& view_frust = ::Render->baseViewFrustum_;

	vis_data& vis = m_object->Visual()->getVisData();
	Fvector p;

	m_object->XFORM().transform_tiny(p, vis.sphere.P);

	float dist = Device.vpSavedView1.GetDistFromCameraMT(p);
	...
	if (dist < IK_CALC_DIST) // if the distance is too big - no need to calc ik.
	{
		if (view_frust.testSphere_dirty(p, vis.sphere.R) || dist < IK_ALWAYS_CALC_DIST) // calc if object is in view frustum or if distance is less than "always calc"
		{
			...
			/*
			Fmatrix predict;
			_pose_extrapolation.extrapolate( predict, EngineTime()  );
			...
			_pose_extrapolation.extrapolate( predict, EngineTime() + 1  );
			DBG_DrawMatrix( predict, 1 );
			*/
		}
	}
...
#ifdef MEASURE_UPDATES
	Device.Statistic->mtIKinematicsTime_ += measure_mt.GetElapsed_sec();
#endif
}


void CIKLimbsController::UpdateKinematics()
{
	if (g_mt_config.test(mtIKinematics))
		Device.AddToAuxThread_Pool(4, fastdelegate::FastDelegate0<>(this, &CIKLimbsController::UpdateIK));
	else
		UpdateIK();
}