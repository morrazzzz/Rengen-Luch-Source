...
public:
	PropValue() :tag(0), m_Owner(0), OnChangeEvent(0), m_function(0) { ; }
	virtual  xr_string	__stdcall GetDrawText(TOnDrawTextEvent OnDrawText) = 0;
	virtual  void	__stdcall	ResetValue() = 0;
	virtual   bool	__stdcall	Equal(PropValue* prop) = 0;
	virtual  void     __stdcall   SetCBChangeEvent(void*userdata, void(__stdcall*function)(void*)) = 0;
...
protected:
	void(__stdcall*m_function)(void*);
	void*m_userdata;
...
public:
	...
	virtual  xr_string __stdcall	GetDrawText(TOnDrawTextEvent OnDrawText) { return ""; }
	virtual  bool	__stdcall	Equal(PropValue* val)
	{
		CustomValue<T>* prop = (CustomValue<T>*)val;
		return (*value == *prop->value);
	}
	virtual const   T&	 __stdcall GetValue() { return *value; }
	virtual  void	__stdcall	ResetValue() { set_value(*value, init_value); }
	virtual  void    __stdcall    SetCBChangeEvent(void*userdata, void(__stdcall*function)(void*)) { m_function = function; m_userdata = userdata; }
	...
	IC bool 			ApplyValue(const T2& val)
	{
		...
		for (PropValueIt it = values.begin(); values.end() != it; ++it) {
			...
			if (CV->ApplyValue(val)) {
				bChanged = true;
				if (CV->m_function)CV->m_function(CV->m_userdata);
...
public:
	...
	virtual  xr_string	__stdcall GetDrawText(TOnDrawTextEvent) { return value.c_str() ? value.c_str() : ""; }
	virtual  	void	__stdcall	ResetValue() { ; }
	virtual	 bool	__stdcall	Equal(PropValue* val) { return (value == ((CaptionValue*)val)->value); }
...
public:
	...
	virtual  xr_string __stdcall	GetDrawText(TOnDrawTextEvent)
	{
	...
	virtual  void  __stdcall      SetCBChangeEvent(void*userdata, void(__stdcall*function)(void*)) { m_function = function; m_userdata = userdata; }
	virtual 	void __stdcall		ResetValue() { ; }
	virtual	 bool	 __stdcall	Equal(PropValue* val) { return true; }
...
public:
	...
	virtual  xr_string __stdcall	GetDrawText(TOnDrawTextEvent OnDrawText);
...
public:
	...
	virtual  void	__stdcall	ResetValue() { xr_strcpy(value, init_value.size() + 1, init_value.c_str()); }
};
//------------------------------------------------------------------------------
...
public:
	...
	virtual  xr_string __stdcall	GetDrawText(TOnDrawTextEvent) { return "[Wave]"; }
};
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
template <>
IC xr_string draw_sprintf<float>(xr_string& s, const float& V, int dec)
{
...
template <>
IC xr_string draw_sprintf<Fvector>(xr_string& s, const Fvector& V, int dec)
{
...
public:
	...
	virtual	 bool	__stdcall	HaveCaption() { return caption[0].size() && caption[1].size(); }
	virtual  bool	__stdcall	GetValueEx() = 0;
};
...
public:
	...
	virtual  bool	__stdcall	Equal(PropValue* val) { return !!CustomValue<T>::value->equal(*((FlagValue<T>*)val)->value, mask); }
	virtual  const T&__stdcall	GetValue() { return *CustomValue<T>::value; }
	virtual  void		__stdcall ResetValue() { CustomValue<T>::value->set(mask, CustomValue<T>::init_value.is(mask)); }
	virtual  bool	__stdcall	GetValueEx() { return !!CustomValue<T>::value->is(mask); }
	bool				ApplyValue(const T& val)
	{
