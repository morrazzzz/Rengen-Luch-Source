...
void CEntityAlive::LoadCfg(LPCSTR section)
{
...
void CEntityAlive::ScheduledUpdate(u32 dt)
{
...
#ifdef MEASURE_UPDATES
	Device.Statistic->scheduler_EntityAlive_ += measure_sc_update.GetElapsed_sec();
#endif
}

BOOL CEntityAlive::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
...
void CEntityAlive::DestroyClientObj()
{
...
void CEntityAlive::OnStartTrade()
{
	callback(GameObject::eTradeStart)();
}

void CEntityAlive::OnStopTrade()
{
	callback(GameObject::eTradeStop)();
}

void CEntityAlive::OnPerformTrade(u32 money_get, u32 money_put)
{
...
//	WOUND_VECTOR_IT last_it;
		...
		it != m_BloodWounds.end();)
	{
		...
		if (pWound->m_fDropTime < EngineTime())
		{
			float size_k = blood_size - m_fStopBloodWoundSize;

			size_k = size_k < 1.f ? size_k : 1.f;

			pWound->m_fDropTime = EngineTime() + (m_fBloodDropTimeMax - (m_fBloodDropTimeMax - m_fBloodDropTimeMin) * size_k) * Random.randF(0.8f, 1.2f);
...
CEntityConditionSimple* CEntityAlive::create_entity_condition(CEntityConditionSimple* ec)
{
	...
	return
		(inherited::create_entity_condition(m_entity_condition));
}
...
bool CEntityAlive::is_locked_corpse()
{
	if (!b_eating)
	{
		if ( m_used_time + m_use_timeout > EngineTimeU())
		{
			return true;
		}
	}
...
void CEntityAlive::RemoveLinksToCLObj(CObject *object)
{
...
void CEntityAlive::OnChangeVisual()
{
	...
	SetBoneSurfListActual(false);
}

void CEntityAlive::fill_hit_bone_surface_areas()
{
	VERIFY(!IsBoneSurfListActual());
	...
	SetBoneSurfListActual(true);
}

BOOL g_ai_use_old_vision = 0;

Fvector	CEntityAlive::get_new_local_point_on_mesh(u16& bone_id)
{
	...
	protectGetLocalPoint_.Enter();

	if (!IsBoneSurfListActual())
		fill_hit_bone_surface_areas();

	auto temp_copy = m_hit_bone_surface_areas; // make a temporary copy, so that other thread can process this object too, without delay

	protectGetLocalPoint_.Leave();
	...
	hit_bone_surface_areas_type::const_iterator i = temp_copy.begin();
	hit_bone_surface_areas_type::const_iterator const e = temp_copy.end();
	...
	for ( ; i != e; ++i )
	{
		...
		R_ASSERT2((*i).first < kinematics->LL_BoneCount(), make_string("Id %u. Max Id %u. [%s]", (*i).first, kinematics->LL_BoneCount(), ObjectNameStr()));
	...
	R_ASSERT2(hit_bones_surface_area > 0.f, make_string("m_hit_bone_surface_areas[%d]", temp_copy.size()));
	...
	R_ASSERT2(i != e, make_string("m_hit_bone_surface_areas[%d]", temp_copy.size()));
...
Fvector CEntityAlive::get_last_local_point_on_mesh	( Fvector const& last_point, u16 bone_id )
{
	if ( bone_id == u16(-1) )
