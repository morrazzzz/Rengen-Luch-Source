...
#endif
{
	...
	if( dbg_draw_animation_movement_controller )
	{
		...
		m_pKinematicsC->CalculateBones(BonesCalcType::force_recalc);
		DBG_OpenCashedDraw();
...
void	animation_movement_controller::GetInitalPositionBlenSpeed	( )
{
	...
	m_control_blend->timeCurrent+=TimeDelta();
	...
	blend_linear_speed =  l/TimeDelta();
	blend_angular_speed = a/TimeDelta();
...
/*
	Fmatrix res = to;
	blend_linear_speed  += blend_linear_accel *TimeDelta() ;
	blend_angular_speed += blend_angular_accel *TimeDelta() ;
	
	inital_position_blending = !clamp_change( res, m_pObjXForm, blend_linear_speed*TimeDelta(), blend_angular_speed*TimeDelta(), 0.00001, 0.000001 ); 
	m_pObjXForm.set( res );
*/
...
//
//	float change_pos_sq_delta = change_pos_delta * change_pos_delta * (( TimeDelta()/0.01f )*( TimeDelta()/0.01f ));
//
...
//		m_pKinematicsC->LL_VisBoxInvalidate();
//		m_pKinematicsC->CalculateBones_Invalidate( );
//		m_pKinematicsC->CalculateBones(BonesCalcType::force_recalc);// TRUE 
//	}
//}