...
void CProjector::LoadCfg(LPCSTR section)
{
...
BOOL CProjector::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
...
void CProjector::ScheduledUpdate(u32 dt)
{
...
void CProjector::TurnOn()
{
	...
	visual->CalculateBones(BonesCalcType::force_recalc);
}
...
void CProjector::UpdateCL()
{
	...
	// Update searchlight 
	angle_lerp(_current.yaw,	_target.yaw,	bone_x.velocity, TimeDelta());
	angle_lerp(_current.pitch,	_target.pitch,	bone_y.velocity, TimeDelta());

	
#ifdef MEASURE_UPDATES
	Device.Statistic->updateCL_Projector_ += measure_updatecl.GetElapsed_sec();
#endif
}


void CProjector::renderable_Render(IRenderBuffer& render_buffer)
{
