...
{
	enemyVertexIDSaved_		= u32(-1);
	enemyPositionSaved_.set(0, 500, 0);
	memParamsInitialized_	= false;
	memObject_.m_object		= nullptr;
}
...
void CStalkerActionCombatBase::execute()
{
	inherited::execute();
	...
	memObject_ = object().memory().memory(object().memory().EnemyManager().selected());

	if (!memObject_.m_object)
		return;

	memParamsInitialized_ = true;

	enemyVertexIDSaved_ = memObject_.m_object_params.m_level_vertex_id;
	enemyPositionSaved_ = memObject_.m_object_params.GetMemorizedPos();
}
...
{
	Fvector	enemy_position = object().memory().EnemyManager().EnemyPosInMemory();

	// Fake to make NPC cover the region at last known enemy position with bulets
	if (!object().memory().VisualMManager().visible_right_now(object().memory().EnemyManager().selected()))
	{
		enemy_position.x += ::Random.randF(-2.f, 2.f);
		enemy_position.y += ::Random.randF(-0.5f, 0.5f);
		enemy_position.z += ::Random.randF(-2.f, 2.f);
	}
...
{
	u32									min_queue_size, max_queue_size, min_queue_interval, max_queue_interval;
	float								distance = object().memory().EnemyManager().EnemyPosInMemory().distance_to(object().Position());
	select_queue_params					(distance,min_queue_size, max_queue_size, min_queue_interval, max_queue_interval);
...
{
	object().sound().play	(
		object().memory().EnemyManager().selected()->human_being() ?
...
{
	object().PlaySearchSound(max_start_time, min_start_time, max_stop_time, min_stop_time, id);
}
...
{
	object().PlayLostSound(max_start_time, min_start_time, max_stop_time, min_stop_time, id);
}
