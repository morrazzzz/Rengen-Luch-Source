...
bool noPathBuildError = false;

void CMovementManager::show_game_path_info	()
{
	Msg							("! Cannot build GAME path! (object %s)",*object().ObjectName());
		...
		case ePathStateBuildGamePath : {
			...
			if (game_path().failed())
			{
				if (!noPathBuildError)
					...
					R_ASSERT2(false, "Path building error, remove -no_build_path_errors to get details");
