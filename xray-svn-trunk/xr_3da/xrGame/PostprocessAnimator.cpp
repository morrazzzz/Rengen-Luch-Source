...
void        CPostprocessAnimator::Load                            (LPCSTR name)
{
    ...
    if (!FS.exist (full_path, "$level$", name))
       if (!FS.exist (full_path, "$game_anims$", name))
		  R_ASSERT2(false, make_string("Can't find motion file '%s'.", name));
#else /*_PP_EDITOR_*/
    string_path full_path;
    ...
    if (ext)
       ...
       if (!xr_strcmp (ext,POSTPROCESS_FILE_EXTENSION))
          ...
          u32 dwVersion = F->r_u32(); //warning is ok, because we need to reed r_u32
...
BOOL CPostprocessAnimator::Process(SPPInfo &PPInfo)
{
	...
	if (m_start_time<0.0f)m_start_time = EngineTime();
	if (m_bCyclic && ((EngineTime() - m_start_time)>f_length)) m_start_time += f_length;

	Update(EngineTime() - m_start_time);

	VERIFY				(_valid(m_factor));
	VERIFY				(_valid(m_factor_speed));
	VERIFY				(_valid(m_dest_factor));
	if(m_bStop)
		m_factor			-=	TimeDelta()*m_factor_speed;
	else
		m_factor			+= m_factor_speed*TimeDelta()*(m_dest_factor-m_factor);
