...
void CAttachableItem::AfterAttachToParent() 
{
...
void CAttachableItem::renderable_Render(IRenderBuffer& render_buffer)
{
	render_buffer.renderingMatrix_ = object().XFORM();

	::Render->add_Visual(object().Visual(), render_buffer);
}

void CAttachableItem::AfterDetachFromParent()
{
...
{
	VERIFY2							(m_valid, make_string("section = %s", object().SectionNameStr()));
...
void attach_draw_adjust_mode()
{
	...
	xr_sprintf(_text, "Adjusting attachable item [%s]", CAttachableItem::m_dbgItem->object().SectionName().c_str());
	F->OutNext			(_text);
