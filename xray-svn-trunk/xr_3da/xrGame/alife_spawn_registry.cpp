...
{
	return						(!!strstr(Core.Params, "-nospawncheck"));
}
...
void CALifeSpawnRegistry::load				(IReader &file_stream, xrGUID *save_guid)
{
	...
	R_ASSERT2					((header().graph_guid() == ai().game_graph().header().guid()) || no_check,"Spawn doesn't correspond to the graph : REBUILD SPAWN!");
