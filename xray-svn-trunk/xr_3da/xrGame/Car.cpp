...
void CCar::reload		(LPCSTR section)
{
	...
	// fuel load
	m_fuel_tank = READ_IF_EXISTS(pSettings, r_float, section, "fuel_tank", 10.0f);
	m_fuel_consumption = READ_IF_EXISTS(pSettings, r_float, section, "fuel_consumption", 0.0005f);
...
// Core events
void	CCar::LoadCfg					( LPCSTR section )
{
...
BOOL	CCar::SpawnAndImportSOData	(CSE_Abstract* data_containing_so)
{
#ifdef DEBUG
	InitDebug();
#endif
	...
	PKinematics(Visual())->CalculateBones(BonesCalcType::force_recalc);
	...
	CInventoryOwner::SpawnAndImportSOData(data_containing_so);

	return							(CScriptEntity::SpawnAndImportSOData(data_containing_so) && R);
...
/* Adopt for cop physics if needed
...
void CCar::ScheduledUpdate(u32 dt)
{
...
#ifdef MEASURE_UPDATES
	Device.Statistic->scheduler_Car_ += measure_sc_update.GetElapsed_sec();
#endif
}
...
void CCar::UpdateEx			(float fov) 
{
	...
	//	Log("UpdateCL",CurrentFrame());
	//XFORM().set(m_pPhysicsShell->mXFORM);
	VisualUpdate(fov);
	if(OwnerActor() && OwnerActor()->IsMyCamera()) 
	{
		cam_Update(TimeDelta(), fov);
...
void CCar::UpdateCL()
{
	...
	{
		#ifdef MEASURE_UPDATES
		Device.Statistic->updateCL_Car_ += measure_updatecl.GetElapsed_sec();
		#endif
...
void	CCar::renderable_Render				(IRenderBuffer& render_buffer)
{
...
void CCar::ExportDataToServer(NET_Packet& P)
{
...
void CCar::UpdateBack()
{
	...
	{
		float k=1.f;
		float time = (EngineTime() - m_break_start);
...
float CCar::EnginePower()
{
	...
	if(b_starting)
	{
		...
		else if(EngineTimeU()-m_dwStartTime>1000) b_starting=false;
	}
	...
	case GE_OWNERSHIP_TAKE:
		{
			...
			PIItem inv_item = smart_cast<PIItem>(O);

			if (inventory().CanTakeItem(inv_item))
			{
				O->H_SetParent(this);
				inventory().TakeItem(inv_item, false, false, duringSpawn);
			}
			else 
			{
	...
	case GE_OWNERSHIP_REJECT:
		{
			...
			if (!O->getDestroy() && inventory().DropItem(inv_item))
			{
...
void CCar::RemoveLinksToCLObj(CObject* O)
{
...
{
	DoorOpen(m_bone_trunk);
}

void CCar::CloseTrunkBone()
{
	DoorClose(m_bone_trunk);
}
