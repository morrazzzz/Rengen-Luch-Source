...
protected:
	...
	float					satisfyingDestinationDistance_;
...
public:
	...
	virtual void	LoadCfg					(LPCSTR caSection);
	...
	//Set satisfying_distance > 0 to make object assume that he has finished path, even though it is only at satisfying_distance from destination
			void	set_level_dest_vertex	(const u32 level_vertex_id, float satisfying_distance = -1.f);
	...
	IC		bool	is_path_varify_state	() const;
	...
	IC		bool	extrapolate_path		() const;
			...
			float	distance_to_destination			() const;
