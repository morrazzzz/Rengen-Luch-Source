...
*/
	...
	{
		base::ScheduledUpdate			(dt);
//		call_member<void>(this,"shedule_Update");
	}

/*	static  void			ScheduledUpdate_static	(base *self, u32 dt)
	{
		self->base::ScheduledUpdate(dt);
	}
*/
};
...
public:
	...
	virtual void			ExportDataToServer	(NET_Packet &packet)
	{
		call<void>("ExportDataToServer",&packet);
	}

	static	void			ExportDataToServer_static	(CGameObject *self, NET_Packet *packet)
	{
		self->CGameObject::ExportDataToServer(*packet);
	}
	...
	static	bool			SpawnAndImportSOData_static	(CGameObject *self, CSE_Abstract* data_containing_so)
	{
		return			(!!self->CGameObject::SpawnAndImportSOData(data_containing_so));
	}
};
