...
void CStalkerActionRunToCover::initialize			()
{
	...
	MemorySpace::CMemoryInfo						mem_object = object().memory().memory(object().memory().EnemyManager().selected());
	...
	if (object().movement().restrictions().accessible(mem_object.m_object_params.m_level_vertex_id)) {
		...
		object().movement().set_desired_position	(&mem_object.m_object_params.GetMemorizedPos());
		return;
	}

	object().movement().set_nearest_accessible_position	(
		mem_object.m_object_params.GetMemorizedPos(),
...
void CStalkerActionRunToCover::execute				()
{
	...
	if (!object().memory().VisualMManager().visible_now(object().memory().EnemyManager().selected())) {
		...
		object().sight().setup						(CSightAction(object().memory().EnemyManager().selected(),true,true,false));
		fire										();
	}
...
void CStalkerActionWaitInCover::execute				()
{
	...
	if (!completed() && !object().memory().VisualMManager().visible_now(object().memory().EnemyManager().selected()))
		return;
