...
void __stdcall OnChangeAnim(void*userdata)
{
	reinterpret_cast<CSE_Visual*>(userdata)->OnChangeAnim(0);
}
void CSE_Visual::FillProps		(LPCSTR pref, PropItemVec &items)
{
	ISE_Abstract* abstract		= smart_cast<ISE_Abstract*>(this); VERIFY(abstract);
	ChooseValue *V 				= PHelper().CreateChoose(items, PrepareKey(pref,abstract->name(),"Model\\Visual").c_str(),		&visual_name,		smVisual);
	//V->OnChangeEvent.bind		(this,&CSE_Visual::OnChangeVisual);
	V->SetCBChangeEvent(static_cast<CSE_Visual*>(this), &::OnChangeVisual);
	V							= PHelper().CreateChoose(items,	PrepareKey(pref,abstract->name(),"Model\\Animation").c_str(),	&startup_animation, smSkeletonAnims,0,(void*)*visual_name);
//	V->OnChangeEvent.bind		(this,&CSE_Visual::OnChangeAnim);
	V->SetCBChangeEvent(static_cast<CSE_Visual*>(this), &::OnChangeAnim);
	PHelper().CreateFlag8		(items, PrepareKey(pref,abstract->name(),"Model\\Obstacle").c_str(),	&flags,	flObstacle);
}
...
void __stdcall OnChangeMotion(void*userdata)
{
	reinterpret_cast<CSE_Motion*>(userdata)->OnChangeMotion(0);
}
void CSE_Motion::FillProps(		LPCSTR pref, PropItemVec &items)
{
	ISE_Abstract* abstract		= smart_cast<ISE_Abstract*>(this); VERIFY(abstract);
	ChooseValue *V				= PHelper().CreateChoose(items, PrepareKey(pref,abstract->name(),"Motion").c_str(),&motion_name, smGameAnim);
	//V->OnChangeEvent.bind		(this,&CSE_Motion::OnChangeMotion);
	V->SetCBChangeEvent(static_cast<CSE_Motion*>(this), &::OnChangeMotion);
}
