...
void CLevelChanger::DestroyClientObj() 
{
...
BOOL CLevelChanger::SpawnAndImportSOData(CSE_Abstract* data_containing_so) 
{
	m_entrance_time				= 0;
	CCF_Shape *l_pShape			= xr_new <CCF_Shape>(this);
	collidable.model			= l_pShape;
	
	CSE_Abstract				*l_tpAbstract = (CSE_Abstract*)(data_containing_so);
	...
	BOOL						bOk = inherited::SpawnAndImportSOData(data_containing_so);
	if (bOk) {
...
void CLevelChanger::ScheduledUpdate(u32 dt)
{
...
#ifdef MEASURE_UPDATES
	Device.Statistic->scheduler_LevelChanger_ += measure_sc_update.GetElapsed_sec();
#endif
}
...
void CLevelChanger::feel_touch_new	(CObject *tpObject)
{
	...
	if (pGameSP)
        pGameSP->ChangeLevel(m_game_vertex_id, m_level_vertex_id, m_position, m_angles, p, r, b, "temp", true);

	m_entrance_time = EngineTime();
}
...
void CLevelChanger::update_actor_invitation()
{
	...
	for(;it!=it_e;++it){
		...
		if (m_entrance_time + 5.0f < EngineTime())
		{
			CUIGameSP* pGameSP = smart_cast<CUIGameSP*>(CurrentGameUI());
			Fvector p,r;
			bool b = get_reject_pos(p,r);
