...
//	m_actuality				= m_actuality && ((m_last_radius + EPS_L) >= radius);
	...
	bool					time_criteria = (EngineTimeU() < m_last_update + m_inertia_time);
...
void CCoverEvaluatorBest::evaluate_cover			(const CCoverPoint *cover_point, float weight)
{
	...
	auto& mem = object().object().memory();
	auto& enemy_mem = mem.EnemyManager();

	auto it = enemy_mem.GetPrestoredAllies().begin();
	auto it_e = enemy_mem.GetPrestoredAllies().end();


	for (; it != it_e; it++)
	{
		const CAI_Stalker* ally = smart_cast<const CAI_Stalker*>((*it).m_object);

		if (ally && ally->current_best_cover() && ally->current_best_cover() == cover_point)
		{
			if (ally->current_best_cover()->m_position.similar(ally->Position(), 2.f))
			{
				//Msg("!%s: Cover is taken by %s", object().object().ObjectNameStr(), ally->ObjectNameStr());
...
void CCoverEvaluatorAmbush::evaluate_cover			(const CCoverPoint *cover_point, float weight)
{
	...
	if (enemy_distance <= m_min_enemy_distance)
		return;
