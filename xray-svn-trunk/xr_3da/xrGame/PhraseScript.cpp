...
//	m_sScriptTextFunc = uiXml.Read(phrase_node, "script_text", 0, NULL);
	...
	LoadSequence(uiXml, phrase_node, "has_any_info",		m_HasAnyInfo);
	LoadSequence(uiXml, phrase_node, "dont_has_any_info",	m_DontHaveAnyInfo);
}
...
bool  CDialogScriptHelper::CheckInfo(const CInventoryOwner* pOwner) const
{
	...
	for (u32 i = 0; i < m_HasAnyInfo.size(); i++)
	{
		if (!Actor()->HasInfo(m_HasAnyInfo[i]))
		{
#ifdef DEBUG
			if (psAI_Flags.test(aiDialogs))
				Msg("----rejected: [%s] dont has info %s", pOwner->Name(), *m_DontHasInfo[i]);
	...
	if (m_HasAnyInfo.size() > 0 && cnt == m_HasAnyInfo.size()) // if all "any infos" are unknown - return false
		return false;

	cnt = 0;

	for (u32 i = 0; i < m_DontHaveAnyInfo.size(); i++)
	{
		if (Actor()->HasInfo(m_DontHaveAnyInfo[i]))
		{
#ifdef DEBUG
			if (psAI_Flags.test(aiDialogs))
				Msg("----rejected: [%s] dont has info %s", pOwner->Name(), *m_DontHasInfo[i]);
	...
	if (m_DontHaveAnyInfo.size() > 0 && cnt == m_DontHaveAnyInfo.size()) // if all "any don't infos" are known - return false
		return false;
...
bool CDialogScriptHelper::Precondition	(const CGameObject* pSpeakerGO, LPCSTR dialog_id, LPCSTR phrase_id) const 
{
	...
	for(u32 i = 0; i<Preconditions().size(); ++i)
	{
		...
		if (strstr(*Preconditions()[i], ".ReturnReverse"))
		{
			...
			if (!lua_function.is_valid())
			{
				R_ASSERT2(false, make_string("Function is not correct %s", preconditionScript.c_str()));
			}
