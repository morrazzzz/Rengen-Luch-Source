...
BOOL CRestrictedObject::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
...
void CRestrictedObject::DestroyClientObj()
{
...
IC	void CRestrictedObject::construct_restriction_string(LPSTR temp_restrictions, u32 const temp_restrictions_size, const xr_vector<ALife::_OBJECT_ID> &restrictions, shared_str current_restrictions, const P &p)
{
	...
	*temp_restrictions			= 0;
	...
	for ( ; I != E; ++I) {
		...
		if (!object || !!strstr(*current_restrictions,*object->ObjectName()) == value)
			continue;
		...
		xr_strcat(temp_restrictions,temp_restrictions_size,*object->ObjectName());
