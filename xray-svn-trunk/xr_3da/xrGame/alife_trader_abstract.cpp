...
void CSE_ALifeTraderAbstract::SpawnTemplate(CSE_ALifeDynamicObject *dynamic_object, const SSpecificCharacterSupplies::TemplateRef& defn)
{
	...
	tmpl.Load(defn.name, character_profile());

	for (int i = 0; i < defn.count; ++i)
	{
...
extern void TraderSupplySpawnScriptCallback(u32 id_parent, u32 id_spawned_item);
	...
	for (int i = 0; i < defn.count; ++i)
	{
		if (::Random.randF(1.f) < defn.probability)
		{
			...
			if (W)
			{
				if (defn.ammo.idx > 0)
				{
					W->set_ammo_type((u8)defn.ammo.idx);
				}
				if (defn.ammo.rounds >= 0)
				{
					W->set_ammo_elapsed((u16)_min(W->get_ammo_magsize(), defn.ammo.rounds));
				}
				...
				if (WGL)
				{
					...
					{
						WGL->grndID_ = (u8)defn.grenade.idx;
					}
			...
			auto alife_item = smart_cast<CSE_ALifeItem*>(item);

			if (alife_item)
				TraderSupplySpawnScriptCallback(dynamic_object->ID, alife_item->ID);
		}
	}
}
