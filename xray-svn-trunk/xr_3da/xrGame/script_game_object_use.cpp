...
shared_str CScriptGameObject::ObjectName				() const
{
	return			(object().ObjectName());
}
...
float CScriptGameObject::GetHemiValue() const
{
	if (object().ROS())
		return object().ROS()->get_luminocity_hemi();
...
void CScriptGameObject::Kill					(CScriptGameObject* who)
{
	...
	if (!l_tpEntity) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"%s cannot access class member Kill!",*object().ObjectName());
		return;
	}
	if (!l_tpEntity->AlreadyDie())
		l_tpEntity->KillEntity					(who ? who->object().ID() : object().ID());
	else
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"attempt to kill dead object %s",*object().ObjectName());
...
ALife::ERelationType CScriptGameObject::GetRelationType	(CScriptGameObject* who)
{
	...
	if (!l_tpEntityAlive1) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"%s cannot access class member GetRelationType!",*object().ObjectName());
		return ALife::eRelationTypeDummy;
	}
	
	CEntityAlive		*l_tpEntityAlive2 = smart_cast<CEntityAlive*>(&who->object());
	if (!l_tpEntityAlive2) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"%s cannot apply GetRelationType method for non-alive object!",*who->object().ObjectName());
		return ALife::eRelationTypeDummy;
	}
...
IC	T	*CScriptGameObject::action_planner()
{
	...
	if (!manager) {
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : cannot access class member action_planner for '%s'!", *manager->ObjectName());
...
void CScriptGameObject::set_enemy_callback	(const luabind::functor<bool> &functor)
{
	...
	monster->memory().EnemyManager().useful_callback().set(functor);
}
...
void CScriptGameObject::set_enemy_callback	(const luabind::functor<bool> &functor, const luabind::object &object)
{
	...
	monster->memory().EnemyManager().useful_callback().set(functor, object);
}
...
void CScriptGameObject::set_enemy_callback	()
{
	...
	monster->memory().EnemyManager().useful_callback().clear();
}
...
void CScriptGameObject::set_const_force(const Fvector &dir,float value,u32 time_interval)
{
	...
	if(!shell){
		ai().script_engine().script_log				(ScriptStorage::eLuaMessageTypeError,"set_const_force : object %s has no physics shell!",*object().ObjectName());
		return;
	}
