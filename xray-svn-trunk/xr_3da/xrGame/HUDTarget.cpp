...
void CHUDTarget::RemoveLinksToCLObj(CObject* O)
{
	if(PP.RQ.O == O)
		PP.RQ.O = NULL;
...
void CHUDTarget::Render()
{
	...
	{ 
		if (PP.RQ.O)
		{
			...
			CPoltergeist* poltergeist = smart_cast<CPoltergeist*> (PP.RQ.O);

			CInventoryOwner* our_inv_owner = smart_cast<CInventoryOwner*>(pCurEnt);

			if ((E && PP.RQ.O->getVisible()) || poltergeist)
			{
				CInventoryOwner* others_inv_owner = smart_cast<CInventoryOwner*>(E);
				CAI_Stalker* pStalker = smart_cast<CAI_Stalker*>(E);

				if (E->g_Alive() && !E->cast_base_monster())
				{
					if (pStalker && psHUD_Flags.test(HUD_NPC_DETECTION) && !Level_Game_Cl_Base().IsDifficultyVeteranOrHigher())
					{
				...
				if (others_inv_owner && psHUD_Flags.test(HUD_INFO_CNAME))
				{
					F->SetColor(subst_alpha(C, u8(iFloor(255.f * (fuzzyShowInfo - 0.5f) * 2.f))));
					F->OutNext("%s", others_inv_owner->cast_game_object()->ObjectName().c_str());

					if (pStalker)
						F->OutNext("Cur logic: %s", pStalker->GetCurrentActionName().c_str());
				}

				fuzzyShowInfo += SHOW_INFO_SPEED * TimeDelta();
			}
			else
			{
				if (Actor()->inventory().m_pTarget && !Actor()->pickUpLongInProgress) // dont show info, if actor ties to pick up group
				{
					if (fuzzyShowInfo > 0.5f)
					{
						F->SetColor(subst_alpha(C, u8(iFloor(255.f * (fuzzyShowInfo - 0.5f) * 2.f))));
						F->OutNext("%s", Actor()->inventory().m_pTarget->Name());

						if (psHUD_Flags.test(HUD_INFO_CNAME))
							F->OutNext("%s", Actor()->inventory().m_pTarget->object().ObjectName().c_str());
					}
			...
			fuzzyShowInfo -= HIDE_INFO_SPEED * TimeDelta();
