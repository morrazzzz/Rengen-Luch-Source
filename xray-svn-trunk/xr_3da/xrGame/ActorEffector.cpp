...
BOOL CAnimatorCamEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	m_objectAnimator->Update	(TimeDelta());
...
{
	float f = (m_end_time - EngineTime()) / m_life_time;
...
void SndShockEffector::Start(CActor* A, float snd_length, float power)
{
	...
	m_end_time			= EngineTime() + m_life_time;
...
BOOL CControllerPsyHitCamEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	if (angle_lerp(m_dangle_current.x, m_dangle_target.x, ANGLE_SPEED, TimeDelta())) {
		m_dangle_target.x = angle_normalize(Random.randFs(DELTA_ANGLE_X));
	}

	if (angle_lerp(m_dangle_current.y, m_dangle_target.y, ANGLE_SPEED, TimeDelta())) {
		m_dangle_target.y = angle_normalize(Random.randFs(DELTA_ANGLE_Y));
	}

	if (angle_lerp(m_dangle_current.z, m_dangle_target.z, ANGLE_SPEED, TimeDelta())) {
...
void CScriptCameraDirection::Update()
{
	...
	if (fsimilar(cam_current_yaw, cam_target_yaw, EPS_ANGLE)) {
		...
		if (angle_normalize_signed(cam_target_yaw - cam_current_yaw) > 0) 
			m_actor->cam_Active()->Move	(kLEFT,		m_speed * TimeDelta());
		else 
			m_actor->cam_Active()->Move	(kRIGHT,	m_speed * TimeDelta());
	}
	...
	if (fsimilar(cam_current_pitch, cam_target_pitch, EPS_ANGLE)) {
		...
		if (angle_normalize_signed(cam_target_pitch - cam_current_pitch) > 0)
			m_actor->cam_Active()->Move	(kDOWN,	m_speed * TimeDelta());
		else 
			m_actor->cam_Active()->Move	(kUP, m_speed * TimeDelta());
	}
