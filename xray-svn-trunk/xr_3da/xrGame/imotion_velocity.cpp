...
void	imotion_velocity::state_end(  )
{
	VERIFY( shell );
	inherited::state_end( );
	shell->AnimToVelocityState( TimeDelta(), default_l_limit * 10, default_w_limit * 10 );
...
{
		VERIFY( shell );
		if( !shell->AnimToVelocityState( TimeDelta(), 2 * default_l_limit, 10.f * default_w_limit ) )
