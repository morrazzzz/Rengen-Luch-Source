...
//	static bool playing = false;
	if (m_start_time > EngineTimeContinual() - m_run_time)
	{
//		playing = true;
		...
		pos.y = this->pos(m_origin, m_destination, EngineTimeContinual() - m_start_time);
		m_shniaga->SetWndPos(pos);		
	}
	else
		ProcessEvent(E_Stop);

	if (m_start_time > EngineTimeContinual() - m_run_time*10/100)
		...
		case E_Begin:
			{
				...
				m_start_time = EngineTimeContinual();
