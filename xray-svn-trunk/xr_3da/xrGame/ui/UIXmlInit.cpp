...
bool CUIXmlInit::InitDragDropListEx(CUIXml& xml_doc, LPCSTR path, int index, CUIDragDropListEx* pWnd)
{
	...
	pWnd->SetAutoGrowY		(tmp!=0);
	tmp						= xml_doc.ReadAttribInt(path, index, "unlimited_x", 0);
	pWnd->SetAutoGrowX		(tmp!=0);
...
bool CUIXmlInit::InitTrackBar(CUIXml& xml_doc, LPCSTR path, int index, CUITrackBar* pWnd)
{
	...
	pWnd->useCustomBounds_ = !!xml_doc.ReadAttribInt(path, index, "custom_bounds", 0);

	if (pWnd->useCustomBounds_)
	{
		float min = xml_doc.ReadAttribFlt(path, index, "min", 0.f);
		float max = xml_doc.ReadAttribFlt(path, index, "max", 1.f);

		if (is_integer)
			pWnd->SetOptIBounds((int)min, (int)max, false);
		else
			pWnd->SetOptFBounds(min, max, false);
	}
