...
class CUIStatic;
class CSimpleArtDetector;
class CAdvArtDetector;
class CEliteArtDetector;
class CLAItem;
...
public:
	...
	void				update						();
	void				SetBoneVisible				(IKinematics* model, CSimpleArtDetector* parent, LPCSTR bonename, bool visible, bool someshit);
	void 				ZvukovojSignal				(HUD_SOUND_ITEM sound, float fvector1, float fvector2, float fvector3, CSimpleArtDetector* parent, bool bool1, bool bool2);
	...
	void				construct					(CSimpleArtDetector* p);
};
...
public:
	...
	virtual void			update();
	void					construct(CAdvArtDetector* p);
...
public:
	...
	void		construct(CEliteArtDetector* p);
	void		Clear();
