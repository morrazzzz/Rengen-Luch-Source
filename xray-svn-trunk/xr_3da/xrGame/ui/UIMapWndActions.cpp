...
static const float	map_resize_speed		= 1550.f;	// y.e./sec
...
void CMapActionZoomControl::execute		()
{
	...
	float dt				= _min(TimeDelta(),time_to);

	if (m_endMovingTime > EngineTime())
	{
...
void CMapActionMinimize::initialize()
{
	...
	m_endMovingTime = EngineTime() + map_zoom_time;
}
