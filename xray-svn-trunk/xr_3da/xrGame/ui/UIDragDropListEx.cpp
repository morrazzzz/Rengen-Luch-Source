...
void CUIDragDropListEx::SetAutoGrowY(bool b)
{
	m_flags.set(flAutoGrowY,b);
}
bool CUIDragDropListEx::IsAutoGrowY()
{
	return !!m_flags.test(flAutoGrowY);
}
void CUIDragDropListEx::SetAutoGrowX(bool b)
{
	m_flags.set(flAutoGrowX, b);
}
bool CUIDragDropListEx::IsAutoGrowX()
{
	return !!m_flags.test(flAutoGrowX);
}
...
void CUIDragDropListEx::OnItemDrop(CUIWindow* w, void* pData)
{
	...
	CUIDragDropListEx* new_ddlist = m_drag_item->BackList();

	bool b = (old_ddlist == new_ddlist) && !GetCustomPlacement();

	if (old_ddlist && new_ddlist && !b)
	{
		CUICellItem* i = old_ddlist->RemoveItem(itm, (old_ddlist == new_ddlist));
		while(i->ChildsCount())
		{
			CUICellItem* _chld = i->PopChild(NULL);
			new_ddlist->SetItem(_chld, old_ddlist->GetDragItemPosition());
		}
...
void CUIDragDropListEx::OnItemDBClick(CUIWindow* w, void* pData)
{
	...
	VERIFY(old_ddlist == this);

	if (old_ddlist&&old_ddlist->GetCustomPlacement())
	{
		CUICellItem* i = old_ddlist->RemoveItem(itm, true);
		old_ddlist->SetItem(i);
	}
...
void CUIDragDropListEx::UpdateHintShow()
{
	if (CursorOverWindow() && !ItemsCount() && m_hint_text.size() && !g_statHint->Owner() && EngineTimeU()>m_dwFocusReceiveTime + 700)
	{
...
CUICellItem* CUIDragDropListEx::RemoveByInvItem(CInventoryItem* itm)
{
	for (WINDOW_LIST_it it = m_ChildWndList.begin(); m_ChildWndList.end() != it; ++it)
	{
		CUICellItem* i = (CUICellItem*)(*it);

		if ((CInventoryItem*)i->m_pData == itm)
		{
...
Ivector2 CUICellContainer::FindFreeCell	(const Ivector2& _size)
{
	...
	if (m_pParentDragDropList->IsAutoGrowY() || m_pParentDragDropList->IsAutoGrowX())
	{
		Grow(m_pParentDragDropList->IsAutoGrowY(), m_pParentDragDropList->IsAutoGrowX());
...
CUICell& CUICellContainer::GetCellAt(const Ivector2& pos)
{
	R_ASSERT(pos.x>=0);
	R_ASSERT(pos.y>=0);
	R_ASSERT(pos.x < m_cellsCapacity.x);
	R_ASSERT(pos.y < m_cellsCapacity.y);
...
void CUICellContainer::Grow(bool grow_y, bool grow_x)
{
	if (grow_y)
		SetCellsCapacity(Ivector2().set(m_cellsCapacity.x,m_cellsCapacity.y + 1));
	if (grow_x)
		SetCellsCapacity(Ivector2().set(m_cellsCapacity.x + 1, m_cellsCapacity.y));
}
...
//	u32 cell_i = 0;
	...
	if ( m_cells_to_draw.size() )
	{
		...
		{
			CUICell& cell = (*it);
			if (!cell.Empty() && (cell.m_item->m_drawn_frame != CurrentFrame()))
			{
				cell.m_item->Draw();
			}
		}
