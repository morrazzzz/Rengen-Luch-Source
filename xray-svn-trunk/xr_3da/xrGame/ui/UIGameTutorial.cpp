...
void CUISequencer::Start(LPCSTR tutor_name)
{
	...
	Device.seqFrame.Add			(this, REG_PRIORITY_LOW - 400);
...
#ifdef MEASURE_ON_FRAME
	Device.Statistic->onframe_UISeq_ += measure_on_frame.GetElapsed_ms_f();
#endif
}
...
void CUISequencer::IR_OnKeyboardPress	(int dik)
{
	...
	if(binded && CurrentGameUI())
	{
		...
		if (ui_game_sp)
		{
			...
			if (ui_game_sp->m_UIStashWnd->IsShown())
			{
				ui_game_sp->m_UIStashWnd->HideDialog();
				return;
			}
			...
			if (ui_game_sp->m_WeaponSelect->IsShown())
			{
				ui_game_sp->m_WeaponSelect->HideDialog();
				return;
			}
