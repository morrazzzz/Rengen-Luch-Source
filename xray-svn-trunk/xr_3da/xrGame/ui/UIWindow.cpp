...
void CUIWindow::Draw()
{
	...
	{
		if (!(*it)->IsShown() || !GameDifficultyAllows())
			continue;
...
{	
	...
	if( WINDOW_LBUTTON_DOWN == mouse_action )
	{
		...
		u32 dwCurTime						= EngineTimeContinual();

		if ((_last_db_click_frame != CurrentFrame()) && (dwCurTime - m_dwLastClickTime < DOUBLE_CLICK_TIME))
		{
            mouse_action			= WINDOW_LBUTTON_DB_CLICK;
			_last_db_click_frame = CurrentFrame();
		}
...
{
	m_dwFocusReceiveTime	= EngineTimeU();
