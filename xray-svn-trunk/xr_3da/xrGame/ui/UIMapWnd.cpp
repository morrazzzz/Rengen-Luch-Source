...
void CUIMapWnd::Init(LPCSTR xml_name, LPCSTR start_from)
{
	...
	btnIndex		= eLocationsToggle;
	strconcat(sizeof(pth),pth, sToolbar.c_str(), ":locations_toggle_btn");
	if(uiXml.NavigateToNode(pth,0)){
		m_ToolBar[btnIndex]				= xr_new <CUI3tButton>(); m_ToolBar[btnIndex]->SetAutoDelete(true);
		xml_init.Init3tButton			(uiXml, pth, 0, m_ToolBar[btnIndex]);
		UIMainMapHeader->AttachChild	(m_ToolBar[btnIndex]);
		Register						(m_ToolBar[btnIndex]);
		AddCallbackStr						(*m_ToolBar[btnIndex]->WindowName(),BUTTON_CLICKED,CUIWndCallback::void_function(this,&CUIMapWnd::OnLocationsShowToggle));

		drawMapLocations_ = true;
	}
...
void CUIMapWnd::SetTargetMap			(CUICustomMap* m, const Fvector2& pos, bool bZoomIn)
{
	...
	if(m==GlobalMap()){
		...
		m_tgtCenter						= m->ConvertRealToLocalNoTransform(pos, false);
...
CMapLocation* CUIMapWnd::UnderSpot(Fvector RealPosition)
{
	...
	Fvector2 m_position_mouse = m_tgtMap->ConvertRealToLocal(RealPositionXZ, false);
	...
	for (it = Spots.begin(); it!=Spots.end(); ++it) {
		if ( (*it).location->IsUserDefined() ) {
			...
			m_position_on_map = m_tgtMap->ConvertRealToLocal((*it).location->GetPosition(), false);
...
void CUIMapWnd::OnLocationsShowToggle(CUIWindow* w, void*)
{
	drawMapLocations_ = !drawMapLocations_;
}
