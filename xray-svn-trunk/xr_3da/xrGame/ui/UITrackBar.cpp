...
void CUITrackBar::SetCurrentOptValue()
{
	...
	if (m_b_is_float)
	{
		float minn = 0.f; float maxx = 0.f;

		GetOptFloatValue(m_f_val, minn, maxx);

		if (!useCustomBounds_) // use console command limits
		{
			m_f_min = minn;
			m_f_max = maxx;
		}
		else // or use developer assigned limits
		{
			// but clamp to console command limit anyway
			m_f_min = _max(m_f_min, minn);
			m_f_max = _min(m_f_max, maxx);
		}
	}
	else
	{
		...
		{
			// but clamp to console command limit anyway
			m_i_min = _max(m_i_min, minn);
			m_i_max = _min(m_i_max, maxx);
		}
	}
...
void CUITrackBar::SetOptIBounds(int imin, int imax, bool update)
{
	// assign and update, if needed
	m_i_min	= imin;
	m_i_max	= imax;

	if (update && (m_i_val < m_i_min || m_i_val > m_i_max))
	{
		clamp(m_i_val, m_i_min, m_i_max);
		OnChangedOptValue();
	}
...
void CUITrackBar::SetOptFBounds(float fmin, float fmax, bool update)
{
	// assign and update, if needed
	m_f_min	= fmin;
	m_f_max	= fmax;

	if (update && (m_f_val < m_f_min || m_f_val > m_f_max))
	{
		clamp(m_f_val, m_f_min, m_f_max);
		OnChangedOptValue();
	}
