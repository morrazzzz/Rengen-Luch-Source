...
bool CUISequenceSimpleItem::IsPlaying()
{
	...
	return (m_time_start+m_time_length)>(EngineTimeContinual()/1000.0f);
}
...
void CUISequenceSimpleItem::OnRender()
{
	...
	if(m_time_start < 0.0f)
		m_time_start				= float(EngineTimeContinual())/1000.0f;
}
...
float CUISequenceSimpleItem::current_factor()
{
	if(m_time_start < 0.0f || fis_zero(m_time_length))
		...
		return ((EngineTimeContinual()/1000.0f)-m_time_start) / m_time_length;
}

void CUISequenceSimpleItem::Update()
{
	inherited::Update();
	float _start					= (m_time_start<0.0f)? (float(EngineTimeContinual())/1000.0f) : m_time_start;

	float gt						= float(EngineTimeContinual())/1000.0f;
	SubItemVecIt _I					= m_subitems.begin();
	SubItemVecIt _E					= m_subitems.end();
	for(;_I!=_E;++_I)
	{
	...
	if(g_pGameLevel && (!m_pda_section || 0 == xr_strlen(m_pda_section)) )
	{
		...
		if(ui_game_sp)
		{
			if ( ui_game_sp->m_PdaMenu->IsShown()			||
				...
				ui_game_sp->m_UIStashWnd->IsShown() ||
				ui_game_sp->UIChangeLevelWnd->IsShown()		||
				ui_game_sp->m_WeaponSelect->IsShown() ||
