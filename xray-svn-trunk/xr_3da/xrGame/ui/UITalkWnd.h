...
public:
	...
	void				SwitchToTrade			();
	void				SwitchToUpgrade			(CInventoryOwner* our = nullptr, CInventoryOwner* other = nullptr);
	void				AddIconedMessage		(LPCSTR text, LPCSTR texture_name, Frect texture_rect, LPCSTR templ_name);

	CUITalkDialogWnd*	GetTalkDialog			() { return UITalkDialogWnd; };
	CUIUpgradeWnd*		GetUpgradeWnd			() { return UIUpgradeWnd; };
	CUITradeWnd*		GetTradeDialog			() { return UITradeWnd; };
protected:
	//диалог
	void				InitTalkDialog			();
	void				AskQuestion				();
