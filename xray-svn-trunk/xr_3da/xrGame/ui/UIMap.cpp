...
Fvector2 CUICustomMap::ConvertRealToLocal(const Fvector2& src, bool for_drawing)// meters->pixels (relatively own left-top pos)
{
	Fvector2 res;
	if( !Heading() ){
		return ConvertRealToLocalNoTransform(src, m_BoundRect);
	}else{
		Fvector2 heading_pivot = GetStaticItem()->GetHeadingPivot();
	
		res = ConvertRealToLocalNoTransform(src, m_BoundRect);
		res.sub(heading_pivot);
		rotation_(res.x, res.y, GetHeading(), res.x, res.y, for_drawing);
...
void CUILevelMap::Update()
{
	...
	if(m_bCursorOverWindow){
		...
		if( EngineTimeU()>(m_dwFocusReceiveTime+500) ){
	...
	if (hintOwnerSpot)
	{
		MapWnd()->HideHintForced();
		MapWnd()->ShowHint(hintOwnerSpot, hintOwnerSpot->GetHint());
		hintOwnerSpot = nullptr;
	}
}
...
void	CUILevelMap::SendMessage			(CUIWindow* pWnd, s16 msg, void* pData)
{
	...
	if(msg==MAP_SHOW_HINT)
	{
		...
		if (hintOwnerSpot) // Check priority if hinted spot is already present
		{
			if (hintOwnerSpot->MapLocation()->GetHintPrior() > sp->MapLocation()->GetHintPrior())
				add_hint = false;
		}
