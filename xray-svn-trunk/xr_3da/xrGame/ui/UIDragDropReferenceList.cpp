...
CUICellItem* CUIDragDropReferenceList::RemoveItem(CUICellItem* itm, bool force_root)
{
	...
	{
		u8 index = u8(vec2.x);
		xr_strcpy(Actor()->quickUseSlotsContents_[index], "");
...
void CUIDragDropReferenceList::ReloadReferences()
{
	if(m_drag_item)
		DestroyDragItem();
	...
	for(u8 i=0; i<m_container->CellsCapacity().x; i++)
	{
		...
		LPCSTR item_name = Actor()->quickUseSlotsContents_[i];

		if(item_name && xr_strlen(item_name))
		{
			PIItem itm = Actor()->inventory().GetItemBySect(item_name, Actor()->inventory().allContainer_);
			if(itm)
			{
				SetItem(create_cell_item(itm, true), Ivector2().set(i, 0));
			}
			else
			{
...
void CUIDragDropReferenceList::OnItemDBClick(CUIWindow* w, void* pData)
{
	...
	if(it != m_references.end())
	{
		...
		PIItem itm = Actor()->inventory().GetItemBySect(Actor()->quickUseSlotsContents_[index], Actor()->inventory().allContainer_);
		if(itm)
...
void CUIDragDropReferenceList::OnItemDrop(CUIWindow* w, void* pData)
{
	...
	if(vec.x!=-1&&vec.y!=-1)
	{
		...
		{
			u8 index = u8(vec2.x);
			shared_str tmp = Actor()->quickUseSlotsContents_[vec.x];
			xr_strcpy(Actor()->quickUseSlotsContents_[vec.x], Actor()->quickUseSlotsContents_[index]);
			xr_strcpy(Actor()->quickUseSlotsContents_[index], tmp.c_str());
			ReloadReferences();
			return;
		}
	}
