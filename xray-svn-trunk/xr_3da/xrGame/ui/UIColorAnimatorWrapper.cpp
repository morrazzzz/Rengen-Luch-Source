...
//	TimeDelta() нельзя - анимация тормознутая получается. Приходится 
...
CUIColorAnimatorWrapper::CUIColorAnimatorWrapper()
	:	colorAnimation		(NULL),
		...
		kRev				(0.0f)
{
	prevGlobalTime	= EngineTimeContinual()/1000.0f;
	...
	if (colorAnimation && !isDone)
	{
		if (!isCyclic)
		{
			if (animationTime < (colorAnimation->iFrameCount / colorAnimation->fFPS))
			{
				...
				//Msg("name: %s, color: %x, frame: %d", *colorAnimation->ObjectName,currColor, currFrame);
				currColor		= color_rgba(color_get_B(currColor), color_get_G(currColor), color_get_R(currColor), color_get_A(currColor));
				// обновим время
				animationTime	+= EngineTimeContinual()/1000.0f - prevGlobalTime;
			}
			else
			{
		...
		{
			currColor	= colorAnimation->CalculateBGR(EngineTimeContinual()/1000.0f, currFrame);
			currColor	= color_rgba(color_get_B(currColor), color_get_G(currColor), color_get_R(currColor), color_get_A(currColor));
		}
