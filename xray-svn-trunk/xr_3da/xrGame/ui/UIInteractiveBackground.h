...
public:
	...
	virtual ~CUIInteractiveBackground() {};
			...
			void SetConstTextureState		(IBState state);
			void RemoveConstTextureState	();
...
{
	if (constantTextureState_ != S_Total && m_states[S_Total])
	{
		m_states[constantTextureState_]->Draw();
	}
	else if (m_states[S_Current])
...
template <class T>
void CUIInteractiveBackground<T>::SetConstTextureState(IBState state)
{
	constantTextureState_ = state;
}

template <class T>
void CUIInteractiveBackground<T>::RemoveConstTextureState()
{
	constantTextureState_ = S_Total;
}
