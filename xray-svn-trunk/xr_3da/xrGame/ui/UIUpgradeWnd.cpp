...
{
	EListType et = eNone;
	if (m_pInv->ModifyFrame() == CurrentFrame())
		et = e1st;
...
void CUIUpgradeWnd::PerformRepair()
{
	...
	funct(item_name, item->GetCondition(), smart_cast<CObject*>(m_pOthersInvOwner)->ObjectName().c_str(), item->object_id());
...
bool CUIUpgradeWnd::CanAddItem(PIItem item)
{
	...
	LPCSTR item_name = item->m_section_id.c_str();
	LPCSTR partner = smart_cast<CObject*>(m_pOthersInvOwner)->ObjectName().c_str();
...
{
	string128 buf;
	xr_sprintf(buf, "%d %s", m_pInvOwner->get_money(), *CStringTable().translate("ui_st_currency"));
...
void CUIUpgradeWnd::SeparateUpgradeItem()
{
	...
	CUIDragDropListEx* list_owner = m_upgrade_selected->CurrentDDList();
...
PIItem CUIUpgradeWnd::get_upgrade_item()
{
