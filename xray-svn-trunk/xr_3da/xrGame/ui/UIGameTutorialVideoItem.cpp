...
void CUISequenceVideoItem::Update()
{
	...
	// deferred start
	if (EngineTimeContinual()>=m_time_start)
	{
	...
	u32 sync_tm				= (0==m_sound._handle())?EngineTimeContinual():(m_sound._feedback()?m_sound._feedback()->play_time():m_sync_time);
	m_sync_time				= sync_tm;
	// processing A&V
...
void CUISequenceVideoItem::Start()
{
	...
	m_sync_time					= 0;
	m_time_start				= EngineTimeContinual()+iFloor(m_delay*1000.f);
...
bool CUISequenceVideoItem::Stop	(bool bForce)
{
	...
	m_wnd->Show					(false);
	if(EngineTimeContinual()>=m_time_start && m_wnd->GetParent()==m_owner->MainWnd())
