...
void CUITalkDialogWnd::AddAnswer(LPCSTR SpeakerName, LPCSTR str, bool bActor)
{
	...
	if (GameConstants::GetUseCharIcons())
	{
		news_data.texture_name = ci.IconName();
		news_data.tex_rect = ci.UIIcon().GetUIStaticItem().GetTextureRect();
		news_data.tex_rect.x2 = 0; news_data.tex_rect.width();
		news_data.tex_rect.y2 = 0; news_data.tex_rect.height();
		news_data.receive_time = GetGameTime();
	}
	else
	{
		TEX_INFO empty_texture = CUITextureMaster::FindItem("ui_iconsTotal_reserved_empty");

		news_data.texture_name = empty_texture.get_file_name();
		news_data.tex_rect = empty_texture.get_rect();
		news_data.tex_rect.x2 = news_data.tex_rect.x2 - news_data.tex_rect.x1;
		news_data.tex_rect.y2 = news_data.tex_rect.y2 - news_data.tex_rect.y1;
		news_data.receive_time = GetGameTime();
	}
