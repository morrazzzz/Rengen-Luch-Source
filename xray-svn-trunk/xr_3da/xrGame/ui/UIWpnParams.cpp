...
void CUIWpnParams::SetInfo(CInventoryItem& cur_wpn)
{
	...
	LPCSTR cur_section = cur_wpn.object().SectionName().c_str();
	...
	float cur_damage = iFloor(g_lua_wpn_params->m_functorDamage(cur_section, str_upgrades, cur_ammo_section)*53.0f) / 53.0f;
...
{
	LPCSTR wpn_section = cur_wpn.object().SectionName().c_str();
