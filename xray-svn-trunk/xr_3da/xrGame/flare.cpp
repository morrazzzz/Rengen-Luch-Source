...
void CFlare::LoadCfg(LPCSTR section)
{
	inherited::LoadCfg		(section);
	m_work_time_sec			= pSettings->r_float(section, "work_time_sec");
}

BOOL CFlare::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
...
void CFlare::SwitchOn()
{
	...
	m_pFlareParticles			= CParticlesObject::Create(pSettings->r_string(SectionName(), "working_particles"), FALSE);
...
void CFlare::SwitchOff()
{
	...
	DestroyParticleInstance		(m_pFlareParticles);
...
void CFlare::UpdateCL()
{
	...
	if( light_render /* && HudItemData()*/ )
	{
		...
		u32 clr					= light_lanim->CalculateBGR(EngineTime(),frame); 
