...
////	Msg("--TRADE::----------------------------------------------");
////	Msg("--TRADE:: - Hello, my name is [%s]", *pThis.base->ObjectName());
...
{
	CEntityAlive* entity = smart_cast<CEntityAlive*>(pThis.base);
...
void CTrade::OnPerformTrade(u32 money_get, u32 money_put)
{
	...
	if (entity)
		entity->OnPerformTrade(money_get, money_put);
}
...
void CTrade::StopTrade()
{
	TradeState = false;
	m_dwLastTradeTime = 0;
//	Msg("--TRADE:: [%s]: Trade stopped...",*pThis.base->ObjectName());
