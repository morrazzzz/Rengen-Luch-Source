...
#ifdef DEBUG
	if(debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject)&&(!!pObject->ObjectName())&&stricmp(debug_output().PH_DBG_ObjectTrackName(),*pObject->ObjectName())==0)
	{
...
/*
	...
	if( mat_injurios!=GAMEMTL_NONE_IDX)
	{
		...
		gcontact_HealthLost+=TimeDelta()*GMLib.GetMaterialByIdx( mat_injurios )->fInjuriousSpeed;
	}
...
void CPHMovementControl::Calculate(const xr_vector<DetailPathManager::STravelPathPoint>& in_path,float speed,  u32& travel_point,  float& precision  )
{
	...
	if(	debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject)&&
		( !!pObject->ObjectName() ) && stricmp(debug_output().PH_DBG_ObjectTrackName(), *pObject->ObjectName() )==0)
	{
...
#ifdef DEBUG
	if(ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject)&&(!!pObject->ObjectName())&&stricmp(PH_DBG_ObjectTrackName(),*pObject->ObjectName())==0)
	{
...
void CPHMovementControl::LoadCfg					(LPCSTR section){
...
float CPHMovementControl::GetCharGravityK()
{
	if (m_character&&m_character->b_exist)
		return m_character->GetCharGravityK();
	else
		return 0.f;
}
...
{
	R_ASSERT3((object->m_pPhysicsShell != NULL), "NO Phisics Shell for object ", *object->ObjectName());
