...
void CHelicopter::init()
{
	...
	m_last_rocket_attack		= EngineTimeU();
...
void CHelicopter::LoadCfg(LPCSTR section)
{
	inherited::LoadCfg					(section);
	m_movement.LoadCfg					(section);
	m_body.LoadCfg						(section);
	m_enemy.LoadCfg						(section);
	...
	HUD_SOUND_ITEM::LoadSound				(section,"snd_shoot", m_sndShot, (u8)sndMaxShotSounds_, SOUND_TYPE_WEAPON_SHOOTING);
	HUD_SOUND_ITEM::LoadSound				(section,"snd_shoot_rocket", m_sndShotRocket, 0, SOUND_TYPE_WEAPON_SHOOTING);
...
BOOL CHelicopter::SpawnAndImportSOData(CSE_Abstract*	data_containing_so)
{
...
//lighting
	...
	m_light_render->set_use_static_occ	(false);
...
void CHelicopter::DestroyClientObj()
{
	...
	m_engineSound.stop					();
	m_brokenSound.stop					();
	DestroyParticleInstance			(m_pParticle);
	m_light_render.destroy				();
	m_movement.DestroyClientObj			();
#ifdef DEBUG
...
void CHelicopter::MoveStep()
{
	...
	if(m_movement.type != eMovNone){
		...
		static float ang = pSettings->r_float	(SectionName(),"magic_angle");
...
#ifdef MEASURE_UPDATES
	Device.Statistic->updateCL_Helicopter_ += measure_updatecl.GetElapsed_sec();
#endif
}

void CHelicopter::ScheduledUpdate(u32 time_delta)
{
#ifdef MEASURE_UPDATES
	...
	inherited::ScheduledUpdate(time_delta);
...
void CHelicopter::RemoveLinksToCLObj(CObject* O )
{
