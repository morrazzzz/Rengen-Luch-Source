...
void CPhraseDialog::load_shared(LPCSTR section)
{
	const ITEM_DATA& item_data = *id_to_index::GetById(m_DialogId);

	R_ASSERT2(&item_data, make_string("Error reading dialog data [%s]. It is missing in xmls. [%s]", m_DialogId.c_str(), section ? section : "no sexion"));
