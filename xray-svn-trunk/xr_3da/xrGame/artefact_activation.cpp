...
SArtefactActivation::SArtefactActivation(CArtefact* af,u32 owner_id)
{
	...
	m_light->set_shadow(gameObjectLightShadows_ == TRUE ? true : false);
	m_light->set_use_static_occ(false);
...
void SArtefactActivation::Load()
{
	...
	LPCSTR activation_seq = pSettings->r_string(*m_af->SectionName(),"artefact_activation_seq");
...
void SArtefactActivation::SpawnAnomaly()
{
	VERIFY(!physics_world()->Processing());
	string128 tmp;
	LPCSTR str			= pSettings->r_string("artefact_spawn_zones",*m_af->SectionName());
...
//. #ifdef DEBUG
		Msg("artefact [%s] spawned a zone [%s] at [%f]", *m_af->ObjectName(), zone_sect, EngineTime());
//. #endif
}
