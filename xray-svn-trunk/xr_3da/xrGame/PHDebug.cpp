...
{
	dbg_ph_draw_mode			=dmSecondaryThread			;
	cash_draw_remove_time	=remove_time+EngineTimeU();
}
...
void DBG_PHAbstructRender()
{
	...
	if(dbg_ph_draw_mode!=dmCashed)
	{
		...
		if(cash_draw_remove_time<EngineTimeU())
		{
...
{
	if( Device.Paused() || CurrentFrame() == previous_frame || !(TimeDelta()>EPS_S) )
		return;
...
static void DBG_DrawTarckObj()
{
	...
	if( !v )
		return;
	DBG_OutText( "visual name: %s ", *trace_object->VisualName() );
