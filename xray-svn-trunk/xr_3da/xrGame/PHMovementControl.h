...
//	dBodyID				GetBody						( )		;
	...
	void				SetCharGravityK(float K) { if (m_character&&m_character->b_exist)m_character->SetCharGravityK(K); }
	float				GetCharGravityK(); // returns 0 if physic character does not exist
