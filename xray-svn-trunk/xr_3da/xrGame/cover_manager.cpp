...
void CCoverManager::compute_static_cover	()
{
	...
	VERIFY2(m_covers->size(), make_string("There are no covers on AI map. Don't expect much from NPCs AI"));

	Msg("* %u AI covers are loaded", m_covers->size());
}
...
public:
	IC 	bool	operator ()		(Cover *lhs, Cover *rhs)
	{
		VERIFY				(lhs);
		VERIFY				(rhs);
		return				(lhs->object().ObjectName()._get() < rhs->object().ObjectName()._get());
	}
	IC 	bool	operator ()		(Cover *cover, shared_str const &id)
	{
		VERIFY				(cover);
		return				(cover->object().ObjectName()._get() < id._get());
	}
};
