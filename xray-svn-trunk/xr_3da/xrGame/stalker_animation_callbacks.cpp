...
//	VERIFY2							( *parameter->m_blend, make_string( "%d %s[%s]", EngineTimeU(), parameter->m_object->ObjectName().c_str(), parameter->m_object->g_Alive() ? "+" : "-") );
...
void CStalkerAnimationManager::assign_bone_callbacks	()
{
	...
	LPCSTR							section = *object().SectionName();
