...
void CDialogHolder::StartMenu(CUIDialogWnd* pDialog, bool bDoHideIndicators)
{
	...
	if(UseIndicators())
	{
		...
		{
			CurrentGameUI()->ShowCrosshair(CB_DIALOG_1, false);
			CurrentGameUI()->ShowGameIndicators(IB_DIALOG_1, false);
		}

		bool b = CurrentGameUI()->HasCrosshairBlock(CB_DIALOG_1);
		m_input_receivers.back().m_flags.set(recvItem::eCrosshair, b);

		b = CurrentGameUI()->HasGameIndicatorsBlock(IB_DIALOG_1);
...
void CDialogHolder::StopMenu(CUIDialogWnd* pDialog)
{
	...
	if( TopInputReceiver()==pDialog )
	{
		if(UseIndicators())
		{
			...
			if (b)
				CurrentGameUI()->ShowCrosshair(CB_DIALOG_1, true);

			b = !!m_input_receivers.back().m_flags.test(recvItem::eIndicators);

			if (b)
				CurrentGameUI()->ShowGameIndicators(IB_DIALOG_1, true);
		}
	...
	if (!TopInputReceiver() || !TopInputReceiver()->NeedCursor())
		GetUICursor().HideCursor();

	if (!TopInputReceiver() || !TopInputReceiver()->NeedDrawCursor())
		GetUICursor().DontDraw();
}
