...
bool doWriteRestrStatistics_ = false; // important, to avoid statistic gathering in game

xr_vector <std::string> not_restrs;
xr_vector <std::string> none_restrs;
xr_vector <std::string> out_restrs;
xr_vector <std::string> in_restrs;
...
#endif
void __stdcall OnChangeProfile(void*userdata)
{
	reinterpret_cast<CSE_ALifeTraderAbstract*>(userdata)->OnChangeProfile(0);
}
void CSE_ALifeTraderAbstract::FillProps	(LPCSTR pref, PropItemVec& items)
{
	PHelper().CreateU32			(items, PrepareKey(pref,*base()->s_name,"Money").c_str(), 	&m_dwMoney,	0, u32(-1));
	PHelper().CreateFlag32		(items,	PrepareKey(pref,*base()->s_name,"Trader\\Infinite ammo").c_str(),&m_trader_flags, eTraderFlagInfiniteAmmo);
#ifdef XRSE_FACTORY_EXPORTS
	RListValue *value		= PHelper().CreateRList	(items,	PrepareKey(pref,*base()->s_name,"npc profile").c_str(),
		&m_sCharacterProfile, 
		&*fp_data.character_profiles.begin(), fp_data.character_profiles.size());
	value->SetCBChangeEvent(this, &::OnChangeProfile);
...
{
#ifdef XRSE_FACTORY_EXPORTS
	PHelper().CreateRToken8		(items,	PrepareKey(pref,*s_name,"Location\\1").c_str(),				&m_tLocations[0],			&*fp_data.locations[0].begin(), fp_data.locations[0].size());
	PHelper().CreateRToken8		(items,	PrepareKey(pref,*s_name,"Location\\2").c_str(),				&m_tLocations[1],			&*fp_data.locations[1].begin(), fp_data.locations[1].size());
	PHelper().CreateRToken8		(items,	PrepareKey(pref,*s_name,"Location\\3").c_str(),				&m_tLocations[2],			&*fp_data.locations[2].begin(), fp_data.locations[2].size());
	PHelper().CreateRToken8		(items,	PrepareKey(pref,*s_name,"Location\\4").c_str(),				&m_tLocations[3],			&*fp_data.locations[3].begin(), fp_data.locations[3].size());
	PHelper().CreateRList	 	(items,	PrepareKey(pref,*s_name,"Connection\\Level name").c_str(),	&m_caConnectionLevelName,	&*fp_data.level_ids.begin(),	fp_data.level_ids.size());
	PHelper().CreateRText	 	(items,	PrepareKey(pref,*s_name,"Connection\\Point name").c_str(),	&m_caConnectionPointName);
#endif
}
...
void CSE_ALifeObject::FillProps				(LPCSTR pref, PropItemVec& items)
{
	...
	PHelper().CreateRText		(items,	PrepareKey(pref,*s_name,"Custom data").c_str(),&m_ini_string);
	if (m_flags.is(flUseSwitches)) {
		PHelper().CreateFlag32	(items,	PrepareKey(pref,*s_name,"ALife\\Can switch online").c_str(),	&m_flags,			flSwitchOnline);
		PHelper().CreateFlag32	(items,	PrepareKey(pref,*s_name,"ALife\\Can switch offline").c_str(),	&m_flags,			flSwitchOffline);
	}                            
	PHelper().CreateFlag32		(items,	PrepareKey(pref,*s_name,"ALife\\Interactive").c_str(),			&m_flags,			flInteractive);
	PHelper().CreateFlag32		(items,	PrepareKey(pref,*s_name,"ALife\\Used AI locations").c_str(),	&m_flags,			flUsedAI_Locations);
#ifdef XRSE_FACTORY_EXPORTS
	PHelper().CreateRToken32	(items,	PrepareKey(pref,*s_name,"ALife\\Story ID").c_str(),				&m_story_id,		&*fp_data.story_names.begin(), fp_data.story_names.size());
	PHelper().CreateRToken32	(items,	PrepareKey(pref,*s_name,"ALife\\Spawn Story ID").c_str(),		&m_spawn_story_id,	&*fp_data.spawn_story_names.begin(), fp_data.spawn_story_names.size());
#endif
}
...
{
	PHelper().CreateU16			(items,	PrepareKey(pref, "ALife\\Count").c_str(),			&m_wCount,			0,0xff);
};	
...
void CSE_ALifeSpaceRestrictor::STATE_Write	(NET_Packet	&tNetPacket)
{
	...
	// For SDK building spawn statistics... as team asked
	if (doWriteRestrStatistics_)
	{
		std::string temp;

		if (m_space_restrictor_type == RestrictionSpace::eRestrictorTypeNone)
		{
			temp = make_string("Restrictor %s. Type: NOT A restrictor", name_replace() ? name_replace() : "");
			not_restrs.push_back(temp);
		}
		else if (m_space_restrictor_type == RestrictionSpace::eDefaultRestrictorTypeNone)
		{
			temp = make_string("Restrictor %s. Type: NONE restrictor", name_replace() ? name_replace() : "");
			none_restrs.push_back(temp);
		}
		else if (m_space_restrictor_type == RestrictionSpace::eDefaultRestrictorTypeOut)
		{
			temp = make_string("Restrictor %s. Type: OUT restrictor", name_replace() ? name_replace() : "");
			out_restrs.push_back(temp);
		}
		else if (m_space_restrictor_type == RestrictionSpace::eDefaultRestrictorTypeIn)
		{
			temp = make_string("Restrictor %s. Type: IN restrictor", name_replace() ? name_replace() : "");
			in_restrs.push_back(temp);
		}
	}
}
...
{
	inherited1::FillProps		(pref,items);
	PHelper().CreateToken8		(items, PrepareKey(pref,*s_name,"restrictor type").c_str(),		&m_space_restrictor_type,	defaul_retrictor_types);
	PHelper().CreateFlag32		(items,	PrepareKey(pref,*s_name,"check for separator").c_str(),	&m_flags,					flCheckForSeparator);
}
...
#ifdef XRSE_FACTORY_EXPORTS
	PHelper().CreateRList		(items,PrepareKey(pref,*s_name,"Level to change").c_str(),		&m_caLevelToChange,		&*fp_data.level_ids.begin(), fp_data.level_ids.size());
#endif
	PHelper().CreateRText		(items,PrepareKey(pref,*s_name,"Level point to change").c_str(),	&m_caLevelPointToChange);

	PHelper().CreateBOOL		(items,PrepareKey(pref,*s_name,"Silent mode").c_str(),	&m_bSilentMode);
}
...
void CSE_ALifeObjectPhysic::FillProps		(LPCSTR pref, PropItemVec& values) 
{
	...
	PHelper().CreateToken32		(values, PrepareKey(pref,*s_name,"Type").c_str(), &type,	po_types);
	PHelper().CreateFloat		(values, PrepareKey(pref,*s_name,"Mass").c_str(), &mass, 0.1f, 10000.f);
    PHelper().CreateFlag8		(values, PrepareKey(pref,*s_name,"Active").c_str(), &_flags, flActive);

    // motions & bones
	PHelper().CreateChoose		(values, 	PrepareKey(pref,*s_name,"Model\\Fixed bones").c_str(),	&fixed_bones,		smSkeletonBones,0,(void*)visual()->get_visual(),8);
}
...
void CSE_ALifeObjectHangingLamp::UPDATE_Read(NET_Packet	&tNetPacket)
{
	...
	tNetPacket.r_u32(nextRestoreHour_);
	tNetPacket.r_float(m_health);
}
...
void CSE_ALifeObjectHangingLamp::UPDATE_Write(NET_Packet	&tNetPacket)
{
	...
	tNetPacket.w_u32(nextRestoreHour_);
	tNetPacket.w_float(m_health);
}
...
void __stdcall OnChangeFlag(void*userdata)
{
	reinterpret_cast<CSE_ALifeObjectHangingLamp*>(userdata)->OnChangeFlag(0);
}
void CSE_ALifeObjectHangingLamp::FillProps	(LPCSTR pref, PropItemVec& values)
{
	...
	inherited2::FillProps		(pref,values);
    PropValue* P				= 0;
	PHelper().CreateFlag16		(values, PrepareKey(pref,*s_name,"Flags\\Physic").c_str(),		&flags,			flPhysic);
	PHelper().CreateFlag16		(values, PrepareKey(pref,*s_name,"Flags\\Cast Shadow").c_str(),	&flags,			flCastShadow);
	PHelper().CreateFlag16		(values, PrepareKey(pref,*s_name,"Flags\\Allow R1").c_str(),	&flags,			flR1);
	PHelper().CreateFlag16		(values, PrepareKey(pref,*s_name,"Flags\\Allow R2").c_str(),	&flags,			flR2);
	P=PHelper().CreateFlag16	(values, PrepareKey(pref,*s_name,"Flags\\Allow Ambient").c_str(),&flags,			flPointAmbient);
	P->SetCBChangeEvent(this, ::OnChangeFlag);
   //P->OnChangeEvent.bind		(this,&CSE_ALifeObjectHangingLamp::OnChangeFlag);
	// 
	P=PHelper().CreateFlag16	(values, PrepareKey(pref,*s_name,"Light\\Type").c_str(), 		&flags,				flTypeSpot, "Point", "Spot");
	P->SetCBChangeEvent(this, ::OnChangeFlag);
	//P->OnChangeEvent.bind		(this,&CSE_ALifeObjectHangingLamp::OnChangeFlag);
	PHelper().CreateColor		(values, PrepareKey(pref,*s_name,"Light\\Main\\Color").c_str(),			&color);
    PHelper().CreateFloat		(values, PrepareKey(pref,*s_name,"Light\\Main\\Brightness").c_str(),	&brightness,		0.1f, 5.f);
	PHelper().CreateChoose		(values, PrepareKey(pref,*s_name,"Light\\Main\\Color Animator").c_str(),&color_animator, 	smLAnim);
	PHelper().CreateFloat		(values, PrepareKey(pref,*s_name,"Light\\Main\\Range").c_str(),			&range,				0.1f, 1000.f);
	PHelper().CreateFloat		(values, PrepareKey(pref,*s_name,"Light\\Main\\Virtual Size").c_str(),	&m_virtual_size,	0.f, 100.f);
	PHelper().CreateChoose		(values, PrepareKey(pref,*s_name,"Light\\Main\\Texture").c_str(),	    &light_texture, 	smTexture, "lights");
	PHelper().CreateChoose		(values, PrepareKey(pref,*s_name,"Light\\Main\\Bone").c_str(),			&light_main_bone,	smSkeletonBones,0,(void*)visual()->get_visual());
	if (flags.is(flTypeSpot))
	{
		PHelper().CreateAngle(values, PrepareKey(pref, *s_name, "Light\\Main\\Cone Angle").c_str(), &spot_cone_angle, deg2rad(1.f), deg2rad(120.f));

		P=PHelper().CreateFlag16	(values, PrepareKey(pref,*s_name,"Flags\\Volumetric").c_str(),	&flags,			flVolumetric);
		P->OnChangeEvent.bind	(this,&CSE_ALifeObjectHangingLamp::OnChangeFlag);
	}

	if (flags.is(flPointAmbient)){
		PHelper().CreateFloat	(values, PrepareKey(pref,*s_name,"Light\\Ambient\\Radius").c_str(),		&m_ambient_radius,	0.f, 1000.f);
		PHelper().CreateFloat	(values, PrepareKey(pref,*s_name,"Light\\Ambient\\Power").c_str(),		&m_ambient_power);
		PHelper().CreateChoose	(values, PrepareKey(pref,*s_name,"Light\\Ambient\\Texture").c_str(),	&m_ambient_texture,	smTexture, 	"lights");
		PHelper().CreateChoose	(values, PrepareKey(pref,*s_name,"Light\\Ambient\\Bone").c_str(),		&light_ambient_bone,smSkeletonBones,0,(void*)visual()->get_visual());
		PHelper().CreateFlag16	(values, PrepareKey(pref,*s_name,"Light\\Ambient\\Abient Shadow").c_str(),	&flags,	flCastAmbShadow);
	}

	// fixed bones
    PHelper().CreateChoose		(values, PrepareKey(pref,*s_name,"Model\\Fixed bones").c_str(),	&fixed_bones,		smSkeletonBones,0,(void*)visual()->get_visual(),8);
    // glow
	PHelper().CreateFloat		(values, PrepareKey(pref,*s_name,"Glow\\Radius").c_str(),	    &glow_radius,		0.01f, 100.f);
	PHelper().CreateChoose		(values, PrepareKey(pref,*s_name,"Glow\\Texture").c_str(),	    &glow_texture, 		smTexture,	"glow");
	// game
	PHelper().CreateFloat		(values, PrepareKey(pref,*s_name,"Game\\Health").c_str(),		&m_health,			0.f, 100.f);
}
...
bool CSE_ALifeSchedulable::NeedServerUpdates(CSE_ALifeDynamicObject *object)
{
...
void CSE_ALifeHelicopter::FillProps(LPCSTR pref, PropItemVec& values)
{
    ...
    PHelper().CreateChoose		(values,	PrepareKey(pref,*s_name,"Engine Sound").c_str(), &engine_sound, smSoundSource);
}
...
void CSE_ALifeCar::FillProps				(LPCSTR pref, PropItemVec& values)
{
  	inherited1::FillProps			(pref,values);
	inherited2::FillProps			(pref,values);
	PHelper().CreateBOOL			(values, PrepareKey(pref,*s_name,"Random health").c_str(),		&random_health);
	if (random_health)
	{
		health				= ::Random.randF(.4f,.7f);
		PHelper().CreateCaption		(values, PrepareKey(pref,*s_name,"Health").c_str(),			shared_str().printf("%.2f",health));
	} else
		PHelper().CreateFloat		(values, PrepareKey(pref,*s_name,"Health").c_str(),			&health,			0.f, 1.0f);

	PHelper().CreateBOOL			(values, PrepareKey(pref,*s_name,"Random fuel").c_str(),		&random_fuel);
	if (random_fuel)
	{
		fuel				= ::Random.randF(fuel_tank/100.f*30.f,fuel_tank/100.f*60.f);
		PHelper().CreateCaption		(values, PrepareKey(pref,*s_name,"Fuel").c_str(),			shared_str().printf("%.2f",fuel));
	} else
		PHelper().CreateFloat		(values, PrepareKey(pref,*s_name,"Fuel").c_str(),			&fuel);
}
...
{
  	inherited::FillProps			(pref,values);
	PHelper().CreateFloat		(values, PrepareKey(pref,*s_name,"Health").c_str(),			&m_health,			0.f, 100.f);
}
