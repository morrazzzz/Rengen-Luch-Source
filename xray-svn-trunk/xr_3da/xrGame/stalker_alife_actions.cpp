...
void CStalkerActionNoALife::initialize()
{
	...
	m_stop_weapon_handling_time					= EngineTimeU();
...
void CStalkerActionNoALife::execute()
{
	...
	if (EngineTimeU() >= m_stop_weapon_handling_time)
...
void CStalkerActionGatherItems::initialize()
{
	...
	CObject const* const selected							= object().memory().ItemManager().selected();
...
void CStalkerActionGatherItems::execute()
{
	...
	if (!object().memory().ItemManager().selected())
		return;

	u32 level_vertex_id = object().memory().ItemManager().selected()->ai_location().level_vertex_id();

//	if (object().movement().restrictions().accessible(level_vertex_id)) {
		object().movement().set_level_dest_vertex	(level_vertex_id);
		object().movement().set_desired_position(&object().memory().ItemManager().selected()->Position());
//	}
//	else {
//		object().movement().set_nearest_accessible_position	(
...
//	}
	...
	object().sight().setup(SightManager::eSightTypePosition, &object().memory().ItemManager().selected()->Position());
}
