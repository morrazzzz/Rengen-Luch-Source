...
DEFINE_VECTOR(shared_str, MAP_LOCATION_ID_VECTOR, MAP_LOCATION_ID_IT);
...
struct SInfoPortionData : CSharedResource
{
	...
	//список особых мест на карте
	MAP_LOCATION_ID_VECTOR	mapLocations_;

	//скриптовые действия, которые активируется после того как 
	//информацию получает персонаж
	CDialogScriptHelper		m_PhraseScript;
...
//	const LOCATIONS_VECTOR&							MapLocations()	const {return info_data()->m_MapLocations;}
	...
	const MAP_LOCATION_ID_VECTOR&					MapLocations()	const { return info_data()->mapLocations_; }
...
protected:
	...
	void			load_shared						(LPCSTR section);
...
public:
	// infoportion is described in xml file and has special properties and actions
	static bool	IsComplexInfoPortion				(LPCSTR info_id);
};