...
void moving_objects::fill_nearest_moving		(moving_object *object)
{
...
#if 0
	Msg							("%6d nearest moving[%d] object[%s]", CurrentFrame(), m_nearest_moving.size(),object->object().ObjectName().c_str());
	{
	...
	struct already_computed {
		...
		{
			return				(object->action_frame() == CurrentFrame());
		}
	};
...
void moving_objects::query_action_dynamic		(moving_object *object)
{
	...
	if (object->action_frame() == CurrentFrame())
		return;
...
#if 0//def DEBUG
	Msg							("%6d end of iteration", CurrentFrame());
#endif // DEBUG
...
#if 0//def DEBUG
	{
		Msg							("Frame[%d], collisions[%d]",CurrentFrame(), m_visited_emitters.size());
		NEAREST_MOVING::iterator	I = m_visited_emitters.begin();
		NEAREST_MOVING::iterator	E = m_visited_emitters.end();
		for ( ; I != E; ++I)
			Msg						("  %s",*(*I)->object().ObjectName());
	}
#endif // DEBUG
