...
{
	if (!m_object->memory().DangerManager().selected())
		return false;
...
_value_type CStalkerPropertyEvaluatorDangerUnknown::evaluate()
{
	...
	switch (m_object->memory().DangerManager().selected()->type())
	{
...
_value_type CStalkerPropertyEvaluatorDangerWithGrenade::evaluate()
{
	...
	return (CDangerObject::eDangerTypeGrenade == m_object->memory().DangerManager().selected()->type());
}
...
_value_type CStalkerPropertyEvaluatorDangerGrenadeExploded::evaluate()
{
	...
	if (CDangerObject::eDangerTypeGrenade != m_object->memory().DangerManager().selected()->type())
		return false;

	return !m_object->memory().DangerManager().selected()->dependent_object();
}
