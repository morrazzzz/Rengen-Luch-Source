...
#define	BEST_DANGER_THRESHOLD 2000.f
#define	BEST_DANGER_DIST_CAP 50.f
#define	BEST_DANGER_TIME_CAP 30*1000
...
void CDangerManager::LoadCfg(LPCSTR section)
{
}
...
void CDangerManager::update()
{
	...
	float					increasing = BEST_DANGER_THRESHOLD;
	m_selected				= 0;
...
float CDangerManager::do_evaluate(const CDangerObject &object) const
{
	...
	if (!object.object() || !object.object()->g_Alive())
		return 0.f;
	...
	float dist = m_object->Position().distance_to(object.object()->Position());
	float elapsed_time = float(EngineTimeU() - object.time());

	clamp(elapsed_time, 0.f, 3600000.f);

	result *= BEST_DANGER_DIST_CAP / dist;
	result *= BEST_DANGER_TIME_CAP / elapsed_time;
...
void CDangerManager::add(const CSoundObject &object)
{
	...
	{
		add(CDangerObject(obj, object.m_object_params.GetMemorizedPos(), object.m_level_time, CDangerObject::eDangerTypeBulletRicochet, CDangerObject::eDangerPerceiveTypeSound));
	...
	{
		add(CDangerObject(obj, object.m_object_params.GetMemorizedPos(), object.m_level_time, CDangerObject::eDangerTypeAttackSound, CDangerObject::eDangerPerceiveTypeSound));
	...
	if ((object.m_sound_type & SOUND_TYPE_INJURING) == SOUND_TYPE_INJURING)
	{
		...
		if (do_add)
			add(CDangerObject(obj, object.m_object_params.GetMemorizedPos(), object.m_level_time, CDangerObject::eDangerTypeEntityAttacked, CDangerObject::eDangerPerceiveTypeSound));
	...
	{
		add(CDangerObject(obj, object.m_object_params.GetMemorizedPos(), object.m_level_time, CDangerObject::eDangerTypeEntityDeath, CDangerObject::eDangerPerceiveTypeSound));
	...
	{
		add(CDangerObject(obj, object.m_object_params.GetMemorizedPos(), object.m_level_time, CDangerObject::eDangerTypeEnemySound, CDangerObject::eDangerPerceiveTypeSound));
...
{
	if (m_object->memory().EnemyManager().selected() && object.object())// && !object.object()->g_Alive())
