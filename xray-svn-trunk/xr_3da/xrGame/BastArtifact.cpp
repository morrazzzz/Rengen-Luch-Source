...
BOOL CBastArtefact::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
...
void CBastArtefact::DestroyClientObj()
{
...
void CBastArtefact::LoadCfg(LPCSTR section) 
{
...
void CBastArtefact::ScheduledUpdate(u32 dt) 
{
...
{
	//Log						("--- A - CBastArtefact",*ObjectName());
	//Log						("--- A - CBastArtefact",renderable.xform);

	//современем энергия по немногу тоже уменьшается
	if(m_fEnergy>0) m_fEnergy -= m_fEnergyDecreasePerTime*TimeDelta();
