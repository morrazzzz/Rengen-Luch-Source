...
{
	m_block_sprint_counter = m_block_sprint_counter + bDisable ? 1 : -1;
...
void CActor::SetWeaponHideState(u32 State, bool bSet, bool need_unholster)
{
	if (g_Alive() && this == Level().CurrentControlEntity())
	{
		inventory().SetSlotsBlocked((u16)State, bSet, need_unholster);
	};
}
...
void	CActor::RemoveAmmoForWeapon	(CInventoryItem *pIItem)
{
	...
	CWeaponAmmo* pAmmo = smart_cast<CWeaponAmmo*>(inventory().GetAmmoFromInv(*(pWM->m_ammoTypes[0])));
	if (!pAmmo) return;
	pAmmo->DestroyObject();
};
