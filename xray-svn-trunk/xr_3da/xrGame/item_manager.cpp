...
void CItemManager::update()
{
	...
	for ( ; I != E; ++I)
		VERIFY3(m_object->movement().restrictions().accessible((*I)->ai_location().level_vertex_id()), *m_object->ObjectName(), *(*I)->ObjectName());
	...
	VERIFY3(
		...
		*m_object->ObjectName(),
		selected() ? *selected()->ObjectName() : "<no selected item>"
	);
...
void CItemManager::remove_links		(CObject *object)
{
	...
	OBJECTS::iterator		I = std::find(objectsArray_.begin(), objectsArray_.end(), (CGameObject*)object);
