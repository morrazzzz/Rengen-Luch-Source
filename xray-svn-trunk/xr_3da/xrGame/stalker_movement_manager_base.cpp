	...
	m_force_update				(false)
{
	...
	stopBeforeDestination_		= false;
	stopBeforeDestinationDistance_ = -1.f;
}
...
//	Msg						("[%6d] m_failed_to_build_path = %s (stalker_movement_manager_base::initialize)",EngineTimeU(),m_failed_to_build_path ? "true" : "false");
}

void stalker_movement_manager_base::set_desired_position(const Fvector *position)
{
	VERIFY2						(!position || accessible(*position) || show_restrictions(&restrictions()),*object().ObjectName());
...
//			Msg						("%d TRUE",EngineTimeU());
		...
		case eVelocityFree : {
			...
			if (m_object->brain().current_action_id() == StalkerDecisionSpace::eWorldOperatorCombatPlanner) {
				...
				if (planner.current_action_id() != StalkerDecisionSpace::eWorldOperatorKillWoundedEnemy)
					Msg					("~ stalker %s is doing bad thing (action %s)",*m_object->ObjectName(),planner.current_action().m_action_name);
			}
#endif // DEBUG
			movement_params.m_mental_state	= eMentalStateFree;
			break;
	...
	VERIFY2	((movement_params.m_mental_state != eMentalStateFree) || movement_params.m_body_state != eBodyStateCrouch,*object().ObjectName());
...
void stalker_movement_manager_base::set_nearest_accessible_position(Fvector desired_position, u32 level_vertex_id, float satisfying_distance)
{
	...
	VERIFY2						(restrictions().accessible(level_vertex_id) || show_restrictions(&restrictions()),*object().ObjectName());

	if (!ai().level_graph().inside(level_vertex_id, desired_position))
		return;

	set_level_dest_vertex		(level_vertex_id, satisfying_distance);
	
	VERIFY2						(restrictions().accessible(desired_position) || show_restrictions(&restrictions()),*object().ObjectName());
...
void stalker_movement_manager_base::update_path()
{
	inherited::update_path();

	if (stopBeforeDestination_ && is_path_varify_state() && !distance_to_destination_greater(stopBeforeDestinationDistance_))
	{
		stopBeforeDestination_ = false;
		set_movement_type(eMovementTypeStand);
	}
...
void stalker_movement_manager_base::set_level_dest_vertex	(u32 const& level_vertex_id, float satisfying_distance)
{
	inherited::set_level_dest_vertex	(level_vertex_id, satisfying_distance);
	m_target.cover_id					("");
}
