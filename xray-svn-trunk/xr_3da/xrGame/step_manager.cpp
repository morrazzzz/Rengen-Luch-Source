...
void CStepManager::reload(LPCSTR section)
{
	...
	{
#ifdef	DEBUG
		Msg( "! no step_params section for :%s section :s", m_object->ObjectName().c_str(), section );
#endif
		return;
	}
	...
	VERIFY3(skeleton_animated, "object is not animated", m_object->VisualName().c_str());
#ifdef	DEBUG
		if( debug_step_info_load )
			Msg( "loading step_params for object :%s, visual: %s, section: %s, step_params section: %s  ", m_object->ObjectName().c_str(), m_object->VisualName().c_str(), section, anim_section );
#endif
	...
	for (u32 i=0; pSettings->r_line(anim_section,i,&anim_name,&val); ++i) {
		...
		R_ASSERT2(anim_name, make_string("Nulled anim_name name sect [%s], line [%u]", anim_section, i));
		...
		if (!motion_id) 
		{
			...
			Msg( "! (CStepManager::reload) no anim :%s object:%s, visual: %s, step_params section: %s ", anim_name, m_object->ObjectName().c_str(), m_object->VisualName().c_str(), anim_section );
		...
		if( debug_step_info_load )
		{
			...
			Msg( "step_params loaded for object :%s, visual: %s, motion: %s, anim set: %s  ", m_object->ObjectName().c_str(), m_object->VisualName().c_str(), anim_name.first, anim_name.second );
		}
#endif
	...
	if( m_steps_map.empty() )
		Msg( "! no steps info loaded for :%s, section :s, step_params section: %s ", m_object->ObjectName().c_str(), section, anim_section );
#endif
	// reload foot bones
...
void CStepManager::on_animation_start(MotionID motion_id, CBlend *blend)
{
	...
	m_time_anim_started = EngineTimeU(); 
	...
	if (it == m_steps_map.end()) {
		...
		if( debug_step_info )
		{
			...
			VERIFY( KA );
			std::pair<LPCSTR,LPCSTR> anim_name = KA->LL_MotionDefName_dbg( motion_id );
			Msg( "! no step_params found for object :%s, visual: %s, motion: %s, anim set: %s  ", m_object->ObjectName().c_str(), m_object->VisualName().c_str(), anim_name.first, anim_name.second );
		}
#endif
	...
	for (u32 i=0; i<m_legs_count; i++) 
	{
		...
		if (offset_time <= cur_time)
		{
			...
			if(b_play && !mtl_pair->CollideParticles.empty())	
			{
				...
				g_pGameLevel->Particles.particlesWaitingToPlay.push_back(ps);
			}
...
void CStepManager::reload_foot_bones()
{
	...
	if(ini&&ini->section_exist("foot_bones")){
		...
		if (!pSettings->line_exist(*m_object->SectionName(),"foot_bones"))
			R_ASSERT2(false,"section [foot_bones] not found in monster user_data");
		load_foot_bones(pSettings->r_section(pSettings->r_string(*m_object->SectionName(),"foot_bones")));
	}
