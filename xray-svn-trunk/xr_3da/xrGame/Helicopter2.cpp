...
//lighting
	if(m_light_render->get_active()){
		...
		{
			int frame;
			u32 clr = m_lanim->CalculateBGR(EngineTime(), frame); // тючтЁр•рхЄ т ЇюЁьрЄх BGR
			Fcolor					fclr;
...
void CHelicopter::SetEnemy(CScriptGameObject* e)
{
	...
	R_ASSERT2(e->ID() <= MAX_OBJ_IDS, "Possible bug here, contact coders");
	m_enemy.destEnemyID = (u16)e->ID();
}
...
//	inherited::Hit(pHDS);
	...
	if(It != m_hitBones.end() && pHDS->hit_type==ALife::eHitTypeFireWound) {
		...
		// cop merge hit_power = AffectHit(hit_power, pHDS->hit_type);
...
void CHelicopter::DieHelicopter()
{
	...
	m_brokenSound.create			(pSettings->r_string(*SectionName(), "broken_snd"),st_Effect,sg_SourceType);
	...
	if(EngineTimeU() != PositionStack.front().dwTime)
		lin_vel.div((EngineTimeU()-PositionStack.front().dwTime)/1000.0f);
...
void SHeliEnemy::LoadCfg(LPCSTR section)
{
...
void CHelicopter::UseFireTrail(bool val)
{
	...
	if(val){
		fireDispersionBase	= pSettings->r_float		(*SectionName(),"fire_dispersion_null");
		fireDispersionBase	= deg2rad					(fireDispersionBase);
	}else{
		fireDispersionBase	= pSettings->r_float		(*SectionName(),"fire_dispersion_base");
		fireDispersionBase	= deg2rad					(fireDispersionBase);
	}
}


void SHeliBodyState::LoadCfg(LPCSTR section)
{
