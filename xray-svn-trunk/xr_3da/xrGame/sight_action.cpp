...
void CSightAction::initialize					()
{
	...
	m_start_time					= EngineTimeU();
	m_state_fire_switch_time		= EngineTimeU();
...
void CSightAction::execute						()
{
	...
	switch (m_sight_type) {
		...
		default:
			Msg("m_sight_type = %u", m_sight_type);
			NODEFAULT;
	}
}
...
#ifdef SIGHT_TEST
	Msg					("%6d eSightTypeCurrentDirection",EngineTimeU());
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d eSightTypePathDirection",EngineTimeU());
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d eSightTypeDirection",EngineTimeU());
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d %s",EngineTimeU(),m_torso_look ? "eSightTypeFirePosition" : "eSightTypePosition");
#endif
}
...
//	Msg						("execute_object(%f)(%s)my_position[%f][%f][%f],object_position[%f][%f][%f]",object().movement().m_head.target.yaw,*m_object_to_look->ObjectName(),VPUSH(m_object->eye_matrix.c),VPUSH(m_object_to_look->Position()));

	if (m_no_pitch)
		object().movement().m_head.target.pitch	= 0.f;

#ifdef SIGHT_TEST
	Msg					("%6d %s",EngineTimeU(),m_torso_look ? "eSightTypeFireObject" : "eSightTypeObject");
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d %s [%f] -> [%f]",EngineTimeU(),m_torso_look ? "eSightTypeFireCover" : "eSightTypeCover",object().movement().m_body.current.yaw,object().movement().m_body.target.yaw);
#endif
}
...
#ifdef SIGHT_TEST
	Msg					("%6d %s",EngineTimeU(),m_torso_look ? "eSightTypeFireSearch" : "eSightTypeSearch");
#endif
}
...
{
	m_internal_state	= 2;
	m_start_state_time	= EngineTimeU();
...
fall_back:
#endif // #ifndef DEBUG
			if ((m_start_state_time + m_stop_state_time < EngineTimeU()) && target_reached()) {
				m_start_state_time	= EngineTimeU();
				m_stop_state_time	= 3500;
				m_internal_state	= 1;
...
void CSightAction::predict_object_position		( bool use_exact_position )
{
	...
	if ( count > 1 ) {
		...
		VERIFY					( EngineTimeU() >= current_position.dwTime );
		...
		if ( EngineTimeU() - previous_position.dwTime < 300 ) {
			if ( current_position.dwTime > previous_position.dwTime ) {
				...
				float const predict_time	= g_aim_predict_time;//*TimeDelta();
...
//			Msg							("%6d switch to mode 1", EngineTimeU());
...
//			m_vector3d					= m_object->sight().object_position();
		...
		case 1 : {
			...
			if ( EngineTimeU() >= m_state_fire_switch_time + 1500 ) {
				if (m_object_to_look->Position().distance_to_sqr(m_object->Position()) > _sqr(5.f)) {
					if (!m_holder_start_position.similar(m_object->Position(),.05f)) {
						m_vector3d			= m_object->sight().object_position();
						m_already_switched	= false;
//						Msg					("%6d switch to mode 0 (reson: holder position changed)", EngineTimeU());
						m_state_fire_object	= 0;
						break;
					}
					...
					if (!m_object_start_position.similar(m_object_to_look->Position(),.05f)) {
						m_vector3d			= m_object->sight().object_position();
//						Msg					("%6d switch to mode 0 (reson: object position changed)", EngineTimeU());
						m_already_switched	= false;
						m_state_fire_object	= 0;
						break;
					}
				...
				if ( !m_already_switched) {
					m_vector3d				= m_object->sight().object_position();
//					Msg						("%6d switch to mode 0 (reson: time interval)", EngineTimeU());
