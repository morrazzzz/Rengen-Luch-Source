...
{
	R_ASSERT(&item);
	R_ASSERT(upgrade_id.size());

	Upgrade* upgrade = upgrade_verify( item.m_section_id, upgrade_id );

	R_ASSERT2(upgrade, make_string("Can't load upgrade from data. Probably need to check this in level spawn. item %s, upgrade %s", item.m_section_id.c_str(), upgrade_id.c_str()));
