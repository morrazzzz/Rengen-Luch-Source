...
#ifdef DEBUG
	global().set_dbg_info		(*object().ObjectName(),"Global");
	head().set_dbg_info			(*object().ObjectName(),"Head  ");
	torso().set_dbg_info		(*object().ObjectName(),"Torso ");
	legs().set_dbg_info			(*object().ObjectName(),"Legs  ");
	script().set_dbg_info		(*object().ObjectName(),"Script");
#endif
};
...
void CStalkerAnimationManager::play_fx(float power_factor, int fx_index)
{
	...
	if (psAI_Flags.is(aiAnimation)) {
		...
		Msg						("%6d [%s][%s][%s][%f]",EngineTimeU(),*object().ObjectName(),"FX",name,power_factor);
	}
#endif
