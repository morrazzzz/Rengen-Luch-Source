...
_action  actions[]		= {
	...
	{ "alive_detector",		kALIVE_DETECTOR			,_both },
	...
	{ "weapon_select",		kWEAPONSELECT			,_both},
...
LPCSTR id_to_action_name(EGameActions _id)
{
	...
	Msg("! Check Key Bindings! Cant find corresponding [action_name] for id");
...
_action* action_name_to_ptr(LPCSTR _name)
{
	...
	Msg("! Check Key Bindings! Cant find corresponding [id] for action_name [%s]", _name);
...
_keyboard* dik_to_ptr(int _dik, bool bSafe)
{
	...
	if (!bSafe)
		Msg("! Check Key Bindings! Cant find corresponding [_keyboard] for dik");
...
_keyboard* keyname_to_ptr(LPCSTR _name)
{
	...
	Msg("! Check Key Bindings! Cant find corresponding [_keyboard*] for keyname [%s]", _name);
...
EGameActions get_binded_action(int _dik)
{
	for(int idx=0; idx<bindings_count; ++idx)
	{
		...
		bool b_is_group_matching	= is_group_matching(binding->m_action->key_group, _both);
