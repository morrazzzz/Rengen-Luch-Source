...
MotionID animation_selector::select_animation		(bool &animation_movement_controller)
{
	...
	if (m_callback_called) {
		if (m_planner->initialized()) {
			...
			m_previous_time		= 0.f;
			if (!m_planner->initialized()) {
//				Msg				("%6d no planner update, planner is not initialized, exiting", EngineTimeU());
				return			(m_object->animation().assign_global_animation(animation_movement_controller));
			}
		}

//		Msg					("%6d updating planner", EngineTimeU());
		m_planner->update	();

		if (!m_planner->initialized()) {
//			Msg				("%6d planner is not initialized after update, exiting", EngineTimeU());
			return			(m_object->animation().assign_global_animation(animation_movement_controller));
		}
