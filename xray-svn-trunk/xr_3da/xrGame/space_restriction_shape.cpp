...
void CSpaceRestrictionShape::build_border	()
{
	...
	VERIFY3							(!border().empty(),"space restrictor has no border",*m_restrictor->ObjectName());
...
void CSpaceRestrictionShape::test_correctness	()
{
	...
	//if (!m_correct && (xr_strcmp(*m_restrictor->ObjectName(), "agr_factory_hold_restrictor") == 0)) {
	//	bool flood_less = m_test_storage.size() > nodes.size();

	//	Msg("NOT Correct restrictor: [%s], flood less = [%u] Dump unique nodes: ", *m_restrictor->ObjectName(), flood_less);
...
{
	VERIFY							(m_restrictor);
	return							(m_restrictor->ObjectName());
}
