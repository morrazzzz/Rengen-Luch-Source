...
void CEncyclopediaArticle::load_shared(LPCSTR section)
{
	const ITEM_DATA& item_data = *id_to_index::GetById(m_ArticleId);

	R_ASSERT2(&item_data, make_string("Error reading encyclopedia article data [%s]. It is missing in xmls. [%s]", m_ArticleId.c_str(), section ? section : "no sexion"));
