...
CALifeSimulator* alifeSimulator_ = nullptr;
	...
	CALifeSimulatorBase			(server,alife_section)
{
	...
	*command_line				= temp;
	...
	Msg("* calling _g start_game_callback");
...
#define LUA_SUPPLY_SPAWN_CB "bind_trader.supply_spawned_callback"

// Callback
void TraderSupplySpawnScriptCallback(u32 parrent_id, u32 spawned_item_id) // puted this function here, just because of pch_script
{
	luabind::functor<void> lua_function;

	R_ASSERT2(ai().script_engine().functor<void>(LUA_SUPPLY_SPAWN_CB, lua_function), make_string("Can't find function %s", LUA_SUPPLY_SPAWN_CB));

	lua_function(parrent_id, spawned_item_id);
}