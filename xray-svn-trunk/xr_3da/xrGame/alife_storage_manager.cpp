...
void CALifeStorageManager::save	(LPCSTR save_name_no_check, bool update_name)
{
	...
	void						*dest_data;
	{
		...
		g_pGamePersistent->Environment().SaveWeather(stream);

		SaveAddionalStuff(stream);
...
void CALifeStorageManager::storage_load(void *buffer, const u32 &buffer_size, LPCSTR file_name)
{
	...
	g_pGamePersistent->Environment().LoadWeather(source);
...
bool CALifeStorageManager::storage_load(LPCSTR save_name_no_check)
{
	LPCSTR game_saves_path		= FS.get_path("$game_saves$")->m_Path;
	...
	g_pGamePersistent->LoadTitle(temp, temp);
	...
	FS.r_close					(stream);
	storage_load				(source_data, source_count, file_name);
	xr_free						(source_data);
...
void CALifeStorageManager::SaveAddionalStuff(IWriter& memory_stream)
{
	// For slonik launcher
	Msg("# Saving additional stuff");

	memory_stream.open_chunk(SHORT_GAME_INFO);

	u32 ms, seconds, mins, hours, day, month, year;
	GetGameDateAndTime(year, month, day, hours, mins, seconds, ms);

	GameGraph::_LEVEL_ID lvl_id = graph().level().level_id();
	LPCSTR lvl_name = *ai().game_graph().header().level(lvl_id).name();

	memory_stream.w_float(g_actor ? Actor()->GetfHealth() : -1.f);

	memory_stream.w_u32(ms);
	memory_stream.w_u32(seconds);
	memory_stream.w_u32(mins);
	memory_stream.w_u32(hours);
	memory_stream.w_u32(day);
	memory_stream.w_u32(month);
	memory_stream.w_u32(year);

	memory_stream.w_stringZ(lvl_name);

	memory_stream.close_chunk();

#ifdef DEBUG
	Msg("* Additional stuff has been saved: health %f, time %u %u %u %u %u %u %u, level %s", g_actor ? Actor()->GetfHealth() : -1.f, ms, seconds, mins, hours, day, month, year, lvl_name);
#endif
}