...
BOOL CPda::SpawnAndImportSOData(CSE_Abstract* data_containing_so) 
{
...
void CPda::LoadCfg(LPCSTR section) 
{
...
void CPda::ScheduledUpdate(u32 dt)	
{
	...
	// use the camera position when demo record is on
	if (freeCamIsOn_)
		Position().set(Device.vCameraPosition);
	else
...
void CPda::BeforeDetachFromParent(bool just_before_destroy) 
{
