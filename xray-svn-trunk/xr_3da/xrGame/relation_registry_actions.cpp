	...
	case ATTACK:
		{
			if(actor)
			{
				...
				if(EngineTimeU() - fight_data_from->attack_time < min_attack_delta_time)
					break;

				fight_data_from->attack_time = EngineTimeU();
			...
			if(stalker)
			{
				...
				const CEntityAlive* stalker_enemy = stalker->memory().EnemyManager().selected();
	...
	case KILL:
		{
			if(stalker)
			{
				...
				delta_rank = CHARACTER_RANK::rank_kill_points(CHARACTER_RANK::ValueToIndex(stalker->GetIORank()));
				if(delta_rank)
					inv_owner_from->ChangeIORank(delta_rank);
			}
		}
		break;
