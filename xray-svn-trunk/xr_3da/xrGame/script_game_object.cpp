...
BIND_FUNCTION10	(&object(),	CScriptGameObject::GetFOV,				CEntityAlive,	GetEyeFovValue,		float,							-1);
BIND_FUNCTION10	(&object(),	CScriptGameObject::GetRange,			CEntityAlive,	GetEyeRangeValue,			float,							-1);
...
LPCSTR CScriptGameObject::WhoHitName()
{
	...
	if (entity_alive)
		return			entity_alive->conditions().GetWhoHitLastTime()?(*entity_alive->conditions().GetWhoHitLastTime()->ObjectName()):NULL;
	else 
	{
...
LPCSTR CScriptGameObject::WhoHitSectionName()
{
	CEntityAlive *entity_alive = smart_cast<CEntityAlive*>(&object());
	if (entity_alive)
		return			entity_alive->conditions().GetWhoHitLastTime()?(*entity_alive->conditions().GetWhoHitLastTime()->SectionName()):NULL;
	else 
	{
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CScriptGameObject : cannot access class member  WhoHitName()");
		return			NULL;
...
void CScriptGameObject::SetAmmoElapsed(int ammo_elapsed)
{
	...
	if (ammo)
	{
		R_ASSERT2(ammo_elapsed < 65535, "set ammo elapsed size must be less than 65535, or contact coders to change this");
		ammo->m_boxCurr = (u16)ammo_elapsed;
	}
}
...
u32 CScriptGameObject::AmmoBoxGetCurSize()
{
	const CWeaponAmmo* ammo_box = smart_cast<const CWeaponAmmo*>(&object());

	if (!ammo_box)
		return(0);

	return (ammo_box->m_boxCurr);
}

void CScriptGameObject::AmmoBoxSetCurSize(u32 value)
{
	CWeaponAmmo* ammo_box = smart_cast<CWeaponAmmo*>(&object());

	if (!ammo_box)
		return;

	ammo_box->m_boxCurr = (u16)value;
}

u32 CScriptGameObject::AmmoBoxGetInitilaSize()
{
	const CWeaponAmmo* ammo_box = smart_cast<const CWeaponAmmo*>(&object());
...
u32 CScriptGameObject::GetInitialPortionsNum() const
{
	const CEatableItem	*item = smart_cast<const CEatableItem*>(&object());
	if (!item)
	{
	...
	return			(item->GetBasePortionsNum());
}

//////////////////////////////////////////////////////////////////////////
bool CScriptGameObject::IsInvItem() const
{
...
bool CScriptGameObject::IsOutfitItem() const
{
	const COutfitBase* item = smart_cast<const COutfitBase*>(&object());
...
bool CScriptGameObject::IsWeaponItem() const
{
	const CWeapon* item = smart_cast<const CWeapon*>(&object());
...
bool CScriptGameObject::IsWeaponMItem() const
{
	const CWeaponMagazined* item = smart_cast<const CWeaponMagazined*>(&object());
...
bool CScriptGameObject::IsWeaponWGLItem() const
{
	const CWeaponMagazinedWGrenade* item = smart_cast<const CWeaponMagazinedWGrenade*>(&object());
...
bool CScriptGameObject::IsTorchItem() const
{
	const CTorch* item = smart_cast<const CTorch*>(&object());
...
bool CScriptGameObject::IsAmmoItem() const
{
	const CWeaponAmmo* item = smart_cast<const CWeaponAmmo*>(&object());
...
bool CScriptGameObject::IsArtefactItem() const
{
	const CArtefact* item = smart_cast<const CArtefact*>(&object());
...
bool CScriptGameObject::IsUniqueItem() const
{
	const CInventoryItem* item = smart_cast<const CInventoryItem*>(&object());

	if (item && item->IsUniqueMarked())
		return true;

	return false;
}

bool CScriptGameObject::IsNeedKnifeToLoot()
{
	CInventoryItem* item = smart_cast<CInventoryItem*>(&object());

	if (item && item->NeedKnifeToLoot())
		return true;
...
int CScriptGameObject::GetAmmoMagSize()
{
	...
	{
		if (&object())
			Msg("! item sect [%s]", object().SectionNameStr());
...
{
	object().SetVisualName(visual);
}

LPCSTR CScriptGameObject::GetVisual	() const
{
	return object().VisualName().c_str();
}
