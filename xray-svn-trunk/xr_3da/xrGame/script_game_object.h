...
public:
	...
	_DECLARE_FUNCTION10	(story_id	,	ALife::_STORY_ID);
			...
			LPCSTR				Name				() const;
			shared_str			ObjectName				() const;
			LPCSTR				Section				() const;

			float				GetHemiValue		() const;
	// CInventoryItem
			u32					Cost				() const;
...
//			void				TeleportActorVehicle(Fvector pos, u32 level_vertex_id, u16 game_vertex_id);
	...
	//передача порции информации InventoryOwner
			...
			void				AddIconedTalkMessage(LPCSTR text, LPCSTR texture_name, const Frect& tex_rect, LPCSTR templ_name);
	...
	//работа с заданиями
			...
			void				SetActiveTask		(LPCSTR task_id, int objective_num);
			bool				IsActiveTask		(CGameTask* t);
			CGameTask*			GetTask				(LPCSTR id);
			...
			u32 GetArtBeltObjectCount() const;

			CScriptGameObject	*GetBeltObjectByName	(LPCSTR object_name) const;
			CScriptGameObject	*GetBeltObjectByIndex	(int iIndex) const;
			CScriptGameObject	*GetBeltObjectById		(int item_id) const;

			CScriptGameObject	*GetArtBeltObjectByName		(LPCSTR object_name) const;
			CScriptGameObject	*GetArtBeltObjectByIndex	(int iIndex) const;
			CScriptGameObject	*GetArtBeltObjectById		(int item_id) const;
...
//////////////////////////////////////////////////////////////////////////
			...
			u32					GetInitialPortionsNum		() const;

			u32					AmmoBoxGetCurSize();
			void				AmmoBoxSetCurSize(u32 value);
			u32					AmmoBoxGetInitilaSize();

			bool				IsInvItem			() const;
			bool				IsEatableItem		() const;
			bool				IsOutfitItem		() const;
			bool				IsWeaponItem		() const;
			bool				IsWeaponMItem		() const;
			bool				IsWeaponWGLItem		() const;
			bool				IsTorchItem			() const;
			bool				IsAmmoItem			() const;
			bool				IsArtefactItem		() const;

			bool				IsQuestItem() const;
			bool				IsUniqueItem() const;

			bool				IsNeedKnifeToLoot();
...
//////////////////////////////////////////////////////////////////////////
			...
			CScriptGameObject		*GetCurrentHelmet() const;
	...
	DetailPathManager::EDetailPathType	detail_path_type	() const;
			...
			u32					add_sound				(bool in_aux_thread, LPCSTR prefix, u32 max_count, ESoundTypes type, u32 priority, u32 mask, u32 internal_type, LPCSTR bone_name);
			u32					add_sound				(bool in_aux_thread, LPCSTR prefix, u32 max_count, ESoundTypes type, u32 priority, u32 mask, u32 internal_type);
			...
			CHARACTER_RANK_VALUE GetGORank				();
			...
			void			set_dest_level_vertex_id_adv(u32 level_vertex_id, float satisfying_dist = -1.f);
			void				set_desired_stop_distance(float stop_dist);
	...
	cphysics_shell_scripted*	get_physics_shell		() const;
			...
			// lost alpha start
			float				GetTorchBatteryStatus				();
			void				SetTorchBatteryStatus				(float value);
