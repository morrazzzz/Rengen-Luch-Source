...
const xr_vector<MemorySpace::CVisibleObject>	&CScriptGameObject::memory_visible_objects	() const
{
	...
	return			(monster->memory().VisualMManager().GetMemorizedObjects());
}
...
const xr_vector<MemorySpace::CSoundObject>	&CScriptGameObject::memory_sound_objects	() const
{
	...
	return			(monster->memory().SoundMManager().GetMemorizedSounds());
}
...
const xr_vector<MemorySpace::CHitObject>		&CScriptGameObject::memory_hit_objects		() const
{
	...
	return			(monster->memory().HitMManager().GetMemorizedHits());
}
...
void CScriptGameObject::SetVisualMemoryEnabled(bool enabled)
{
	...
	if (!custom_monster)
		...
		custom_monster->memory().VisualMManager().enable(enabled);
}
...
CScriptGameObject* CScriptGameObject::GetCurrentOutfit() const
{
	...
	CCustomOutfit* current_equipment = inventoryOwner->GetOutfit();
...
float CScriptGameObject::GetCurrentOutfitProtection(int hit_type)
{
	...
	return current_equipment->GetDefHitTypeProtection(ALife::EHitType(hit_type));
}

CScriptGameObject* CScriptGameObject::GetCurrentHelmet() const
{
	CInventoryOwner* inventoryOwner = smart_cast<CInventoryOwner*>(&object());

	if (!inventoryOwner)
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CInventoryOwner : cannot access class member GetCurrentHelmet!");

		return(0);
	}

	CGameObject* current_helm = inventoryOwner->GetCurrentHelmet() ? &inventoryOwner->GetCurrentHelmet()->object() : 0;

	return(current_helm ? current_helm->lua_game_object() : 0);
}
...
void CScriptGameObject::add_animation			(LPCSTR animation, bool hand_usage, bool use_movement_controller)
{
	...
	if (stalker->movement().current_params().cover()) {
		ai().script_engine().script_log(eLuaMessageTypeError, "Cannot add animation [%s]: object [%s] is in smart_cover!", animation, stalker->ObjectName().c_str());
	}
...
void CScriptGameObject::set_dest_level_vertex_id(u32 level_vertex_id)
{
	...
	if (!stalker)
		...
		stalker->movement().set_level_dest_vertex(level_vertex_id, -1.f);
	}
}

void CScriptGameObject::set_dest_level_vertex_id_adv(u32 level_vertex_id, float satisfying_dist)
{
	CAI_Stalker					*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker)
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : cannot access class member set_dest_level_vertex_id!");
		...
		THROW2(stalker->movement().restrictions().accessible(level_vertex_id), *stalker->ObjectName());
		stalker->movement().set_level_dest_vertex(level_vertex_id, satisfying_dist);
	}
}
...
CHARACTER_RANK_VALUE CScriptGameObject::GetGORank()
{
	CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker)
	{
		ai().script_engine().script_log	(ScriptStorage::eLuaMessageTypeError,"CAI_Stalker : cannot access class member GetGORank!");

		return(CHARACTER_RANK_VALUE(0));
	}
	else
		return(stalker->GetIORank());
}
...
void CScriptGameObject::set_desired_position	(const Fvector *desired_position)
{
	...
	if (!stalker)
		...
		THROW2										(desired_position || stalker->movement().restrictions().accessible(*desired_position),*stalker->ObjectName());
...
void  CScriptGameObject::set_desired_direction	(const Fvector *desired_direction)
{
	...
	if (!stalker)
		...
		if (fsimilar(desired_direction->magnitude(), 0.f))
			ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : [%s] set_desired_direction - you passed zero direction!", stalker->ObjectName().c_str());
		else {
			if (!fsimilar(desired_direction->magnitude(), 1.f))
				ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : [%s] set_desired_direction - you passed non-normalized direction!", stalker->ObjectName().c_str());
		}
...
void CScriptGameObject::set_desired_stop_distance(float dist_to_stop)
{
	CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker)
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : cannot access class member movement!");
	else
		stalker->movement().SetDestinationStopDistance(dist_to_stop);
}
		...
		if (mental_state != eMentalStateDanger) {
			if (stalker->brain().initialized()) {
				if (stalker->brain().current_action_id() == StalkerDecisionSpace::eWorldOperatorCombatPlanner) {
					ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : set_mental_state is used during universal combat!, object[%s]", stalker->ObjectName().c_str());
					//					return;
				}
			}
		}
...
u32	CScriptGameObject::GetInventoryObjectCount() const
{
	...
	if (l_tpInventoryOwner)
		return			(l_tpInventoryOwner->inventory().GetItemsCount());
	else {
...
CScriptGameObject	*CScriptGameObject::GetObjectByName	(LPCSTR caObjectName) const
{
	CInventoryOwner		*l_tpInventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (l_tpInventoryOwner) {
		CInventoryItem	*l_tpInventoryItem = l_tpInventoryOwner->inventory().GetItemByName(caObjectName, l_tpInventoryOwner->inventory().allContainer_);
...
CScriptGameObject	*CScriptGameObject::GetObjectByIndex	(int iIndex) const
{
	CInventoryOwner		*l_tpInventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (l_tpInventoryOwner) {
		CInventoryItem	*l_tpInventoryItem = l_tpInventoryOwner->inventory().GetItemByIndex(iIndex, l_tpInventoryOwner->inventory().allContainer_);
		CGameObject		*l_tpGameObject = smart_cast<CGameObject*>(l_tpInventoryItem);
		if (!l_tpGameObject)
			return		(0);
		else
...
u32	CScriptGameObject::GetBeltObjectCount() const
{
	...
	if (inv_owner)
		return(inv_owner->inventory().belt_.size());
	else
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"CScriptGameObject : cannot access class member GetBeltObjectCount!");

		return(0);
	}
}

u32	CScriptGameObject::GetArtBeltObjectCount() const
{
	CInventoryOwner* inv_owner = smart_cast<CInventoryOwner*>(&object());

	if (inv_owner)
		return(inv_owner->inventory().artefactBelt_.size());
	else
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CScriptGameObject : cannot access class member GetArtBeltObjectCount!");

		return(0);
	}
}

CScriptGameObject* CScriptGameObject::GetBeltObjectByName(LPCSTR object_name) const
{
	CInventoryOwner* inv_owner = smart_cast<CInventoryOwner*>(&object());

	if (inv_owner)
	{
		CInventoryItem* inv_item = inv_owner->inventory().GetItemBySect(object_name, inv_owner->inventory().belt_);
		CGameObject* g_obj = smart_cast<CGameObject*>(inv_item);
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CScriptGameObject : cannot access class member GetBeltObjectByName!");
...
CScriptGameObject* CScriptGameObject::GetBeltObjectByIndex(int iIndex) const
{
	...
	if (inv_owner)
	{
		CInventoryItem* inv_item = inv_owner->inventory().GetItemByIndex(iIndex, inv_owner->inventory().belt_);
		CGameObject* g_obj = smart_cast<CGameObject*>(inv_item);
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CScriptGameObject : cannot access class member GetBeltObjectByIndex!");
		return(0);	
	}
}
...
CScriptGameObject* CScriptGameObject::GetBeltObjectById(int item_id) const
{
	...
	if (inv_owner)
	{
		R_ASSERT2(item_id <= MAX_OBJ_IDS, "Possible bug here, contact coders");

		CInventoryItem* inv_item = inv_owner->inventory().GetItemByID((u16)item_id, inv_owner->inventory().belt_);
		CGameObject* g_obj = smart_cast<CGameObject*>(inv_item);
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CScriptGameObject : cannot access class member GetBeltObjectById!");
		return(0);	
	}
}

CScriptGameObject* CScriptGameObject::GetArtBeltObjectByName(LPCSTR object_name) const
{
	CInventoryOwner* inv_owner = smart_cast<CInventoryOwner*>(&object());

	if (inv_owner)
	{
		CInventoryItem* inv_item = inv_owner->inventory().GetItemBySect(object_name, inv_owner->inventory().artefactBelt_);
		CGameObject* g_obj = smart_cast<CGameObject*>(inv_item);
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CScriptGameObject : cannot access class member GetArtBeltObjectByName!");
		
		return(0);
	}
}

CScriptGameObject* CScriptGameObject::GetArtBeltObjectByIndex(int iIndex) const
{
	CInventoryOwner* inv_owner = smart_cast<CInventoryOwner*>(&object());

	if (inv_owner)
	{
		CInventoryItem* inv_item = inv_owner->inventory().GetItemByIndex(iIndex, inv_owner->inventory().artefactBelt_);
		CGameObject* g_obj = smart_cast<CGameObject*>(inv_item);
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CScriptGameObject : cannot access class member GetArtBeltObjectByIndex!");
		return(0);	
	}
}

CScriptGameObject* CScriptGameObject::GetArtBeltObjectById(int item_id) const
{
	CInventoryOwner* inv_owner = smart_cast<CInventoryOwner*>(&object());
	...
	if (inv_owner)
	{
		...
		CInventoryItem* inv_item = inv_owner->inventory().GetItemByID((u16)item_id, inv_owner->inventory().artefactBelt_);
		CGameObject* g_obj = smart_cast<CGameObject*>(inv_item);
	...
	{
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "CScriptGameObject : cannot access class member GetArtBeltObjectById!");
		return(0);	
	}
}
...
bool CScriptGameObject::weapon_strapped	() const
{
	...
	//	Msg					( "[%6d][%s] weapon_strapped = %s", EngineTimeU(), stalker->ObjectName().c_str(), result ? "true" : "false" );
	return			(result);
}
...
bool CScriptGameObject::weapon_unstrapped	() const
{
	...
	//	Msg					( "[%6d][%s] weapon_unstrapped = %s", EngineTimeU(), stalker->ObjectName().c_str(), result ? "true" : "false" );
	return			(result);
}
...
bool CScriptGameObject::IsNPCTrader()
{
	CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());

	if (!stalker || !stalker->IsTrader())
		return false;
	else
		return true;
}
