...
void CScriptParticlesCustom::PreDestroy()
{
	if ( m_owner )
		m_owner->m_particles				= NULL;
	CParticlesObject::PreDestroy();
}

void CScriptParticlesCustom::ScheduledUpdate(u32 _dt)
{
	...
	CParticlesObject::ScheduledUpdate(_dt);
...
#ifdef MEASURE_UPDATES
	Device.Statistic->scheduler_ScriptParticlesCustom_ += measure_sc_update.GetElapsed_sec();
#endif
}
...
CScriptParticles::~CScriptParticles()
{
	if(m_particles)
	{
		...
		DestroyParticleInstance		(m_particles);
		m_particles					= 0;
	}
}
