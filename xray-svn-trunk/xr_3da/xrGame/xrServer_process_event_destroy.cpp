...
{
	string1024						buff;
	CSE_Abstract*	e_dest			= Server_game_sv_base->get_entity_from_eid	(eid);
	if(e_dest)
...
void xrServer::Process_event_destroy	(NET_Packet& P, ClientID sender, u32 time, u16 ID, NET_Packet* pEPack)
{
	...
	u16								id_dest	= ID;
#ifdef DEBUG
	Msg								("sv destroy object %s [%d]", ent_name_safe(id_dest).c_str(), CurrentFrame());
#endif

	CSE_Abstract*					e_dest = Server_game_sv_base->get_entity_from_eid(id_dest);	// кто должен быть уничтожен
	if (!e_dest) 
	{
#ifdef DEBUG
	...
	else 
	{
		...
		{
			Server_game_sv_base->u_EventGen(tmpP, GE_OWNERSHIP_REJECT, parent_id);
			tmpP.w_u16(id_dest);
			tmpP.w_u8(1);
		...
		Server_game_sv_base->u_EventGen(tmpP, GE_DESTROY, id_dest);
	...
	if (e_dest->m_bALifeControl && ai().get_alife()) {
		VERIFY(Server_game_sv_base);
		if (ai().alife().objects().object(id_dest,true))
			Server_game_sv_base->alife().release(e_dest, false);
	}
