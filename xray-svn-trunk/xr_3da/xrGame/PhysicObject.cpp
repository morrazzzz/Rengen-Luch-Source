...
BOOL CPhysicObject::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
	...
	if (data_containing_so->s_flags.is(M_SPAWN_UPDATE)) {
		NET_Packet				temp;
		temp.B.count			= 0;
		data_containing_so->UPDATE_Write			(temp);
		if (temp.B.count > 0)
		{
			R_ASSERT(false);
			temp.r_seek			(0);
...
void		CPhysicObject::play_bones_sound()
{
	if(!bones_snd_player)
	{
		Msg( "! no sound loaded for obj: %s, model :%s - can not play", CObject::ObjectName().c_str(), VisualName().c_str() );
		return;
	}
...
void CPhysicObject::RunStartupAnim(CSE_Abstract *D)
{
	if(Visual()&&smart_cast<IKinematics*>(Visual()))
	{
		...
		smart_cast<IKinematics*>(Visual())->CalculateBones	(force_recalc);
	}
}
...
{
	if( !check_blend( m_anim_blend, CObject::ObjectName().c_str(), SectionName().c_str(), VisualName().c_str() ) )
		return;
...
void	CPhysicObject::		anim_time_set					( float time )
{
	...
	{
#ifdef	DEBUG	
		Msg( " ! can not set blend time %f - it must be in range 0 - %f(timeTotal) obj: %s, model: %s, anim: %s", time, m_anim_blend->timeTotal, ObjectName().c_str(), VisualName().c_str(), smart_cast<IKinematicsAnimated*>( PPhysicsShell()->PKinematics() )->LL_MotionDefName_dbg( m_anim_blend->motionID ).first );
#endif
		return;
	}
	...
	K->CalculateBones(force_recalc);
}


void CPhysicObject::DestroyClientObj()
{
...
void CPhysicObject::LoadCfg(LPCSTR section)
{
...
void CPhysicObject::ScheduledUpdate(u32 dt)
{
...
if(dbg_draw_doors)
{
	...
	if( !is_active( bones_snd_player ) )
		return;
	bones_snd_player->update( TimeDelta(), *this );
...
{
	return							!!( READ_IF_EXISTS(pSettings, r_bool, SectionName(), "is_ai_obstacle", true ) );
}

void CPhysicObject::ExportDataToServer			(NET_Packet& P) 
{	
	P.w_u8				(0);
