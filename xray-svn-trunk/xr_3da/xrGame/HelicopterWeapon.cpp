...
void CHelicopter::MGunUpdateFire()
{
	...
	float time_f = EngineTime() - delta_t;

	float fire_time;
	if(pSettings->line_exist(*SectionName(),"fire_time"))
		fire_time = pSettings->r_float(*SectionName(),"fire_time");
	else
		fire_time = -1;

	float no_fire_time;
	if(pSettings->line_exist(*SectionName(),"no_fire_time"))
		no_fire_time = pSettings->r_float(*SectionName(),"no_fire_time");
	else
		no_fire_time = -1;
...
void CHelicopter::OnShot		()
{
	...
	if(m_enemy.bUseFireTrail){
		...
		float	dt		= EngineTime() - m_enemy.fStartFireTime; VERIFY(dt>=0);
		...
		static float fire_trace_width = pSettings->r_float(*SectionName(),"fire_trace_width");
	...
	HUD_SOUND_ITEM::PlaySound(m_sndShot, fire_pos, this, false, false, u8(-1), Random.randF(MIN_RND_FREQ__SHOT, MAX_RND_FREQ__SHOT));
...
void CHelicopter::MGunFireStart()
{
	...
	if(FALSE==IsWorking() && m_enemy.bUseFireTrail){
		...
		m_enemy.fStartFireTime = EngineTime();
...
void CHelicopter::UpdateWeapons		()
{
	...
	// lerp angle
	angle_lerp	(m_cur_rot.x, m_tgt_rot.x, PI, TimeDelta());
	angle_lerp	(m_cur_rot.y, m_tgt_rot.y, PI, TimeDelta());
	...
	if( isOnAttack() ){
		...
		if(m_allow_fire){
			...
			if( between(d,m_min_rocket_dist,m_max_rocket_dist) &&
				(EngineTimeU()-m_last_rocket_attack > m_time_between_rocket_attack) ) {
...
void CHelicopter::startRocket(u16 idx)
{
	if((getRocketCount()>=1)&&m_use_rocket_on_attack) {
		...
		HUD_SOUND_ITEM::PlaySound(m_sndShotRocket, xform.c, this, false, false, u8(-1), Random.randF(MIN_RND_FREQ__SHOT, MAX_RND_FREQ__SHOT));
