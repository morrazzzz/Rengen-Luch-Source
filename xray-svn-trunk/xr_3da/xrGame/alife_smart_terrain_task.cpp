...
void CALifeSmartTerrainTask::setup_patrol_point				(const shared_str &patrol_path_name, const u32 &patrol_point_index)
{
	...
	R_ASSERT2				(patrol_path, make_string("patrol_path_name %s", *patrol_path_name ? *patrol_path_name : "null"));
...
u32	CALifeSmartTerrainTask::level_vertex_id						() const
{
	if (m_level_vertex_id == u32(-1))	{
		...
		R_ASSERT2				(ai().game_graph().valid_vertex_id(m_game_vertex_id), make_string("Vertex [%d] is not valid!!!", m_game_vertex_id));
