...
void CPatrolPathManager::select_point(const Fvector &position, u32 &dest_vertex_id)
{
	...
	if (!actual() || !m_path->vertex(m_curr_point_index)) {
		switch (m_start_type) {
			case ePatrolStartTypeFirst : {
				...
				VERIFY3		(accessible(vertex) || show_restrictions(m_object),*m_path_name,*m_game_object->ObjectName());
				break;
			}
			...
			case ePatrolStartTypePoint : {
				VERIFY3		(m_path->vertex(m_start_point_index),*m_path_name,*m_game_object->ObjectName());
	...
	VERIFY3					(m_path->vertex(m_curr_point_index) || show_restrictions(m_object),*m_path_name,*m_game_object->ObjectName());
	...
	VERIFY3				(m_path->vertex(target) || show_restrictions(m_object),*m_path_name,*m_game_object->ObjectName());
	...
	VERIFY3				(accessible(m_dest_position) || show_restrictions(m_object),*m_path_name,*m_game_object->ObjectName());
	m_actuality			= true;
	m_completed			= false;
}
...
{
	if (!m_path) {
		ai().script_engine().script_log(eLuaMessageTypeError,"Path not specified (object %s)!",*m_game_object->ObjectName());
		return				("");
	}
	VERIFY					(m_path);
	return					(m_path_name);
...
void CPatrolPathManager::set_previous_point	(int point_index)
{
	...
	if (!m_path->vertex(point_index)) {
		ai().script_engine().script_log(eLuaMessageTypeError,"Start point violates path bounds %s (object %s)!",*m_path_name,*m_game_object->ObjectName());
		return;
	}
	VERIFY					(m_path);
