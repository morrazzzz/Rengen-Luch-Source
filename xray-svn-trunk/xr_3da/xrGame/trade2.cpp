...
u32	CTrade::GetItemPrice(PIItem pItem, bool b_buying)
{
	...
	bool is_trading_with_trader_npc = pThis.inv_owner->IsTrader() || pPartner.inv_owner->IsTrader(); // ralationship is not influencing traders

	if (attitude == NO_GOODWILL || is_trading_with_trader_npc)
		relation_factor		= 0.5f;
	else
	...
	if (buying)
	{
		if( ! pThis.inv_owner->trade_parameters().enabled(CTradeParameters::action_buy(0),pItem->object().SectionName()) )
			return 0;

		p_trade_factors		= &pThis.inv_owner->trade_parameters().factors(CTradeParameters::action_buy(0),pItem->object().SectionName());
	}
	else
	{
		if( ! pThis.inv_owner->trade_parameters().enabled(CTradeParameters::action_sell(0),pItem->object().SectionName()) )
			return 0;

		p_trade_factors		= &pThis.inv_owner->trade_parameters().factors(CTradeParameters::action_sell(0),pItem->object().SectionName());
	}
...
#if 0
	float deficit_factor = _partner->inv_owner->deficit_factor(pItem->object().SectionName());
#else
	float deficit_factor = 1.f;
#endif

	// total price calculation
	u32	result = iFloor(base_cost * condition_factor * action_factor * deficit_factor * actor_factor);
