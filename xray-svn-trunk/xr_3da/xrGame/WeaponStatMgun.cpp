...
void CWeaponStatMgun::LoadCfg(LPCSTR section)
{
	inheritedPH::LoadCfg(section);
	inheritedShooting::LoadCfg	(section);

	m_sounds.LoadSound(section, "snd_shoot", "sndShot", (u8)sndMaxShotSounds_, false, SOUND_TYPE_WEAPON_SHOOTING);
...
BOOL CWeaponStatMgun::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
	if(!inheritedPH::SpawnAndImportSOData(data_containing_so))
		return FALSE;
...
void CWeaponStatMgun::DestroyClientObj()
{
	inheritedPH::DestroyClientObj();
	
	processing_deactivate();
}

void CWeaponStatMgun::ExportDataToServer(NET_Packet& P)	// export to server
{
	inheritedPH::ExportDataToServer	(P);
...
void CWeaponStatMgun::UpdateBarrelDir()
{
	...
	m_cur_x_rot		= angle_inertion_var(m_cur_x_rot,m_tgt_x_rot,0.5f,3.5f,PI_DIV_6,TimeDelta());
	m_cur_y_rot		= angle_inertion_var(m_cur_y_rot,m_tgt_y_rot,0.5f,3.5f,PI_DIV_6,TimeDelta());
}
...
void CWeaponStatMgun::renderable_Render	(IRenderBuffer& render_buffer)
{
	inheritedPH::renderable_Render	(render_buffer);
