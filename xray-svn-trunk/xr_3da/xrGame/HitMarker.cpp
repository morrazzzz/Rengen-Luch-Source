...
void CHitMarker::RemoveLinksToCLObj(CObject* obj)
{
...
{
	return ((EngineTime() - m_StartTime) < m_lanim->Length_sec());
}
...
{
	int frame;
	u32 clr = m_lanim->CalculateRGB(EngineTime() - m_StartTime, frame);
...
{
	return (2.0f*(EngineTime() - m_LastTime) < m_LightAnim->Length_sec());
}

void SGrenadeMark::Draw(float cam_dir)
{
	int frame;
	u32 clr = m_LightAnim->CalculateRGB(2.0f*(EngineTime() - m_LastTime), frame);
	m_UIStaticItem->SetTextureColor(subst_alpha(m_UIStaticItem->GetTextureColor(), color_get_A(clr)));
