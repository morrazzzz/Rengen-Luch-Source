...
//	int								m_iCumulativeItemVolume;
	...
	CHARACTER_RANK_VALUE			GetRankServer			();
	CHARACTER_RANK_VALUE			T_GetRank				();
	CHARACTER_REPUTATION_VALUE		Reputation				();
	void							SetRankServer			(CHARACTER_RANK_VALUE val);
...
SERVER_ENTITY_DECLARE_BEGIN(CSE_ALifeCustomZone,CSE_ALifeSpaceRestrictor)
	...
	xr_vector<ALife::_OBJECT_ID_u32>	suroundingArtsID_; // stored ids of artefacts, that are in the anomaly ownership

	u16								maxSurroundingArtefacts_; // max number of arts for init spawn and respawn
...
SERVER_ENTITY_DECLARE_BEGIN(CSE_ALifeAnomalousZone, CSE_ALifeCustomZone)
	...
	u32								nextRespawnTime_; // in hours

	u32								artRespawnTime_; // timer value
	u8								anomalyHasRespawner_; // is respawning arts?
	u8								anomalyDoesTimerRespawn_; // enable/disable timed respawn of arts

	virtual bool					NeedServerUpdates(CSE_ALifeDynamicObject *object);
#ifdef XRGAME_EXPORTS
			void					ServerUpdate(); // update me offline and online
	void							RespawnArtefacts		(bool no_time_check = false);
#else
			void					ServerUpdate()	{};
#endif

#ifdef XRGAME_EXPORTS
			void					spawn_artefacts			(bool respawn);
...
public:
	...
	IC		int						GetRankServer			(){ return rankServer_; }
#ifdef XRGAME_EXPORTS
			void					kill					();
			bool					has_detector			();
