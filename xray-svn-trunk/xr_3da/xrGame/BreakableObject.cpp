...
void CBreakableObject::LoadCfg		(LPCSTR section)
{
...
BOOL CBreakableObject::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
...
void CBreakableObject::ScheduledUpdate(u32 dt)
{
	...
	if (m_pPhysicsShell && !bRemoved&&EngineTimeU() - m_break_time>m_remove_time && Device.GetDistFromCamera(Position()) > GameConstants::GetDontRemovedDestroyableDist())
		SendDestroy();
...
void CBreakableObject::ExportDataToServer(NET_Packet& P)
{
}
...
void CBreakableObject::DestroyClientObj()
{
	DestroyUnbroken();
	if(m_Shell)
	{
...
//	Msg				("ge_destroy: [%d] - %s",ID(),*ObjectName());
