...
void CComplexMapSpot::Update()
{
	...
	m_last_delay += TimeDeltaU();
	if ( m_last_delay > 310 )
	{
		m_last_delay = 0;
		if ( GetGameTime() > m_timer_finish )
		{
			/*if ( !m_infinity_time )
			{
		...
		else
		{
			if ( !m_infinity_time )
			{
				ALife::_TIME_ID dt = m_timer_finish - GetGameTime();
				m_timer->TextItemControl()->SetText( GetTimeAsString( dt, etpTimeToMinutes, ':', true).c_str() );
			}
		}
	}
	...
	{
		m_timer->Show( (m_timer->GetWndSize().x > 5.0f) && (GetGameTime() < m_timer_finish) );
	}
}
