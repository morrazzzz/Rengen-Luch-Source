...
{
	SZoneObjectInfo():object(NULL),zone_ignore(false),dw_time_in_zone(0),f_time_affected(EngineTime()),small_object(false),nonalive_object(false) {}
	CGameObject*			object; 
	bool					small_object;
...
protected:
	...
	u16						deathSpawnReenableDelay_; // the delay for nextDeathSpawnArtOKTime_

	u32						nextDeathSpawnArtOKTime_; // the time when the anomaly can start to produce arts again if entity dies in it after previous death-spawn. To work around the art spawn abuse
