...
void CAttachmentOwner::DestroyClientObj()
{
#ifdef DEBUG
	if (!attached_objects().empty()) {
		Msg						("Object %s has attached items :",*smart_cast<CGameObject*>(this)->ObjectName());
...
CAttachableItem* CAttachmentOwner::attachedItem			(shared_str& section) const
{
	...
	xr_vector<CAttachableItem*>::const_iterator	E = m_attached_objects.end();
	for ( ; I != E; ++I)
		if (!xr_strcmp((*I)->item().object().SectionName(), section) && !(*I)->item().IsInvalid())
			return		(*I);
...
void CAttachmentOwner::LOG_AttachedItemsList()
{
	for (u32 i = 0; i < m_attached_objects.size(); ++i)
		if (m_attached_objects[i]){
			Msg("Attached item # %u = %s", i, m_attached_objects[i]->item().object().SectionName().c_str());
		}
