...
{
	if( EngineTimeU() - last_update <  extrapolate_pose_update_mseconds  )
		return;
	m_points.push_back(point().set(pose().set(m), EngineTime()));
	last_update = EngineTimeU();
}

void	points::init	( const Fmatrix &m )
{
	m_points.fill_in(point().set(pose().set(m), EngineTime()));
	last_update = EngineTimeU();
}
