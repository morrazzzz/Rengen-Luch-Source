...
//	Interface for adding callback functions on server objects for executing them(callbacks) when server object gets spawned in online.
...
CClientSpawnManager::~CClientSpawnManager()
{
	...
	Msg("# ClientSpawnManager: %s empty", (m_registry.empty()) ? "is" : "isnt");
}

void CClientSpawnManager::add_spawn_callback(ALife::_OBJECT_ID requesting_id, ALife::_OBJECT_ID requested_id, const luabind::functor<void>& functor, const luabind::object &object)
{
	CSpawnCallback					callback;
	callback.m_callback.set			(functor,object);

	add_spawn_callback(requesting_id, requested_id, callback);
}

void CClientSpawnManager::add_spawn_callback(ALife::_OBJECT_ID requesting_id, ALife::_OBJECT_ID requested_id, const luabind::functor<void>& lua_function)
{
	CSpawnCallback					callback;
...
void CClientSpawnManager::add_spawn_callback(ALife::_OBJECT_ID requesting_id, ALife::_OBJECT_ID requested_id, const CALLBACK_TYPE& object_callback)
{
	CSpawnCallback					callback;
...
void CClientSpawnManager::add_spawn_callback(ALife::_OBJECT_ID requesting_id, ALife::_OBJECT_ID requested_id, CSpawnCallback& spawn_callback)
{
...
void CClientSpawnManager::remove_spawn_callback(REQUESTED_REGISTRY& registry, ALife::_OBJECT_ID requesting_id, ALife::_OBJECT_ID requested_id, bool no_warning)
{
	...
	if (I == registry.end())
	{
		if (!no_warning)
		{
			...
			CSE_ALifeDynamicObject* requesting = Alife()->objects().object(requesting_id, true);
			CSE_ALifeDynamicObject* requested = Alife()->objects().object(requesting_id, true);

			LogStackTrace("Wrong Spawn callback removal");

			Msg("Requesting %s Requested %s", requesting ? requesting->name() : "null", requesting ? requested->name() : "null");
...
void CClientSpawnManager::remove_spawn_callback(ALife::_OBJECT_ID requesting_id, ALife::_OBJECT_ID requested_id)
{
	REQUEST_REGISTRY::iterator I = m_registry.find(requesting_id);
	...
	remove_spawn_callback((*I).second, requesting_id, requested_id);
...
void CClientSpawnManager::clear(ALife::_OBJECT_ID requested_id)
{
	...
	for ( ; I != E; )
	{
		remove_spawn_callback((*I).second, (*I).first, requested_id, true);
