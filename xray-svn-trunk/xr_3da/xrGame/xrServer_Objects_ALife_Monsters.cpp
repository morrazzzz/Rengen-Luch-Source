...
#ifdef XRGAME_EXPORTS
				if (char_info.data()->characterProfileRank_ == NO_RANK || _abs(spec_char.GetSpecCRank() - char_info.data()->characterProfileRank_)<RANK_DELTA)
				{
...
void CSE_ALifeTraderAbstract::set_specific_character	(shared_str new_spec_char)
{
	...
	if (selected_char.SpecificStoryID() != u32(-1))
		xmlStoryID_ = selected_char.SpecificStoryID();
...
//----
	if (NO_RANK == GetRankServer())
		SetRankServer(selected_char.GetSpecCRank());
...
CHARACTER_RANK_VALUE CSE_ALifeTraderAbstract::GetRankServer()
{
	specific_character();
...
CHARACTER_RANK_VALUE CSE_ALifeTraderAbstract::T_GetRank()
{
	return GetRankServer();
}

void CSE_ALifeTraderAbstract::SetRankServer(CHARACTER_RANK_VALUE val)
{
	specific_character();
...
CSE_ALifeCustomZone::CSE_ALifeCustomZone	(LPCSTR caSection) : CSE_ALifeSpaceRestrictor(caSection)
{
	...
	suroundingArtsID_.clear();

	maxSurroundingArtefacts_ = READ_IF_EXISTS(pSettings, r_u16, caSection, "max_surrounding_arts", 2);
	R_ASSERT(maxSurroundingArtefacts_ < 256);
}
...
#ifdef XRGAME_EXPORTS 
	u8 surounding_artifacts = 0;
	tNetPacket.r_u8(surounding_artifacts);

	suroundingArtsID_.clear();

	for (u32 i = 0; i < surounding_artifacts; i++)
	{
		u32 art_id = 0;
		tNetPacket.r_u32(art_id);

		suroundingArtsID_.push_back(art_id);
	}
#endif
...
void CSE_ALifeCustomZone::STATE_Write	(NET_Packet	&tNetPacket)
{
	...
	R_ASSERT(suroundingArtsID_.size() < 256);

	if (IsForGameSave())
	{
		u8 surounding_artifacts = (u8)suroundingArtsID_.size();
		tNetPacket.w_u8(surounding_artifacts);

#ifdef XRGAME_EXPORTS // spawn compiler makes 0 artefact count
		for (u32 i = 0; i < surounding_artifacts; i++)
		{
			CSE_ALifeDynamicObject* object = Alife()->objects().object(suroundingArtsID_[i], true);

			R_ASSERT(object);

			if (object)
				tNetPacket.w_u32(suroundingArtsID_[i]);
			else
			{
				// handle undefined or yet uknown situations
				xr_vector<u32>::iterator stored_id = std::find(suroundingArtsID_.begin(), suroundingArtsID_.end(), suroundingArtsID_[i]);

				suroundingArtsID_.erase(stored_id);

				Msg("! Rafa needs to check this - inexisting artifact has been found in anomaly owned artefact list: anom ID %u artefact ID %u", ID, suroundingArtsID_[i]);
			}
		}
#endif
...
void CSE_ALifeCustomZone::FillProps		(LPCSTR pref, PropItemVec& items)
{
	...
	PHelper().CreateU32			(items,PrepareKey(pref,*s_name,"on/off mode\\Shift time (sec)").c_str(),	&m_start_time_shift,0,100000);
	PHelper().CreateU32			(items,PrepareKey(pref,*s_name,"on/off mode\\Enabled time (sec)").c_str(),	&m_enabled_time,	0,100000);
	PHelper().CreateU32			(items,PrepareKey(pref,*s_name,"on/off mode\\Disabled time (sec)").c_str(),	&m_disabled_time,	0,100000);
}
...
CSE_ALifeAnomalousZone::CSE_ALifeAnomalousZone(LPCSTR caSection) : CSE_ALifeCustomZone(caSection)
{
	...
	artRespawnTime_				= READ_IF_EXISTS(pSettings, r_u32, caSection, "art_respawn_time", 48);
	anomalyHasRespawner_		= READ_IF_EXISTS(pSettings, r_u8, caSection, "is_respawning_arts", 1);
	anomalyDoesTimerRespawn_	= READ_IF_EXISTS(pSettings, r_u8, caSection, "respawn_on_timer", 0);
...
#ifdef XRGAME_EXPORTS //Only for game
	tNetPacket.r_u32		(nextRespawnTime_);
#endif
}
...
void CSE_ALifeAnomalousZone::STATE_Write	(NET_Packet	&tNetPacket)
{
	...
	if (IsForGameSave())
		tNetPacket.w_u32(nextRespawnTime_); // store hour of next respawn
}
...
{
	inherited::FillProps(pref, items);
	PHelper().CreateFloat			(items,PrepareKey(pref,*s_name,"offline interactive radius").c_str(),			&m_offline_interactive_radius,	0.f,	100.f);
	PHelper().CreateU16				(items,PrepareKey(pref,*s_name,"ALife\\Artefact spawn places count").c_str(),	&m_artefact_spawn_count,		32,		256);
	PHelper().CreateFlag32			(items,PrepareKey(pref,*s_name,"ALife\\Visible for AI").c_str(),				&m_flags,						flVisibleForAI);
}

bool CSE_ALifeAnomalousZone::NeedServerUpdates(CSE_ALifeDynamicObject* object)
{
	return (anomalyHasRespawner_ == 1 && anomalyDoesTimerRespawn_ == 1);
}
...
:CSE_ALifeAnomalousZone(caSection),CSE_Visual(caSection)
{
	...
	startup_animation = READ_IF_EXISTS(pSettings, r_string, caSection, "idle_anim_name", "idle");
}
...
void __stdcall OnChangeVisual(void*userdata)
{
	reinterpret_cast<CSE_Visual*>(userdata)->OnChangeVisual(0);
}
void CSE_ALifeZoneVisual::FillProps(LPCSTR pref, PropItemVec& values)
{
	...
	ChooseValue *V 				= PHelper().CreateChoose(values, PrepareKey(pref,abstract->name(),"Model\\Visual").c_str(),		&visual_name,		smVisual);
	//V->OnChangeEvent.bind		(this,&CSE_Visual::OnChangeVisual);
	V->SetCBChangeEvent(static_cast<CSE_Visual*>(this), &::OnChangeVisual);
	PHelper().CreateFlag8		(values, PrepareKey(pref,abstract->name(),"Model\\Obstacle").c_str(), &VisualFlags(), flObstacle);
}
...
{
  	inherited::FillProps			(pref,items);
    PHelper().CreateU8				(items,PrepareKey(pref,*s_name, "Team").c_str(),		&s_team, 	0,64,1);
    PHelper().CreateU8				(items,PrepareKey(pref,*s_name, "Squad").c_str(),	&s_squad, 	0,64,1);
    PHelper().CreateU8				(items,PrepareKey(pref,*s_name, "Group").c_str(),	&s_group, 	0,64,1);
   	PHelper().CreateFloat			(items,PrepareKey(pref,*s_name,"Personal",	"Health" 				).c_str(),&fHealth,	0,2,5);
}

bool CSE_ALifeCreatureAbstract::used_ai_locations	() const
{
	if (!m_flags.test(CSE_ALifeObject::flUsedAI_Locations) && fHealth <= 0.f)
		return(false);
	else
		return(true);
}
...
void CSE_ALifeCreatureAbstract::kill_entity			(ALife::_OBJECT_ID id)
{
	...
	{
#ifdef XRGAME_EXPORTS
		m_game_death_time			= GetGameTime(); //from on_death()
#endif
...
CSE_ALifeMonsterAbstract::CSE_ALifeMonsterAbstract(LPCSTR caSection)	: CSE_ALifeCreatureAbstract(caSection), CSE_ALifeSchedulable(caSection)
{
	...
	rankServer_					= (pSettings->line_exist(caSection,"rank")) ? pSettings->r_s32(caSection,"rank") : 0;
...
void CSE_ALifeMonsterAbstract::FillProps		(LPCSTR pref, PropItemVec& items)
{
	...
	PHelper().CreateFlag32		(items,	PrepareKey(pref,*s_name,"ALife\\No move in offline").c_str(),	&m_flags,			flOfflineNoMove);
	PHelper().CreateFlag32		(items,	PrepareKey(pref,*s_name,"Use smart terrain tasks").c_str(),	&m_flags,			flUseSmartTerrains);

	if (pSettings->line_exist(s_name,"SpaceRestrictionSection")) {
		LPCSTR					gcs = pSettings->r_string(s_name,"SpaceRestrictionSection");
		PHelper().CreateChoose	(items, PrepareKey(pref,*s_name,"out space restrictions").c_str(),&m_out_space_restrictors, smSpawnItem, 0, (void*)gcs, 16);
		PHelper().CreateChoose	(items, PrepareKey(pref,*s_name,"in space restrictions").c_str(),&m_in_space_restrictors,  smSpawnItem, 0, (void*)gcs, 16);
	}
}
...
bool CSE_ALifeMonsterAbstract::NeedServerUpdates(CSE_ALifeDynamicObject *object)
{
	return						(CSE_ALifeSchedulable::NeedServerUpdates(object) && (get_health() > EPS_L));
}
#ifdef XRGAME_EXPORTS
...
BOOL CSE_ALifeCreatureActor::NeedDataExport()
{
...
void CSE_ALifeMonsterRat::FillProps			(LPCSTR pref, PropItemVec& items)
{
	...
	// personal characteristics
   	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Personal",	"Field of view" 		).c_str(),&fEyeFov,							0,170,10);
   	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Personal",	"Eye range" 			).c_str(),&fEyeRange,						0,300,10);
   	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Personal",	"Minimum speed" 		).c_str(),&fMinSpeed,						0,10,0.1f);
   	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Personal",	"Maximum speed" 		).c_str(),&fMaxSpeed,						0,10,0.1f);
   	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Personal",	"Attack speed" 			).c_str(),&fAttackSpeed,					0,10,0.1f);
   	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Personal",	"Pursuit distance" 		).c_str(),&fMaxPursuitRadius,				0,300,10);
   	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Personal",	"Home distance" 		).c_str(),&fMaxHomeRadius,					0,300,10);
	// morale																			
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Morale",		"Success attack quant" 	).c_str(),&fMoraleSuccessAttackQuant,		-100,100,5);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Morale",		"Death quant" 			).c_str(),&fMoraleDeathQuant,				-100,100,5);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Morale",		"Fear quant" 			).c_str(),&fMoraleFearQuant,				-100,100,5);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Morale",		"Restore quant" 		).c_str(),&fMoraleRestoreQuant,				-100,100,5);
	PHelper().CreateU16  			(items, PrepareKey(pref,*s_name,"Morale",		"Restore time interval" ).c_str(),&u16MoraleRestoreTimeInterval,	0,65535,500);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Morale",		"Minimum value" 		).c_str(),&fMoraleMinValue,					-100,100,5);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Morale",		"Maximum value" 		).c_str(),&fMoraleMaxValue,					-100,100,5);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Morale",		"Normal value" 			).c_str(),&fMoraleNormalValue,				-100,100,5);
	// attack																			 	
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Attack",		"Hit power" 			).c_str(),&fHitPower,						0,200,5);
	PHelper().CreateU16  			(items, PrepareKey(pref,*s_name,"Attack",		"Hit interval" 			).c_str(),&u16HitInterval,					0,65535,500);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Attack",		"Distance" 				).c_str(),&fAttackDistance,					0,300,10);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Attack",		"Maximum angle" 		).c_str(),&fAttackAngle,					0,180,10);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Attack",		"Success probability" 	).c_str(),&fAttackSuccessProbability,		0,100,1);
}	
...
void CSE_ALifeMonsterZombie::FillProps		(LPCSTR pref, PropItemVec& items)
{
	...
	PHelper().CreateU16  			(items, PrepareKey(pref,*s_name,"Attack",		"Hit interval" 			).c_str(),&u16HitInterval,					0,65535,500);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Attack",		"Distance" 				).c_str(),&fAttackDistance,					0,300,10);
	PHelper().CreateFloat			(items, PrepareKey(pref,*s_name,"Attack",		"Maximum angle" 		).c_str(),&fAttackAngle,					0,100,1);
}
...
void CSE_ALifeHumanAbstract::STATE_Read		(NET_Packet &tNetPacket, u16 size)
{
	...
	if (inherited1::xmlStoryID_ != u32(-1))
		m_story_id = inherited1::xmlStoryID_;
}
...
void CSE_ALifeHumanAbstract::FillProps		(LPCSTR pref, PropItemVec& items)
{
  	inherited1::FillProps		(pref,items);
  	inherited2::FillProps		(pref,items);
	PHelper().CreateFlag32		(items,	PrepareKey(pref,*s_name,"Group behaviour").c_str(),			&m_flags,			flGroupBehaviour);
}
