...
void CGameTask::Load(const TASK_ID& id)
{
	...
	for (int i = 0; i < tag_num; i++)
	{
		...
		if (i == 0)
		{
			...
			R_ASSERT2(*objective.icon_texture_file_name, make_string("%s: The specified task icon %s is not parsed in xml texture file. Probably, some problem with includes", *objective.icon_texture_node_name, *objective.description));
		}
		//.
...
void CGameTask::RemoveMapLocations(bool notify)
{
	for (u32 i = 0; i < m_Objectives.size(); ++i)
		if (m_Objectives[i].LinkedMapLocation() != NULL)
			m_Objectives[i].RemoveMapLocations(notify);
}
...
ETaskState SGameTaskObjective::UpdateState	()
{
	...
	{
		if(GetGameTime() > parent->m_TimeToComplete)
		{
...
{
	if (LinkedMapLocation() && !notify)
		Level().MapManager().RemoveMapLocation(LinkedMapLocation());
}
