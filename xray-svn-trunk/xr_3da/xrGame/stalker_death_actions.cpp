...
{
	if (object().inventory().GetTotalWeight() <= 0)
		return							(false);
	...
	if (EngineTimeU() - object().GetLevelDeathTime() > 500)
		return							(false);
...
void CStalkerActionDead::initialize		()
{
	...
	if (active_slot == RIFLE_SLOT || active_slot == RIFLE_2_SLOT) {
	...
	for ( ; I <= E; ++I) 
	{
		...
		if (item)
			object().inventory().MoveToRuck		(item);
	}
}
