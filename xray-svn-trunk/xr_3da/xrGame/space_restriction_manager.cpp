...
IC	void CSpaceRestrictionManager::collect_garbage				()
{
	...
	for ( ; I != E; ) {
		if (!(*I).second->m_ref_count && (EngineTimeU() >= (*I).second->m_last_time_dec + time_to_delete)) {
			J						= I;
			++I;
			xr_delete				((*J).second);
...
bool CSpaceRestrictionManager::has_restrictions_data(ALife::_OBJECT_ID id)
{
	CLIENT_RESTRICTIONS::iterator	I = m_clients->find(id);
	...
	return ((*I).second.m_restriction) != nullptr;
}
