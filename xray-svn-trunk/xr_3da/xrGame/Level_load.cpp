...
{
	R_ASSERT(Particles.staticParticles.empty());
	...
	if (FS.exist(fn_game, "$level$", "level.ps_static")) 
	{
		...
		for (IReader *OBJ = F->open_chunk_iterator(chunk); OBJ; OBJ = F->open_chunk_iterator(chunk,OBJ)) {
			...
			Particles.staticParticles.push_back		(pStaticParticles);
		}
		FS.r_close		(F);
	}
	...
	{
		Msg("No Valumetric Fog data (level.fog_vol)");
	}

	if (g_pGamePersistent->levelEnvironment)
	{
		if (auto rain = g_pGamePersistent->levelEnvironment->eff_Rain)
		{
