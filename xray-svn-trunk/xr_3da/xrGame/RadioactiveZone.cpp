...
void CRadioactiveZone::LoadCfg(LPCSTR section) 
{
...
void CRadioactiveZone::Affect(SZoneObjectInfo* O) 
{
	...
	if(!O->object || O->f_time_affected+one > EngineTime()) 
		return;
...
/*		Msg			(	"Zone[%s]-hit->[%s] Power=%3.3f Frame=%d Time=%3.3f", 
						ObjectName().c_str(), 
						O->object->ObjectName().c_str(), 
						send_power, 
						CurrentFrame(), 
						tg);*/
///		Msg( "Zone hit ___   damage = %.4f    Frame=%d ", send_power, CurrentFrame() );
#endif
