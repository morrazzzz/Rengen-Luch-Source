...
void CStalkerCombatPlanner::update()
{
	...
	if (inherited::solution().empty())
	{
		EvaluatorsDebug();

		//R_ASSERT2(false, object().ObjectName().c_str());
		Msg("@ Reinitializing combat planner for %s, since there is no solution for applicating a behavior scheme", object().ObjectName().c_str());

		initialize(); // Resets states and conditions for planner
...
void CStalkerCombatPlanner::initialize()
{
	...
	if (!m_loaded && object().memory().EnemyManager().selected())
	{
...
//	VERIFY					(object().memory().enemy().selected());
	...
	if (m_object->memory().VisualMManager().visible_now(m_object->memory().EnemyManager().selected()))
	{
		if (m_object->memory().EnemyManager().selected()->human_being())
...
void CStalkerCombatPlanner::finalize()
{
	...
	object().memory().DangerManager().time_line(EngineTimeU() + 3000);
...
{
	add_evaluator			(eWorldPropertyPureEnemy		, xr_new <CStalkerPropertyEvaluatorEnemies>(m_object, "is_there_enemies"));
	add_evaluator			(eWorldPropertyEnemy			, xr_new <CStalkerPropertyEvaluatorEnemies>(m_object, "is_there_enemies_delayed", false, POST_COMBAT_WAIT_INTERVAL, &m_last_wounded));
	add_evaluator			(eWorldPropertySeeEnemy			, xr_new <CStalkerPropertyEvaluatorSeeEnemy>(m_object, "see enemy"));
	add_evaluator			(eWorldPropertyEnemyLocalized	, xr_new <CStalkerPropertyEvaluatorEnemyLocalized>(m_object, "enemy localized", 2000));
	add_evaluator			(eWorldPropertyKnowWhereEnemy	, xr_new <CStalkerPropertyEvaluatorKnowWhereEnemy>(m_object, "know where enemy", 10000, 5000));
	...
	add_evaluator			(eWorldPropertyEnemyToStrong	, xr_new <CStalkerPropertyEvaluatorEnemyToStrong>(m_object, "enemy_too_strong"));
	...
	add_evaluator			(eWorldPropertyEnemyOnThePath	, xr_new <CStalkerPropertyEvaluatorEnemyOnThePath>(m_object, "player on the path"));
	...
	if (!strstr(Core.Params, "-no_grenades_throwing"))
...
#pragma todo("Cop merge AA: Check if we need to add new evaluators conditions for our new actions")

void CStalkerCombatPlanner::add_actions()
{
	CStalkerActionBase* action;
	bool grenades_throwing = (strstr(Core.Params, "-no_grenades_throwing") == nullptr);
	...
	//add_condition			(action, eWorldPropertyEnemyToStrong, true);
	...
	add_condition			(action, eWorldPropertyEnemyOnThePath, false);
	...
	action					= xr_new <CStalkerActionReachFireDistance>(m_object, "reach_fire_dist");
	...
	add_operator			(eWorldOperatorReachFireDist, action);
...

	{
		...
		add_condition			(action, eWorldPropertyUseSuddenness, false);
		add_condition			(action, eWorldPropertyReadyToKill, true);
		add_condition			(action, eWorldPropertyEnemyLocalized, false);
		add_condition			(action, eWorldPropertyLookedOut, true);
		add_condition			(action, eWorldPropertyPositionHolded, true);
	...
	add_condition			(action, eWorldPropertyUseSuddenness, false);
	add_condition			(action, eWorldPropertyReadyToKill, true);
	add_condition			(action, eWorldPropertyKnowWhereEnemy, true);
	add_condition			(action, eWorldPropertySeeEnemy, false);
	add_condition			(action, eWorldPropertyEnemyWounded, false);
	add_condition			(action, eWorldPropertyTooFarToKillEnemy, false);
	add_condition			(action,eWorldPropertyInSmartCover, false);
	...
	action					= xr_new <CStalkerActionKillEnemyOnThePath>(m_object, "kill enemy, if enemy is on my path");

	add_condition			(action, eWorldPropertyCriticallyWounded, false);
	add_condition			(action, eWorldPropertyUseSuddenness, false);
//	add_condition			(action, eWorldPropertySeeEnemy, true); // Cop commented
	add_condition			(action, eWorldPropertyEnemyOnThePath, true);
	add_condition			(action, eWorldPropertyEnemyWounded, false);
	add_condition			(action, eWorldPropertyTooFarToKillEnemy, false);
	...
	add_operator			(eWorldOperatorKillEnemyOnThePath,	action);
	...
	add_condition			(planner, eWorldPropertyPureEnemy, true);

	add_effect				(planner, eWorldPropertyPureEnemy, false);
...

	{
		...
		add_condition			(planner,eWorldPropertyEnemyWounded,		false);
		add_condition			(planner,eWorldPropertyEnemyOnThePath,		false);
		add_condition			(planner,eWorldPropertyInCover,				true);
...
void CStalkerCombatPlanner::EvaluatorsDebug()
{
	Msg("--2 %s : PositionHolded %d CriticallyWounded %d DangerGrenade %d UseSuddenness	%d ReadyToKill %d ItemToKill %d \
		ItemCanKill %d ReadyToDetour %d InCover %d EnemyDetoured %d SeeEnemy %d LookedOut %d EnemyWounded %d \
		EnemyOnThePath %d TooFarToKillEnemy %d EnemyLocalized %d KnowWhereEnemy %d ThrowGrenade %d",
		object().ObjectNameStr(),
		evaluator(eWorldPropertyPositionHolded).evaluate(),
		evaluator(eWorldPropertyCriticallyWounded).evaluate(),
		evaluator(eWorldPropertyDangerGrenade).evaluate(),
		evaluator(eWorldPropertyUseSuddenness).evaluate(),
		evaluator(eWorldPropertyReadyToKill).evaluate(),
		evaluator(eWorldPropertyItemToKill).evaluate(),
		evaluator(eWorldPropertyItemCanKill).evaluate(),
		evaluator(eWorldPropertyReadyToDetour).evaluate(),
		evaluator(eWorldPropertyInCover).evaluate(),
		evaluator(eWorldPropertyEnemyDetoured).evaluate(),
		evaluator(eWorldPropertySeeEnemy).evaluate(),
		evaluator(eWorldPropertyLookedOut).evaluate(),
		evaluator(eWorldPropertyEnemyWounded).evaluate(),
		evaluator(eWorldPropertyEnemyOnThePath).evaluate(),
		evaluator(eWorldPropertyTooFarToKillEnemy).evaluate(),
		evaluator(eWorldPropertyEnemyLocalized).evaluate(),
		evaluator(eWorldPropertyKnowWhereEnemy).evaluate(),
		(strstr(Core.Params, "-no_grenades_throwing")) == nullptr ? evaluator(eWorldPropertyShouldThrowGrenade).evaluate() : "null"
		);
}