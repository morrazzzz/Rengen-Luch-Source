...
void CPlanner::update				()
{
	...
	if (m_use_log) {
		if (m_solution_changed) {
			...
			Msg						("%6d : Solution for object %s [%d vertices searched]",EngineTimeU(),object_name(),ai().graph_engine().solver_algorithm().data_storage().get_visited_node_count());
...
//		VERIFY2						(!m_failed,"Problem solver couldn't build a valid path - verify your conditions, effects and goals!");
	...
	//R_ASSERT2(!solution().empty(), object_name());

	if (solution().empty())
	{
		return; // avoid crash - reinitialize in combat planer
	}
