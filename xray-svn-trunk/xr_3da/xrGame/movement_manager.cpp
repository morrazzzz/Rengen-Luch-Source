...
CMovementManager::CMovementManager	(CCustomMonster *object)
{
	...
	satisfyingDestinationDistance_	= -1.f;
}
...
void CMovementManager::LoadCfg			(LPCSTR section)
{
...
BOOL CMovementManager::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
	return(restrictions().SpawnAndImportSOData(data_containing_so));
}

void CMovementManager::DestroyClientObj()
{
	level_path_builder().remove		();
	detail_path_builder().remove	();
	restrictions().DestroyClientObj	();
}
...
void CMovementManager::set_level_dest_vertex(const u32 level_vertex_id, float satisfying_distance)
{
	if (ai().level_graph().valid_vertex_id(level_vertex_id) && restrictions().accessible(level_vertex_id))
	{
		satisfyingDestinationDistance_ = satisfying_distance; // if <= 0, then its not used
...
float CMovementManager::distance_to_destination() const
{
	if (path().size() < 2)
		return 0.f;
...
{
//	Msg								("[%6d][%s][on_restrictions_change]",EngineTimeU(),*object().ObjectName());
	m_path_actuality				= false;
	level_path_builder().remove		();
	detail_path_builder().remove	();
...
//	static int i=0;
//	Msg									("[%6d][%6d][%4d][%f] build_level_path",EngineTimeU(),CurrentFrame(),++i,timer.GetElapsed_sec()*1000.f);
}
