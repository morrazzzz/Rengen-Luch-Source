...
void CObjectActionCommand::initialize	()
{
	...
	object().inventory().InputAction((u16)m_command,CMD_START);
}
...
void CObjectActionShow::initialize		()
{
	...
	if (slot_item)
		object().inventory().MoveToRuck(slot_item);

	object().inventory().MoveToSlot(m_item->BaseSlot(), m_item);
}
...
void CObjectActionShow::execute		()
{
	...
	if (!hud_item){
		object().inventory().MoveToSlot(m_item->BaseSlot(), m_item);
		object().inventory().ActivateSlot(m_item->BaseSlot());
		return;
	}
...
void CObjectActionHide::execute		()
{
	inherited::execute				();
	VERIFY							(m_item);
	object().inventory().ActivateSlot(NO_ACTIVE_SLOT);
...
static bool try_advance_ammo		(CWeapon const& weapon)
{
	...
	for(u8 i = 0; i < u8(weapon.m_ammoTypes.size()); ++i) 
	{
		...
		for(TIItemContainer::iterator l_it = inventory.belt_.begin(); inventory.belt_.end() != l_it; ++l_it)
		{
		...
		for(TIItemContainer::iterator l_it = inventory.ruck_.begin(); inventory.ruck_.end() != l_it; ++l_it)
		{
			CWeaponAmmo *l_pAmmo = smart_cast<CWeaponAmmo*>(*l_it);
			if(l_pAmmo && !xr_strcmp(l_pAmmo->SectionName(), l_ammoType)) 
			{
...
void CObjectActionReload::initialize		()
{
	...
	object().inventory().InputAction(kWPN_RELOAD,	CMD_START);
}
...
void CObjectActionFire::initialize		()
{
	...
	if (!m_object->can_kill_member())
		object().inventory().InputAction(kWPN_FIRE, CMD_START);
	else
		object().inventory().InputAction(kWPN_FIRE, CMD_STOP);
}
...
void CObjectActionThrowMissile::initialize	()
{
	...
	object().inventory().InputAction(kWPN_ZOOM, CMD_START);
...
void CObjectActionThrowMissile::execute		()
{
	inherited::execute		();
	if (completed())
		object().inventory().InputAction(kWPN_ZOOM, CMD_STOP);
}
