...
void CObjectHandler::LoadCfg			(LPCSTR section)
{
...
void CObjectHandler::reinit			(CAI_Stalker *object)
{
	...
	m_r_hand					= kinematics->LL_BoneID(pSettings->r_string(*planner().m_object->SectionName(),"weapon_bone0"));
	m_l_finger1					= kinematics->LL_BoneID(pSettings->r_string(*planner().m_object->SectionName(),"weapon_bone1"));
	m_r_finger2					= kinematics->LL_BoneID(pSettings->r_string(*planner().m_object->SectionName(),"weapon_bone2"));
...
BOOL CObjectHandler::SpawnAndImportSOData(CSE_Abstract* data_containing_so)
{
	if (!inherited::SpawnAndImportSOData(data_containing_so))
		return					(FALSE);

	CSE_Abstract				*abstract = static_cast<CSE_Abstract*>(data_containing_so);
...
void CObjectHandler::OnItemTake		(CInventoryItem *inventory_item, bool duringSpawn)
{
	...
	if (inventory_item->useful_for_NPC() && (inventory_item->object().SectionName() == m_item_to_spawn)) {
	...
	if (weapon)
	{
		...
		cam_recoil_copy.camRelaxSpeed = cam_recoil_copy.camRelaxSpeed_AI;
...
void CObjectHandler::OnItemDrop		(CInventoryItem *inventory_item)
{
	...
	if (m_infinite_ammo && planner().object().g_Alive() && !inventory_item->useful_for_NPC()) {
		...
		if (weapon_ammo) {
			Level().spawn_item		(*weapon_ammo->SectionName(),planner().object().Position(),planner().object().ai_location().level_vertex_id(),planner().object().ID());
			m_item_to_spawn			= weapon_ammo->SectionName();
...
void CObjectHandler::weapon_bones	(int &b0, int &b1, int &b2) const
{
	...
	THROW3						(weapon->can_be_strapped(),"Cannot strap weapon",*weapon->ObjectName());
...
//		Msg						( "[%6d][%s] actualizing: weapon_strapped = false", EngineTimeU(), planner().object().ObjectName().c_str() );
		weapon->strapped_mode	(false);
		return;
	}

	THROW3						(weapon->can_be_strapped(),"Cannot strap weapon",*weapon->ObjectName());
//	Msg							( "[%6d][%s] actualizing: weapon_strapped = true", EngineTimeU(), planner().object().ObjectName().c_str() );
	weapon->strapped_mode		(true);
}
...
//		Msg						( "[%6d][%s] weapon_strapped = false0", EngineTimeU(), planner().object().ObjectName().c_str() );
		return					(false);
	}
...
//		Msg						( "[%6d][%s] weapon_strapped = %s1", EngineTimeU(), planner().object().ObjectName().c_str(), almost_strapped && !planner().m_storage.property(ObjectHandlerSpace::eWorldPropertyStrapped2Idle) ? "true" : "false" );
...
bool CObjectHandler::weapon_unstrapped	() const
{
	CWeapon						*weapon = smart_cast<CWeapon*>(inventory().ActiveItem());
	if (!weapon) {
//		Msg						( "[%6d][%s] no active item!!(%d)(%s)", EngineTimeU(), planner().object().ObjectName().c_str(), inventory().GetActiveSlot(), inventory().ItemFromSlot(3) ? inventory().ItemFromSlot(3)->object().ObjectName().c_str() : "<no_item>" );
		return					(true);
	}
...
bool CObjectHandler::weapon_unstrapped	(CWeapon *weapon) const
{
	...
	if (!weapon->can_be_strapped()) {
//		Msg						( "[%6d][%s] weapon_unstrapped = true0", EngineTimeU(), planner().object().ObjectName().c_str() );
		return					(true);
	}
...
//		Msg						( "[%6d][%s] weapon_unstrapped = %s1", EngineTimeU(), planner().object().ObjectName().c_str(), almost_unstrapped && !planner().m_storage.property(ObjectHandlerSpace::eWorldPropertyStrapped2Idle) ? "true" : "false" );
