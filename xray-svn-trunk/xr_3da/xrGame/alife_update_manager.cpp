...
CTimer serverAnomaliesUpdateTimer_;
	...
	CALifeSimulatorBase		(server,section)
{
	...
	maxTimeALifeGraphRegUpdate_		= pSettings->r_float(section, "max_time_graph_itter");
	maxTimeALifeScRegUpdate_		= pSettings->r_float(section, "max_time_schedule_itter");

	maxObjectsALifeScRegUpdate_		= pSettings->r_u32(section, "objs_per_sc_registry_itter");

	m_changing_level		= false;
	m_first_time			= true;

	serverAnomaliesUpdateTimer_.Start();
	anomalyPoolCreated_		= false;
	anomaliesOnServer_.clear();
	anomalyPoolUpdIndex_	= 200;
...
#ifdef MEASURE_MT
	Device.Statistic->mtALifeTime_ += measure_sc_update.GetElapsed_sec();
#endif
}

void CALifeUpdateManager::ScheduledUpdate(u32 dt)
{
	...
	if (serverAnomaliesUpdateTimer_.GetElapsed_sec() * 1000.f > 1000.f) // Update just a part of anomalies every "some seconds" to not overload frame update
	{
		UpdateServerAnomalies();
...
void CALifeUpdateManager::SetGraphProcessTimeLimit(float seconds)
{
	graph().SetGraphItterTimeLimit(seconds);
}

void CALifeUpdateManager::SetRegidtryUpdateLimits(const u32 objects_per_update, const float time_limit)
{
	scheduled().SetUpUpdateLimits(objects_per_update, time_limit);
}
...
{
	g_pGamePersistent->LoadTitle		("st_creating_new_game", "New Creating New Game");
...
void CALifeUpdateManager::load			(LPCSTR game_name, bool no_assert, bool new_only)
{
	...
	if (new_only || !CALifeStorageManager::storage_load(game_name)) {
...
#endif
	g_pGamePersistent->LoadTitle("st_server_connecting", "Connecting to Server", true, g_pGameLevel->name());
}
...
void CALifeUpdateManager::reload(LPCSTR section)
{
	...
	SetGraphProcessTimeLimit(maxTimeALifeGraphRegUpdate_);

	SetRegidtryUpdateLimits(maxObjectsALifeScRegUpdate_, maxTimeALifeScRegUpdate_);
}
