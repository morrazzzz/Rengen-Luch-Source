	...
	for(;_b!=_e;++_b)
	{
		if(strstr(_b->first.c_str(), "anm_")==_b->first.c_str())
		{
			...
			else
			{
				...
				pm->m_anim_speed = xr_strlen(str_item) > 0 ? (float)atof(str_item)	: 1.f;
			}
...
{
	if (!bForce && m_upd_firedeps_frame == CurrentFrame())
		return;
	...
	m_upd_firedeps_frame = CurrentFrame();
	...
	if(ka)
	{
		...
		ka->dcast_PKinematics()->CalculateBones(BonesCalcType::force_recalc);
	}
}
...
void attachable_hud_item::render_me(IRenderBuffer& render_buffer)
{
	render_buffer.renderingMatrix_ = m_item_transform;
	::Render->add_Visual(m_model->dcast_RenderVisual(), render_buffer);

	//debug_draw_firedeps(); // mt bugs here

	m_parent_hud_item->render_hud_mode(m_model, render_buffer);
}
...
Fmatrix hud_item_measures::GetAddonsParams(u8 index)
{
	Fmatrix buffer;
	Fvector rot = m_custom_addon_offset[1][index];
	rot.mul(PI / 180.f);
	buffer.setHPB(rot.x, rot.y, rot.z);
	buffer.translate_over(m_custom_addon_offset[0][index]);
	return buffer;
}
...
void hud_item_measures::load(const shared_str& sect_name, IKinematics* K)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		m_custom_addon_offset[0][i].set(0, 0, 0);
		m_custom_addon_offset[1][i].set(0, 0, 0);
	}
	...
	m_hands_offset_ironsight[0][0].set	(0,0,0);
	m_hands_offset_ironsight[1][0].set	(0,0,0);

	strconcat					(sizeof(val_name),val_name,"aim_hud_offset_pos",_prefix);
	m_hands_offset_ironsight[0][1]		= pSettings->r_fvector3(sect_name, val_name);
	strconcat					(sizeof(val_name),val_name,"aim_hud_offset_rot",_prefix);
	m_hands_offset_ironsight[1][1]		= pSettings->r_fvector3(sect_name, val_name);

	strconcat					(sizeof(val_name),val_name,"gl_hud_offset_pos",_prefix);
	m_hands_offset_ironsight[0][2]		= pSettings->r_fvector3(sect_name, val_name);
	strconcat					(sizeof(val_name),val_name,"gl_hud_offset_rot",_prefix);
	m_hands_offset_ironsight[1][2]		= pSettings->r_fvector3(sect_name, val_name);


	m_hands_offset_scope[0][0].set	(0,0,0);
	m_hands_offset_scope[1][0].set	(0,0,0);

	strconcat					(sizeof(val_name),val_name,"aim_scope_hud_offset_pos",_prefix);
	m_hands_offset_scope[0][1]		= READ_IF_EXISTS(pSettings, r_fvector3, sect_name, val_name, m_hands_offset_ironsight[0][1]);
	strconcat					(sizeof(val_name),val_name,"aim_scope_hud_offset_rot",_prefix);
	m_hands_offset_scope[1][1]		= READ_IF_EXISTS(pSettings, r_fvector3, sect_name, val_name, m_hands_offset_ironsight[1][1]);

	strconcat					(sizeof(val_name),val_name,"gl_scope_hud_offset_pos",_prefix);
	m_hands_offset_scope[0][2]		= READ_IF_EXISTS(pSettings, r_fvector3, sect_name, val_name, m_hands_offset_ironsight[0][2]);
	strconcat					(sizeof(val_name),val_name,"gl_scope_hud_offset_rot",_prefix);
	m_hands_offset_scope[1][2]		= READ_IF_EXISTS(pSettings, r_fvector3, sect_name, val_name, m_hands_offset_ironsight[1][2]);
...
void player_hud::load(const shared_str& player_hud_sect)
{
	...
	m_model->dcast_PKinematics()->CalculateBones(BonesCalcType::force_recalc);
}
...
void player_hud::render_hud(IRenderBuffer& render_buffer)
{
	...
	render_buffer.renderingMatrix_ = m_transform;
	::Render->add_Visual(m_model->dcast_RenderVisual(), render_buffer);
	
	if(m_attached_items[0])
		m_attached_items[0]->render_me(render_buffer);

	if(m_attached_items[1])
		m_attached_items[1]->render_me(render_buffer);
}
...
void player_hud::update_inertion(Fmatrix& trans)
{
	if (inertion_allowed())
	{
		...
		// tend to forward
		st_last_dir.mad						(diff_dir, TENDTO_SPEED * TimeDelta());
...
custom_addon::custom_addon(LPCSTR section)
{
	attach_offset[0].set(0.f, 0.f, 0.f);
	attach_offset[1].set(0.f, 0.f, 0.f);

	attach_offset[0] = READ_IF_EXISTS(pSettings, r_fvector3, section, "hud_attach_pos", attach_offset[0]);
	attach_offset[1] = READ_IF_EXISTS(pSettings, r_fvector3, section, "hud_attach_rot", attach_offset[1]);

	addon_section	 = READ_IF_EXISTS(pSettings, r_string, section, "addon_name", NULL);
}
