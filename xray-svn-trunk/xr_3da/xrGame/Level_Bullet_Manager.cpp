...
//	VERIFY						( m_thread_id == GetCurrentThreadId() );
	...
	u32 const time_delta		= TimeDeltaU();
	if (!time_delta)
		return;
...
#ifdef MEASURE_MT
	Device.Statistic->mtBulletsTime_ += measure_mt.GetElapsed_sec();
#endif
}
...
//	VERIFY						(data.collide_time <= bullet.life_time + EngineTime());
	...
	case EVENT_HIT:
		{
			...
			if (_dynamic)	
			{
				...
				// Msg("Dynamic object hit event: %s, id: %d, bullet target: %d, repeated: %d", R.O->ObjectName().c_str(), targetId, E.bullet.targetID, E.Repeated);
