...
bool CALifeMonsterDetailPathManager::target	(const GameGraph::_GRAPH_ID &game_vertex_id, const u32 &level_vertex_id, const Fvector &position)
{
	if (!ai().game_graph().valid_vertex_id(game_vertex_id))
	{
#if 0
		Msg("!Alife path construction error while using the graph point %d (level [%s] [%d] [%d] [%d] [%d], position of graph point [%f] [%f] [%f], position of destination [%f] [%f] [%f])",

			game_vertex_id,
			ai().game_graph().header().levels().find(ai().game_graph().vertex(game_vertex_id)->level_id())->second.name().c_str(),
			ai().game_graph().vertex(game_vertex_id)->vertex_type()[0],
			ai().game_graph().vertex(game_vertex_id)->vertex_type()[1],
			ai().game_graph().vertex(game_vertex_id)->vertex_type()[2],
			ai().game_graph().vertex(game_vertex_id)->vertex_type()[3],
			VPUSH(ai().game_graph().vertex(game_vertex_id)->level_point())),
			VPUSH(position);

		ai().script_engine().print_stack();

		R_ASSERT2(false, "Alife path construction error");
#endif
		Msg("! Alife path constuction error. Use mixed bins for more info");

		return false;
	}


	R_ASSERT						(ai().game_graph().valid_vertex_id(game_vertex_id));
	R_ASSERT						((ai().game_graph().vertex(game_vertex_id)->level_id() != ai().alife().graph().level().level_id()) || ai().level_graph().valid_vertex_id(level_vertex_id));
	R_ASSERT						((ai().game_graph().vertex(game_vertex_id)->level_id() != ai().alife().graph().level().level_id()) || (ai().cross_table().vertex(level_vertex_id).game_vertex_id() == game_vertex_id));
	R_ASSERT						((ai().game_graph().vertex(game_vertex_id)->level_id() != ai().alife().graph().level().level_id()) || ai().level_graph().inside(level_vertex_id,position));
...
//	Msg								("[%6d][%s][%f][%f][%f]",EngineTimeU(),object().name_replace(),VPUSH(m_destination.m_position));
}
...
bool CALifeMonsterDetailPathManager::target					(const CALifeSmartTerrainTask &task)
{
	bool successful = target(task.game_vertex_id(), task.level_vertex_id(), task.position());

	R_ASSERT2(successful, make_string("Alife path constuction: Failed to process smart terain task: task patrol path = %s, patrol point index = %u", *task.patrol_path_name(), task.patrol_point_index()));
...
bool CALifeMonsterDetailPathManager::target					(const CALifeSmartTerrainTask *task)
{
	//Msg	("[%6d][%s][%s]",EngineTimeU(),object().name_replace(),*task->patrol_path_name());

	bool successful = target(*task);
...
void CALifeMonsterDetailPathManager::update()
{
	...
	m_last_update_time = GetGameTime();
}
...
void CALifeMonsterDetailPathManager::follow_path				(const ALife::_TIME_ID &time_delta)
{
	...
	for ( ; m_path.size() > 1;) {
		...
		m_walked_distance				= 0.f;
		m_path.pop_back					();
//		Msg									("%6d %s changes graph point from %d to %d",EngineTimeU(),object().name_replace(),object().m_tGraphID,(GameGraph::_GRAPH_ID)m_path.back());
