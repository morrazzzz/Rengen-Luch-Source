...
void CWeaponKnife::LoadCfg	(LPCSTR section)
{
	// verify class
	...
	{
		m_sounds.LoadSound(section, "snd_shoot_2", "sndShot2", 0, false, SOUND_TYPE_WEAPON_SHOOTING);
...
void CWeaponKnife::OnMotionMark(u32 state, const motion_marks& M)
{
	...
	if (H_Parent())
	{
		...
		if (GetState() == eFire)
			PlayHUDMotion("anm_shoot1_end", TRUE, this, GetState());
		else
			PlayHUDMotion("anm_shoot2_end", TRUE, this, GetState());
	}
}
...
void CWeaponKnife::switch2_Attacking	(u32 state)
{
	...
	if(state==eFire)
		PlayHUDMotion("anm_shoot1_start",		FALSE, this, state);
	else
		PlayHUDMotion("anm_shoot2_start",		FALSE, this, state);
...
{
	PlaySound((bHasShoot2Sound && GetState() == eFire2) ? "sndShot2" : "sndShot", get_LastFP(), Random.randF(MIN_RND_FREQ__SHOT, MAX_RND_FREQ__SHOT));
}
