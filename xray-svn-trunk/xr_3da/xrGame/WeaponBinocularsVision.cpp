...
void SBinocVisibleObj::Draw()
{
	...
	Fvector2 screen_center; screen_center.set(UI_BASE_WIDTH / 2.f, UI_BASE_HEIGHT / 2.f);

	float dist = m_lt.GetWndPos().distance_to(screen_center);

	if (dist <= displayRadius_)
		m_lt.Draw();

	dist = m_lb.GetWndPos().distance_to(screen_center);

	if (dist <= displayRadius_)
		m_lb.Draw();

	dist = m_rt.GetWndPos().distance_to(screen_center);

	if (dist <= displayRadius_)
		m_rt.Draw();

	dist = m_rb.GetWndPos().distance_to(screen_center);
...
void SBinocVisibleObj::Update()
{
	...
	if (m_flags.is(flTargetLocked)){
		...
		cur_rect.lt.x	+= (mn.x-cur_rect.lt.x)*m_upd_speed*TimeDelta();
		cur_rect.lt.y	+= (mn.y-cur_rect.lt.y)*m_upd_speed*TimeDelta();
		cur_rect.rb.x	+= (mx.x-cur_rect.rb.x)*m_upd_speed*TimeDelta();
		cur_rect.rb.y	+= (mx.y-cur_rect.rb.y)*m_upd_speed*TimeDelta();
...
void CBinocularsVision::Update()
{
	...
	const CVisualMemoryManager::VISIBLES& vVisibles = pActor->memory().visual().GetMemorizedObjects();
	...
	for (; v_it!=vVisibles.end(); ++v_it)
	{
		...
		if( found != m_active_objects.end() ){
			...
			new_vis_obj->displayRadius_		= screenRadius_;
...
void CBinocularsVision::Load(const shared_str& section)
{
	...
	screenRadius_		 = READ_IF_EXISTS(pSettings, r_float, section.c_str(), "display_area_radius", 750.f);

	m_sounds.LoadSound(section.c_str(), "found_snd", "found_snd", 0, false, SOUND_TYPE_NO_SOUND);
	m_sounds.LoadSound(section.c_str(), "catch_snd", "catch_snd", 0, false, SOUND_TYPE_NO_SOUND);
}
