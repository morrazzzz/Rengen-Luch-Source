...
void moving_objects::resolve_collision_first	(boxes &current, moving_object *object0, moving_object *object1, possible_actions &action) const
{
	...
	if (target._0.intersects(start._1)) {
		MSG						("%6d [t0s1] [%s][%s]",CurrentFrame(),*object0->object().ObjectName(),*object1->object().ObjectName());
		action					= possible_action_1_can_wait_2;
		return;
	}

	if (target._1.intersects(start._0)) {
		MSG						("%6d [t1s0] [%s][%s]",CurrentFrame(),*object0->object().ObjectName(),*object1->object().ObjectName());
		action					= possible_action_2_can_wait_1;
		return;
	}

	if (start._0.intersects(current._1)) {
		MSG						("%6d [s0c1] [%s][%s]",CurrentFrame(),*object0->object().ObjectName(),*object1->object().ObjectName());
		action					= possible_action_2_can_wait_1;
		return;
	}

	if (start._1.intersects(current._0)) {
		MSG						("%6d [s1c0] [%s][%s]",CurrentFrame(),*object0->object().ObjectName(),*object1->object().ObjectName());
		action					= possible_action_1_can_wait_2;
		return;
	}

	if (current._0.intersects(target._1)) {
		MSG						("%6d [c0t1] [%s][%s]",CurrentFrame(),*object0->object().ObjectName(),*object1->object().ObjectName());
		action					= possible_action_2_can_wait_1;
		return;
	}

	if (current._1.intersects(target._0)) {
		MSG						("%6d [c1t0] [%s][%s]",CurrentFrame(),*object0->object().ObjectName(),*object1->object().ObjectName());
		action					= possible_action_1_can_wait_2;
		return;
	}
...
{
#if 0
	if (object0->action_frame() == CurrentFrame()) {
		Msg						("%6d Oooooooops",CurrentFrame());
		{
			...
			for ( ; I != E; ++I) {
				Msg				("    %s",(*I)->object().ObjectName().c_str());
			}
		}
		{
		...
		Msg						("%6d Eng of \"Oooooooops\"",CurrentFrame());
	}
#endif // 0
	VERIFY2						(object0->action_frame() != CurrentFrame(), make_string("%d %s",CurrentFrame(),*object0->object().ObjectName()));
	VERIFY2						(object0->action_frame() < CurrentFrame(), make_string("%d %s",CurrentFrame(),*object0->object().ObjectName()));

	VERIFY2						(object1->action_frame() != CurrentFrame(), make_string("%d %s",CurrentFrame(),*object0->object().ObjectName()));
	VERIFY2						(object1->action_frame() < CurrentFrame(), make_string("%d %s",CurrentFrame(),*object0->object().ObjectName()));
	...
	VERIFY2						(false,make_string("NODEFAULT: [%s][%s]",*object0->object().ObjectName(),*object1->object().ObjectName()));
}
