...
public:
	...
	IC		void			SetState			(u32 v)				{m_hud_item_state = v; m_dw_curr_state_time=EngineTimeU();ResetSubStateTime();}
	IC		void			SetNextState		(u32 v)				{m_nextState = v;}
	IC		u32				CurrStateTime		() const			{return EngineTimeU()-m_dw_curr_state_time;}
	IC		void			ResetSubStateTime	()					{m_dw_curr_substate_time=EngineTimeU();}
...
public:
	...
	virtual	BOOL				SpawnAndImportSOData(CSE_Abstract* data_containing_so)				{return TRUE;};
	...
	virtual SSnd*				PlaySound			(LPCSTR alias, const Fvector& position, float freq = 1.f);
			void				StopAllSounds		(LPCSTR exept_these = nullptr)		{ m_sounds.StopAllSounds(exept_these); };
	...
	shared_str					GetCurrentHUDMotion(){ return m_current_motion; };
	...
	virtual bool				UseScopeParams			() { return false; };
...
public:
	...
	bool                        bNeedUpdateAddons;

	virtual void				on_renderable_Render	(IRenderBuffer& render_buffer) = 0;
