...
IC	CSSafeMapIterator::CSafeMapIterator()
{
	...
	is_time_limited = use_time_limit;
...
{
	bool time_res = (is_time_limited && !m_first_update && (m_timer.GetElapsed_sec() >= m_max_process_time));
...
TEMPLATE_SPEZIALIZATION
IC	float CSSafeMapIterator::get_process_time()
{
	return m_max_process_time;
}

TEMPLATE_SPEZIALIZATION
IC	void CSSafeMapIterator::set_is_time_limited(const bool &limited)
{
