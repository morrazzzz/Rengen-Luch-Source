...
LPCSTR CScriptGameObject::GetNpcIcon()
{
	...
	if (monster) {
		shared_str icon_name = pSettings->r_string(monster->SectionName(),"icon");
...
void  CScriptGameObject::AddIconedTalkMessage(LPCSTR text, LPCSTR texture_name, const Frect& tex_rect, LPCSTR templ_name)
{
...
bool CScriptGameObject::IsInvUpgradeEnabled()
{
	...
	return pInventoryOwner->IsUpgradeEnabled(Actor());
}
...
void CScriptGameObject::IterateInventory	(luabind::functor<void> functor, luabind::object object)
{
	...
	TIItemContainer::iterator	I = inventory_owner->inventory().allContainer_.begin();
	TIItemContainer::iterator	E = inventory_owner->inventory().allContainer_.end();
	for ( ; I != E; ++I)
...
{
	DropItem(pItem);
#pragma todo("will not work, since item is droped only next frame")
	R_ASSERT(false);
	CInventoryItem* item = smart_cast<CInventoryItem*>(&pItem->object());
...
void CScriptGameObject::MakeItemActive(CScriptGameObject* pItem)
{
	...
	if (item_in_slot)
		owner->inventory().MoveToRuck(item_in_slot);

	owner->inventory().MoveToSlot(item->BaseSlot(), item);
	owner->inventory().ActivateSlot(item->BaseSlot());
}
...
int CScriptGameObject::CharacterRank()
{
	...
	if (!monster)
	{
		...
		return pInventoryOwner->GetIORank();
	}

	return monster->GetMonsterRank();
}
...
void CScriptGameObject::SetCharacterRank(int char_rank)
{
	...
	return pInventoryOwner->SetIORank(char_rank);
}
...
void CScriptGameObject::ChangeCharacterRank(int char_rank)
{
	...
	return pInventoryOwner->ChangeIORank(char_rank);
}
...
bool CScriptGameObject::has_restrictions()
{
	CCustomMonster	*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster)
	{
	...
	return Level().space_restriction_manager().has_restrictions_data(object().ID());
}
...
void CScriptGameObject::enable_attachable_item	(bool value)
{
	...
	//Msg("enable_attachable_item for [%s][%d]", attachable_item->object().SectionName().c_str(), value);
...
bool CScriptGameObject::attachable_item_enabled	() const
{
	...
	//Msg("attachable_item_enabled for [%s]", attachable_item->object().SectionName().c_str());
...
bool CScriptGameObject::night_vision_enabled() const
{
	...
	return									(Actor()->GetActorNightVisionStatus());
}
...
CScriptGameObject *CScriptGameObject::active_detector() const
{
	...
	if (result) {
		CArtDetectorBase *detector = smart_cast<CArtDetectorBase*>(result);
		VERIFY(detector);
...
CGameTask* CScriptGameObject::GetTask(LPCSTR id)
{
	return Level().GameTaskManager().HasGameTask(id);
}

void CScriptGameObject::SetActiveTask(LPCSTR task_id, int objective_num)
{
	Level().GameTaskManager().SetActiveTask(task_id, objective_num);
}
...
void CScriptGameObject::deactivate_slot()
{
	...
	inventory_owner->inventory().ActivateSlot(NO_ACTIVE_SLOT);
}
...
bool CScriptGameObject::MoveToSlot(CScriptGameObject *obj, bool not_activate)
{
	...
	R_ASSERT2(obj->ID() <= MAX_OBJ_IDS, "Possible bug here, contact coders");

	item = actor->inventory().GetItemByID((u16)obj->ID(), actor->inventory().allContainer_);
	if (!item)
		return false;

	return actor->inventory().MoveToSlot(item->BaseSlot(), item, not_activate);
}
...
bool CScriptGameObject::MoveToRuck(CScriptGameObject *obj)
{
	...
	return actor->inventory().MoveToRuck(item);
}

void CScriptGameObject::RemoveFromInventory(u32 obj_id)
{
	R_ASSERT2(obj_id <= MAX_OBJ_IDS, make_string("obj_id is beyond of max objects ids. %u", obj_id));
...
void CScriptGameObject::activate_slot	(u32 slot_id)
{
	...
	inventory_owner->inventory().ActivateSlot((TSlotId)(slot_id + 1));
}
...
void CScriptGameObject::SetTorchBatteryStatus(float value)
{
	CTorch *torch = smart_cast<CTorch*>(&object());
	if (torch)
	{
...
float CScriptGameObject::GetTorchBatteryStatus()
{
	CTorch *torch = smart_cast<CTorch*>(&object());
	if (torch)
...
bool CScriptGameObject::suitable_smart_cover(CScriptGameObject* object)
{
	...
	if (inventory_item)
		return							(inventory_item->BaseSlot() == RIFLE_SLOT);

	CInventoryItem const* best_weapon = stalker->best_weapon();
	if (!best_weapon)
		return							(false);

	return								(best_weapon->BaseSlot() == RIFLE_SLOT);
}
...
{
	VERIFY2(!m_door, make_string("object %s has been registered as a door already", m_game_object->ObjectName().c_str()));
...
{
	VERIFY2(m_door, make_string("object %s is not a door", m_game_object->ObjectName().c_str()));
...
{
	VERIFY2(m_door, make_string("object %s hasn't been registered as a door already", m_game_object->ObjectName().c_str()));
