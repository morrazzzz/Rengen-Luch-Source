...
void CSightManager::LoadCfg			(LPCSTR section)
{
}
...
void CSightManager::Exec_Look		(float time_delta)
{
...
#ifdef SIGHT_DEBUG
	if ( object().ObjectName() == "level_prefix_stalker" ) {
		Msg				("[%6d][%s] BEFORE BODY [%f] -> [%f]",EngineTimeU(), object().ObjectName().c_str(), object().movement().m_body.current.yaw,object().movement().m_body.target.yaw);
		Msg				("[%6d][%s] BEFORE HEAD [%f] -> [%f]",EngineTimeU(), object().ObjectName().c_str(), object().movement().m_head.current.yaw,object().movement().m_head.target.yaw);
	}
...
#ifdef DEBUG
	if ( g_ai_dbg_sight )
		Msg							( "%6d [%s] before body[%f]->[%f], head[%f]->[%f]", EngineTimeU(), object().ObjectName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
	vfValidateAngleDependency		(body.current.yaw,body.target.yaw,head.current.yaw);
#ifdef DEBUG
	if ( g_ai_dbg_sight )
		Msg							( "%6d [%s] after  body[%f]->[%f], head[%f]->[%f]", EngineTimeU(), object().ObjectName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
...
#ifdef DEBUG
	if ( g_ai_dbg_sight )
		Msg							( "%6d [%s] after2 body[%f]->[%f], head[%f]->[%f]", EngineTimeU(), object().ObjectName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
	...
	if ( object().ObjectName() == "level_prefix_stalker" ) {
		Msg				("[%6d][%s] AFTER  BODY [%f] -> [%f]",			EngineTimeU(), object().ObjectName().c_str(),object().movement().m_body.current.yaw,object().movement().m_body.target.yaw);
		Msg				("[%6d][%s] AFTER  HEAD [%f][%f] -> [%f][%f]",	EngineTimeU(), object().ObjectName().c_str(), object().movement().m_head.current.yaw,object().movement().m_head.current.pitch,object().movement().m_head.target.yaw,object().movement().m_head.target.pitch);
	}
#endif // #ifdef SIGHT_DEBUG
...
#ifdef DEBUG
	if ( g_ai_dbg_sight )
		Msg							( "%6d [%s] after3 body[%f]->[%f], head[%f]->[%f]", EngineTimeU(), object().ObjectName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
...
//			Msg				("%6d started turning in place",EngineTimeU());
...
//		object().movement().m_body.target.yaw	= object().movement().m_head.current.yaw;
		...
		m_turning_in_place	= false;
//		Msg					("%6d stopped turning in place",EngineTimeU());
		object().movement().m_body.target.yaw	= object().movement().m_body.current.yaw;
	}
...
//	if ( object().ObjectName() == "level_prefix_stalker" ) {
//		Msg							("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  CurrentFrame(), EngineTimeU(), m_current.m_head.m_factor,		s_factor_lerp_speed*time_delta,		lerp ( m_current.m_head.m_factor,		factors.x, s_factor_lerp_speed*time_delta ), factors.x );
//		Msg							("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  CurrentFrame(), EngineTimeU(), m_current.m_shoulder.m_factor,	s_factor_lerp_speed*time_delta,		lerp ( m_current.m_shoulder.m_factor,	factors.y, s_factor_lerp_speed*time_delta ), factors.y );
//		Msg							("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  CurrentFrame(), EngineTimeU(), m_current.m_spine.m_factor,	s_factor_lerp_speed*time_delta,		lerp ( m_current.m_spine.m_factor,		factors.z, s_factor_lerp_speed*time_delta ), factors.z );
//	}
	...
	switch (m_aiming_type) {
		...
		case aiming_weapon : {
			...
			aimers::weapon		aimer(
				...
				aiming_position(),
				pSettings->r_string(object().SectionName().c_str(),"bone_spin"),
				pSettings->r_string(object().SectionName().c_str(),"bone_shoulder"),
				pSettings->r_string(object().SectionName().c_str(),"weapon_bone0"),
				pSettings->r_string(object().SectionName().c_str(),"weapon_bone2"),
		...
		case aiming_head : {
			...
			LPCSTR	bones[]		=
			{
				pSettings->r_string(object().SectionName().c_str(),"bone_spin"),
				pSettings->r_string(object().SectionName().c_str(),"bone_shoulder"),
				pSettings->r_string(object().SectionName().c_str(),"bone_head"),
			};
...
void CSightManager::enable							(bool const value)
{
	...
	m_enabled			= value;
//	Msg					("[%d][%s] sight_enabled[%c]", EngineTimeU(), object().ObjectName().c_str(), value ? '+' : '-');
