...
{
	VERIFY2(RC_float == C->type, make_string("C->type = %u, Engine - Shader constant type missmatch", C->type));
...
{
	VERIFY2(RC_int == C->type, make_string("C->type = %u, Engine - Shader constant type missmatch", C->type));
	VERIFY(RC_1x1 == L.cls);
...
IC void dx10ConstantBuffer::seta(R_constant* C, R_constant_load& L, u32 e, const Fvector4& A)
{
	...
	static const u16 line_size = 4 * sizeof(float);
	u32 base = (u32)L.index + line_size *e;
	Fvector4* it = Access((u16)base);

	VERIFY((base + line_size) <= m_uiBufferSize);
