...
SVS* CResourceManager::_CreateVS(LPCSTR _name)
{
	...
	else
	{
		...
		CHECK_OR_EXIT(!FAILED(_hr), make_string("Shader compilation error\n\n Check Log."));
...
CRT* CResourceManager::_CreateRT(LPCSTR Name, xr_vector<RtCreationParams>& vp_params, D3DFORMAT f, u32 SampleCount, bool useUAV)
{
	...
	else
	{
		...
		if (Device.b_is_Ready)
			RT->create(Name, vp_params, f, SampleCount, useUAV);
...
CTexture* CResourceManager::_CreateTexture(LPCSTR _Name, bool mt_anyway)
{
	...
	else
	{
		...
		if (Device.b_is_Ready && !bDeferredLoad)
			T->LoadTexture(false, mt_anyway);

		return T;
	}
}

void CResourceManager::_DeleteTextureRef(const CTexture* T)
{
