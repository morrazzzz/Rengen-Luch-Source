...
IC void CBackend::BackendRender(D3DPRIMITIVETYPE t, u32 baseV, u32 startV, u32 countV, u32 startI, u32 PC)
{
	//VERIFY(vs);
...
IC void CBackend::BackendRender(D3DPRIMITIVETYPE t, u32 startV, u32 PC)
{
...
IC void CBackend::ApplyVertexLayout()
{
	...
	if (it == decl->vs_to_layout.end())
	{
		...
		res = HW.pDevice->CreateInputLayout(&decl->dx10_dcl_code[0], decl->dx10_dcl_code.size() - 1, m_pInputSignature->GetBufferPointer(), m_pInputSignature->GetBufferSize(), &pLayout);
...
ICF void CBackend::ApplyRTandZB()
{
	if (m_bChangedRTorZB)
	{
		...
		{
			bool result = false;
			for (size_t i = 0; i < sizeof(pRT) / sizeof(pRT[0]); i++)
			{
				result |=!! pRT[i];
			}
			VERIFY(result);
