...
ECamEffectorType   CCameraManager::RequestCamEffectorId ()
{
	...
	for (index	= (ECamEffectorType)effCustomEffectorStartID; GetCamEffector(index); index = (ECamEffectorType)(index + 1) ) 
		{ ; }
...
EEffectorPPType   CCameraManager::RequestPPEffectorId ()
{
	...
	for (index	=	(EEffectorPPType)effCustomEffectorStartID; GetPPEffector(index); index = (EEffectorPPType)(index + 1)) 
		{ ; }
...
void CCameraManager::Update(const Fvector& P, const Fvector& D, const Fvector& N, float fFOV_Dest, float fASPECT_Dest, float fFAR_Dest, u32 flags)
{
#ifdef DEBUG
	if (!Device.Paused()) {
		VERIFY				(dbg_upd_frame != CurrentFrame());// already updated !!!
		dbg_upd_frame		= CurrentFrame();
	}
#endif // DEBUG
	// camera
	...
	float src					= 10*TimeDelta();	clamp(src,0.f,1.f);
	float dst					= 1-src;
...
extern ENGINE_API float camAspectK;
ENGINE_API float hudFovRuntime_K = 1.f; // for aiming in CWeapon
...
void CCameraManager::ApplyDeviceInternal(float _viewport_near)
{
	...
	Device.fASPECT				= m_cam_info.fAspect * camAspectK;
	...
	if (Render->currentViewPort == SECONDARY_WEAPON_SCOPE)
	{
		...
		Device.fFOV = g_pGamePersistent->weaponSVPShaderParams->svpParams.y;
		// Предупреждаем что мы изменили настройки камеры
		Device.m_SecondViewport.isCamReady = true;

		Device.vpSavedView2.fFov = g_pGamePersistent->weaponSVPShaderParams->svpParams.y;
	}
	else
		Device.m_SecondViewport.isCamReady = false;

	Device.mProject.build_projection(deg2rad(Device.fFOV), Device.fASPECT, _viewport_near * viewPortNearK, m_cam_info.fFar);
	...
	if (Render->currentViewPort == SECONDARY_WEAPON_SCOPE)
	{
		...
		if(psSVP1FrustumOptimize)
			second_vp_shrinked_project_m.build_projection(deg2rad(Device.fFOV * psSVP1FrustumFovK), psSVP1FrustumWidthK, psSVP1FrustumHeightK, _viewport_near * viewPortNearK, m_cam_info.fFar);
		else
			second_vp_shrinked_project_m.build_projection(deg2rad(Device.fFOV), Device.fASPECT, _viewport_near * viewPortNearK, m_cam_info.fFar);
		...
		Device.vpSavedView2.SetShrinkedFullTransform_saved(second_vp_shrinked_full_m);
	}
