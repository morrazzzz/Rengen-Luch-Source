...
void CConsole::Register_callbacks()
{
	...
	ec().assign_callback( DIK_PRIOR, text_editor::ks_Shift,  Callback( this, &CConsole::Page_up      ) );
	ec().assign_callback( DIK_NEXT,  text_editor::ks_Shift,  Callback( this, &CConsole::Page_down    ) );
}
...
void CConsole::Page_up() // PAGE_UP+Shift
{
	scroll_delta += 16;
	if (scroll_delta > int(LogFile->size()) - 1)
	{
...
void CConsole::Page_down() // PAGE_DOWN+Shift
{
	scroll_delta -= 16;
	if (scroll_delta < 0)
	{
...
void CConsole::End_tips()
{
	...
	m_start_tip = m_select_tip - (NeedFullOpen() ? VIEW_TIPS_COUNT : VIEW_TIPS_COUNT_2) + 1;
	check_next_selected_tip();
}

void CConsole::PageUp_tips()
{
	m_select_tip -= (NeedFullOpen() ? VIEW_TIPS_COUNT : VIEW_TIPS_COUNT_2);
