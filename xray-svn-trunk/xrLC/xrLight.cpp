...
void CBuild::Light()
{
	...
	//Lmaps
	{
		...
		thNUM = (CPU::GetLogicalCoresNum()) - 1; // one less than all

		clamp(thNUM, (u32)1, thNUM); // minimum 1 thread for dual or single threaded cpu

		if (strstr(Core.Params, "-threads "))
			sscanf(strstr(Core.Params, "-threads ") + xr_strlen("-threads "), "%d", &thNUM);
...
void CBuild::LightVertex()
{
	...
	NUM_THREADS = (CPU::GetLogicalCoresNum() / 2) - 1; // one less than half
