...
void			base_Face::CacheOpacity	()
{
	...
	if (M.surfidx >= pBuild->textures.size())
		R_ASSERT2(false, "Count of loaded textures is less than material texture index");
	...
	{
		flags.bOpaque	= true;
		if (strangeFaceLogging) clMsg("Strange face detected... Has alpha without texture... Texture name = [%s]. Material index %u", T.name, dwMaterial);
	}
}
