...
void CBuild::BuildRapid		(BOOL bSaveForOtherCompilers)
{
	...
	for (vecFaceIt it=g_faces.begin(); it!=g_faces.end(); it++)
	{
		...
		if (!bAlready) 
		{
			...
			CL.add_face_ptr		( F->v[0]->P,F->v[1]->P,F->v[2]->P ,F );
		}
	}
	...
	*/
	...
	clMsg("Exporting cform rcast: Size = %u", mu_refs.size());
	CTimer T; T.Start();
	...
	clMsg("Exporting cform rcast: Finished. Time spent = %fms", T.GetElapsed_sec() * 1000.f);
	...
	if (bSaveForOtherCompilers)
	{
		...
		for (u32 k=0; k<CL.getTS(); k++){
			CDB::TRI& T			= CL.getT( k );
			base_Face* F = reinterpret_cast<base_Face*>(T.GetPTR());
		...
		for (size_t i = 0; i < CL.getTS(); i++)
			MFS->w(&CL.getT()[i], CDB::TRI::GetSizeof());
		MFS->close_chunk		();
