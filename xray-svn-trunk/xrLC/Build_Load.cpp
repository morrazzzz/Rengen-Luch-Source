...
void CBuild::Load(const b_params& Params, const IReader& _in_FS)
{
	...
	Status("Processing textures...");
	{
		...
		size_t size_b_texture = sizeof(b_texture);
#ifdef _WIN64
		size_b_texture -= 4;
#endif
		u32 tex_count = F->length() / size_b_texture;

		VERIFY(F->length() % size_b_texture == 0);
		for (u32 t = 0; t < tex_count; t++)
		{
			Progress(float(t) / float(tex_count));

			b_texture		TEX;
			F->r(&TEX, size_b_texture);
			...
			else
			{
				...
				if (!THM) // KD: first part of textures fix - start. no nead to die if there is no texture
				{
				...
				VERIFY(sizeof(STextureParams::ETFormat) == 4);
