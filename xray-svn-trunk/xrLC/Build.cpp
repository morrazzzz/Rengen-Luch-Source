...
BOOL					b_notessellation = FALSE;
// KD start
BOOL					b_norgb = FALSE;
BOOL					b_skipinvalid = FALSE;
float					f_lmap_quality = 0.f;
u32						f_lmap_j_quality = 0;
// KD end

bool					strangeFaceLogging = true;
bool					lighLogging = false;
...
public:
	...
	{
		thMessages = FALSE; low = _low; high = _high;
	}
...
public:
	...
	virtual void	Execute()
	{
		...
		MU_THREADS = CPU::GetLogicalCoresNum() - 2; // use 2 less than all of processor threads

		clamp(MU_THREADS, (u32)1, MU_THREADS); // minimum 1 thread for dual or single threaded cpu

		if (strstr(Core.Params, "-threads "))
			sscanf(strstr(Core.Params, "-threads ") + xr_strlen("-threads "), "%d", &MU_THREADS);
