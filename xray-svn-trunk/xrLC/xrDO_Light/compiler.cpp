...
public:
	...
	base_color()
	{
		rgb.set(0, 0, 0); hemi = 0; sun = 0; _tmp_ = 0;
	}
...
{
	s32 _a = iFloor(a*255.f); clamp(_a, 0, 255); return u8(_a);
};

const int	LIGHT_Count = 7;

__declspec(thread)		double		t_time = 0;
...
void xrLoad(LPCSTR name)
{
	...
	// Load CFORM
	{
		...
		Msg("Load CFORM: Openning build.cform [%s]", N);

		R_ASSERT(FS.exist("$level$", "build.cform"));
	...
	{
		// copy
		Msg("Load details: Openning build.details");

		R_ASSERT(FS.exist("$level$", "build.details"));

		string_path	fname;
		FS.update_path(fname, "$level$", "level.details");

		FS.BackUpFile(fname, true);
	...
	// Lights
	{
		Msg("Load Lights: Openning build.lights");
	...
	// Load level data
	{
		Msg("Load level data: Openning build.prj");
			...
			for (u32 t = 0; t < tex_count; t++)
			{
				...
				else
				{
					...
					if (!THM)
					{
						...
						if (strstr(Core.Params, "-allow_replace_thm_tga"))
						{
							string_path message;
							xr_sprintf(message, "Cannot Find THM %s, Do you wan't to use default one?", th_name);

							int	result = MessageBox(NULL, message, "Error reading THM", MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON1 | MB_TASKMODAL | MB_TOPMOST);

							switch (result)
							{
							case IDYES: {
								FS.update_path(th_name, "$textures$", strconcat(sizeof(th_name), th_name, "no_thm", ".thm"));

								THM = FS.r_open(th_name);

								if (!THM)
								{
									R_ASSERT2(false, make_string("Please create dummy thm: %s", th_name));
					...
					if (!bLOD)
					{
						if (BT.bHasAlpha || BT.THM.flags.test(STextureParams::flImplicitLighted))
						{
							...
							if (!BT.pSurface)
							{
								...
								{
									string_path message;
									xr_sprintf(message, "Cannot Find TGA %s, Do you wan't to use default one?", N);

									int	result = MessageBox(NULL, message, "Error reading TGA", MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON1 | MB_TASKMODAL | MB_TOPMOST);

									switch (result)
									{
									case IDYES: {
										BT.pSurface = Surface_Load("no_tga", w, h);

										if (!BT.pSurface)
										{
											R_ASSERT2(false, make_string("Please create dummy tga texture: %s", FS.combine("$textures$", "no_tga", ".tga").c_str()));
...
//				FPU::m64r();
				...
				thPerformance = float(double(t_count) / t_time);
			}
		}
	}
};
...
void	xrLight()
{
	...
	NUM_THREADS = CPU::GetLogicalCoresNum() - 1; // leave one thread unused for system and shits
	...
	Msg("^LIGHT THREADS %u", NUM_THREADS);
