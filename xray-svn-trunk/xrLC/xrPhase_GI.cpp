...
static Fvector		GetPixel_7x7		(CDB::RESULT& rpinf)
{
	...
	base_Face* F										= reinterpret_cast<base_Face*>(clT.GetPTR());
	if (0==F)											return R;
...
public:
	...
	virtual void	Execute	()
	{
		...
		GI_THREADS = CPU::GetLogicalCoresNum() - 1; // leave one thread unused for system and shits

		clamp(GI_THREADS, (u32)1, GI_THREADS); // minimum 1 thread for dual or single threaded cpu

		if (strstr(Core.Params, "-threads "))
			sscanf(strstr(Core.Params, "-threads ") + xr_strlen("-threads "), "%d", &GI_THREADS);
