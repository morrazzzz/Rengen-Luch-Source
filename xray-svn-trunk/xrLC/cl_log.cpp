...
//	if (stageTimer_.IsStarted())
		Msg("* Phase time: %f ms", stageTimer_.GetElapsed_sec() * 1000.f);
...
{
	CTimer total_timer; total_timer.Start();
	...
	Msg("\"Rentgen-Luch Level Builder v5.0\"\nCompilation date: %s\n",__DATE__);
	{
		char tmpbuf[128];
...
*/
	...
	while (TRUE)
	{
		...
		if (LogSize - LogFile->size() > 150) // save log every N lines
			FlushLog(false);

		bool keep_top_index = false;

		if (((int)SendMessage(hwLog, LB_GETCURSEL, 0, 0) == LogSize - 1)) // Check if user did not change selected line
			keep_top_index = true;

		if (LogSize - LogFile->size() > 1)
		{
			bWasChanges		= TRUE;
			...
			if (keep_top_index) // scroll to uptodate line, if user didnt change it manualy
				SendMessage(hwLog, LB_SETCURSEL, LogSize - 1, 0);
		}
		...
		if (progress > 0.005f) {
			...
			xr_sprintf(tbuf, "Remain:     %s\nElapsed:    %s\nTotal time: %s", make_time(secRemain).c_str(), make_time(secElapsed).c_str(), make_time((u32)total_timer.GetElapsed_sec()).c_str());
			SetWindowText(hwTime, tbuf);
		}
		else {
			xr_sprintf(tbuf, "\nElapsed:    %s\nTotal time: %s", make_time(secElapsed).c_str(), make_time((u32)total_timer.GetElapsed_sec()).c_str());
			SetWindowText(hwTime, tbuf);
		}
