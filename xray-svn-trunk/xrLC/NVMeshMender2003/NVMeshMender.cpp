								...
								D3DXVECTOR3 workingPosition)
{
		...
		for( unsigned int i = 0; i < neighborGroups.size(); ++i )
		{
			...
			R_ASSERT2(curGroup.size() != 0, "should not be a zero group here.");
								...
								const float& minCreaseAngle)
{
	...
	R_ASSERT2((tri->group != NO_GROUP), "error!: tri should have a group set");
	tri->handled = true;
...
bool MeshMender::SharesEdgeRespectSplits(	Triangle* triA, 
				...
				xr_vector< Vertex >& theVerts)
{
	R_ASSERT2(triA && triB, "invalid data passed to SharesEdgeNoSplit");
...
bool MeshMender::SharesEdge(Triangle* triA, 
				...
				xr_vector< Vertex >& theVerts)
{
	R_ASSERT2(triA && triB, "invalid data passed to SharesEdge");
...
void MeshMender::SetUpData(
				  ...
				  const NormalCalcOption computeNormals)
{
	R_ASSERT2(((theIndices.size() % 3) == 0), "expected the indices to be a multiple of 3");
	unsigned int i;
						...
						xr_vector< Vertex >&   theVerts )
{
	...
	for(size_t i = 0 ; i < len ; ++ i )
	{
		...
		R_ASSERT2(D3DXVec3Length(&(theVerts[i].normal)) > 0.00001f,
...
void MeshMender::UpdateTheIndicesWithFinalIndices(xr_vector< unsigned int >& theIndices )
{
	...
	R_ASSERT2(((theIndices.size() / 3) == m_Triangles.size()), "invalid number of tris, or indices.");
