...
void read_levels(CInifile *Ini, xr_set<CLevelInfo> &levels, bool rebuild_graph, xr_vector<LPCSTR> *needed_levels)
{
	...
	for (u32 k = 0; Ini->r_line("levels",k,&_N,&V); k++) {
		...
		if (!FS.exist(file_name)) {
			Msg("! There is no ai-map for the level %s! (level is not included into the game graph). Path: %s", S, file_name);
			continue;
		}
	...
	)
{
	...
	for ( ; I != E; ++I) {
		...
		generate_temp_file_name		("local_graph_",*tLevel.m_name, levelgraph);
		generate_temp_file_name		("raw_cross_table_",*tLevel.m_name, levelgct);
		string_path					level_folder;
		FS.update_path				(level_folder,"$game_levels$",*tLevel.m_name);
		xr_strcat						(level_folder,"\\");
		CGameGraphBuilder().build_graph	(levelgraph, levelgct, level_folder);
...
//					(*K).second->vfAddEdge		((*M).second.tGraphID,tGraphEdge);
	...
	FS.BackUpFile					(l_caFileName, true);
