...
#include "../xr_3da/xrGame/graph_engine.h"
#include "../xr_3da/xrGame/xrmessages.h"
#include "../xr_3da/xrGame/xrServer_Objects_ALife_All.h"
#include "factory_api.h"
#include "../xr_3da/xrGame/clsid_game.h"
#include "game_spawn_constructor.h"
#include "../xr_3da/xrGame/patrol_path_storage.h"
#include "space_restrictor_wrapper.h"
#include "../xr_3da/xrGame/object_broker.h"
#include "../xr_3da/xrGame/restriction_space.h"
...
//	chunk->close			();
	...
	{
		string256			temp;
		xr_sprintf			(temp, sizeof(temp), "Can't create entity '%s' !\n", section_name);
...
void CLevelSpawnConstructor::correct_objects					()
{
	...
	if (dwStart >= dwFinish) {
		string4096			S;
		xr_sprintf				(S, sizeof(S), "There are no graph vertices in the game graph for the level '%s' !\n",*m_level.name());
	...
	for (int i=0; i<(int)m_spawns.size(); i++) {
		...
		if (m_spawns[i]->used_ai_locations() && !level_graph().inside(level_graph().vertex(m_spawns[i]->m_tNodeID),position)) {
			...
			clMsg("[%s][%s][%s] : position changed from [%f][%f][%f] -> [%f][%f][%f]; Reason : used_ai_locations", *m_level.name(), *m_spawns[i]->s_name, m_spawns[i]->name_replace(), VPUSH(position), VPUSH(new_position));
		...
		if (game_graph().vertex(dwBest)->level_id() != m_level.id()) {
			string4096	S1;
			char		*S = S1;
			S			+= xr_sprintf(S, sizeof(S), "Corresponding graph vertex for the spawn point is located on the ANOTHER level\n",m_spawns[i]->name_replace());
			S			+= xr_sprintf(S, sizeof(S), "Current level  : [%d][%s]\n",m_level.id(),*game_graph().header().level(m_level.id()).name());
			S			+= xr_sprintf(S, sizeof(S), "Conflict level : [%d][%s]\n",game_graph().vertex(dwBest)->level_id(),*game_graph().header().level(game_graph().vertex(dwBest)->level_id()).name());
			S			+= xr_sprintf(S, sizeof(S), "Probably, you filled offsets in \"game_levels.ltx\" incorrect");
		...
		if (dwBest == u32(-1)) {
			string4096	S1;
			char		*S = S1;
			S			+= xr_sprintf(S, sizeof(S), "Can't find a corresponding GRAPH VERTEX for the spawn-point %s\n",m_spawns[i]->name_replace());
			S			+= xr_sprintf(S, sizeof(S), "Level ID    : %d\n",m_level.id());
			S			+= xr_sprintf(S, sizeof(S), "Spawn index : %d\n",i);
			S			+= xr_sprintf(S, sizeof(S), "Spawn node  : %d\n",m_spawns[i]->m_tNodeID);
			S			+= xr_sprintf(S, sizeof(S), "Spawn point : [%7.2f][%7.2f][%7.2f]\n",m_spawns[i]->o_Position.x,m_spawns[i]->o_Position.y,m_spawns[i]->o_Position.z);
