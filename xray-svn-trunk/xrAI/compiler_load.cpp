...
void xrLoad(LPCSTR name, bool draft_mode)
{
	...
	if (!draft_mode)	{
		...
		// Load CFORM
		{
			...
			R_ASSERT2(FS.exist(N), make_string("build.cform is missing!"));

			if (!FS.exist(N))
			{
				Msg("!Nothing specified or internal problem. Exiting");
			...
			R_ASSERT2(fs, make_string("FS cant open '%s'", N));
		...
		// Load level data
		{
			...
			Msg("Load level data: Openning build.prj [%s]", N);
			...
			Status			("Processing textures...");
			{
				...
				for (u32 t = 0; t < tex_count; t++)
				{
					...
					else
					{
						...
						xr_sprintf(N, "%s.thm", N);

						IReader* THM;

						R_ASSERT2(FS.exist("$textures$", N), make_string("%s %s does not exist", "$textures$", N));

						if (!FS.exist("$textures$", N))
							continue;

						THM = FS.r_open("$textures$", N);
...
//
	// Load lights
	{
		...
		Msg("Load lights: Openning build.prj [%s]", N);
...
//	g_params.Init		();
	...
	// Load initial map from the Level Editor
	{
		...
		Msg("Load initial map from the Level Editor: Openning build.aimap [%s]", file_name);
