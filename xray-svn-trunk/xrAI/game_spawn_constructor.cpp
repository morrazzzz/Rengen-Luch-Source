...
#include "xrai.h"
#include "../xr_3da/xrGame/server_entity_wrapper.h"
...
#pragma todo("Implement check for changed files, to decide if we need to rebuild graph. Make graph building to rebuild only changed 'level.graph's. Make a launch key 'compile_game' which will make building of all stages and making build only changed files")
	if (!FS.exist(game_graph_name))
	{
		Msg("Building game.graph, since it is missing");

		xrMergeGraphs(name, false);
	}
...
void CGameSpawnConstructor::save_spawn(LPCSTR output)
{
	...
	LPCSTR file_name				= *spawn_name(output);

	FS.BackUpFile					(file_name, true);
...
shared_str CGameSpawnConstructor::spawn_name(LPCSTR output)
{
	...
	Msg("Output spawn file path %s", file_name);
