...
AccessLock				protectFilesVector;
AccessLock				protectFileOpenning;
#endif

AccessLock	fileRegistrationLock;
...
LPCSTR GetGameDataPath()
{
	static string_path gameDataPath;
	if (FS.path_exist("$game_data$"))
	{
		xr_strcpy(gameDataPath, FS.get_path("$game_data$")->m_Path);
		auto len = xr_strlen(gameDataPath);
		if (gameDataPath[len-1] == '\\') gameDataPath[len-1] = '\0';
	}
	else
	{
		FS_Path* pFSRoot = FS.get_path("$fs_root$");
		strconcat(sizeof(gameDataPath), gameDataPath, pFSRoot->m_Path, "gamedata");
	}
	return gameDataPath;
}
...
//	if file already exist - update info
	LockFilesVectorMutex();
	files_it			I = files.find(desc);
	UnlockFilesVectorMutex();
	if (I != GetFilesEndIter()) {
		desc.name		= I->name;
	...
	while (temp[0]) 
	{
		...
		if (!exist(path))	
		{
            ...
            desc.modif			= u32(-1);
            std::pair<files_it,bool> fit = files.insert(desc); 
...
void CLocatorAPI::ProcessArchive(LPCSTR _path, LPCSTR base_path)
{
	...
	Msg("Processing archive %s.", _path);
...
void CLocatorAPI::ProcessOne(const char* path, const _finddata_t& F)
{
	...
	if (F.attrib&_A_SUBDIR) {
		...
		if (ext && 0==strncmp(ext,".db",3)) {
			ProcessArchive(N);
		} else if (ext && 0==strncmp(ext,".xdb",4)) {
			ProcessArchive(N, GetGameDataPath());
		} else {
			Register(N,0xffffffff,0,0,F.size,F.size,(u32)F.time_write);
		}
	}
...
IC bool is_find_dir(const _finddata_t& x) {
	return (x.attrib & _A_SUBDIR) > 0;
}

IC bool pred_str_ff(const _finddata_t& x, const _finddata_t& y)
{
	if (is_find_dir(x) != is_find_dir(y)) {
		return !is_find_dir(x);
	}
...
*/
	...
	Msg("* Init FileSystem %f ms", t.GetElapsed_sec() * 1000.f);
	//-----------------------------------------------------------
...
const CLocatorAPI::file* CLocatorAPI::exist			(const char* fn)
{
	...
	return (it != GetFilesEndIter()) ? &(*it) : 0;
}
...
const CLocatorAPI::file* CLocatorAPI::exist2		(LPCSTR path, LPCSTR name, LPCSTR ext)
{
	string_path		nm;

	strconcat(sizeof(nm), nm, name, ext);
	update_path(nm, path, nm);

	return			exist(nm);
}

std::string CLocatorAPI::combine(LPCSTR path, LPCSTR name, LPCSTR ext)
{
	string_path nm;
...
void CLocatorAPI::combine(string_path& res, LPCSTR path, LPCSTR name, LPCSTR ext)
{
	string_path nm;

	strconcat(sizeof(nm), nm, name, ext);
	update_path(res, path, nm);
}
...
void CLocatorAPI::file_rename(LPCSTR src, LPCSTR dest, bool bOwerwrite)
{
	...
	if (S != GetFilesEndIter()){
		files_it D		= file_find_it(dest);
		if (D != GetFilesEndIter()){
...
int	CLocatorAPI::file_length(LPCSTR src)
{
	...
	return (I != GetFilesEndIter()) ? I->size_real : -1;
}
...
u32 CLocatorAPI::get_file_age(LPCSTR nm)
{
	...
	return (I != GetFilesEndIter()) ? I->modif : u32(-1);
}
...
void CLocatorAPI::ProcessExternalArch()
{
	...
	for( ;it!=it_e; ++it)
	{
		...
		ProcessArchive		(full_mod_name, GetGameDataPath());
	}
}

void CLocatorAPI::BackUpFile(LPCSTR fn, bool bDeleteSource)
{
	if (exist(fn))
	{
		R_ASSERT(fn);
		//R_ASSERT2(strstr(fn, "."), make_string("File name better to have extension: %s", fn));

		xr_string ext = ".no_extension";
		...
		Msg("* Backing up %s into %s", fn, backup_fn.c_str());

		if (bDeleteSource)
		{
			file_rename(fn, backup_fn.c_str(), true);
		}
		else
		{
			file_copy(fn, backup_fn.c_str());
		}
	}
}

IC CLocatorAPI::files_it CLocatorAPI::GetFilesEndIter()
{
#if defined __BORLANDC__
	return files.end();
#else
	protectFilesVector.Enter();
	files_it end_iter = files.end();
	protectFilesVector.Leave();
	return end_iter;
#endif
}

IC void CLocatorAPI::LockFilesVectorMutex()
{
#if defined __BORLANDC__
...
IC void CLocatorAPI::UnlockFilesVectorMutex()
{
#if defined __BORLANDC__
	//Do nothing in editor
#else
