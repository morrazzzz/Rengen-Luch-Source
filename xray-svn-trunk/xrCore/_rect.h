...
public:
	...
	Self(_vector2<T> t, _vector2<T> tt)
	{
		lt = t;
		rb = tt;
	};

	Self(T X1, T Y1 = (T)0, T X2 = (T)0, T Y2 = (T)0)
	{
		lt.set(X1, Y1);
		rb.set(X2, Y2);
	};
	...
	// create with one point and a size for x and y
	IC	SelfRef	set2(const T base_x, const T base_y, const T size_x, const T size_y)		{ x1 = base_x; y1 = base_y; x2 = base_x + size_x; y2 = base_y + size_y; return *this; };
	// create with two vectors, representing min and max limit
	IC	SelfRef	set(const Tvector &mn, const Tvector &mx)									{ x1 = mn.x; y1 = mn.y; x2 = mx.x; y2 = mx.y; return *this; };
	// create from other rect
	IC	SelfRef	set(const Self &r)															{ x1 = r.x1; y1 = r.y1;	x2 = r.x2; y2 = r.y2; return *this; };
	IC	SelfRef	null( )										{ lt.null(); rb.null(); return *this; };
