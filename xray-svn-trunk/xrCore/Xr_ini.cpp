...
CInifile::~CInifile( )
{
	...
	catch (...)
	{
		...
		MessageBox(NULL, m_file_name, "Error while deleting ini interface for", MB_OK | MB_ICONERROR | MB_TASKMODAL);

		Msg("Error while deleting ini interface for [%s]", m_file_name);
	}
}
...
LPCSTR	CInifile::r_string(LPCSTR S, LPCSTR L)const
{
	...
	if (A != I.Data.end() && xr_strcmp(*A->first, L) == 0)
		...
		Debug.fatal(DEBUG_INFO, "Can't find variable(line) %s in section [%s]. Using '0' value", L, S);
    ...
    if (it != data.Data.end()) 
	{
    	...
    	if (0==xr_strcmp(*it->first, *I.first)) 
		{
			if (E_IMGE == false) {
				...
				Msg("!Can't save texture [%s] to ltx, it's already exist in section [%s]",line,sect);
			}
            *it  = I;
		} else 
		{
