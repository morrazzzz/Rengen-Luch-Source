...
public:
	...
	const_reference operator[]	(size_type _Pos) const				{ { VERIFY(_Pos < size()); if (_Pos >= size()){ LogStackTrace("_Pos < size()"); Msg("Size %u, pos %u", size(), _Pos); } } return (*(begin() + _Pos)); }
	reference operator[]		(size_type _Pos)					{ { VERIFY(_Pos < size()); if (_Pos >= size()){ LogStackTrace("_Pos < size()"); Msg("Size %u, pos %u", size(), _Pos); } } return (*(begin() + _Pos)); }
};
