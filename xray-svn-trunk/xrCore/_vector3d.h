...
public:
	...
	Self(TYPE X, TYPE Y = (T)0, TYPE Z = (T)0)
	{
		x = X;
		y = Y;
		z = Z;
...
IC void spline1(float t, Fvector* p, Fvector* ret)
{
	float t2 = t * t;
	float t3 = t2 * t;
	float m[4];
...
IC void spline2(float t, Fvector* p, Fvector* ret)
{
	float s = 1.0f - t;
	float t2 = t * t;
	float t3 = t2 * t;
...
IC void spline3(float t, Fvector* p, Fvector* ret)
{
