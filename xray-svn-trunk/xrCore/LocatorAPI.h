...
private:
	...
	IC files_it					GetFilesEndIter(); //Multitreading safe access to Files ending iterator. Ensure ending iterator is not being changed while we get it. Not perfect, but at least better
	IC void						LockFilesVectorMutex();
	IC void						UnlockFilesVectorMutex();
public:
								CLocatorAPI		();
								~CLocatorAPI	();
	...
	const file*					exist2			(LPCSTR path, LPCSTR name, LPCSTR ext);
	...
	std::string					combine			(LPCSTR path, LPCSTR name, LPCSTR ext);
	void						combine			(string_path& res, LPCSTR path, LPCSTR name, LPCSTR ext);
	...
	// renames "file" into "~file"
	void						BackUpFile			(LPCSTR fn, bool bDeleteSource);
