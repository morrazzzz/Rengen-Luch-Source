...
void _STDCALL EncodeFile(_PPMD_FILE* EncodedFile,_PPMD_FILE* DecodedFile,
                            int max_order,MR_METHOD mr_method)
{
    rcInitEncoder();
    StartModelRare(max_order, mr_method);
...
static void _STDCALL StartModelRare(int max_order,MR_METHOD mr_method)
{
    ...
    if( first_time )
    {
        ...
        if(max_order < 2 ) // we are in solid mode
        {                     
            OrderFall = ::MaxOrder;
        ...
        ::OrderFall = ::MaxOrder = max_order;
        ::MRMethod  = mr_method;
        
        InitSubAllocator();
        RunLength = InitRL = -((max_order < 12) ? max_order : 12) - 1;
        ...
        if( !trained_model || _PPMD_E_GETC(trained_model) > max_order)
        {
